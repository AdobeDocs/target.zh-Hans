...
有关在[!DNL Adobe Target]中使用[!UICONTROL 可视化体验编辑器] (VEC)的信息。
...
在目标&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 可视化体验编辑器]**&#x200B;选项卡上，您可以输入默认的可视化体验编辑器URL。
...
>[!NOTE]
>
>如果页面包含混合内容（例如，安全网站中的非安全页面），某些浏览器（如 Firefox）可能会阻止页面在 VEC 中显示。 如果您的页面未显示，请单击浏览器地址栏中URL旁边的图标，然后单击&#x200B;**[!UICONTROL 在此页面上禁用保护]**。 此问题不会影响向网站访客显示页面。
...
在页面加载时（或页面加载失败后），[!UICONTROL 体验]面板、[!UICONTROL 修改]面板和体验顶部的设置（叠加、修改、配置等）均可访问。
...
在 VEC 中取消页面加载之后，您可以在活动中的体验之间进行切换，而无需等待页面加载完成。 若要再次查看VEC中的页面，必须单击&#x200B;**[!UICONTROL 重新加载]**&#x200B;按钮。
