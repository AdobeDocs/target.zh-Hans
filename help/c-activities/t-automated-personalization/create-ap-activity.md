...
1. 要使用可视化体验编辑器 (VEC)，请单击**[!UICONTROL 可视（默认）]**。
   ...
   如果您希望使用基于表单的体验编辑器，请选择**[!UICONTROL 表单]**。

   有关这两种编辑器的更多信息，请参阅[体验](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)。
...
1. 按照[体验](../../c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D)中的说明修改页面元素，然后单击**[!UICONTROL 下一步]**。
...
1. 单击**[!UICONTROL 管理内容]**以配置可用的组合。
   ...
   “[!UICONTROL 体验]”列表显示了为活动选择的每个内容块以及该活动被分配到的位置。您可以通过将鼠标悬停在所需的体验上，然后单击排除图标来排除特定的体验；或者，也可以通过选中相关体验对应的复选框，然后单击对话框右上角的“排除”图标来批量排除/包含多个体验。

   ![](assets/ap_content_batch_exclude.png)

   您可以通过单击**状态**下拉列表来筛选此列表视图，以仅查看排除或包含的活动。
1. 单击**[!UICONTROL 选件]**以选择内容块并将它们分配给[报表组](../../c-reports/offer-reporting-groups-in-automated-personalization.md#concept_194128C0B56B4B26AAB57DB49892960C)。
   ...
   您可以通过将鼠标悬停在所需的选件上，然后单击文件夹图标来将特定的体验添加到报表组；或者，也可以通过选中相关体验对应的复选框，然后单击对话框右上角的“报表组”文件夹图标来将多个体验批量包含到报表组中。
...
1. （视情况而定）单击**[!UICONTROL 选件]**以选择内容片段并将其分配给报表组，或通过定位仅允许特定访客查看特定选件。
   ...
   有关将选件定位到特定受众的信息，请参阅 [Target 自动个性化选件](../../c-activities/t-automated-personalization/ap-target-offers.md#task_F207ED7A41B84FD39BB6FCBFABF4B23E)。
1. （视情况而定）单击**[!UICONTROL 排除组]**以选择要从活动中排除的任何元素组合。

   ![步骤结果](assets/exclusion_groups.png)
...
1. 单击**[!UICONTROL 继续]**以显示**[!UICONTROL 目标和设置]**页面。
1. 使用以下设置配置活动，然后单击**[!UICONTROL 保存并关闭]**。
   ...
   | 转化量度 | 默认情况下，转换量度与优化目标量度相同。但是，您可以通过取消选中 [!UICONTROL 与优化目标] 选项相同的转换量度来定义单独的转化量度。 |
