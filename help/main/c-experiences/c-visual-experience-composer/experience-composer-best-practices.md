...
以下最佳实践有助于确保您的体验按预期运行。在[!DNL Adobe Target]中使用[!UICONTROL Visual Experience Composer] (VEC)时，您还应了解其他提示和限制。
...
如果您还使用访客API服务，请将访客API脚本放置在at.js上方。

### 您可以在帐户级别（为该帐户中创建的所有活动启用）或单个活动级别启用增强型体验编辑器。

要在帐户级别启用增强型体验编辑器，请单击[!UICONTROL Administration > Visual Experience Composer]，然后将开关切换到“开启”位置。

在可视化体验编辑器中创建活动时，要在活动级别启用增强型体验编辑器，请单击[!UICONTROL Configure > URL]，然后将开关切换到“开启”位置。

### 如果增强型可视化允许列表编辑器无法在您的网站的安全页面上加载，则您可以“体验”某些IP地址。
...
请参阅[可视化体验编辑器中使用的元素选择器](/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337)。
...
### 从内容库(Scene7)插入图像并编辑HTML会破坏图像URL。
...
### VEC中的customCode操作不适用于Internet Explorer 8。
