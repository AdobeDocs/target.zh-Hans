---
keywords: AB;A/B;AB...n；错误；错误；错误；错误；错误；陷阱；重要；胜；统计差；统计功率；流量分配；
description: 了解如何避免公司在Adobe [!DNL Target] 和其他测试解决方案中执行A/B测试时最常犯的错误和错误。
title: 如何避免常见A/B测试错误？
feature: A/B 测试
...
[!DNL Adobe Target]中的A/B测试是大多数数字营销优化项目的支柱，可帮助营销人员将优化的目标体验优惠给访客和客户。 本节概述了公司在执行A/B测试时可能遇到的十大重大缺陷。 此外，本节还介绍了避免这些陷阱的方法，从而使您的公司可以通过这种测试工作获得更高的投资回报率 (ROI)，并对报告的 A/B 测试结果更有信心。
...
## 皮特福尔2:声明多个优惠测试的获胜者，无统计学显着差异{#section_FA83977C71DB4F69B3D438AF850EF3B6}
...
我们建议您不要在测试期间更改流量分配百分比，因为这可能会扭曲测试结果，直到数据正常化。
例如，假设您有一个A/B测试活动，其中80%的流量分配给体验A（控件），20%的流量分配给体验B。在测试期间，您会将每个体验的分配更改为50%。 几天后，您会将流量分配更改为100%，改为体验B。

在此方案中，如何将用户分配到体验？

如果您将体验B的分配拆分人工更改为100%，则最初分配给体验A（控件）的访客将保留在其初始分配的体验（体验A）中。 流量分配的变化只影响新进入者。

如果您想更改百分比或极大影响访客流入每个体验，我们建议您创建新活动或复制活动，然后编辑流量分配百分比。

如果您在测试期间更改了不同体验的百分比，则数据需要几天时间才能正常化，尤其是当许多购买者正在返回访客时。
另一个示例是，如果您的A/B测试的流量分配被拆分为50/50，然后您将拆分更改为80/20，则更改后的头几天，结果可能看起来有偏差。 如果平均转化时间较长，即某人需要几个小时甚至几天的时间才能进行购买，则这些延迟的转化可能会影响您的报表。 因此，在第一次体验中，数量从50%增加到80%，平均转化时间为两天，只有来自50%人口的访客在第一天进行转化，尽管今天有80%的人口进入了这一体验。 这让它看起来像是转化率一落千丈，但在80%的访客花了两天时间才转化之后，它将再次恢复正常。

## 皮特福尔8:没有考虑新奇效应{#section_90F0D24C40294A8F801B1A6D6DEF9003}
...
## 皮特福尔十：使用不反映业务目标{#section_F0CD6DC7993B4A6F9BEEBB31CD1D9BEE}的量度
...
## 结论：通过识别和绕过缺陷{#section_54D33248163A481EBD4421A786FE2B15}成功进行A/B测试
