...
description: 了解最佳实践，以使您的体验在使用[!UICONTROL 可视化体验编辑器] (VEC)时按预期工作。
title: '[!UICONTROL 可视化体验编辑器]的最佳实践和限制是什么？'
...
# [!UICONTROL 可视化体验编辑器]最佳实践和限制

为确保您的体验按预期运行，请在使用[!DNL Adobe Target] [!UICONTROL 可视化体验编辑器] (VEC)时遵循最佳实践。 了解关键提示和限制以最大限度地提高性能并避免常见问题。
...
+++查看详细信息
如果您还使用[!UICONTROL 访客API服务]，请将访客API脚本放置在at.js上方。

+++

### 您可以在帐户级别启用[!UICONTROL 增强型体验编辑器]（为该帐户中创建的所有活动启用），也可以在活动级别启用该编辑器。

+++查看详细信息
要在帐户级别启用[!UICONTROL 增强型体验编辑器]，请单击[!UICONTROL [!UICONTROL 管理] > [!UICONTROL 可视化体验编辑器]]，然后将[!UICONTROL 启用增强型体验编辑器]开关切换到“开启”位置。

在[!UICONTROL 可视化体验编辑器]中创建活动时，要在活动级别启用[!UICONTROL 增强型体验编辑器]，请单击[!UICONTROL 配置> [!UICONTROL 页面交付]]，然后将[!UICONTROL 启用增强型体验编辑器]开关切换到“开启”位置。

+++

### 如果[!UICONTROL 增强型Experience Composer]无法在您的网站的安全页面中加载，您可以列入允许列表某些IP地址。

+++查看详细信息
加载[!UICONTROL 增强型体验编辑器]的问题可以通过列入允许列表以下IP地址来解决。 这些IP地址用于用于[!UICONTROL 增强型体验编辑器]代理的[!DNL Adobe]服务器。 仅在编辑活动时才需要这些地址。 您网站的访客不需要列入允许列表这些IP地址。
...
[!DNL Target]不需要ID，但使用ID会提高通过体验编辑器创建的体验的可靠性。 在交付体验时，[!DNL Target]使用CSS选择器修改您的内容。 在编辑体验时，[!UICONTROL 可视化体验编辑器]会将选择器定位到要修改的HTML元素的最近祖先（具有非null ID属性）。 因此，不建议使用任何机制（包括 JavaScript 库）来设置或修改 HTML ID 属性。 尽管这些ID可能可供[!DNL Target]体验编辑器用于创建活动，但如果JavaScript修改ID，则创建体验时使用的ID在体验运行时可能不可用。 如果 ID 不可用，则定位到该 ID 的选择器将会失败。
...
+++详细信息
块级元素不应在内联元素中使用，例如锚点、跨距等。 这样做会导致内联元素失去其高度和宽度，因此[!UICONTROL 可视化体验编辑器]中的叠加工具可能无法按预期工作。
...
查看[!UICONTROL 可视化体验编辑器]](/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337)中使用的[元素选择器。
...
+++详细信息
当您使用[!UICONTROL 增强型体验编辑器]时，网站会在后台被代理服务器操作，该代理服务器会更新所有链接URL以使它们在代理中工作。 如果添加基本标记，则所有这些URL都将由浏览器解析，因此看起来已损坏。
...
+++详细信息
启用[!UICONTROL 增强型体验编辑器]后，将更新top和self变量的值以禁用iframe嵌套。 使用 X-frame-options 标头可添加 iframe 嵌套而不是自定义 JavaScript 代码。
...
使用[!UICONTROL 可视化体验编辑器]设计活动时，请考虑以下注意事项。

### [!UICONTROL 移动]功能不支持z-index。
...
+++详细信息
如果您在非实时站点（如暂存环境）上使用[!UICONTROL 增强型体验编辑器]，如果您的站点阻止RIP，则可能会看到超时和拒绝访问错误。
...
+++详细信息
在mbox中不允许执行诸如[!UICONTROL 编辑类]和[!UICONTROL 重新排列]之类的操作。
...
### [!UICONTROL 更改图像]操作不适用于轮播中的图像。

+++详细信息
例如，如果您的页面包含一个带有六个图像的轮播，并且您想要将一个图像与该轮播的第二个图像交换，则[!UICONTROL 更改图像]操作不起作用。

解决方法是选择父容器并使用[!UICONTROL 编辑HTML]操作编辑轮盘的HTML以更新所需图像的图像源。
...
使用[!UICONTROL Insert Element]操作选择此div以插入图像作为此虚拟文本div的同级。
...
+++详细信息
虽然[!UICONTROL 浏览]模式可用于VEC中的点击跟踪，但[!UICONTROL 浏览]模式不能用于在页面上添加点击跟踪。
