---
description: “Target Recommendations”的介绍，该活动会自动显示可能根据先前用户活动或其他算法对客户感兴趣的产品或内容。“推荐”有助于将客户导向到他们可能还不知道的相关项目。
keywords: 推荐;
seo-description: Adobe Target Recommendations活动的简介，可根据先前用户活动或其他算法自动显示可能会吸引客户的产品或内容。“推荐”有助于将客户导向到他们可能还不知道的相关项目。
seo-title: Adobe Target中的推荐活动简介
solution: Target
title: 推荐介绍
...
我们都知道我们在零售业中看到的推荐类型。越来越多的客户期待这些推荐，并将它们用作探索其他可用选项的起点。如果您考虑自己的购物行为，则这些推荐的效果非常好。我们中几乎每个人都购买了我们在推荐中看到的一个产品，无论是在商店中还是在数字资产上。

下图显示了一个推荐，该建议显示了通常随新手机一起购买的附件，包括充电站、案例和耳机。

![推荐显示其他人随新电话购买的附件。](/help/c-recommendations/assets/intro-1.png)

但我们并不总是考虑数字化优先品牌如何提高客户期望。我们消费媒体和内容的方式越来越受到个性化推荐的驱动。了解打开Netflix、Spetify或YouTube时看到的第一件事。这些品牌通过推荐开始客户体验。在一个比以往更为替代的世界中，您可以在互动点为客户确定最相关的内容。
...
![说明Target如何在不同位置提供推荐的插图](/help/c-recommendations/assets/intro-3.png)
...
![显示三个元素的插图，这些元素做出推荐](/help/c-recommendations/assets/intro-4.png)

推荐的推荐应具有相关性和个性化。这意味着您需要三件事来推动相关性和个性化：

* **营销人员控制** ，有助于推动推荐项目的相关性。作为营销人员，您可以为表格提供有价值的上下文，并且您知道您的产品或内容的哪些属性与推荐模型相关。如果您正在运行一个视频站点，您知道用户可能有兴趣从同一个导演看到电影，但可能不在乎看到同一工作室制作的电影。[!DNL Target] 为您提供允许您利用此域知识增强算法的控件。
* **完善模型** ，让您了解目录和互动活动中的数百万项项目。[!DNL Target] 拥有超过十年经验的复杂机器学习功能，我们每年处理数十亿条推荐。
* **用户上下文** 以确保推荐及时、与用户相关。您不希望推荐某人刚刚观看的视频，也不要推荐刚添加到购物车中的那个衬衫。Target的丰富用户配置文件可用于推荐，以确保个性化。
...
* **您需要推荐哪些项目？** 首先，考虑要推荐的项目。这可能是产品、视频或内容。
...
* 要向收到推荐的用户显示的数据。例如，电影的名称和缩略图图象URL。
* 用于应用营销和销售控制的数据。例如，电影的等级，以便不建议使用NC-17电影。
* 用于确定项目与其他项目的相似性的数据。例如，电影的流派或电影中的演员。
...
必须选择或创建正确的标准，以实现您想要的个性化效果。标准就像一个漏斗，它将您从整个目录带到了最后一组推荐。
...
静态过滤器是与您不希望频繁更改的目录属性相关的广泛适用规则。

![集合和排除插图](/help/c-recommendations/assets/intro-16.png)

例如，在内容上下文中，您可能希望包含建议中的所有电影，但不包括评级为NC-17的电影。在零售环境中，您可能在全球不同部分拥有多个品牌，但您只想推荐在美国提供的产品。您可能还希望排除区域专用标签中的产品。

这些是广泛适用的目录属性，您可能希望在多个建议中使用这些属性，并且您不希望它们经常更改。

### 算法(推荐密钥和逻辑)

下一步是选择推荐密钥和逻辑。您可以在这里决定推荐的依据。

![算法插图](/help/c-recommendations/assets/intro-17.png)

您需要选择的第一件事就是推荐键。推荐密钥是您“查找”推荐的内容。这就是您推荐的内容。
...
* **基于普及率的算法** 包括最受欢迎的、最畅销的卖家。
* **基于内容的算法** 包括内容相似性。
* **基于项目的协作筛选算法** 包括查看/查看、查看/购买以及购买/购买。请注意，“购买”可以是任何转化。
* **个性化算法** 包括最近查看的、站点亲和力和配置文件增强的协作筛选。
* **利用自带算法** ，您可以使用自己的自定义算法。

### 在线业务规则

最后一步是应用在线业务规则。在这种情况下，您可以根据访客在数字属性上的操作，使用域知识和最新上下文来支持算法。

![在线业务规则插图](/help/c-recommendations/assets/intro-18.png)

例如，在内容上下文中，您可能希望排除访客之前观看过的电影、建议同一导演推荐电影或在同一生殖器中提升电影。在零售环境中，您可能希望排除现成的产品，以$5-500的价格显示项目，或从同一品牌提升项目。
