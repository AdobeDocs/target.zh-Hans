...
如果您计划使用Platform Web SDK，以下部分将介绍将此集成部署到您的站点所需的步骤：
...
必须符合用户帐户要求，然后才能在[!DNL Analytics]中创建基于[!DNL Target]的活动。 请参阅[用户权限要求](/help/main/c-integrating-target-with-mac/a4t/account-reqs.md)。

### 步骤3：创建Edge配置

使用边缘配置工具使用[!DNL Adobe Experience Platform]创建Edge配置。 配置[创建和配置数据流](https://experienceleague.adobe.com/docs/experience-platform/datastreams/configure.html#)。
...
必须先满足用户帐户要求，然后才能在[!DNL Analytics]中创建基于[!DNL Target]的活动。 请参阅[用户权限要求](/help/main/c-integrating-target-with-mac/a4t/account-reqs.md)。
...
请参阅[Experience Cloud访客ID服务](https://experienceleague.adobe.com/docs/id-service/using/implementation/setup-target.html)文档中的&#x200B;*为Experience Cloud实施Target ID服务*。
...
有关新实施的信息，请参阅[JavaScript实施指南](https://experienceleague.adobe.com/docs/analytics/implementation/js/overview.html)中的&#x200B;*Analytics实施概述*。

有关迁移，请参阅[Analytics实施指南](https://experienceleague.adobe.com/docs/analytics/implementation/js/migrate-from-hcode.html)中的&#x200B;*迁移到JavaScript的AppMeasurement*。
...
此设置具有全局效果，这意味着at.js进行的每次调用都将在&#x200B;**请求中发送** analyticsLogging： &quot;client_side&quot;[!DNL Target]，并且将为每个请求返回分析有效负载。 设置此选项后，返回的有效负载格式如下所示：
...
然后，可以通过[数据插入API](https://helpx.adobe.com/analytics/kb/data-insertion-api-post-method-adobe-analytics.html)将该有效负载转发到Analytics。 对于自动分配和自动定位活动，还必须转发sessionId。 有关详细信息，请参阅[Adobe Target SDK](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/integration/a4t-reporting.html){target=_blank}指南中的&#x200B;*Analytics for Target (A4T)报表*。

如果不希望进行全局设置并且想要使用按需方法，请通过传入[analyticsLogging： &quot;client_side&quot;](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-getoffers-atjs-2.html){target=_blank}来使用at.js函数&#x200B;**getOffers()**。 仅为此调用返回分析有效负载，[!DNL Target]后端未将该有效负载转发到[!DNL Analytics]。 如果采用这种方法，每个at.js [!DNL Target]请求都会默认返回有效负载，但只有在需要和指定时才会返回。
...
然后，可以通过[!DNL Analytics]数据插入API[将该有效负载转发到](https://helpx.adobe.com/analytics/kb/data-insertion-api-post-method-adobe-analytics.html)。

### 步骤 8：验证实施 {#step8}

更新JavaScript库后加载页面，以确认`mboxMCSDID`调用中的[!DNL Target]参数值与`sdid`页面查看调用中的[!DNL Analytics]参数值匹配。
...
Adobe建议您删除之前的集成，以简化实施并免除解决系统间差异的麻烦。 您可以删除为之前的SC与T&amp;T集成部署的任何代码，包括`mboxLoadSCPlugin`。
