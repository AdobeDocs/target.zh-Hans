...
* 修复了导致某些客户的默认工作区的 Target UI 中不显示任何活动的问题。(TGT-45286)
* 更新了“不允许重复”标志的行为。排除的重复优惠标志已更新，以允许重复优惠（如果它们是默认内容优惠（对于 API v3、v4）），如果选项引用默认内容优惠且未定义模板，则允许重复选项。(TNT-46617)
* 修复了一个问题，在该问题中，查询参数被添加到 URL，导致页面无法加载到可视化体验编辑器 (VEC) 中。(TGT-44873)
* 修复了体验中文本/HTML 中某些字符被错误转义的问题。(TGT-44600)
...
* 性能增强以禁止重复功能（包括减少加载时间），同时在 [!UICONTROL Automated Personalization] 活动中[管理排除项](/help/main/c-activities/t-automated-personalization/managing-exclusions.md#concept_4EF78013F80E48EFA024AE0274C9F037)。
