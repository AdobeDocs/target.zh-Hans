...
description: 了解存储在访客配置文件中的特定于访客的属性，以提供可在您的Adobe中使用的相关信息 [!DNL Target] 活动。
title: 什么是配置文件属性？
feature: Audiences
...
中的配置文件属性 [!DNL Adobe Target] 是特定于访客的参数。 这些属性存储在访客的配置文件中，提供了可在您的活动中使用的关于访客的信息。

用户配置文件包含网页访客的人口统计和行为信息。 此信息可能包括年龄、性别、购买的产品、上次访问时间等，这些信息 [!DNL Target] 用于将其提供给访客的内容个性化。
...
1. 单击 **[!UICONTROL 创建脚本]**.
   ...
   | 参数类型 | 描述 |
   |--- |--- |
   | mbox | 创建 mbox 时，直接通过页面代码传入。请参阅[将参数传递到全局 mbox](/help/main/c-implementing-target/c-implementing-target-for-client-side-web/t-mbox-download/c-understanding-global-mbox/pass-parameters-to-global-mbox.md)。<br>****&#x200B;注意： 限制每个 mbox 调用只能包含 50 个唯一的配置文件属性。[!DNL Target]如果必须将50个以上的配置文件属性传递到 [!DNL Target]，请使用配置文件更新API方法传递它们。 有关更多信息，请参阅 [在 [!DNL Adobe Target] API文档](https://developers.adobetarget.com/api/#updating-profiles). |
...
您可以使用配置文件脚本，在多次访问中捕获访客属性。配置文件脚本是在 [!DNL Target] 使用服务器端JavaScript的形式。 例如，您可以使用配置文件脚本捕获访客访问您网站的频率以及该访客上次访问的时间。

配置文件脚本与配置文件参数不同。配置文件参数使用 [!DNL Target].
...
配置文件脚本会对每个位置请求运行配置文件属性“catchers”。收到位置请求后， [!DNL Target] 确定应运行的活动，并显示与该活动和该体验相适的内容。 [!DNL Target] 还会跟踪活动的成功情况并运行任何相关的配置文件脚本。 通过此过程，您可以跟踪有关访问的信息，如访客的位置、时间、访客访问网站的次数、访客以前购买过的次数等。 然后，这些信息将添加到访客的配置文件中，以便您可以更好地跟踪访客在您网站上的活动。
...
例如，通过单击 [!UICONTROL 信息] 图标([!UICONTROL 受众] > [!UICONTROL 配置文件脚本])。

的 [!UICONTROL 脚本信息] 选项卡包含以下信息：名称、描述和脚本代码。

![配置文件脚本信息卡](assets/profile_script_info_card.png)

单击 **[!UICONTROL 查看完整详细信息]** 查看引用所选配置文件脚本的受众和活动。

![配置文件脚本信息卡 >“脚本使用情况”选项卡](assets/profile_script_info_card_usage_tab.png)

>[!NOTE]
>
>的 [!UICONTROL 脚本使用情况] 选项卡不会在以下情况下显示引用所选配置文件脚本的活动：
>
> * 活动位于 [!UICONTROL 草稿] 状态。
...
* 不仅要注意脚本性能，还要注意所有脚本的组合性能。作为最佳做法， [!DNL Adobe] 建议总共使用少于5,000个指令。 计算指令数量并不明显，但需要注意的重要一点是，超过2,000条指令的脚本会自动禁用。 有效配置文件脚本的数量不应超过300个。 每个脚本都通过每个mbox调用执行。 应只运行所需数量的脚本。
* 在正则表达式中，在开头使用点和星形符号(例如： `/.*match/`, `/a|.*b/`)。 正则表达式搜索将从字符串中的任意位置开始（除非与 `^` 绑定），因此其已经假定包含点和星形符号。如果此类正则表达式与足够长的输入数据相匹配（最少可达几百个字符），则脚本执行可能会中断。
...
* 以下建议可帮助您限制配置文件脚本的复杂性。 配置文件脚本可以执行有限数量的指令。
   ...
   * 避免递归。
   * 配置文件脚本在添加到之前应进行性能测试 [!DNL Target]. 所有配置文件脚本都会在每个mbox请求中执行。 如果配置文件脚本无法正确执行，则执行mbox请求的时间会更长，这可能会影响流量和转化。
   * 如果配置文件脚本过于复杂，请考虑使用 [响应令牌](/help/main/administrating-target/response-tokens.md) 中。
...
>[!NOTE]
>
>使用 [!DNL console.log] 配置文件脚本中不输出配置文件值，因为配置文件脚本在服务器端执行。

* **将配置文件脚本添加为响应令牌以调试配置文件脚本：**

   在 [!DNL Target]，单击 **[!UICONTROL 管理]**，单击 **[!UICONTROL 响应令牌]**，然后启用要调试的配置文件脚本。

   无论您何时通过 [!DNL Target] 在上面，部分回应来自 [!DNL Target] 包含给定配置文件脚本的值，如下所示：

   ![](assets/debug_profile_script_1.png)

* **使用mboxTrace调试工具调试配置文件脚本。**

   此方法需要授权令牌，您可以通过单击 **[!UICONTROL Target]** > **[!UICONTROL 管理]** > **[!UICONTROL 实施]** > **[!UICONTROL 生成授权令牌]** 在 [!UICONTROL 调试器工具] 中。
...
由于配置文件脚本在服务器端执行，因此它们无法直接读取页面。数据必须通过 mbox 请求或其他[将数据传入 Target 的方法](/help/main/c-implementing-target/c-considerations-before-you-implement-target/c-methods-to-get-data-into-target/methods-to-get-data-into-target.md#concept_0069C0EFB56C4700BB33F2F35C2B9B17)传递。在数据输入 [!DNL Target]，配置文件脚本可以将数据作为mbox参数或配置文件参数读取。
...
创建一个以毫秒为单位的“天数”变量。如果mbox名称为 `orderThankyouPage`，请设置名为的本地（不可见）用户配置文件属性 `lastPurchaseTime` 以显示当前日期和时间的值。 读取上次购买时间的值，如果定义， [!DNL Target] 返回自上次购买时间以来经过的时间除以一天中的毫秒数（其结果为自上次购买后的天数）。
...
创建一个名为 `frequency`，将其初始化为上一个值，或者如果没有上一个值，则初始化为0。 如果 mbox 名称为 `orderThankyouPage`，则返回递增的值。
...
创建一个名为 `adobeQA` 跟踪用户 [活动QA](/help/main/c-activities/c-activity-qa/activity-qa.md).
...
## 培训视频：配置文件脚本 ![教程徽章](/help/main/assets/tutorial.png)
