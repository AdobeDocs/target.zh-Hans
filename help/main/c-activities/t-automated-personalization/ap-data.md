...
| [!DNL Adobe Experience Platform Real-time CDP] audiences | UPS | 与共享的Platform Real-time CDP受众 [!DNL Target] via [!UICONTROL 目标]. |  |
| [!DNL Adobe Experience Platform Real-time CDP] 属性 | AEP | 共享平台Real-time CDP属性 [!DNL Target] via [!UICONTROL 目标]. |  |

## 阻止功能 [!DNL Target] 机器学习算法

可以从阻止功能 [!DNL Target] 机器学习算法，防止在任何系统中使用它们 [!UICONTROL Automated Personalization] 或 [!UICONTROL 自动定位] 模型或活动。

有关更多信息，请参阅 [模型API(列入阻止列表)概述](https://experienceleague.adobe.com/docs/target-dev/developer/api/models-api/models-api.html){target=_blank} 在 *[!DNL Adobe Target]开发人员指南*.
...
| 移动设备 — OS - OS X ([!DNL Android]， [!DNL Windows]，等等) | 指定用户是否使用了OSX(或 [!DNL Android]， [!DNL Windows]，等等)设备访问活动。 | 0 为 False，1 为 True | MOB_targeting.mobile.os[操作系统]<br>例如，MOB_targeting.mobile.osOSx、MOB_targeting.mobile.osWindows、MOB_targeting.mobile.osAndroid、MOB_targeting.mobile.osLinux |
...
| 浏览器 - 工作日/周末 | 访客访问活动时的工作状态（周末、工作时间或工作日休息时间）。 | 星期六和星期日是周末<br>周一至周五09:00-18:00为工作时间<br>1800后星期一至星期五至0900是工作日休息时间 | ENV_UserHourType |
...
| 地域 - 州/地区 | 访客访问活动时所在的州或地区。 | 犹他州<br>新南威尔士州 | 地理状态<br>地理区域 |
...
| 访客配置文件 - 首次访问 | 指定用户首次访问进行交互的时间 [!DNL Target]. | 双精度，毫秒 | ses_PROFILE_CREATION_TIME |
...
| 访客配置文件 — 上次 [!DNL Target] 互动 | 指定上次与交互的时间 [!DNL Target]. 交互发生于每 [!DNL Target] 请求，因为当前的 [!DNL Target] 会更新每个请求中的配置文件。 | 双精度，毫秒 | SES_PROFILE_UPDATE_TIME |
| 访客配置文件 - 活动之前查看的页面数 | 指定在访客进入活动之前的页面查看总次数（展示次数），包括当前访问/会话。 | 双精度（仅限正整数） 1、2、3等。 | SES_TOTAL_PAGE_VIEWS |
| 访客配置文件 - 当前访问中的页面查看次数 | 指定在访客进入活动之前，当前访问/会话中的页面查看次数。 精度更高的展示次数。这些展示次数并不是实际的页面查看次数，而是达到请求的次数 [!DNL Target]. [!DNL Target] 无法区分用户是因为超时还是任何其他原因而未能接收或查看内容。 | 双精度（仅限正整数） | SES_SESSION_POSITION |
| 访客配置文件 - 当前访问的开始时间 | 指定当前访问/会话的时间 [!DNL Target] 已启动。 访问方式 [!DNL Target] 无需进入活动即可启动。 只需调用任意 [!DNL Target] 请求。 访客可能需要一段时间才能进入活动并拍摄快照。 | 双精度，毫秒 | SES_SESSION_START |
...
| 访客配置文件 - 进入活动前的访问时间 | 指定上次与的交互之间的差异 [!DNL Target] 以及当前访问开始的时间。 此属性可视为在用户进入活动并拍摄快照之前，访问/会话的持续时间。<br>如果会话开始时间和上次更新时间由同一触发时，则会出现负值 [!DNL Target] 呼叫。 负值应视为 0（零）。 | 双精度，毫秒 | SES_SESSION_TIME |
