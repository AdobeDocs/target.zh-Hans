...
**限制**：对于 POST 请求，为 5 KB。2,083 个字符减去 GET 请求的 URL 长度。

对于 GET 请求，尽管后端的限制为 5 KB，但由于 Microsoft Internet Explorer 将 URL 限制为 2,083 个字符，因此其实际限制为 2,083 个字符减去 URL 的当前长度。
...
对于被记录到 Target 数据库中的参数，上述限制适用于标准 mbox 请求。除非因 Web 浏览器限制而缩短请求，否则将应用这些限制。
...
此限制是由于 Microsoft Internet Explorer URL 长度限制所致。
...
如果您使用的是全局 mbox，则限制适用于为该页面返回的整套内容。限制选件大小会提升页面加载时间。如果超出限制，则会出现以下消息：
