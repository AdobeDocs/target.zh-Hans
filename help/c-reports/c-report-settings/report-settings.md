...
帮助您设置要显示在[!DNL Adobe Target]报表中的元素的信息。 可保存报表设置，以供日后使用。
...
根据需要配置单个活动的报表后，您最多可以保存该报表的 10 个不同预设（量度、日期范围、受众、高级设置等）。所有[!DNL Target]用户都可以显示、编辑和删除各种预设，无论这些预设是由谁创建的。
...
1. （视情况而定）如果您希望此视图为默认/收藏报表，请将&#x200B;**[!UICONTROL 设为默认预设]**&#x200B;切换至开启位置。
1. 单击&#x200B;**[!UICONTROL 保存]**。
...
为报表选择新的&#x200B;**[!UICONTROL 开始]**&#x200B;日期和&#x200B;**[!UICONTROL 结束]**&#x200B;日期。您还可以使用&#x200B;**[!UICONTROL 从活动]**&#x200B;和&#x200B;**[!UICONTROL 直到活动]**&#x200B;结尾复选框的开始。
...
选择要用于报表的环境（主机组）。 有关更多信息，请参阅[主机](/help/administrating-target/hosts.md#concept_516BB01EBFBD4449AB03940D31AEB66E)。

### 重置报表数据

重置报告数据以删除旧数据。 当前的访客将保留在活动中。此选项仅适用于具有[!UICONTROL 审批者]权限的用户。
...
[!UICONTROL 排除极值]切换选项仅适用于“收入”和“参与”量度类型的活动。 有关更多信息，请参阅[排除超常订单](/help/c-reports/c-report-settings/excluding-extreme-orders.md#task_2AE7743FFCDD466DAEEB720BE5F33DAA)。

## 下载 {#section_77E65C50BAAF4AB79242DB3A8778ADEF}

单击&#x200B;**[!UICONTROL 下载]**&#x200B;图标以[!DNL .csv]格式下载报表数据，以便快速导入Excel、Access或其他数据分析项目。
...
单击&#x200B;**[!UICONTROL 刷新]**&#x200B;图标可刷新报表的表格和图表视图，而无需刷新整个页面、其配置或日期范围。
...
* **图表视图**:单击图 **[!UICONTROL 表]** 视图图标以图表形式视图报表。
* **自动细分**:(仅适用于Automated Personalization(AP)和自动目标(AT)活动。)单击**[!UICONTROL 自动区段]图标以视图[自动区段报表](/help/c-reports/c-personalization-insights-reports/automated-segments-report.md)。
* **重要属性**:(仅适用于Automated Personalization(AP)和自动目标(AT)活动。)单击**[!UICONTROL 重要属性]图标以视图[重要属性报表](/help/c-reports/c-personalization-insights-reports/important-attributes-report.md)。
...
“运行平均值”反映累积转换(从报告窗口的开始到图表上所示日期)除以累积访客。
...
要访问[!UICONTROL 计数方法]选项，在图表模式下查看报表时，请单击&#x200B;**[!UICONTROL 我的主要目标]**&#x200B;下拉列表，然后选择计数方法。
...
* **A/B测试(包括自动分配和Automated Personalization)**:体验创建的顺序（按降序）。
* **体验定位(XT)**:体验在活动中的顺序。
* **Multivariate Test(MVT)**:按体验名称字母排序。
* **Recommendations**:体验创建的顺序（按降序）。

在使用“计数方法”选项时，请考虑以下注意事项：

* 对于[自动目标活动](/help/c-activities/auto-target/auto-target-to-optimize.md)，没有选项可选择“访客”作为计数方法。 自动定位是唯一一种无法按访客绘制图形的活动类型。
* 对于使用[Analytics作为报告源(A4T)](/help/c-integrating-target-with-mac/a4t/a4t.md)的活动，您无法累积绘制访客、访问或印象。
