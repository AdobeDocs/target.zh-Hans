...
实施时需要执行多个步骤 [!DNL Adobe Analytics] 作为的报表源 [!DNL Adobe Target] (A4T)。 根据您是否使用实施A4T，此过程会有所不同 [[!DNL Adobe Experience Platform Web SDK]](https://experienceleague.adobe.com/docs/experience-platform/edge/home.html) 或使用at.js。
...
在实施A4T之前，必须为您配置 [!DNL Analytics] 和 [!DNL Target]. [使用此表单可请求进行配置](https://survey.adobe.com/jfe/form/SV_ekBHTLSoP5Zki2y).
...
创建Edge配置，使用 [!DNL Adobe Experience Platform] 使用边缘配置工具。 配置 [创建和配置数据流](https://experienceleague.adobe.com/docs/experience-platform/datastreams/configure.html?lang=zh-Hans).
...
在 [!DNL Target] UI，单击 **[!UICONTROL Administration]** > **[!UICONTROL Visual Experience Composer]**，然后选择以下任一选项 **[!UICONTROL Select per activity]** 或 **[!UICONTROL Adobe Analytics]**.

* **[!UICONTROL Select per activity]** 可让您选择 [!DNL Target] 和 [!DNL Analytics] 创建每个活动时。
* **[!UICONTROL Adobe Analytics]** 集 [!DNL Analytics] 作为您创建的所有活动的报表源。
...
实施之后 [!DNL Analytics] 作为的报表源 [!DNL Target]，您必须进行配置， [!DNL Analytics] 和 [!DNL Target]. [使用此表单可请求进行配置](https://survey.adobe.com/jfe/form/SV_ekBHTLSoP5Zki2y){target=_blank}.

### 步骤 2：设置用户权限

在创建 [!DNL Analytics]中基于的活动 [!DNL Target]. 请参阅[用户权限要求](/help/main/c-integrating-target-with-mac/a4t/account-reqs.md)。

### 步骤 3：实施 Experience Cloud 访客 ID 服务。

通过访客ID服务，您可以识别以下各项中的用户： [!DNL Adobe Experience Cloud] 解决方案。 实施或迁移到所需版本的Experience Cloud访客ID。 有关更多信息，请参阅[实施之前](/help/main/c-integrating-target-with-mac/a4t/before-implement.md)中的“实施要求”。
...
有关新的实施，请参阅 [JavaScript实施概述](https://experienceleague.adobe.com/docs/analytics/implementation/js/overview.html) 在 *Analytics实施指南*.

有关迁移，请参阅 [迁移到JavaScript的AppMeasurement](https://experienceleague.adobe.com/docs/analytics/implementation/js/migrate-from-hcode.html) 在 *Analytics实施指南*.
...
的默认设置 [!DNL Target] 和 [!DNL Analytics] 从实施的角度来看，集成就是使用从页面传递的SDID来拼合 [!DNL Target] 和 [!DNL Analytics] 一起在后端自动请求。

您可以控制如何以及何时发送与相关的分析数据 [!DNL Target] 到 [!DNL Analytics] 作报告用途。 如果您不想选择使用默认设置，即 [!DNL Target] 和 [!DNL Analytics] 通过SDID自动拼合分析数据，设置 **analyticsLogging =客户端** via **window.targetGlobalSettings**. 注意：低于 2.1 的任何版本均不支持此方法。
...
此设置具有全局效果，这意味着at.js发出的每个调用都具有 **analyticsLogging： &quot;client_side&quot;** 发送于 [!DNL Target] 将为每个请求返回请求和分析有效负载。 设置此选项后，返回的有效负载格式如下所示：
...
如果不希望进行全局设置并且想要使用按需方法，请使用at.js函数 [getOffers()](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-getoffers-atjs-2.html){target=_blank} 通过传入 **analyticsLogging： &quot;client_side&quot;**. 仅针对此调用返回分析有效负载，并且 [!DNL Target] 后端未将有效负载转发到 [!DNL Analytics]. 通过采用这种方法，每个at.js [!DNL Target] 默认情况下，请求会返回有效负载，但只有在需要和指定时才返回。
...
更新JavaScript库后加载页面，以确认 `mboxMCSDID` 参数值 [!DNL Target] 调用匹配 `sdid` 参数值 [!DNL Analytics] 页面查看调用。
...
在 [!DNL Target]，单击 **[!UICONTROL Administration > Reporting]** 并选择 **[!UICONTROL Select per activity]** 或 **[!UICONTROL Adobe Analytics]** 以启用选项。
