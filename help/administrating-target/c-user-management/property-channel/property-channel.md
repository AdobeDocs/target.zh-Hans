...
属性与动态标签管理（激活）中的属性类似，不同之处在于它们各自使用一段独特的代码片段。
...
### 为什么我会收到一条错误消息，指示没有与此活动关联的属性与此活动相关联，即使分配了属性？

如果您使用 [!DNL Target][!DNL Adobe Launch] 并收到一条错误消息，表明没有与该活动关联的属性，则将该 `at_property` 参数与该 `targetPageParams` 函数一起传递。

### 如果重定向页面和活动URL属于不同的属性，是否会记录点击跟踪转换？

单击跟踪不会记录在页面和活动URL属于不同属性的页面上。

请考虑以下场景(适用于. js和mbox. js)：

* 第页属于属性1。
* 第页属于属性2。
* 在活动中，第页重定向到包含点击跟踪的第页。

当访客在浏览器中打开第页时，他或她被重定向到第页。由于第页没有提供活动的资格，其Target电话不包含点击跟踪。

如果重定向页面和活动URL属于同一属性，则clicktrack可按预期工作。有关详细信息，请参阅 [单击跟踪](/help/c-activities/r-success-metrics/click-tracking.md)。
