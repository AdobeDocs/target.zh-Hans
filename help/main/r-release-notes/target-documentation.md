...
| [Target Recommendations APIs](https://experienceleague.adobe.com/docs/target-dev/developer/api/recommendations-api/overview.html?lang=zh-Hans){target=_blank} | 有关[!UICONTROL Recommendations] API的信息，该API允许您以编程方式与[!DNL Target]推荐服务器交互。 这些 API 可与多种应用程序栈集成，以执行一般将通过 [!DNL Target] UI 执行的功能。 |
| [服务器端投放 API](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/server-side-overview.html?lang=zh-Hans){target=_blank} | 关于构成 Adobe Target 投放 API 的资源的信息。 |
