...
seo-description: 自动分配可在两个或更多体验中标识一个入选者，并在测试继续运行和学习期间，自动为入选者重新分配更多流量以提高转化。
seo-title: 自动分配
solution: Target
title: 自动分配
topic: Standard
...
自动分配可识别两个或更多体验中的入选方，并自动将更多流量分配给入选方，以在测试继续运行和学习的同时提高转化率。
...
自动分配会智能地为新访客分配体验，直到最佳体验的置信区间与任何其他体验的置信区间都不重叠为止。通常此过程可能会产生误报，但自动分配可使用基于 [Bernstein 不等式](https://en.wikipedia.org/wiki/Bernstein_inequalities_(probability_theory))的置信区间来抵消重复评估所产生的影响。从这一点来看，我们将会获得真实入选者。当自动分配停止时，前提是访问到页面的访客没有明显的时间依赖关系，因此自动分配将返回一个体验，该体验的实际响应不会低于实际响应的实际响应的1%(相对)。
...
**在使用自动分配功能评估入选方将需要多长时间时，我能否使用示例大小计算器？**
...
**我是否应该从自动分配活动中删除效果不佳的体验以加快确定入选方的过程？**

没有理由删除效果不佳的体验。自动分配可更频繁地自动提供高性能体验，并更少地提供性能不佳的体验。在活动中留下表现欠佳的体验不会显著影响确定入选方的速度。

20%的访客随机分配给所有体验。性能欠佳体验的流量最低(20%除以体验数量)。
