...
1. 与组织成员共享活动链接URL以进行测试。
   ...
   您可以导航站点以查看其他页面，因为[!UICONTROL Activity QA]模式具有粘性。 对于版本2.*x*&#x200B;或更高版本的at.js实施，此情况为真。 对于at.js 1.*x*&#x200B;实施，仅当访客的浏览器不阻止第三方Cookie时，才会出现这种情况。
...
如果您的网站部署了at.js 2.*x*，请使用[Target QA书签](/help/main/c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)强制自己退出[!UICONTROL Activity QA]。 在网站上加载具有空值的页面（如下一个项目符号中所述），在部署at.js 2.*x*&#x200B;时，*不会*&#x200B;从浏览器中删除QA Cookie。
...
* 如果您有at.js 1.*x*，那么如果您使用Safari或其他阻止第三方Cookie的浏览器，则[!UICONTROL Activity QA]模式无粘性。 在这些情况下，您必须将预览参数添加到您导航到的每个URL。 如果您实施了[CNAME](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/implement-cname-support-in-target.html?lang=zh-Hans){target=_blank}，则同样如此。
