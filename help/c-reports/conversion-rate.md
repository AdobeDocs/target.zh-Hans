---
keywords: 定位
description: 了解Adobe [!DNL Target] 如何显示和计算每个体验的转化率、提升度、置信度（统计意义）和置信区间。
...
如果您插入下单(`orderConfirmPage`)mbox并将其选作转化mbox，则会为每个体验显示AOV、RPV和销售数据。
...
要计算这些计算量度，请下载Target的[完整置信度计算器](/help/assets/complete_confidence_calculator.xlsx) Excel文件以输入活动值，或查看Target](/help/assets/statistical-calculations.pdf)使用的[统计计算。
...
对于 A4T，我们会使用 Student t 检验来计算连续变量（而非二进制量度）。在 Analytics 中，会始终跟踪访客，并计入所执行的每项操作。因此，如果访客进行了多次购买或多次访问了某个成功量度，则会计入这些额外的点击。这会使量度变为连续变量。要执行Student t检验计算，需要“平方和”来计算方差，该方差用在t统计量的分母中。 [本文档介绍所](/help/assets/statistical-calculations.pdf) 用数学公式的详细信息。可从[!DNL Analytics]中检索平方和。 要获取平方和数据，您需要导出样本时间段内要优化的量度在访客级别对应的数值。
...
## 为什么[!DNL Target]建议使用Student的t检验？ {#t-test}

A/B测试是一种实验，用于将控制变体（也称为体验）中某些业务量度的平均值与一个或多个替代体验中该相同量度的平均值进行比较。

[!DNL Target] 建议使用两个 [示例学生t测试](https://en.wikipedia.org/wiki/Student%27s_t-test#:~:text=The%20t%2Dtest%20is%20any,the%20test%20statistic%20were%20known.)，因为与z测试等替代方法相比，这些测试需要的假设要少，并且是在控制体验和替代体验之间对业务量度进行两对比较（定量）的适当统计测试。
...
虽然业务量度本身可能根据某些任意分布进行分配，但此量度（在每个变体内）的平均值分布应通过[中心极限定理](https://en.wikipedia.org/wiki/Central_limit_theorem)收敛到正态分布。 请注意，虽然无法保证平均值的取样分布会以多快的速度收敛到正常，但鉴于在线测试中的访客数量，通常会达到此条件。

根据平均值的这种正常性，可以显示要使用的测试统计量以遵循t分布，因为它是正分布值（业务量度的手段差）与基于来自数据的估计值（手段差的标准误差）的缩放项的比值。 如果测试统计数据遵循t分布，则&#x200B;**Student的t检验**&#x200B;是适当的假设检验。

### 为何不使用其他测试

**z-test**&#x200B;不适当，因为在典型的A/B测试情景中，测试统计数据的分母不是从已知方差派生的，而是必须从数据中估计。

**不使用卡** 方检验，因为这些检验适用于确定两个变体之间是否存在定性关系（即两个变体之间没有差异的空假设）。T测试更适合于&#x200B;_quantimulative_&#x200B;比较量度的情景。

**Mann-Whitney U测试**&#x200B;是非参数测试，当平均业务量度（对于每个变量）的抽样分布未正常分布时，此测试是合适的。 但是，如前所述，考虑到在线测试中涉及的流量量，中心极限定理通常适用，因此t检验可以安全地应用。

当测试具有两个以上体验（“A/Bn测试”）时，可以应用更复杂的方法，例如&#x200B;**ANOVA**（将t测试归纳为两个以上变体）。 但是，ANOVA回答了“所有变体是否具有相同的均值”的问题，而在典型的A/Bn测试中，我们更感兴趣的是&#x200B;_哪个特定变体_&#x200B;最好。 因此，在[!DNL Target]中，我们应用常规t测试来比较每个变体与控制体，并使用Bonferroni校正来考虑多个比较。
