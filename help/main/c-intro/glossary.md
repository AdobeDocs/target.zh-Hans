...
一组将会看到定位活动的相似活动参加者。  受众是具有相同特征的一群人，如新访客、回访访客或来自 Midwest 的 help/c-integrating-target-with-mac/a4t/a4t-at-aa.mds。 通过使用“受众”功能，您可以将不同的内容和体验锁定到特定的受众，即在适当的时间向适当的人员展示恰当的信息，从而优化网站营销。 如果某位访客被识别为目标受众的一员，则 Target 会根据在活动创建过程中定义的标准确定要向该访客显示的体验。
...
自动个性化 (AP) 可将各种产品建议或消息进行组合，并且使用先进的机器学习技术，根据每位访客的个人客户轮廓将不同的变体与其匹配，以便个性化内容并促进转化。
...
[!UICONTROL 增强型体验编辑器]是[!UICONTROL 可视化体验编辑器]的扩展，可帮助您编辑尚未包含[!DNL Target]实现的防iframe嵌套网站或页面的体验。 如果无法在[!UICONTROL 可视化体验编辑器]中打开您的页面，请尝试使用[!UICONTROL 增强型体验编辑器]。
...
一个页面、整个网页、可能形成购买漏斗的一组页面或一些其他逻辑序列的页面上的一个产品建议、图像、文本、按钮、视频，以及这些不同元素的组合。 它还可以是语音助手的回复、客户服务脚本，甚至是饮料机中提供的独特口味。 您可以在 [!DNL Target] 活动中测试或个性化体验。
...
一个非可视化体验和产品建议创建界面，当可视化体验编辑器不可用或不实用时，它有助于创建在 A/B 测试、体验定位、自动个性化和“推荐”活动中使用的体验。 例如，您可以使用基于表单的编辑器为电子邮件、网亭和语音助手中的交付创建体验和产品建议。

## G {#section_D7B3B62BD3A449EFAA11DF9704773B28}

**geo location parameter（地理位置参数）**

可让您根据访客的地理位置锁定活动和体验。 也称为地理位置定位参数。
...
在页面（而不是 URL）中传递。 这些参数可用于传递来自数据库中的数据，并跟踪销售数据或运行 Adobe 推荐。 也称为页面参数。

**多变量测试**

多变量测试 (MVT) 可通过比较页面上各元素中的产品建议组合，来确定哪个组合对特定受众的效果最佳，以及哪个元素对活动成功的影响最大。
...
* 这是自[访客轮廓生命周期](/help/main/c-target/c-visitor-profile/visitor-profile-lifetime.md)到期以来该访客第一次访问该网站。

有关详细信息，请参阅[如何计算新访客和回访访客指标？](/help/main/c-reports/reporting-frequently-asked-questions.md#methodology) （位于报告问答中）。请与下方的“回访访客”进行对比。
...
产品建议是指在活动和测试期间显示在 mbox 中的内容。 体验决定了满足特定条件时显示的产品建议。
...
页面查看是指对移动或浏览器应用程序、电子邮件或 Internet 站点网页（包括应用程序屏幕视图、应用程序屏幕状态、移动网页以及社交网络页面）的单次查看。 每次加载或刷新网页、加载应用程序、呈现或生成目标内容或者通过已打开或已查看的电子邮件显示内容时，都会发生页面查看。
...
将值作为引荐 URL 参数和值传递给目标页面的 mbox。 例如，使用这些参数和值来锁定访客源。

**回访访客**

如果用户以前访问过该网站，停留了至少 30 分钟，然后带着相同的 Cookie 再次返回该网站，则该访问者被包括在“回访访客”区段中。 只要访客在其轮廓生命周期内返回，此访客即为回访访客。

有关详细信息，请参阅“报表常见问题”中的[如何计算新访客和回访访客指标？](/help/main/c-reports/reporting-frequently-asked-questions.md#methodology)。请与上方的“新访客”进行对比。
...
可使用远程产品建议在 [!DNL Target] 以外托管内容，[!DNL Target] 会引用这些内容并将其交付到用户网站。 出于方便使用或安全原因，这些内容可能位于内容管理或其他系统中。

**reports（报表）**

报表提供了有关活动进度和结果的信息，可帮助您根据数据做出决策。 报告数据可以帮助您确定何时结束测试，向您显示哪个体验或产品建议已入选，并提供确定后续操作所需的洞察。

## S {#section_9482B06EBB09486784492F838289E54D}

**script profile parameter（脚本配置文件参数）**

与 mbox 内轮廓参数类似，不同之处在于，此参数使用在 Target 后端运行的 JavaScript 进行定义，而不是在浏览器中定义。
...
用于衡量活动是否成功的参数。 成功量度包括关键业务衡量指标，可帮助您确定 Target 活动中的给定体验或产品建议是否成功。 例如，您可以确定新产品建议或向购物车添加物品是否会增加每位访客带来的收入。 成功量度可用于发现注册、订购或购买漏斗等方面的问题，也可仅仅用于提高访客或客户参与度。
...
在 Target Standard 中，WYSIWYG 用户界面可让您轻松地在站点环境中创建和测试个性化体验和产品建议。 您可以通过拖放、交换和修改网页（或产品建议）的布局和内容来为 Target 活动创建体验和产品建议。
