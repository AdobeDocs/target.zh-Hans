...
要在创建活动时应用受众URL定位，请在&#x200B;**[!UICONTROL 体验]**&#x200B;页面（三步引导式工作流的步骤一）上单击&#x200B;**[!UICONTROL 配置]**&#x200B;图标（![配置图标](/help/main/assets/icons/Setting.svg)），单击&#x200B;**[!UICONTROL 页面交付]**，然后指定所需的URL。
...
通过[!UICONTROL 模板规则]添加的受众URL定位和URL定位将评估为URL定位（请参阅下面的URL定位）。

### URL定位 {#url-targeting}

若要在创建受众时应用URL定位，请单击将&#x200B;**[!UICONTROL 网站页面]**&#x200B;拖放到[!UICONTROL 创建受众]窗格中，单击&#x200B;**[!UICONTROL 网站页面]**，从第一个下拉列表中选择一个选项（[!UICONTROL 当前页面]、[!UICONTROL 上一页面]或[!UICONTROL 登陆页面]），从第二个下拉列表中选择[!UICONTROL URL]，指定计算器，然后指定所需的URL。
...
[!DNL Target] 限制每个 mbox 调用只能包含 50 个唯一的轮廓属性。 如果您需要将50个以上的配置文件属性传递到[!DNL Target]，则可以使用[!UICONTROL 配置文件更新] API方法来传递它们。 有关更多信息，请参阅 Adobe Target API 文档中的[配置文件更新](https://developers.adobetarget.com/api/#authentication-tokens)。
