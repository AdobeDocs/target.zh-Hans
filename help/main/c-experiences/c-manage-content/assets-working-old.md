...
1. （可选）在[!UICONTROL 卡片视图]和[!UICONTROL 列表视图]之间进行切换，单击内容库右上角的[!UICONTROL 卡片视图]图标或[!UICONTROL 列表视图]图标。 您还可以在查看[!UICONTROL 列表视图]时使用[!UICONTROL 视图设置]进一步配置列。
...
查看[!UICONTROL 代码选件]页面时，您可以对项目执行以下操作（方法是将鼠标悬停在选件或文件夹上，然后选择相应的操作图标）。
...
* **移动**：单击“移动”图标，导航到要将选件或文件夹移动到的位置，然后单击&#x200B;**[!UICONTROL 放置]**&#x200B;图标。 例如，您可以将一个或多个文件夹移动到另一个文件夹以创建子文件夹。 单击[!UICONTROL 清除选定内容]可取消选择您选择的选件或文件夹。
* **删除**：删除选件或文件夹。 请参阅[删除项](#delete)时的注意事项。

## 图像产品建议选项

查看[!UICONTROL 图像选件]页面时，您可以对项目执行以下操作（方法是将鼠标悬停在选件或文件夹上，然后选择相应的操作图标）。

下图显示了查看[!UICONTROL 卡片视图]时悬停的图标。

在卡片视图中，![将图标悬停在“图像选件”选项卡上](/help/main/c-experiences/c-manage-content/assets/image-offers-hover-icons.png)

下图显示了查看[!UICONTROL 列表视图]时悬停的图标。 要显示图标，请单击列表中的项目。
