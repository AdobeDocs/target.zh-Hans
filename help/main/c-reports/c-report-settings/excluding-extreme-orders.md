...
description: 了解如何在Adobe中排除极端值以影响报表 [!DNL Target] 所以一些不寻常的命令不会影响您的活动结果。
title: 如何在报表中排除极端值？
feature: Reports
...
您可以在 [!DNL Adobe Target] 所以一些不寻常的命令不会影响您的活动结果。 例如，与个人购买者购买个人制服相比，一位教练为整个团队购买统一制服可能会产生一个超常订单。
...
如果某个值被标记为极端值，则其订单值会被替换为上个月体验的平均订单值（排除极端值后计算出来的平均值）。在 [!UICONTROL 订单详细信息] 报表和CSV下载中，查看每日结果。
