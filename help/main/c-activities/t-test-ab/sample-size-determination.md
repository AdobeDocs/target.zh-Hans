...
在[!UICONTROL A/B Test]中成功的[!DNL Adobe Target]活动需要足够的访客（样本量）来提高转化率。 您如何知道运行A/B测试的时长？ 本文包含有关[!UICONTROL Auto-Allocate]活动和[!UICONTROL Adobe Target]样本量计算器的信息，可帮助您确保活动具有足够的访客来实现目标。
...
每次比较两个产品建议时，获得误报（即：即使转化率没有差异，也观察到具有统计意义的显著差异）的概率等于显著性水平。例如，如果有五个产品建议 A/B/C/D/E，A 是控制产品建议，且已经完成了四组比较（控制产品建议与 B，控制产品建议与 C，控制产品建议与 D，控制产品建议与 E）；那么即使当置信水平为 95% 时，由于 Pr（至少一个误报）= 1 - Pr（无误报）= 1 - 0.95 = 18.5%，误报的概率也有 18.5%。在这种情况下，误报的定义为：报告的控制产品建议优于替代产品建议，或者报告的替代产品建议优于控制产品建议，但实际上它们之间没有区别。
