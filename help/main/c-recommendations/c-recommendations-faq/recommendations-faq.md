...
* 如果某个项目之前已添加但现在应该排除，则会根据“项目属性已更新……”排除该项目。 上文讨论的时间线取决于信息源来源（通过mbox/API为15分钟，通过信息源为12-24小时）。
...
“标准属性权重”适用于大部分（但不是全部）标准类型（不仅仅是“内容相似度”）。 此类权重会为某些属性值赋予更大的权重。 在以下示例中，输出推荐中增加了耐克产品。
