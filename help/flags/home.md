...
**查看新功能的控件。** Target将发布到特定用户、组织、区域或自定义属性。 从一小部分开始，验证体验，然后展开 — 全部从控制台中展开，无需更改代码。

**运行A/B试验。**&#x200B;向受众的不同区段提供表现更好的不同变体。 在完整版本发布之前，使用结果做出明智的产品决策。

**降低发布风险。**&#x200B;将大型更改分解为更小的、受控的转出。 如果出现错误或性能问题，请仅禁用受影响的功能 — 应用程序的其余部分将保持稳定。

**跨应用程序协调。**&#x200B;功能组允许您一起管理多个功能标志，从而跨应用程序共享通用转出受众。

**导出您的数据。**&#x200B;将标记数据导出到您的首选报表环境，以便与其他Adobe数据一起进行分析和测量。
