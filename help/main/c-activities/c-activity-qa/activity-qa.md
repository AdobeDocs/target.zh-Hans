...
* [!UICONTROL 活动QA] 有粘性。 在中浏览网站之后 [!UICONTROL 活动QA]，您的 [!DNL Target] 会话必须过期或您必须拥有 [!DNL Target] 从以下位置释放您 [!UICONTROL 活动QA] 以典型访客身份查看您的网站之前。
   ...
   * **[!DNL Adobe Experience Platform Web SDK]**：如果您的网站具有 [[!UICONTROL 平台Web SDK]](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/aep-web-sdk.html){target=_blank} 部署后，您可以通过以下方式手动强制自己退出该模式：在网站上加载页面，其中 `at_qa_mode` 具有空值的参数(例如， `https://www.mysite.com/?at_qa_mode=`)。
