---
keywords: 远程选件；远程选件选择矩阵；缓存的内容；动态内容；URL类型
...
>[!NOTE]
>
>远程选件可在以下位置创建： [!UICONTROL 选件] > [!UICONTROL 代码选件] 页面或中的 [基于Forms的体验编辑器](/help/main/c-experiences/form-experience-composer.md). 您无法在可视化体验编辑器(VEC)中创建或应用远程选件。 内容将插入到 [!DNL Target] 请求位置，因此这些位置可能不适合全局 [!DNL Target] 请求。
>
...
* 如果选件与位于同一域中， [!DNL Target] 请求，使用 [!UICONTROL 已缓存] 选项允许您在描述选件位置时使用相对URL。
...
* 使用 [远程选件选择表](#reference_B23BEDD29DDD47709A7651AFD27E776B) （下文）以帮助您选择最适合您特定案例的优惠。 如果您有任何问题，请咨询客服专员。
...
1. 指定重定向URL类型。
   ...
   参见 [重定向URL类型：缓存或动态](#url-type) 有关更多信息，请参阅下文。
...
缓存的远程选件提供了增强的安全性，因为有人登录到 [!DNL Target] 无法更改内容。 要更改内容，用户需登录到内容管理系统或其他系统并在其中更改内容。
...
当访客加载的网站具有包含远程选件的体验时，您可能不希望由 [!DNL Target] 定期缓存并交付内容。相反，您需要调用托管内容的系统，可能传入特定信息，以便为每个用户返回动态（或不同的）选件。 例如，如果某位用户登录到一个信用卡网站，且该网站中具有包含动态远程选件的体验，则您能够将相应参数传递到 URL 中，以获取该用户的帐户信息。随后，网站便会提供特定于该用户的信息，例如账户余额。

您可以单击 **[!UICONTROL 添加参数]** 添加一个或多个 [!DNL Target] 请求或请求参数。

## 在活动中使用远程选件

您必须使用应用远程选件 [!UICONTROL 基于表单的体验编辑器]. 您当前无法使用VEC应用远程选件。

此 [!DNL Adobe Target] [!UICONTROL 基于表单的体验编辑器] 是一个非可视化体验和选件创建界面，在创建在中使用的体验时非常有用。 [!UICONTROL A/B测试]， [!UICONTROL 体验定位] (XT)， [!UICONTROL Automated Personalization] (AP)，和 [!UICONTROL Recommendations] 当可视化体验编辑器不可用或不实用时的活动。 例如，您可以使用 [!UICONTROL 基于表单的体验编辑器] 以创建使用远程选件的体验。
...
1. 单击 **[!UICONTROL 内容]** 部分，然后单击 **[!UICONTROL 更改远程选件]**.

   ![更改远程选件选项](/help/main/c-experiences/c-manage-content/assets/change-remote-offer.png)

1. 从中选择所需的远程选件 [!UICONTROL 选择远程选件] 对话框，然后单击 **[!UICONTROL 完成]**.
...
选件会在您提交页面后执行。不可见的iframe收集数据，将其从框架中复制出来，然后插入到页面上，从而加载您传递的值。

![remote_offer_howitworks_2图像](assets/remote_offer_howitworks_2.jpeg)

## 远程选件选择矩阵 {#reference_B23BEDD29DDD47709A7651AFD27E776B}
