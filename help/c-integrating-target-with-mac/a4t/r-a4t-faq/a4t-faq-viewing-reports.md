...
description: 查找在使用目标分析(A4T)时查看报告时常被问到的问题的答案。 A4T允许您对目标报告使用Analytics活动。
...
如果对该活动进行了分类，并且报表中仍显示“未指定”行，请确保报表未使用非[!DNL Target]度量显示数据。 除非报告使用[!DNL Target]特定的度量，否则“未指定”行将包含与未与[!DNL Target]关联的调用的事件。 该行不包含任何与[!DNL Target]相关的信息(例如访客/访问／展示次数)。
