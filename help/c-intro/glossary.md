...
一个参与量度，它根据分配给站点上访问的页面的值计算汇总分数，从访客第一次看到活动的第一个显示mbox开始计算。
...
记录测试或活动结果，如注册、订单或销售。通常情况下，转化 mbox 不显示任何内容，而只是进行监听。转化 mbox 的常见位置是注册完成页、订单感谢页或点进处。您还可以使用全局 mbox 计算转化量度。
...
可以属于任何类型的测试或活动。无需编写任何附加代码。在您域上的访客会话都将保留。

**dynamic mbox（动态 mbox）**

使用DHTML和AJAX等技术，使富Internet应用程序(RIA)能够在页面加载后操作HTML。
...
访客在站点中的兴趣，根据网站上的时间、点击量、转化率和其他互动指标衡量访客的兴趣。
...
允许您根据访客的地理位置定位活动和体验。也称为地理位置定位参数。
...
登陆页面活动允许您使用定位来显示不同访问的不同登陆页面内容。否则，登陆页将对每次访问均显示相同的内容。登陆页面活动会比较页面的不同版本，帮助您了解登陆页面的哪个版本可以产生更成功的结果。在 Target Standard 中，被“体验定位”**所取代。
...
在页面（而不是 URL）中传递。参数用于传递数据库中的数据，以及跟踪销售数据或运行Adobe Recommendations。也称为页面参数。
...
选件是活动和测试期间mbox中显示的内容。体验决定了满足特定条件时显示的选件。
...
一个参与量度，用于测量访客第一次看到活动的第一个展示mbox时访问的页面数。
...
将值作为引荐 URL 参数和值传递给目标页面的 mbox。例如，使用这些数据定位访客的源。
...
报告提供有关活动进度和结果的信息，可帮助您根据数据做出决策。报告数据可帮助您确定何时结束测试，向您展示哪个体验或选件是入选方，并提供确定后续行动所需的洞察。

## S {#section_9482B06EBB09486784492F838289E54D}

**script profile parameter（脚本配置文件参数）**

类似于mbox配置文件参数，但使用在Target后端上运行而不是浏览器中运行的JavaScript定义。

**segment（区段）**

用于定位活动的指定标准集。只有符合标准的访客才会看到针对该区段的活动的内容。某些区段可在多个活动中重复使用，而其他区段则特定于某个活动。
...
用于衡量活动是否成功的参数。成功量度包括关键业务措施，这些措施使您能够确定给定体验或选件中给定体验的成功与否。例如，您可以确定新选件还是向购物车添加物品，从而提高每个访客的收入。成功量度可用于发现注册、订购或购买漏斗的问题，但也只能与访客或客户互动相关。

## T {#section_1841144841AF41169AF1E83B90C18292}

**test（测试）**

一种活动，可根据您指定的成功量度比较两个或更多体验，以便选择最有可能提供所需结果的体验。

**time on site（网站逗留时间）**

一种参与类型，表示从访客看到活动的第一个显示mbox到带有mbox的会话中最后一页加载的时间(以秒计)。

## U {#section_6407B81326FF43049D92265746E6C402}

**URL parameters（URL 参数）**

当任何访客查看带有 mbox 的页面时，所有的引荐页面和网页 URL 变量都将被自动地传递到 mbox 中。这些URL变量随后可通过显示定位或报表中的细分过滤器来使用。

## V {#section_0B85CE959BA948CDAC3BAEFB4EEDFAD2}

**visitor（访客）**

访客是指访问您网站的任何人员。根据活动条件评估访客，以确定访客是否包含在活动中。请参阅“参加者”**。

**可视化体验编辑器**

Target Standard是一个WYSIWYG用户界面，可让您在站点上下文中轻松创建和测试个性化体验和选件。您可以通过拖放、交换和修改网页的布局和内容，为Target活动创建体验和选件。
