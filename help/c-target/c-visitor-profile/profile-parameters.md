---
description: 配置文件属性是特定于访客的参数。这些属性存储在访客的配置文件中，提供了可在您的营销活动中使用的关于访客的信息。
keywords: 配置文件脚本;配置文件脚本属性;配置文件脚本最佳实践;调试
seo-description: 配置文件属性是特定于访客的参数。这些属性存储在访客的配置文件中，提供了可在您的营销活动中使用的关于访客的信息。
seo-title: 配置文件属性
solution: Target
title: 配置文件属性
...
配置文件属性是特定于访客的参数。这些属性存储在访客的配置文件中，提供了可在您的营销活动中使用的关于访客的信息。

## 配置文件属性 {#concept_01A30B4762D64CD5946B3AA38DC8A201}

配置文件属性是特定于访客的参数。这些属性存储在访客的配置文件中，提供了可在您的营销活动中使用的关于访客的信息。

当访客浏览您的网站或者返回网站进行另一个会话时，可使用保存的配置文件属性定位内容或记录信息以便细分客户群。
...
* 参数和值区分大小写。请与营销活动或测试期间您接收的参数和值的大小写进行匹配。
* 请参阅下面的“脚本配置文件参数的JavaScript参考”部分以了解更多JavaScript语法。

## 查看配置文件脚本信息卡 {#section_18EA3B919A8E49BBB09AA9215E1E3F17}
...
## Target在某些情况下禁用配置文件脚本 {#section_C0FCB702E60D4576AD1174D39FBBE1A7}
...
## 最佳做法 {#section_64AFE5D2B0C8408A912FC2A832B3AAE0}
...
## 个人资料脚本常见问题解答 {#section_1389497BB6D84FC38958AE43AAA6E712}
...
## 脚本配置文件参数的JavaScript参考

需要简单的Javascript知识才能有效使用脚本配置文件参数。此部分用作快速参考，只需几分钟即可利用此功能实现高效工作。

“脚本配置文件参数”位于mbox/配置文件选项卡下。您可以编写返回任何Javascript类型(字符串、整数、数组等)的Javascript程序。
...
创建一个以毫秒为单位的天变量。如果mbox名称是 `orderThankyouPage`，则将名为 `lastPurchaseTime` nocle(不可见)的用户配置文件属性设置为采用当前日期和时间的值。将读取上次购买时间的值，如果定义，我们将返回自上次购买时间起所通过的时间，除以一天中的毫秒数(结果导致上次购买后的天数)。
...
创建一个名为频率的变量，将其初始化为上一个值或0(如果没有之前的值)。如果mbox名称为 `orderThankyouPage`，则返回递增的值。
...
创建一个调用 `monetaryValue`的变量，查找给定访客的当前值(如果没有以前的值，则设置为0)。如果mbox名称是 `orderThankyouPage`，则通过添加前一个值和传递给mbox的 `orderTotal` 参数的值返回新的货币值。
...
| --- | --- |
| `page.url` | 当前URL。 |
| `page.protocol` | 用于页面(http或https)的协议。 |
| page.domain | 当前URL域(第一个斜杠之前)。`www.acme.com``http://www.acme.com/categories/men_jeans?color=blu e&size=small`例如，在。 |
| `page.query` | 当前页面的查询字符串。完成后的所有内容？.`blue&size=small``http://www.acme.com/categories/mens_jeans?color=blue&size=small`例如，在。 |
| `page.param(‘<par_name>’)` | 表示的参数的值 `<par_name>`。如果您当前的URL是Google的搜索页面和您输入 `page.param('hl')`的内容，您将获得URL的“en” `http://www.google.com/search?hl=en& q=what+is+asdf&btnG=Google+Search`。 |
| `page.referrer` | 上述操作集适用于引介和登陆(即referrer. url将是引用方的url地址)。 |
| `landing.url`, `landing.protocol`, `landing.query`, 和 `landing.param` | 类似于页面，但对于登陆页面也是如此。 |
| `mbox.name` | 活动mbox的名称。 |
| `mbox.param(‘<par_name>’)` | 处于活动mbox中给定名称的mbox参数。 |
| `profile.get(‘<par_name>’)` | 客户端创建的用户配置文件参数(按名称 `<par_name>`)。例如，如果用户设置了名为“性别”的配置文件参数，则可以使用“profile.性别”提取该值。返回为当前访客设置的“`profile.<par_name>`”值；如果未设置值，则返回null。 |
| `user.get(‘<par_name>’)` | 返回为当前访客设置的“`user.<par_name>`”值；如果未设置值，则返回null。 |
| `user.categoryAffinity` | 返回最佳类别的名称。 |
| `user.categoryAffinities` | 返回具有最佳类别的数组。 |
| `user.isFirstSession` | 如果是访客的第一个会话，则返回true。 |
| `user.browser` | 返回HTTP头中的用户代理。例如，您可以创建定位以仅定位Safari用户的表达式： `if (user.browser != null && user.browser.indexOf('Safari') != -1) { return true; }` |

### 常用运营商


所有标准JavaScript操作符都具有和可用。JavaScript操作符可用于字符串和数字(以及其他数据类型)。快速简介：

| 运算符 | 描述 |
| --- | --- |
| `==` | 表示相等。当任一边上的操作数相等时保持true。 |
| `!=` | 表示不平等。当任一边上的操作数不相等时保持true。 |
| `<` | 指示左侧的变量小于右侧的变量。如果变量相等，将评估为false。 |
| `>` | 指示左侧的变量大于右侧的变量。如果变量相等，将评估为false。 |
| `<=` | 相同， `<` 除非变量相等，否则它将评估为true。 |
| `>=` | 相同， `>` 除非变量相等，否则它将评估为true。 |
| `&&` | 逻辑上“AND”左侧和右侧的表达式-只有当双方为true时才是true(否则，才为false)。 |
| `||` | 逻辑上“ORS”左侧和右侧的表达式-如果其中某个侧边为true，则仅为true(否则)。 |
| `//` | 检查源是否包含目标布尔型中包含的所有元素(Array源、Array目标)。<br>`//` 从目标中提取子字符串(对应于regexp)并解码它 `Array/*String*/ decode(String encoding, String regexp, String target)`。<br>该功能还支持使用常数字符串值、分组(`condition1 || condition2) && condition3`和正则表达式(`/[^a-z]$/.test(landing.referring.url)`. |
