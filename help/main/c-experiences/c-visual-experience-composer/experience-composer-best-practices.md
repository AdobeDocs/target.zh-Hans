...
description: 了解最佳实践，以使您的体验在Adobe Target中使用可视化体验编辑器(VEC)时按预期工作。
title: 可视化体验编辑器的最佳实践和限制是什么？
...
以下最佳实践有助于确保您的体验按预期运行。在[!DNL Adobe Target]中使用可视化体验编辑器(VEC)时，您还应了解其他提示和限制。
...
**如果增强型可视化Experience Composer无法在您的网站的安全页面上加载，则您可以列入允许列表某些IP地址。**
...
**添加多个页面时，会同时打开体验边栏和页面边栏。这会缩小可视化体验编辑器的宽度，以显示要进行优化的网站。因此，可重新整理站点在减少的空间中可能开始出现与预期不同的外观。**
