...
如果不希望进行全局设置并且希望使用按需方法，请使用at.js函数 [getOffers()](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-getoffers-atjs-2.html){target=_blank} 通过传递 **analytics日志记录：&quot;client_side&quot;**. 仅会为此调用返回分析有效负载，并且 [!DNL Target] 后端不会将有效负载转发到 [!DNL Analytics]. 通过采用这种方法，每个at.js [!DNL Target] 默认情况下，请求会返回有效负载，但只有在需要和指定时才会返回。
