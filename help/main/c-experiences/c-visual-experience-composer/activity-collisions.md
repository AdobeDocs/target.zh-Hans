...
[!DNL Adobe Target]中的[!UICONTROL 活动概述]页面上的[!UICONTROL 冲突]选项卡列出了您网站上的活动冲突。

当多个活动被设置为将内容发布到同一页面时，就会发生活动冲突。 如果发生活动冲突，您可能无法在页面上看到预期的内容。

“活动概述”页面上提供了“[!UICONTROL 冲突]”选项卡，该选项卡可以通知您活动是否包含潜在的冲突。

要访问[!UICONTROL 冲突]选项卡，请单击&#x200B;**[!UICONTROL 活动]** >从列表中单击所需活动>然后单击左边栏中的&#x200B;**[!UICONTROL 冲突]**。
...
[!UICONTROL 冲突]列表显示每个使用mbox且使用相同URL的[!DNL Target]方案。 对于每个潜在冲突，此列表都会显示活动URL、发生冲突的mbox名称以及任何符合这两个条件的活动。 如果存在多个 mbox，则会列出每个 mbox。
