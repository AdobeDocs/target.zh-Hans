...
description: 了解 [!DNL Target] 如何使用TLS（传输层安全）协议来维护最高的安全标准并提升客户数据的安全性。
...
>[!IMPORTANT]
>
>在2020年3月1日之后，Adobe Target将不再支持针对Visual Experience Composer(VEC)、Enhanced Experience Composer(EEC)、活动投放、API等的TLS 1.1加密。 请在2020年3月1日之前升级到TLS 1.2，以避免出现任何问题。
...
但是，如果您是使用TSL 1.1但不支持TLS 1.2的客户之一，则您应计划将域/基础结构移动到TLS 1.2。在2020年3月1日之前，我们将继续支持TLS 1.1协议。 从2020年3月1日开始，目标将不支持将通过增强体验书写器功能用于VEC的TLS 1.1协议。

尽管我们强烈建议大家今后使用 TLS 1.2，但如果您是新客户且确实“不”**&#x200B;支持 TLS 1.2，请联系客户关怀团队，告知他们您需要对增强型体验编辑器使用 TLS 1.1。但是，请计划迁移到TLS 1.2，因为2020年3月1日之后将不支持您。

## 活动投放{#section_46CA5943E4354B259014C2BF340AECD6}

从2020年3月1日开始，目标服务器将不再支持TLS 1.1。通过此更改，目标服务器将不再接受来自使用不支持TLS 1.2（或更高版本）的旧设备或Web浏览器的访客的请求。 因此，仅支持 TLS 1.1（或默认支持 TLS 1.1）的旧设备和浏览器将不会收到来自 Adobe Target 的活动内容。网站的默认内容将会呈现出来。

一些将受到影响的旧设备和浏览器包括：

* Google Chrome(Chrome for Android)版本29及更早版本
* Opera Browser(Opera Mobile)12.17版及更早版本
* Mozilla Firefox（适用于移动设备的Firefox）26及更早版本
* Android 4.3 及更早版本
...
从2020年3月1日开始，目标 API将不再支持TLS 1.1加密。 访问该 API 的客户应当确认他们不会受到影响。
...
1. 打开“Security（安全）”选项卡，然后在“Connection（连接：）”下检查TLS版本信息

   ![TLS版本详细信息](/help/c-implementing-target/c-considerations-before-you-implement-target/assets/chrome-tls-version.png)

>[!NOTE]
>
>这些说明自发布之日起为最新说明，并且可能发生更改。 如果这些说明发生更改，快速Internet搜索应会有所帮助。  其他浏览器具有类似步骤。

## 支持TLS版本低于1.2 {#section_B5DA97A34EF248EB927610A5DA71EF2F}的浏览器的预期行为

本节介绍仅在使用at.js或mbox.js实施时，支持1.2以下TLS版本的浏览器所应具备的条件。 为便于比较，本节还介绍支持TLS 1.2的浏览器的预期。
...
### 活动针对浏览器版受众（Internet Explorer、6、7或8版）
