---
keywords: 实现；javascript库；js;atjs；设备上决策；设备上决策；规则伪像
description: 了解设备决策使用的JSON规则对象。
title: 在设备上决策中，JSON规则对象有什么作用？
feature: at.js
role: Developer
...
规则对象是[!DNL Adobe Target]设备上决策活动的JSON表示形式。 规则对象由[!DNL Target]生成并传播到Akamai CDN，以确保存在尽可能接近您的访客的规则对象。 规则对象包含元数据，可确保活动的精确执行和投放，同时允许通过事件跟踪进行实时分析。
