...
1. 安装 at.js 2.x。
   ...
   Download at.js 2.x via the Adobe Target UI located in [!UICONTROL Administration > Implementation]. at.js 2.x 也可以通过 Adobe Launch 进行部署。但是，Adobe Target 扩展当前不是最新的，不受支持。
...
at.js 2.x API允许您以多种方 [!DNL Target] 式自定义实施，但在此过程中遵循正确的操作顺序很重要。
...
| 6 | 拨叫其他 `triggerView({"page": false})` | 这是SPA框架的可选步骤，它可能在不发生视图更改的情况下重新呈现页面上的特定组件。 在这种情况下，您必须调用此API，以确保在SPA框 [!DNL Target] 架重新渲染组件后重新应用体验。 您可以按期望的次数执行此步骤，以确保体验 [!DNL Target] 在SPA视图中持续存在。 |
...
| 2 | 通过调用API更新缓 `getOffers()` 存 | 如果此视图更改有可能使当前访客符合更多活动的条件或使其不符合活动的条件，则 [!DNL Target] 这是可选的步骤。 此时，您还可以选择发送其他数据，以 [!DNL Target] 实现进一步定位功能。 |
| 3 | 调用 `triggerView()` | 如果已执行步骤2，则必须等待请 [!DNL Target] 求并应用优惠以缓存，然后执行此步骤。 每个视图只能执行此步骤一次。 |
| 4 | 调用 `triggerView()` | 如果您尚未执行步骤2，则您可以在完成步骤1后立即执行此步骤。 如果已执行步骤2和步骤3，则应跳过此步骤。 每个视图只能执行此步骤一次。 |
| 5 | 调用页 [!DNL Analytics] 面视图信标 | 此信标将与步骤2、3和4关联的SDID发送至 [!DNL Analytics] 数据拼接。 |
| 6 | 拨叫其他 `triggerView({"page": false})` | 这是SPA框架的可选步骤，它可能在不发生视图更改的情况下重新呈现页面上的特定组件。 在这种情况下，您必须调用此API，以确保在SPA框 [!DNL Target] 架重新渲染组件后重新应用体验。 您可以按期望的次数执行此步骤，以确保体验 [!DNL Target] 在SPA视图中持续存在。 |
...
### 在SPA教程徽章中实 ![施at.js 2.x](/help/assets/tutorial.png)

>[!VIDEO](https://video.tv.adobe.com/v/26248)

请 [参阅在单页应用程序(SPA)中实施Adobe Target的at.js 2.x](https://helpx.adobe.com/target/kt/using/atjs2-single-page-application-technical-video-implement.html) 以了解更多信息。
