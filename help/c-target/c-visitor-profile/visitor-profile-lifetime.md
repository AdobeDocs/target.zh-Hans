...
默认情况下，访客配置文件会存储 14 天。配置文件生命周期可延长。
...
您使用的 [!DNL Target] JavaScript 库（[!DNL mbox.js] 或 [!DNL at.js]）决定了是否需要下载新文件：
...
如果您在一个客户代码下有两个网站，并且访客访问了这两个网站，则将配置文件设置为最后访问的网站上的配置文件的生命周期。例如，如果网站 1 上的配置文件生命周期为 84 天，而网站 2 上的生命周期为 14 天，且访客是先访问网站 1，然后再访问网站 2，则访客的配置文件会在 14 天后过期。但如果访客先访问网站 2，然后再访问网站 1，则访客的配置文件会在 84 天后过期。
