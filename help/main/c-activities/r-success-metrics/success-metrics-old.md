...
默认情况下，转化事件设置为“[!UICONTROL 递增计数并保持用户处于活动中]”。 转化次数只计算一次，不计算重复转化次数，访客始终会看到活动内容。

设置为“[!UICONTROL 递增计数并保持用户处于活动中]”的收入量度仅记录同一访客发出的第一个订单的详细信息。 所有后续订单都会增加转化计数，但不会将收入添加到RPV/AOV/销售中，并且不会包含在[!UICONTROL 订单详细信息]报表中。

>[!NOTE]
>
>对于使用[Analytics作为报表源](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T)的活动，目标量度将始终使用“[!UICONTROL 递增计数并保持用户处于活动中]”和“[!UICONTROL 每次展示]”设置。 这是&#x200B;*不可配置的*。
...
某些量度（如[!UICONTROL 自定义得分]和[!UICONTROL 每位访客带来的收入]）需要自定义实施，这些实施需要传递订单总数和订单ID等信息。

## 高级设置 {#section_7CE95A2FA8F5438E936C365A6D43BC5B}

可使用高级设置来管理成功的衡量方式。 选项包括添加依赖项、选择是将用户保留在活动中还是将其删除，以及是为每个参加者还是每次展示都计算一次量度。

要访问[!UICONTROL 高级设置]选项，请单击&#x200B;**[!UICONTROL 垂直省略号]** > **[!UICONTROL 高级设置]**。

![“高级设置”菜单](/help/main/c-activities/r-success-metrics/assets/advanced-settings.png)

>[!NOTE]
>
>如果您将 [!DNL Adobe Analytics] 用作报表源，则设置会由 [!DNL Analytics] 服务器来管理。 [!UICONTROL 高级设置]选项将不可用。 有关详细信息，请参阅将[Adobe Analytics作为Adobe Target (A4T)的报表源](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。
