...
description: 了解如何使用基于键的推荐，这些键使用访客行为上下文在[!UICONTROL 推荐]活动中显示相关结果。
title: 如何使[!UICONTROL 推荐]基于[!UICONTROL 推荐键]？
...
* **[!UICONTROL 单个会话]**：基于其他访客在单个会话中所执行的操作。
...
以下算法可用于[!UICONTROL 基于购物车的]算法类型：

### [!UICONTROL 查看了这个项目，也查看了]的人员
...
[!UICONTROL 基于热门程度的]算法类型允许您根据项目在整个网站中的整体热门程度或根据用户最喜爱或查看次数最多的类别、品牌、流派等中的项目热门程度进行推荐。

以下算法可用于[!UICONTROL 基于热门程度的]算法类型：

### 整个网站查看的次数最多 {#most-viewed}
...
### [!UICONTROL 网站上的最畅销商品] {#top-sellers}
...
>title="轮廓属性"
>abstract="您可以使用轮廓脚本创建轮廓属性。 创建并激活一个轮廓脚本后，就可以与这个算法一起使用与该脚本对应的轮廓属性。"
...
显示“前x个”，其中&#x200B;*x*&#x200B;是任意[!DNL Analytics]量度。 使用来自mbox的行为数据时，您可以使用[!UICONTROL 最畅销]或[!UICONTROL 最常查看]（x =“已出售”或x =“已查看”）。 如果您使用的是来自[!DNL Adobe Analytics]的行为数据，则可以使用x = &quot;购物车添加次数&quot;或一些其他[!DNL Analytics]量度。

## [!UICONTROL 基于项目]

[!UICONTROL 基于项目的]推荐类型允许您根据用户当前正在查看或最近查看的项目的相似项目进行推荐。

以下算法可用于[!UICONTROL 基于项目的]算法类型：

### [!UICONTROL 查看了这个项目，也查看了那个项目的人] {#viewed-viewed}
...
### [!UICONTROL 查看了这个项目，但购买了那个项目的人] {#viewed-bought}
...
### [!UICONTROL 购买了这个项目，也购买了那个项目的人] {#bought-bought}
...
### [!UICONTROL 具有相似属性的项目] {#similar-attributes}

根据当前页面活动或以往的访客行为来推荐类似的项目或媒体。

如果选择[!UICONTROL 具有相似属性的项目]或[!UICONTROL 具有相似属性的媒体]，则可以选择设置内容相似度规则。

使用内容相似度生成推荐对新项目特别有效，对于使用[!UICONTROL 查看了这个项目]、[!UICONTROL 查看了那个项目]的人以及基于过去行为的其他逻辑的推荐，这些项目不太可能显示。 您还可以使用内容相似度为没有过去购买数据或其他历史数据的新访客生成有用的推荐。
...
[!UICONTROL 基于用户的]算法类型允许您根据用户的行为提供推荐。

以下算法可用于[!UICONTROL 基于用户的]算法类型：

### [!UICONTROL 最近查看的项目] {#recently-viewed}

使用访客的历史记录（跨会话）根据设计中的版块数量显示访客最近查看过的 *x* 个项目。

[!UICONTROL 最近查看的项目]算法返回特定于给定[环境](/help/main/administrating-target/hosts.md)的结果。 如果两个网站属于不同的环境，并且访客在两个网站之间切换访问，则每个网站仅显示相应网站的最近查看的项目。 如果两个网站位于同一环境中，并且访客在两个网站之间切换访问，则访客会看到两个网站的最近查看的相同项目。

>[!NOTE]
>
>您无法将[!UICONTROL 最近查看的项目]标准用于备份推荐。

可以筛选[!UICONTROL 最近查看的项目]或[!UICONTROL 最近查看的媒体]，以便仅显示具有特定属性的项目。

* [!UICONTROL 最近查看的项目]标准可配置，与推荐中的其他标准类似。
...
>[!NOTE]
>
>[!UICONTROL 最近查看的项目]兼顾了排除全局设置和活动的选定收藏集设置。 如果某个项目被全局排除项排除在外，或者未包含在选定的收藏集中，则不会显示该项目。 因此，在使用[!UICONTROL 最近查看的项目]标准时，通常应使用“所有收藏集”设置。

### [!UICONTROL 为您推荐] {#recommended-for-you}

根据每位访客的浏览、查看和购买历史记录推荐商品。

通过此算法，您可以为新访客和回访访客提供个性化内容和体验。 推荐列表根据访客最近的活动进行加权，在会话中更新，并随着用户浏览您的网站而更加个性化。

查看次数和购买次数均用于确定推荐的项目。 指定的推荐键（例如，[!UICONTROL 当前项目]）用于应用您选择的任何包含规则筛选器。
...
[!UICONTROL 自定义标准]算法类型允许您根据上传的自定义文件提供推荐。
...
在[!UICONTROL 自定义标准]中添加包含规则后，这会将原本为静态的推荐转变为基于访客兴趣的动态推荐。
...
[!UICONTROL 推荐键]下拉列表中提供了以下推荐键：

### [!UICONTROL 当前项目] {#current-item}
...
使用以下位置使用您网站上的[!UICONTROL 当前项目]推荐键：

* 单项目页面，例如产品页面。
* 请勿在空的搜索结果页面上使用。

### [!UICONTROL 上次购买的项目] {#last-purchased}
...
使用以下位置上使用您网站上的[!UICONTROL 上次购买的项目]推荐键：
...
1. 从&#x200B;**[!UICONTROL 推荐键]**&#x200B;下拉列表中选择自定义配置文件属性（例如，“[!UICONTROL 上次添加到观看列表的节目]”）。
1. 然后选择您的&#x200B;**[!UICONTROL 推荐逻辑]**（例如，[!UICONTROL 查看了这个项目，也查看了那个项目]的人员）。
...
1. 然后选择要与此键一起使用的&#x200B;**[!UICONTROL 推荐逻辑]**（例如，“[!UICONTROL 最畅销商品]”）。

   显示[!UICONTROL 按唯一值]分组选项。

1. 选择与您所选的键匹配的实体属性。 在这种情况下，“[!UICONTROL 最喜爱的品牌]”与`entity.brand`匹配。

   [!DNL Recommendations]现在为每个品牌生成“[!UICONTROL 最畅销商品]”列表，并根据访客的[!UICONTROL 最喜爱的品牌]配置文件属性中存储的值向访客展示相应的“[!UICONTROL 最畅销商品]”列表。

### [!UICONTROL 上次查看的项目] {#last-viewed}
...
使用您网站上的[!UICONTROL 上次查看的项目]推荐键：

* 主页，“我的帐户”页面，站外广告。
* 请勿在产品页面或与购买相关的页面上使用。

### [!UICONTROL 查看次数最多的项目] {#most-viewed-logic}
...
在网站上使用[!UICONTROL 当前类别]推荐键：

* 单类别页面。
* 请勿在空的搜索结果页面上使用。

### [!UICONTROL 最喜爱类别] {#favorite-category}
...
### [!UICONTROL 网站亲和度] {#site-affinity}

根据项目间关系的确定性来推荐项目。 您可以配置此标准，以确定需要多少数据，才能使用“[!UICONTROL 包含规则]”滑块显示推荐。 例如，如果您选择“非常强”，则会推荐具有最强匹配确定性的产品。
