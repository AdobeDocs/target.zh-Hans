...
导入源文件后，或通过API或mbox接收实体更新后，以下更改将在60分钟内反映出来：

* 在设计模板中返回的项目属性。
* 全局排除规则中使用的项目属性，用于防止项目包含在返回建议中。
* 在包含规则的条件中使用的项目属性，这些规则会影响返回建议中包含或排除的项目。

以下更改在执行下一个算法运行之前不会反映(在12-24小时内)：

* 用于活动的集合规则中使用的项目属性。
* 促销中使用的项目属性，基于与活动关联的属性或集合。
* 项目在“最热门的卖家”或“最常查看的算法”中显示的项目类别。
* 当属性更改时，建议的项目排名是用作算法自定义密钥的自定义属性。
* 根据推荐逻辑(当推荐逻辑为“具有类似属性”的项目时)、使用“内容相似性”加权因子或使用“属性权重”因子时的推荐项目排名。

>[!NOTE]
>
>当源文件的状态更改从“导入项目”变为“准备搜索索引更新”时，将被视为导入。更新可能需要超过60分钟才能反映在目录搜索用户界面中；当源状态变为“已完成更新”时，目录搜索处于最新状态。即使目录搜索尚未更新，您的站点仍会反映上面列出的时间范围内的更新。最新目录搜索索引更新时间将显示在“目录搜索”页面上。
