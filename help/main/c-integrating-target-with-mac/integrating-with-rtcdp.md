...
#### Adobe Experience Platform Web SDK或Experience Platform服务器端API实施
...
>[!NOTE]
>
>Real-time CDP Profile Attributes功能在测试版中提供，可用于HTML选件和 [JSON选件](/help/main/c-experiences/c-manage-content/create-json-offer.md).

请考虑以下事项：

* 给定选件中的属性必须来自同一Experience Platform沙盒。 (换言之，选件不能包含来自不同Experience Platform沙箱的属性。)
* 给定选件内的属性可能来自不同的来源；即 [!DNL Target] 配置文件和Experience Platform配置文件。 (换言之，无论属性来自 [!DNL Target] 或从Experience Platform配置文件中。)
...
作为在线营销人员，您希望AEP/统一配置文件与共享属性值 [!DNL Target] 提供实时个性化。 通过使用Real-time CDP Profile Attributes，您可以在 [!DNL Target] 选件。 例如，您可以使用 `${aep.profile.favoriteColor}`，或者使用令牌的忠诚度等级和忠诚度点值 `${aep.loyalty.tier}` 和 `${aep.loyalty.points}`.
