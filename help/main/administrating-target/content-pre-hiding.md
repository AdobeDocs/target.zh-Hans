...
1. [!DNL Target]通过实时[!UICONTROL 可视化体验编辑器]和[!UICONTROL 增强型体验编辑器]活动构建规则集。 规则集列出了投放可能更改的选择器和区域。

   请注意，不支持[!UICONTROL 基于表单的编辑器]活动。

1. 库将从Adobe CDN中提取该规则集，并仅在个性化内容仍在加载时预隐藏匹配元素。

1. 在&#x200B;**[!UICONTROL 目标和设置]**&#x200B;中，您可以为各个活动禁用&#x200B;**[!UICONTROL 内容预隐藏]**，但前提是在帐户级别启用它。 [了解详情](#content-pre-hiding-activity)
...
在启用内容预隐藏之前，它将对实例处于关闭状态。 使用&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 实施]**&#x200B;启用该功能，设置默认值，并访问实施团队的下载内容。

1. 在[!DNL Target]中，单击&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 实现]**。

1. 从&#x200B;**[!UICONTROL 内容预隐藏]**&#x200B;菜单中，启用内容预隐藏选项。

   ![](assets/content-pre-hiding-1.png)

1. 如果需要，请更新&#x200B;**[!UICONTROL 预隐藏超时]**（以秒为单位）。
...
在为实例启用预隐藏的情况下，选择每个活动是否在&#x200B;**[!UICONTROL 目标和设置]**&#x200B;中使用它。 为其启用预隐藏的活动在活动时包括在目标行为中。

然后，[!DNL Target]根据在[!UICONTROL 可视化体验编辑器] (VEC)和[!UICONTROL 基于表单的编辑器]中创作的实时活动构建轻量级规则集，其中描述了投放可以更改的选择器和区域。

创建或编辑活动时：

1. 访问要启用预隐藏选项的活动。

1. 访问&#x200B;**[!UICONTROL 编辑活动]**&#x200B;下拉列表并选择&#x200B;**[!UICONTROL 编辑目标和设置]**。

   ![](assets/content-pre-hiding-3.png)

1. 从&#x200B;**[!UICONTROL 内容预隐藏]**&#x200B;菜单中，打开&#x200B;**[!UICONTROL 启用内容预隐藏]**&#x200B;选项以选择加入或退出预隐藏。
