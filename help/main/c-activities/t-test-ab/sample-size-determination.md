...
[自动分配](/help/main/c-activities/automated-traffic-allocation/automated-traffic-allocation.md)活动是一种A/B测试，可在两个或更多体验中标识入选者。 [!UICONTROL 自动分配]测试会自动为入选者重新分配更多流量以提高转化，同时测试会继续运行和学习。
...
解决方案是[!UICONTROL 自动分配]。 [!UICONTROL 自动分配]降低了确定入选体验的成本和开销。 [!UICONTROL 自动分配]监视所有体验的目标量度性能，并相应地向高性能体验发送更多新参加者。 同时，也会保留足够的流量来探索其他体验。 即使活动仍在运行，您也可以从结果中看到该活动的好处：优化与学习同时进行。

[!UICONTROL 自动分配]将访客逐步推向入选体验，而不是要求您等到活动结束才能确定入选者。 您可以更快地从提升度中受益，因为原本将被发送到不太成功的体验的活动参加者现在会看到潜在的入选体验。

使用[!UICONTROL 自动分配]时，[!DNL Target]会在活动页面的顶部显示一个标记，在活动达到最低转化次数，且具有足够的置信度之前，该标记会一直指示“还没有入选者”。 然后，[!DNL Target]通过在活动页面的顶部显示徽章来声明入选体验。
...
如果您选择使用手动[!UICONTROL A/B测试]活动而不是[!UICONTROL 自动分配]，则[!DNL Target]样本量计算器可帮助您确定成功测试所需的样本量。 手动A/B测试属于固定水平测试，因此该计算器会有所帮助。 为[!UICONTROL 自动分配]活动使用计算器是可选的，因为[!UICONTROL 自动分配]为您声明了一个入选者。 计算器会给出所需样本量的粗略估计。 继续阅读可了解有关如何使用计算器的更多信息。
...
样本量计算器（上文提供了链接）可帮助您配置 RPV 量度。 打开计算器时，您将看到一个标签为[!UICONTROL RPV量度]的选项卡。 使用计算器的 RPV 版本时，您需要以下信息：
...
通过使用[!UICONTROL 自动分配]活动，[!DNL Target]在两个或更多体验中标识一个入选者，并在测试继续运行和学习期间，自动为入选者重新分配更多流量以提高转化。 [!UICONTROL 自动分配]使您能够在消除猜测工作的同时轻松实现转化目标。
