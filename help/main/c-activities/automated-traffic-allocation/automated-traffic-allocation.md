...
使用三步引导式工作流[创建A/B活动](/help/main/c-activities/t-test-ab/t-test-create-ab/test-create-ab.md)时，请在&#x200B;**[!UICONTROL 定位]**&#x200B;页面上选择&#x200B;**[!UICONTROL 自动分配到最佳体验]**&#x200B;选项（步骤2）。
...
[!UICONTROL 自动分配]活动可降低确定入选体验的成本和开销。 [!UICONTROL 自动分配]监视所有体验的目标量度性能，并相应地向高性能体验发送更多新参加者。 同时，也会保留足够的流量来探索其他体验。 即使活动仍在运行，您也可以看到该测试对结果带来的好处：优化与学习并行进行。

[!UICONTROL 自动分配]将访客逐步推向入选体验，而不是要求您等到活动结束才能确定入选者。 您可以更快地从提升度中受益，因为原本将被发送到不太成功的体验的活动参加者现在会看到潜在的入选体验。
...
[!UICONTROL 自动分配]跨体验执行所有隐式比较并生成“真”入选者。 测试中没有“控制”体验的概念。

[!UICONTROL 自动分配]智能地将新访客分配给体验，直到最佳体验的置信区间与任何其他体验的置信区间不重叠。 通常此过程可能会产生误报，但[!UICONTROL 自动分配]使用基于[Bernstein不等式](https://en.wikipedia.org/wiki/Bernstein_inequalities_%28probability_theory%29){target=_blank}的置信区间来补偿重复评估。 此时，就会有真正的赢家。 当[!UICONTROL 自动分配]停止时，如果访问页面的访客不存在实质性时间依赖关系，则[!UICONTROL 自动分配]返回其真实响应不比入选体验的真实响应差1%（相对）的体验的概率至少为95%。

## 何时使用[!UICONTROL 自动分配]与[!UICONTROL A/B测试]或[!UICONTROL Automated Personalization]活动 {#section_3F73B0818A634E4AAAA60A37B502BFF9}

* 如果您希望从一开始就优化活动并尽快确定入选体验，则使用&#x200B;**[!UICONTROL 自动分配]**。 通过更频繁地提供高性能体验，可以提高整体活动性能。
* 当您想要在优化网站之前表征所有体验的性能时，请使用标准&#x200B;**[A/B测试](/help/main/c-activities/t-test-ab/test-ab.md#task_05E33EB15C4D4459B5EAFF90A94A7977)**。 A/B测试有助于您对所有体验进行排名，而[!UICONTROL 自动分配]会查找表现最好的体验，但无法保证区分表现较差的体验。
* 如果您希望使用最复杂的优化算法，例如根据个人配置文件属性构建预测的机器学习模型，请使用[Automated Personalization](/help/main/c-activities/t-automated-personalization/automated-personalization.md#task_8AAF837796D74CF893CA2F88BA1491C9)。 [!UICONTROL 自动分配]查看体验的汇总行为（与标准A/B测试类似），并且不区分访客。

## [!UICONTROL 自动分配]的主要优势 {#section_0913BF06F73C4794862561388BBDDFF0}
...
[!UICONTROL 自动分配]背后的整体逻辑同时包含累积数据的测量性能（如转化率）和置信区间。 与流量在体验之间平均分配的标准A/B测试不同，[!UICONTROL 自动分配]会更改跨体验的流量分配。
...
激活[!UICONTROL 自动分配]活动后，不允许从Tar[!DNL]get UI中进行以下操作：

* 将“流量分配”模式切换为“手动”
* 更改目标量度类型
* 更改“[!UICONTROL 高级设置]”面板中的选项
...
在使用[!UICONTROL 自动分配]时请考虑以下信息：

### [!UICONTROL 自动分配]功能仅对一个高级量度设置起作用： [!UICONTROL 递增计数并保持用户处于活动中]

不支持以下高级量度设置：[!UICONTROL 增量计数]、[!UICONTROL 释放用户]、[!UICONTROL 允许再次进入和增量计数]以及[!UICONTROL 释放用户和禁止再次进入]。
...
[!UICONTROL 自动分配]擅长区分高性能体验（和查找入选者）。 可能有些时候，您并没有充分区分性能较差的体验。

如果要在所有体验之间产生统计上的显着差异，您可能需要考虑使用手动流量分配模式。

### 时间相关（或上下文不同）的转化率可能会导致分配数量存在偏差。

在[!UICONTROL 自动分配]活动中，在标准A/B测试中因同等影响所有体验而可忽略的某些因素不能被忽略。 算法易受观察到的转化率的影响。
...
在使用[!UICONTROL 自动分配]活动时查询以下常见问题和答案：
...
使用[!UICONTROL 自动分配]提供80%的流量，随机提供20%的流量。 确定入选者后，80%的流量会流向该入选者，而所有体验仍会在该20%的流量中获取一些流量，包括入选体验。
...
另一方面，[!UICONTROL 自动分配]是一个A/B测试，它将生成一个聚合入选者（最受欢迎的体验，但不一定是每个访客最有效的体验）。
...
### 使用[!UICONTROL 自动分配]时，我能否使用样本量计算器来估算活动确定入选者所需的时间？

您可以使用现有的[!DNL Adobe Target] [样本量计算器](/help/main/c-activities/t-test-ab/sample-size-determination.md#section_6B8725BD704C4AFE939EF2A6B6E834E6)来估计测试运行的时长。 （与传统A/B测试一样，如果您测试多个选件或多个转化量度/假设验证，请应用Bonferroni校正。） 该计算器专为传统的定界A/B测试而设计，仅提供估计值。 为[!UICONTROL 自动分配]活动使用计算器是可选的，因为[!UICONTROL 自动分配]为您声明了一个入选者。 您不需要挑选固定的时间点来查看测试结果。 提供的值在统计上始终有效。

内部[!DNL Adobe]试验发现以下内容：

* 当仅测试两个体验时，当体验之间的性能差异较大时，[!UICONTROL 自动分配]比固定水平面测试（即样本量计算器建议的时间范围）更快地找到入选者。 但是，当体验之间的性能差异较小时，[!UICONTROL 自动分配]可能需要额外的时间来识别入选者。 在这些情况下，固定时程测试通常会在没有统计上显着结果的情况下结束。
* 当测试两个以上的体验时，当单个体验的效果远远超过所有其他体验时，[!UICONTROL 自动分配]比固定水平面测试（即样本量计算器建议的时间范围）更快地找到入选者。 当两个或更多体验都“入选”其他体验，但彼此紧密匹配时，[!UICONTROL 自动分配]可能需要额外的时间来确定哪个体验更优秀。 在这些情况下，固定范围测试通常会得出结论认为“入选”体验优于表现较差的体验，但未确定哪个体验更优秀。

### 我是否应该从[!UICONTROL 自动分配]活动中删除性能不佳的体验，以加快确定入选者的过程？

确实没有必要删除性能不佳的体验。 [!UICONTROL 自动分配]会自动更频繁地提供高性能的体验，而较少地提供性能不佳的体验。 在活动中保留性能不佳的体验不会显着影响确定入选者的速度。

20% 的访客将被随机分配给所有体验。 提供给性能不佳体验的流量很少（20%除以体验数量）。

### 能否在[!UICONTROL 自动分配]活动中途更改目标指标？ {#change-metric}
...
### 能否在[!UICONTROL 自动分配]活动中途更改报表源？ {#change-reporting}
...
### 运行[!UICONTROL 自动分配]活动时能否使用[!UICONTROL 重置报表数据]选项？

建议不要对[!UICONTROL 自动分配]活动使用[!UICONTROL 重置报表数据]选项。 虽然它删除可见的报表数据，但此选项不会从[!UICONTROL 自动分配]模型中删除所有训练记录。 不要对[!UICONTROL 自动分配]活动使用[!UICONTROL 重置报表数据]选项，请创建新活动并停用原始活动。 （本指导还适用于[!UICONTROL 自动定位]和[!UICONTROL Automated Personalization]活动。）

### [!UICONTROL 自动分配]如何针对环境构建模型？

[!UICONTROL 自动分配]仅基于默认环境中记录的流量和转化行为构建模型。 默认情况下，[!UICONTROL 生产]是默认环境，但可以在[!DNL Target]中更改默认环境（[管理>环境](/help/main/administrating-target/environments.md)）。

如果点击发生在其他（非默认）环境中，则根据默认环境中观察到的转化行为来分配流量。 该点击的结果（转化或非转化）已记录用于报告目的，但在[!UICONTROL 自动分配]模型中未考虑。

选择其他环境时，报表会显示该环境的流量和转化率。 报表的默认选定环境是选定的帐户范围的默认环境。 无法为每个活动设置默认环境。

### [!UICONTROL 自动分配]活动能否在测试过程中调整回看窗口以考虑随时间变化趋势？

例如，活动是否可以考虑12月份来决定如何分配流量，而不是查看9月份的访客数据（测试开始的时间）？

否，[!UICONTROL 自动分配]考虑整个活动的性能。

### 如果获胜体验与访客在获得活动资格时看到的不同，则[!UICONTROL 自动分配]是否向回访访客显示获胜体验？

[!UICONTROL 自动分配]使用粘性决策的原因与[!UICONTROL A/B测试]活动具有粘性相同。 流量分配仅适用于新访客。
...
以下视频演示了如何使用 Target 三步引导式工作流创建 A/B 测试。 从4:45开始讨论[!UICONTROL 自动分配]。
