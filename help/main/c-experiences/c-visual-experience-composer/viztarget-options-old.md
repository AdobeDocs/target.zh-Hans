...
编辑[!UICONTROL A/B]和[!UICONTROL 体验定位]活动的文本和HTML时，可以使用多种富文本格式选项。 您可以选择字体、选择字体样式、更改文本对齐方式以及使用其他标准文本格式设置选项。 修改 HTML 时，您可以在 HTML 的代码视图和富编辑视图之间切换。
...
使用[!UICONTROL 样式]面板查看或编辑所选元素的现有样式的值。 您还可以添加其他样式。

要访问[!UICONTROL 样式]面板，请单击VEC中的某个页面元素，然后单击&#x200B;**[!UICONTROL 编辑]** > **[!UICONTROL 样式]**。

[!UICONTROL 样式]面板将显示在VEC的右侧。 该面板包含一个样式列表，允许您编辑样式或将其添加到所选元素。 如果您习惯使用层叠样式表 (CSS)，或者从开发人员那里接收代码，则可以使用实时 CSS 编辑器查看更改并添加样式。

![“样式”面板](/help/main/c-experiences/c-visual-experience-composer/assets/styles-panel-new.png)

应用不同的样式时，在更改任何部分后，您始终可以通过单击显示在[!UICONTROL 样式]面板右上角的[!UICONTROL 还原]图标还原您所做的更改。 单击[!UICONTROL 还原]图标将还原在当前部分的面板上所做的所有更改。

展开每个部分以编辑或添加样式，具体如下文中所述。 要保存更改，请单击面板顶部的[!UICONTROL 返回]图标以返回到面板的主显示屏，然后单击&#x200B;**[!UICONTROL 保存]**。

主面板上以及各个部分面板上每个选项旁边的蓝色圆点表示您已更改了相应的样式。 此视觉指示器可让您在单击[!UICONTROL 保存]之前轻松查看更改。
...
* **[!UICONTROL CSS编辑器]**
  ...
  请注意上图中[!UICONTROL 排版规则]、[!UICONTROL 边框]和[!UICONTROL 大小]选项旁边的蓝点。 这些圆点表示您已更改这些部分。 如果打开这些部分的面板，则蓝色圆点将显示在您更改的特定选项旁边。
...
可用于[!UICONTROL A/B]、[!UICONTROL Automated Personalization]和[!UICONTROL 多变量测试]活动。
...
[!UICONTROL 推荐]支持[!UICONTROL 此项前插入] DIV、SECTION和ARTICLE标记的内容。
...
[!UICONTROL 推荐]支持[!UICONTROL 此项后插入] DIV、SECTION和ARTICLE标记的内容。
...
当前，某些VEC操作（如[!UICONTROL 重新排列]和[!UICONTROL 移动]）假定源父元素和目标父元素的同级元素已完全加载。 如果在父DOM元素（源或目标）下发生延迟加载，则这些VEC操作可能会导致不一致行为。 我们正在寻找一种更可靠的方法，以使VEC操作在延迟加载的DOM元素中正常工作。 作为临时解决方法，您可以在这些场景中使用[!UICONTROL 自定义代码]来呈现您的体验。

### [!UICONTROL 调整大小]

调整页面上的元素大小。 选择[!UICONTROL 调整大小]后，元素的右下角会出现一个句柄，允许您拖动该角调整大小。 按住 Shift 键保持相同的宽高比。
...
有关由于延迟加载DOM元素而导致[!UICONTROL 移动]和[!UICONTROL 重新排列]操作不一致行为的详细信息，请参阅上面的[!UICONTROL 重新排列]。
...
* 如果产品建议包含 HTML 内容，请参阅 [at.js 工作原理](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/at-js/how-atjs-works.html?lang=zh-Hans){target=_blank}中的“at.js 如何渲染包含 HTML 内容的产品建议”了解更多信息。
