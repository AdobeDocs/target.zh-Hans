...
description: 了解如何使用配置文件属性在Adobe [!DNL Target] 中设置测试，该测试比较多个活动，但不允许相同的访客参与每个活动。
...
您可以使用[!DNL Adobe Target]中的配置文件属性来设置测试，以比较两个或更多活动，但不允许相同的访客参与每个活动。
...
在创建配置文件属性后，通过要求用户配置文件参数`user.twogroups`与GroupA指定的值匹配，设置第一个活动以定向所需群体。
