...
使用在中创建的体验片段(XF) [!DNL Adobe Experience Manager] (AEM)in [!DNL Target] 活动来帮助优化或个性化。

>[!NOTE]
>
>在中使用AEM体验片段时，请考虑以下事项 [!DNL Target]:
> 
...
![按体验片段类型过滤：HTML或JSON](/help/main/c-experiences/c-manage-content/assets/fragment-types.png)

您可以将鼠标悬停在列表中的某个体验片段上，然后单击 [!UICONTROL 查看] 图标 ![“查看”图标](assets/icon_info.png) 要查看有关体验片段的其他信息，包括其名称、类型、选件ID、选件路径和上次修改信息。 单击选件使用情况选项卡，以查看引用此选件的活动。

![体验片段信息弹出窗口](/help/main/c-experiences/c-manage-content/assets/xf-info-popup.png)

您可以在 [!DNL Target] 活动 [可视化体验编辑器](/help/main/c-experiences/c-visual-experience-composer/visual-experience-composer.md) (VEC)或 [基于表单的体验编辑器](/help/main/c-experiences/form-experience-composer.md).
...
>
>中不支持体验片段 [!DNL Recommendations] 活动。 但是，要将体验片段用于推荐，您可以创建 [!UICONTROL A/B测试] 活动(包括 [!UICONTROL 自动分配] 和 [!UICONTROL 自动定位])或 [!UICONTROL 体验定位] (XT)活动和 [将推荐作为选件](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md).
...
>[!NOTE]
>
>在中导出为JSON的体验片段 [!DNL Target] 不能在使用VEC创建的活动中使用；基于VEC的活动仅支持HTML体验片段。 如果要使用JSON体验片段，请在使用 [基于表单的体验编辑器](/help/main/c-experiences/form-experience-composer.md).
...
## 从导出到目标的体验片段中移除 ClientLib 和无关 HTML

在 [!DNL Target] 在由AEM交付的页面上，目标页面已包含所有必需的客户端库。 另请注意，选件中不重要的HTML元素也不是必需的。
