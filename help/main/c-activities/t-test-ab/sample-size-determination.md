...
成功 [!UICONTROL A/B测试] 活动 [!DNL Adobe Target] 需要足够的访客（样本量）来提高转化率。 您如何知道A/B测试的运行时间？ 本文包含有关 [!UICONTROL 自动分配] 活动和 [!UICONTROL Adobe Target] 样本量计算器，可帮助您确保活动有足够的访客来实现您的目标。
...
>[!IMPORTANT]
>
>提前停止活动是执行A/B测试时可能会遇到的十大陷阱之一。 有关更多信息，请参阅 [十种常见的A/B测试陷阱以及避免方法](/help/main/c-activities/t-test-ab/common-ab-testing-pitfalls.md#concept_578A7947C9554868B30F12DFF9E3F8E3).

[!DNL Adobe Target] 提供了一些工具，可帮助确保您的活动具有足够大的样本量来实现转化目标：自动分配。

## 自动分配 {#auto-allocate}

安 [自动分配](/help/main/c-activities/automated-traffic-allocation/automated-traffic-allocation.md) 活动是一种A/B测试类型，用于在两个或更多体验中标识入选者。 自动分配测试会在测试继续运行和学习期间，自动为入选者重新分配更多流量以提高转化。
...
解决方案是 [!UICONTROL 自动分配]. [!UICONTROL 自动分配可降低确定入选体验的成本和开销。][!UICONTROL 自动分配会监控所有体验的目标量度性能，并按比例将更多新参加者发送到高性能体验。]同时，也会保留足够的流量来探索其他体验。即使活动仍在运行，您也可以在结果中看到活动的好处：优化与学习并行进行。
...
在 A/B 测试中，最开始假设两种选件均具有相同的转化率。然后根据这个假设计算观察到的结果的概率。如果此概率（p值）小于某些预定义的阈值（显着性级别）， [!DNL Target] 结论是，初始假设（两个选件具有相同的转化率）不正确。 因此，在给定显着性级别，A和B的转化率具有统计学上的差异。
...
![samplesicontrol图像](assets/samplesizecontrol.png)

在此示例中，您可能会决定是否能够检测到5%的提升度(对应于替代选件(100%+5%)的转化率)&#42;5% = 5.25%)，在80个测试中，有80个测试是足够的，因此您需要每个选件的样本量为100,000位访客。 如果网站每天有20,000位访客，并且您正在测试两个选件，则应允许测试运行2个&#42;100,000/20,000 = 10天后才能确定替代选件在统计上是否显着优于控制选件。

再次建议将所需的时间始终四舍五入至最接近的整周，以避免出现周内效应。因此在本例中，在评估结果之前，测试将会运行两周。

### “每次访问带来的收入”量度 {#section_C704C0861C9B4641AB02E911648D2DC2}

当使用每次访问收入(RPV)作为量度时，会添加额外的差异源，因为RPV是每订单收入和转化率的乘积(RPV =收入/ #visitors =（每订单收入） &#42; #orders)/访客数=每订单收入 &#42; (#visitors &#42; CTR)/ #visitors =每订单收入 &#42; CTR)，每个CTR都具有自己的差异。 转化率的方差可以直接使用数学模型进行估计，但每个订单的收入方差是特定于活动的。 因此，请使用过去活动中对此差异的了解，或运行A/B测试几天来估计收入差异。 差异根据CSV下载文件中找到的“销售总和”、“销售总和的平方”和“访客数”的值计算。 建立此模型后，使用电子表格计算完成测试所需的时间。
...
通过使用 [!UICONTROL 自动分配] 活动， [!DNL Target] 在两个或更多体验中标识一个入选者，并在测试继续运行和学习期间自动为入选者重新分配更多流量以提高转化。 [!UICONTROL 自动分配简化了实现转化目标的过程，同时又免去了估算工作。]
