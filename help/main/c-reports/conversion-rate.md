---
keywords: 定位
description: 了解如何Adobe [!DNL Target] 显示和计算每个体验的转化率、提升度、置信度和置信区间。
...
[!DNL Target] 建议使用 [韦尔奇的T检验](https://en.wikipedia.org/wiki/Welch%27s_t-test)，因为与z测试等替代方法相比，这些方法需要的假设更少，并且是在控制体验和替代体验之间对业务量度进行两两（定量）比较的适当统计测试。

### 更详细

运行在线A/B测试时，会将每个用户/访客随机分配到一个变体。 随后，我们会测量感兴趣的业务量度（例如，转化、订单、收入等） 适用于每个变体中的访客。 然后，我们使用的统计测试来测试平均业务量度（例如转化率、每用户订单数、每用户收入等）的假设 与给定替代变量相等。

虽然业务量度本身可能根据某些任意分布进行分配，但此量度（在每个变体内）的平均值分布应会通过 [中心极限定理](https://en.wikipedia.org/wiki/Central_limit_theorem). 请注意，虽然无法保证平均值的取样分布会以多快的速度收敛到正常，但鉴于在线测试中的访客数量，通常会达到此条件。

根据平均值的这种正常性，可以显示要使用的测试统计量以遵循t分布，因为它是正分布值（业务量度的手段差）与基于来自数据的估计值（手段差的标准误差）的缩放项的比值。 的 **t测试** 假设检验统计量服从t分布，则为适当的假设检验。

### 为何不使用其他测试

A **z测试** 在技术上不适当，因为在典型的A/B测试情景中，测试统计数据的分母不是从已知方差派生的，而是必须从数据中估计。 但是，对于足够大的样本量，z检验和t检验是相同的。

**Chi — 平方检验** 之所以不使用，是因为它们适用于确定两个变体之间是否存在定性关系（即两个变体之间没有差异的空假设）。 T测试更适合于 _定量_ 比较量度。

的 **Mann-Whitney U检验** 是非参数测试，当平均业务量度（对于每个变体）的采样分布未正常分布时，此测试非常合适。 但是，如前所述，考虑到在线测试中涉及的流量量，中心极限定理通常适用，因此t检验可以安全地应用。

更复杂的方法，如 **阿诺瓦** 当测试具有两个以上体验（“A/Bn测试”）时，可以应用（将t测试归纳为两个以上变体）。 然而，方差分析回答了“所有变体是否具有相同的含义”的问题，而在典型的A/Bn测试中，我们更感兴趣的是 _特定变体_ 最好。 在 [!DNL Target]因此，我们会应用常规t测试来比较每个变体与一个控制体，并使用Bonferroni校正来考虑多个比较。