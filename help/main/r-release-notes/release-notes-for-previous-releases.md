...
* **某些活动中缺少位置。** 修复了某些活动中缺少&#x200B;**[!UICONTROL Location]**&#x200B;的问题。 (TGT-54951)

* **报告量度列顺序。** 更新的[!DNL Target]界面允许对报表量度重新排序，而无需清除完整的选择并按顺序重新添加量度。 以前，要求用户取消选择所有量度并按所需顺序再次选择它们，这在启用许多量度以及调整列放置以限制水平滚动时会很耗时。 (TGT-53044)
...
* **应用于其他视图时保留的自定义代码。** 修复了在同一&#x200B;**[!UICONTROL Activity]**&#x200B;中为另一个&#x200B;**[!UICONTROL View]**&#x200B;添加或保存自定义代码时，可能会删除应用于一个&#x200B;**[!UICONTROL View]**&#x200B;的自定义代码的问题。 (TGT-53933)
+++
...
* **在“活动”视图中可见的受众属性。** 修复了从&#x200B;**[!UICONTROL Activity]**&#x200B;查看的受众规则详细信息时未显示从&#x200B;**[!UICONTROL Audiences]**&#x200B;部分打开同一受众时显示的特定属性的问题。 (TGT-54742)

* **导出“活动”列表页面上的CSV。** 添加了&#x200B;**[!UICONTROL Export CSV]**&#x200B;操作，以便您能够从用户界面导出活动列表（包括应用过滤器时），而无需仅依赖API进行例行导出。 (TGT-51466)
...
* **从活动创建受众时显示的受众名称和描述。** 修复了在从活动流创建或编辑受众时，与直接在&#x200B;**[!UICONTROL Audiences]**&#x200B;下创建受众相比，受众&#x200B;**[!UICONTROL Name]**&#x200B;和&#x200B;**[!UICONTROL Description]**&#x200B;字段未明确脱颖而出的问题。 (TGT-54837)
...
* [!UICONTROL Global Exclusions].**中的**&#x200B;长ID列表 修复了在更新后的界面中，**[!UICONTROL Global Exclusions]**&#x200B;中粘贴或输入较长ID列表与旧版相比可能被截断的问题，该问题会导致排除列表不完整。 (TGT-54422)
...
* [!UICONTROL Visual Experience Composer].**中的**&#x200B;增强型体验编辑器(EEC)状态指示器 EEC指示器指示是否启用增强型体验编辑器。 其演示文稿已修订，不再类似于交互式切换，因为它仅用作非交互式状态显示。 (TGT-54828)

* **[!UICONTROL Visual Experience Composer].**&#x200B;中的可折叠左边栏 现在，当活动处于打开状态以进行编辑时，可以折叠左边栏。 这改进了对于包含多个受众和页面的活动（包括在较小的显示屏上）的&#x200B;**[!UICONTROL Components]**&#x200B;和&#x200B;**[!UICONTROL Properties]**&#x200B;的访问权限。 (TGT-54269)
...
* **在新的可视化体验编辑器UI中进行身份验证后出现间歇性灰色屏幕。** 修复了以下问题：用户登录新的可视化体验编辑器后，可能会间歇性地看到灰色屏幕，导致无法进一步编辑。 此更新提高了身份验证后加载的可靠性。 (TGT-54626)
...
* **可视化体验编辑器Web编辑器中可见的设计/浏览模式选择。** 此修复程序解决了在“设计”、“浏览”和“预览”模式之间切换可正常工作，但界面中未明确指示活动模式的问题。 所选模式现在会突出显示，以便您查看所处的模式。 (TGT-54790)

* 可视化体验编辑器(Chrome)中的&#x200B;**专用网络URL。** 除非允许本地网络访问，否则Chrome可能会阻止专用网络上的页面。 **[!UICONTROL Visual Experience Composer]**&#x200B;现在请求获取在框架化Experience Cloud中使用的权限，因此Chrome可以显示允许提示而不是静默失败。 在允许访问后，如果您的环境支持VEC，则可以在VEC中加载内部URL。 (TGT-54346)
...
* **在实施屏幕上添加闪烁管理切换。** 已向[!UICONTROL Implementation]屏幕添加新的切换开关以控制启用闪烁管理设置。 此切换允许管理员直接从“实施”屏幕配置闪烁管理。 (TGT-52247)
...
* **VEC更改未反映在使用Shadow DOM （Salesforce Lightning Web组件）的网站上。** 此修复程序解决了在使用Lightning Web Components (LWC)的基于Salesforce的站点的实时站点上未保存或反映在Adobe Target中所做的更改（例如CTA颜色更改）的问题。 CMS不接受来自Target活动的更新，此问题在A/B测试和其他活动类型中始终出现。 (TGT-54059)
...
* **自定义代码在应用于其他视图时丢失。** 此修复了将自定义代码添加到其他视图并将这些代码应用于其他视图时，应用于一个视图的自定义代码丢失的问题。 以前，当用户将自定义代码应用于活动中的多个视图时，以前应用的自定义代码将会消失。 此修复方法可确保在将修改应用于其他视图时，在所有视图间正确保留自定义代码。 (TGT-54230)
...
* 当后端返回null时，**将experienceEditorType默认为“visual”。** 此修复程序解决了在某些情况下将`experienceEditorType`值返回为null的问题，该问题会导致编辑器类型未正确初始化的UI中出现意外行为。 (TGT-54690)
...
* **无法将图像添加到活动 — 未知错误。** 此修复程序解决了用户在尝试在活动中添加或替换图像时遇到“未知错误”的问题。 该问题与Scene7 URL同步和身份验证失败有关。 此修复确保可以在可视化体验编辑器中正确添加和替换图像。 (TGT-54137)
...
* 删除后未自动重新添加&#x200B;**属性令牌。** 此修复了在编辑活动时，会自动重新添加用户已取消选择的属性令牌的问题。 以前，当用户删除自动检测到的属性并在步骤之间导航时，系统将重新引入已删除的属性，而不会出现任何警报或提示，从而导致在意外的属性上启用活动。 此更新可确保删除资产令牌后，该令牌将在所有后续编辑和导航期间保持被删除状态。 (TGT-54040)
...
* **已列入允许列表的URL管理功能。** 已向[!UICONTROL Administration]部分添加新的“列入允许列表的URL”管理功能，以管理允许的远程选件URL模式。 此功能包括在表中显示列入允许列表的URL、搜索和过滤URL、通过模式对话框添加新URL以及在确认后删除URL的功能。 该功能可帮助客户控制哪些URL可用于远程选件，从而提高安全性和合规性。 (TGT-54486)
...
* **在GraphQL中支持按createdAt和modifiedAt进行筛选。** GraphQL API中添加了对按`createdAt`和`modifiedAt`日期字段筛选活动的支持。 此增强功能可以在Target UI中进行准确筛选，减少客户端数据处理，并且无需多个API调用即可检索筛选后的活动数据。 (TGT-54499)
...
* **为Web编辑器选件启用工作区列。** 此更新解决了Web编辑器中的其他工作区中出现[!UICONTROL Default Workspace]中的选件所造成的客户混淆。 尽管此行为按预期工作，但在所有工作区中均可刻意看到[!UICONTROL Default Workspace]选件，但客户报告UI未明确工作区来源，尤其是在非默认工作区（如“审批者”）中创建活动时更是如此。 为了提高清晰度，Web编辑器的选件列表中现已启用[!UICONTROL Workspace]列，允许用户轻松区分每个选件属于哪个工作区，并防止误解显示的其他选件。 (TGT-54138)
* 在新选项卡中打开具有target=&quot;_blank&quot;的&#x200B;**链接。** 此修复了以下问题：在[!UICONTROL Browse]模式下单击时，包含具有~target=&quot;_blank&quot;~的链接的已创作网站将在新的浏览器选项卡中打开，从而中断编辑器内预览体验。 出现此行为是因为创作页面的本机链接属性未被扩展插入的JavaScript截获，这与旧版UI不同，在旧版UI中，锚点元素会进行转换并覆盖其目标以保留在编辑器中导航。 此更新现在确保在Web编辑器中正确处理使用~target=&quot;_blank&quot;~的链接，以便在创作过程中不再打开外部选项卡。 (TGT-54134)
* **取消选择属性警告。** 此更新引入了可视警告，当用户在活动编辑器中取消选择自动检测到的属性时，该警告会明确通知用户。 以前，删除自动检测到的属性不会指示该属性将被永久删除，这可能会导致意外丢失定位配置。 此修复程序会添加一个警告图标（与旧版UI中的行为一致），以通知用户取消选择资产会将其从活动中移除。 (TGT-54121)
* **[!UICONTROL Workspaces]下拉列表在[!UICONTROL Users]部分中限制为20个。** 此修复解决了[!UICONTROL Administration] > [!UICONTROL Users]部分中的[!UICONTROL Workspaces]下拉菜单仅显示20个工作区的问题，即使用户有权访问更多工作区。 `licenseGroups`的基础GraphQL调用也被限制为20个结果，这会导致UI显示不完整的列表，尽管用户有权访问组织中的更多工作区。 更新会移除此硬限制，以便现在可以正确返回并显示整组可用工作区。 (TGT-53820)
* **修复了选件模式未显示工作区列的问题。** 修复了选件模式在更新的UI中不显示Workspace列的问题。 这会给客户带来困惑，因为[!UICONTROL Default Workspace]中的选件与所选工作区中的选件一起显示，但未指明它们的来源。 工作区列现在已启用，以便客户可以清楚地识别每个选件属于哪个工作区。 (TGT-52320)
...
* **如果已删除，Activity Edit则不应添加自动检测到的属性。** 此修复了以下问题：编辑活动会自动重新引入用户之前删除的自动检测属性。 重新打开活动进行编辑时，系统错误地还原了已删除的属性，导致[!UICONTROL Properties List]中的行为不一致并混淆。 此更新可确保在删除自动检测到的属性后，该属性会在所有后续编辑期间保持被删除状态，并且除非用户明确将其添加回来，否则该属性不会重新出现。 (TGT-54182)
* **如果已删除，则不添加自动检测到的属性。** 此修复可确保，一旦用户手动从活动中移除自动检测到的属性，系统在活动编辑器中的后续导航期间将不再重新引入该属性。 以前，如果用户取消选择自动检测的属性，移至[!UICONTROL Targeting]步骤，然后返回到[!UICONTROL Experiences]，则编辑器将根据活动编辑器状态切片中存储的自动检测列表重新填充移除的属性。 更新后的逻辑现在将自动检测到的属性与~ActivityState~切片中的当前属性进行比较，并阻止重新添加用户已删除的任何自动检测到的属性。 这会导致各步骤之间出现一致的行为并尊重用户意图。 (TGT-54181)
* **将自动检测到的文本添加到属性列表。** 此增强功能会更新[!UICONTROL Properties List]，以明确标记系统自动检测的任何属性。 当用户可见的[!UICONTROL Properties List]中也存在自动检测到的属性时，该属性现在会使用存储在~ActivityEditorSlice~状态中的值，在其名称旁边显示“（自动检测）”文本。 这反映了旧版UI的行为，并帮助用户轻松区分手动选择的属性和自动识别的属性。 (TGT-54120)
* **将自动检测到的[!UICONTROL Properties]添加到状态。** 此更新确保~ActivityEditorSlice.ExperienceEditor~状态一致地维护所有自动检测到的属性ID的最新列表，这些属性ID从Web编辑器传递到Activity [!UICONTROL Experiences]选项卡中。 每次用户导航到[!UICONTROL Experiences]选项卡时，都会使用任何新检测到的属性刷新状态，同时会防止出现重复项，从而确保准确跟踪和可靠的下游行为。 (TGT-54119)
...
* **[!UICONTROL Environment]下拉列表仅显示100个结果。** 此修复解决了具有超过100个环境的客户在[!UICONTROL Recommendations]内的[!UICONTROL Environment]下拉菜单中只能看到前100个条目的限制。 基础GraphQL查询(~getEnvironmentsV2~)采用硬编码页面大小100分页，导致UI仅显示部分列表，即使有其他页面可用。 对于拥有超过100个环境的客户，此问题会导致缺少选项和不完整的选择体验。 更新将提高限制，以便返回并显示所有环境，从而确保完全可见，无论环境计数是多少。 (TGT-53903)
...
* **修复了[!UICONTROL Reports]箭头未明确指示可展开列的问题。** 修复了报表表格未明确显示其他列可以在更新后的UI中展开的问题。 向列标题附近的[!UICONTROL Reports]箭头添加了消失的工具提示，以帮助客户了解更多列可用。 (TGT-53479)
...
* **无法将[!UICONTROL Advanced Settings]用于[!UICONTROL Revenue]指标。** 此修复解决了在[!UICONTROL Goals & Settings]中为[!UICONTROL Revenue]量度配置[!UICONTROL Advanced Settings]时，用户遇到403“访问被拒绝”错误的问题。 添加与主目标关联的依赖项条件时，出现问题；后端错误地要求编辑者权限，即使用户已有足够的权限创建和编辑活动。 因此，尽管配置有效，但保存活动失败。 更新更正了权限检查，以便具有适当访问权限的用户可以成功添加收入量度依赖关系，而不会触发禁止的资源错误。 (TGT-54092)
* **修复了“添加”按钮未应用于选定图像的问题。** 修复了一个问题，该问题阻止客户在活动创建流程中选择或更新图像时添加某些图像。 例如，当客户搜索特定资产时，如果在搜索“ipp”时返回图像，则单击[!UICONTROL Add]按钮不会应用选定的图像，并且不会创建任何修改。 选择其他图像（如`Homepage-banner-1-moz.jpg`）可继续按预期工作。 此更新确保所有有效图像都可以在更新后的UI中一致应用。 (TGT-53610)
...
* **修复了搜索未查看子文件夹的问题。** 修复了在搜索选件时未从更新后的UI中的子文件夹返回结果的问题。 客户只有在手动导航到存储选件的文件夹时，才能找到该选件，这使得搜索行为与API功能不一致。 搜索功能现在支持递归查看文件夹，因此客户可以找到选件，而无需单独打开每个文件夹。 (TGT-51954)
...
* **具有隐藏或无效选择器的优惠决策在更新的UI中不可编辑。** 解决了更新UI中无法编辑与隐藏或无效选择器相关联的优惠决策的问题，除非该元素在可视化体验编辑器(VEC)中可见。 现在支持直接从面板进行编辑，从而恢复旧版UI中可用的功能，并确保能够修改优惠决策，而不管选择器可见性如何。 (TGT-53899)
...
* **在活动中编辑条件导致页面崩溃。** 解决了在更新的UI中编辑活动标准导致页面崩溃并出现与`useCrudActionsCtx`相关的控制台错误的问题。 现在，标准编辑器可正确加载和运行，确保活动可编辑而不会中断。 (TGT-53971)
* **[!UICONTROL Message]列间歇性地无法在更新的UI中显示产品数据。** 解决了在更新的[!UICONTROL Recommendations] UI中，[!UICONTROL Catalog Search]中的[!UICONTROL Message]列间歇性地无法显示产品数据的问题，即使信息源中存在值也是如此。 列现在可以一致地显示所有产品的正确报文值，确保可靠的可见性，而无需手动重新配置列。 (TGT-52777)
* 在更新的UI中保存活动后，**[!UICONTROL Download Recommendations Data]按钮不可见。** 解决了在更新后的UI中，即使在重新保存之后，某些保存的活动也不会显示[!UICONTROL Download Recommendations Data]按钮的问题。 现在，按钮在所有活动中均可一致显示，从而确保用户能够可靠导出推荐数据而无需解决办法。 (TGT-53802)
* **从集合中打开某些产品时返回“未找到请求的资源”，并且模式模式缺少close选项。** 解决了在更新的Recommendations UI中，从收藏集打开某些产品时触发“未找到请求的资源”错误并显示一个没有“关闭”选项的空白模式的问题。 该模式现在可正确加载产品详细信息，并且始终提供关闭选项以便正常退出。 (TGT-53986)
...
* 在更新的UI中使用[!DNL Adobe Analytics]作为报表源时出现&#x200B;**[!UICONTROL Goals & Settings]错误消息。** 解决了更新的[!UICONTROL Overview] UI中，目标部分显示“出现错误”错误的问题。 我们无法完成您的请求。 如果选择[!DNL Adobe Analytics] (A4T)作为报表源，如果问题仍然存在，请联系[!DNL Adobe Client Care]。 目标现在可以正确显示为[!UICONTROL Adobe Analytics]个量度，从而确保跨报表源的一致可见性。 (TGT-54021)
...
* **更正了韩文和日文UI中的几个本地化错误。** （TGT-54003、TGT-54004、TGT-54006、TGT-54007和TGT-54018）
...
* **具有实体属性匹配的按属性列出的促销活动在活动保存后无法加载推荐键。** 修复了在保存活动后进行编辑时，规则类型为[!UICONTROL Entity Attribute Matching]且类型为[!UICONTROL Promotion by Attribute]的促销活动未加载推荐键的问题。 问题是由未通过GraphQL请求`customKeyId`导致的。 现在，推荐键可在编辑促销活动期间正确加载。 (TGT-53117)
* 从ExpB切换到ExpA时，**推荐会以可视方式持续存在。** 解决了在体验B中插入推荐，然后切换到体验A以使推荐选件框可见的问题。 这只是视觉上的不一致；现在，在体验之间切换时，修改可正确呈现，从而确保准确的UI行为。 (TGT-53911)
* **推荐键未加载[!UICONTROL Promotion by Attribute]，其中[!UICONTROL Entity Attribute]匹配。** 解决了规则类型为[!UICONTROL Entity Attribute Matching]且类型为[!UICONTROL Promotion by Attribute]的促销活动在保存活动后进行编辑时无法加载推荐键的问题。 现在，可通过GraphQL正确检索推荐键，从而确保促销活动按预期显示和运行。 (TGT-53917)
* **编辑有关隐藏的HTML元素的推荐在更新的UI中不起作用。** 解决了[!UICONTROL New Create]和VEC UI中无法编辑应用于隐藏HTML元素的推荐活动的问题。 此功能现在可按预期工作，从而恢复与旧版UI的等同性，并确保无论元素可见性如何，推荐都可以修改。 (TGT-53953)
* **无法在更新的UI中编辑隐藏HTML元素的推荐活动。** 解决了在更新的UI中，无法编辑应用于隐藏HTML元素的推荐活动的问题。 此功能现在可按预期工作，从而恢复与旧版UI的等同性，并确保无论元素可见性如何，推荐都可以修改。 (TGT-53951)
* **推荐目录在更新的UI中间歇性地缺少属性值。** 解决了在更新的[!UICONTROL Recommendations] UI中，目录搜索列表间歇性地无法显示某些属性值（例如，消息）的问题，即使这些值存在于产品馈送中也是如此。 现在，搜索结果中的属性值加载一致，无需重新配置列，提高了目录管理的可靠性和效率。 (TGT-52769)
* 更新后的UI中的[!DNL Recommendations]活动缺少&#x200B;**[!UICONTROL Download Recommendations]按钮。** 解决了在更新的[!DNL Recommendations] UI中，使用推荐的A/B活动无法显示[!UICONTROL Download Recommendations]按钮的问题。 现在，按钮可正确显示，允许用户按预期导出推荐数据，这与旧版UI中的功能一致。 (TGT-53768)
* 更新后的概述UI中缺少&#x200B;**[!UICONTROL Download Recommendation Data]按钮。** 解决了在更新的[!UICONTROL Overview] UI中，[!UICONTROL Download Recommendation Data]按钮对包含推荐的活动不可见的问题。 按钮现在可正确显示，从而确保用户无需切换回旧版UI即可直接导出推荐数据。 (TGT-53772)
* **编辑活动条件有时会在更新的UI中导致空白屏幕。** 解决了更新UI中单击[!UICONTROL Edit Criteria in Experiences]有时会导致某些活动出现空白屏幕的问题。 现在，标准编辑器可以在所有活动中可靠地加载，从而确保用户能够不受中断地编辑。 (TGT-53961)
* **无法在更新的UI中编辑序列条件。** 解决了在更新的UI中，尝试编辑[!UICONTROL Sequence Criteria]导致标准弹出窗口在加载时停留并显示空白屏幕的问题。 标准编辑器现在可正确加载，允许用户编辑和更新序列标准而不会中断。 (TGT-53985)
...
* **[!UICONTROL Multivariate Test] (MVT)位置和图形报告问题阻止生成报告。** 解决了MVT活动无法在Target UI中生成[!UICONTROL Location Contribution]和图形报表的问题，该问题显示错误“出现错误。 我们无法完成您的请求。” 现在，报告可在UI中正确加载，确保完全可见。 (TGT-53654)
* 由于[!UICONTROL Element]贡献报表错误，**MVT报表未加载。** 修复了Target UI中MVT活动报表加载失败，并显示“无法获取元素贡献报表”错误的问题。 现在，报表可正确显示，从而确保元素贡献的全面可见。 (TGT-53691)
* **将订单详细信息导出到[!UICONTROL Experience Targeting] (XT)活动的CSV问题。** 修复了[!UICONTROL Export Order Details to CSV]选项在XT活动中错误显示并返回空文件的问题。 现在，仅对AP活动显示选项，以确保准确的导出功能并防止混淆。 (TGT-53798)
...
* **[!UICONTROL Delete Modification]按钮问题阻止移除活动修改。** 解决了[!DNL Target] UI中的[!UICONTROL Delete Modification]按钮不起作用，导致用户无法删除活动中的修改的问题。 现在，按钮按预期工作，允许毫不延迟地可靠地删除修改。 (TGT-53728)
* **在更新的UI中无法识别首选选择器。** 解决了更新后的UI中，VEC的CSS选择器列表中未显示首选选择器（例如`data-target-component-id`）的问题。 用户现在可以可靠地选择首选属性而不是动态生成的类名，从而确保在进行SPA页面更新时稳定定位。 (TGT-53908)
* **活动位置在[!UICONTROL Edit]和[!UICONTROL Overview]页之间对齐方式不匹配。** 解决了[!UICONTROL Overview]页面中的活动位置编号与[!UICONTROL &#x200B; Edit Experience]页面中的更新不符的问题。 现在，两个视图中的位置保持一致，确保精确的对齐并防止位置缺失或编号错误。 （TGT-53960 和 TGT-53954）
* **无法在更新的VEC中切换回[!UICONTROL Design]模式。** 解决了更新后的VEC UI中，用户在[!UICONTROL Browse]模式下导航到新页面后无法切换回[!UICONTROL Design]模式的问题。 [!UICONTROL Design]切换功能现在可以正常工作，允许修改无缝地跨页面应用。 （TGT-53988 和 TGT-53993）
* **查询参数未显示在活动概述中。** 解决了在更新的UI中，查询参数未显示在活动的[!UICONTROL Overview]页面中，从而导致[!UICONTROL Overview]和页面交付URL之间不一致的问题。 现在，查询参数可正确显示，从而确保活动位置在各个视图中完全表示并保持一致。 (TGT-53701)
...
* **[!UICONTROL Observer]用户现在可以在更新的UI中查看活动内容。** 观察者角色用户在更新后的活动UI中恢复可见性。 以前，观察者无法查看旧版UI中可用的修改、选件和内容更改。 (TGT-53785)
* **[!UICONTROL Approver]用户现在可以编辑活动目标，但没有编辑器权限错误。** 解决了活动创建UI中阻止审批者级别用户保存对高级目标设置更改的权限问题。 受影响的用户收到`403 Forbidden.Resource`错误，需要编辑器权限，尽管具有足够的访问权限。 (TGT-53819)
...
* **已在“仅此活动”报表中还原多受众选择。** 解决了活动创建UI中阻止用户在[!UICONTROL Goals & Settings]的[!UICONTROL This activity only]部分下选择多个受众的问题。 (TGT-53283)
* **基于受众的报告图表现在可正确显示转化数据。** 解决了[!UICONTROL Reports]选项卡中在选择非默认受众时导致图形失败的问题。 虽然提供了数据和置信度量度，但可视化图表只显示一条实线，导致分析困难。 (TGT-53769)
* **[!UICONTROL Targeting] UI现在明确指示已排除的受众规则。** 解决了在活动创建UI的[!UICONTROL Targeting]部分中，未明确显示设置为[!UICONTROL Exclude]的受众规则的问题。 这会导致查看定位逻辑时出现混淆，尤其是对于排除特定URL的受众。 (TGT-53809)
...
* **已更正zh_CN页面编辑器上下文中“quote”的错误翻译。** 更正了zh_CN区域设置中的上下文翻译错误，在该错误中，“quote”一词被不准确地翻译为“报价”，这意味着商业价格报价。 在“排版规则”>“标题样式”>“块引用”部分中，预期含义是指格式元素（引用块）而不是定价。 (TGT-53841)
* **更正了zh_CN页面编辑器上下文中“已移除引号”的错误翻译。** 更正了zh_CN区域设置中的一个翻译错误，该错误中“quote removed”被不准确地呈现为“移除了报价”，这意味着商业报价。 在“排版规则”>“标题样式”>“块引用”部分中，术语是指格式元素（引用块）而不是定价。 (TGT-53843)
* **更正了zh_CN页面编辑器上下文中“重新排列引号”的错误翻译。** 更正了zh_CN区域设置中的上下文翻译错误，该错误中“quote reorized”被不准确地翻译为“重新排列了报价”，这意味着商业报价。 在“排版规则”>“标题样式”>“块引用”部分中，术语是指格式元素（引用块）而不是定价。 (TGT-53844)
* **更正了zh_CN页面编辑器上下文中“quote changed”的错误翻译。** 更正了zh_CN区域设置中的一个翻译错误，该错误中“quote changed”未准确地呈现为“更改了报价”，从而建议使用商业报价。 在“排版规则”>“标题样式”>“块引用”部分中，术语是指格式元素（引用块）而不是定价。 (TGT-53845)
...
* **推荐的CSS选择器更改现在可以正确保存。** 解决了活动创建UI中阻止用户更新推荐投放的CSS选择器的问题。 更改在保存后已恢复，阻止对定向容器的更新。 (TGT-53835)
* **页面加载事件选择在推荐修改中现在持续存在。** 解决了在将推荐的事件类型从[!UICONTROL View]切换到[!UICONTROL Page Load]时，活动创建UI中阻止用户保存更改的问题。 尽管选择似乎成功，但在导航离开并阻止活动发布后已恢复。 (TGT-53957)
...
* 已添加&#x200B;**个IMS预筛选器以保护GQL端点免受跨组织数据泄露的影响。** 解决了“管理员”选项卡中影响licenseGroups和targetProperties GraphQL端点的安全漏洞。 问题源于将JIL API与管理员客户端令牌一起使用，可能会利用此令牌访问跨组织产品数据。 (TGT-53837)
...
* **在活动创建UI中为保存的活动编辑已还原。** 解决了阻止用户在保存修改后编辑活动的问题。 受影响的活动在“[!UICONTROL Applying initial modifications]”中仍然卡住，阻止进一步更新并隐藏[!UICONTROL Cancel]按钮。 (TGT-53631)
* **VEC不再在“[!UICONTROL Applying initial modifications]”上停止。** 解决了VEC中的一个性能问题，该问题在加载具有大量修改的体验时会导致长时间延迟。 受影响的用户看到UI在“[!UICONTROL Applying initial modifications]”上停滞了数分钟，尤其是在体验B场景中。 (TGT-53727)
* **VEC现在加载修改时没有根元素。**
...
* **VEC不再在“[!UICONTROL Applying initial modifications]”上间歇性停止。** 解决了VEC中的一个间歇性问题，该问题导致体验无法加载并停留在“[!UICONTROL Applying initial modifications]”上。 此行为不一致，有时会触发重定向循环或需要手动清除缓存。 (TGT-53916)
...
* VEC中的&#x200B;**[!UICONTROL Design]模式不再像[!UICONTROL Browse]模式那样间歇性地运行。** 解决了UI中[!UICONTROL Design]模式间歇性地表现为[!UICONTROL Browse]模式的问题，该模式允许可单击的`<a>`链接并阻止元素选择。 这会导致悬停框消失并阻止修改工作流。 (TGT-53136)
* 在[!UICONTROL Composer]模式下的&#x200B;**按钮点击不再触发页面重定向。** 解决了更新后的VEC UI中的一个问题：在[!UICONTROL Composer]模式下单击某个按钮会导致意外重定向到该按钮的目标URL。 这会阻止用户编辑call-to-action (CTA)元素，并中断创作工作流。 (TGT-53137)
* **自动个性化活动中的选件代码更新现在无错误保存。** 解决了在更新[!UICONTROL Automated Personalization]活动中的优惠代码时，在新建用户界面中导致“[!UICONTROL Invalid user input]”错误的问题。 该错误阻止用户保存更改，即使输入有效也是如此。 (TGT-53586)
* VEC中的&#x200B;**[!UICONTROL Design]模式现在阻止可编辑组件的链接导航。** 解决了更新后的VEC中存在的一个问题：即使在[!UICONTROL Design]模式下，可单击元素（如按钮和链接）也会触发页面重定向。 此行为模拟[!UICONTROL Browse]模式并阻止用户修改关键组件。 (TGT-53696)
...
* **编辑期间现有活动不再卡在无限加载循环中。** 解决了在新建用户界面中编辑VEC中的现有活动导致页面停留在无限加载循环中的问题。 此问题不会影响新创建的活动，是由页面上预先存在的修改触发的。 (TGT-53913)
* **现在，在VEC中正确加载了包含修改的现有活动页面。** 解决了在更新的VEC中通过保存的修改编辑现有活动时导致页面停滞在加载阶段的问题。 新活动已加载且没有问题，但之前配置的活动无法呈现。 (TGT-53967)
...
* **受众排除规则在[!DNL Target] UI中显示为包含不正确。** 在活动中编辑定位时，使用排除规则配置的受众似乎包含在内。 尽管在执行期间正确应用了排除逻辑，但UI无法正确反映规则，缺少“排除”标签。 [!DNL Target] UI现在可以在受众配置和定位工作流中正确显示排除规则，从而确保活动设置的明确性和一致性。 (TGT-53808)
* **[!UICONTROL Targeting]部分未指示受众规则设置为排除。** 使用排除逻辑配置的受众错误地显示为包含在活动创建UI的[!UICONTROL Targeting]部分中。 尽管后端正确应用了排除规则，但UI无法直观地表示该规则，在活动设置期间会忽略“排除”标签并导致混淆。 [!UICONTROL Targeting]部分现在可以清楚地显示排除规则，确保受众配置和定位可视化之间的一致性。 (TGT-53809)
...
* **推荐框在VEC中难以查找和选择。** 在(VEC)中添加推荐选件后，单击左侧面板中的修改不会高亮显示或滚动到页面上相应的推荐框。 这使得查找和编辑选件变得困难，尤其是在选择器下隐藏或最小化样式时。 现在，单击推荐修改可正确高亮显示并滚动到关联的元素，从而提高更新的活动创建过程中的可用性和编辑效率。 (TGT-52571)
...
* **修改现有活动时无法编辑标准内容。** 编辑活动时，[!UICONTROL Criteria]内容部分显示为禁用，按钮灰显且无响应。 通过确保[!UICONTROL Criteria]配置在活动更新期间完全可编辑，此问题已得到解决。 客户现在可以修改[!UICONTROL Criteria]内容，而无需切换选择或使用解决方法，从而改进了活动创建更新过程中的灵活性和可用性。 (TGT-53812)
* **无法在活动中编辑标准。** 从活动内访问标准时，[!UICONTROL Edit Criteria]和[!UICONTROL Remove Criteria]选项已禁用。 但是，可通过[!UICONTROL Recommendations]选项卡成功编辑相同的标准。 现在，可以从活动编辑工作流和[!UICONTROL Recommendations]选项卡中完全编辑标准，从而确保一致且高效的编辑体验。 (TGT-53814)
...
* **报告在与滚动条交互后崩溃。**&#x200B;单击[!UICONTROL Reports]选项卡中的滚动条导致页面崩溃，并伴有JavaScript错误：
...
* **与水平滚动条交互时，更新的UI中的[!UICONTROL Reports]选项卡崩溃。** [!UICONTROL Reports]视图在使用水平滚动条访问视图外的量度时间歇性地崩溃，并出现“出现错误”错误。 现在，滚动条可以可靠地工作，允许客户查看和分析所有量度，而无需采取变通方法，例如缩小或使用按住Shift键滚动鼠标。 (TGT-53824)
...
* **改进了在选择VEC中的元素期间查看子元素时的工作流。** 为了提高活动创建期间的可用性和准确性，当鼠标悬停在父HTML元素上或选择它时，VEC现在会显示子元素。 此增强功能使客户能够更好地了解页面的结构并做出更准确的修改，从而简化更新后UI中的编辑工作流。 (TGT-53416)
...
* **修复了由于受众ID无效而无法复制活动的问题。** 客户尝试在更新的活动创建流程中复制活动时，遇到因受众ID无效（例如 — 1752722444307）导致的错误。 此后端验证问题可防止同一工作区中的活动重复。 此问题已得到解决，现在可以成功复制活动而不会出现与受众相关的错误。 (TGT-53717)
* **修复了[!UICONTROL Manage Content]模式的[!UICONTROL Automated Personalization]活动中仅活动受众出现无效用户输入错误的问题。** 客户在AP活动的[!UICONTROL &#x200B; Manage Content]模式中配置仅限该活动的受众时遇到无效的用户输入错误。 尽管之前已成功使用受众，但还是出现了此问题。 现在，组合的受众配置可正确保存，而不会触发验证错误。 (TGT-53749)
...
* **已将特定于Target的Web SDK文档页面移动到Adobe Target存储库。** 作为Web SDK文档重构的一部分，特定于[!DNL Target]的内容已从常规Web SDK文档迁移到[!DNL Adobe Target] [开发人员指南](https://experienceleague.adobe.com/zh-hans/docs/target-dev/developer/a4t/overview-a4t?lang=en){target=_blank}。 此更改改进了内容可搜索性，并确保相应的产品团队维护特定于解决方案的指导。 (TGT-53374)
...
* **优惠决策选项在初始活动创建期间始终可见。** 解决了在更新的UI中，首次创建A/B活动流程时，特别是在无痕模式下访问启用了优惠决策的租户时，无法显示[!UICONTROL Offer Decision]选项的问题。 该选项仅在导航到[!UICONTROL Targeting]步骤并返回到[!UICONTROL Experiences]之后显示。 此修复程序确保[!UICONTROL Offer Decision]选项在初始设置期间立即可用，从而提高可用性并减少混淆。 (TGT-51888)
...
* **修复了当`includeContent=true`.**&#x200B;时，重定向选件未在有效负载中包含`redirectOptions`的问题 检索具有`includeContent=true `的重定向选件的客户在响应有效负载中缺少`redirectOptions`字段。 这种不一致会影响工作流，例如选件复制和活动创建。 在请求内容时，重定向选件现在正确包含`redirectOptions`。 (TGT-53737)
...
* **已还原在更新的UI中创建的[!UICONTROL Recommendations]活动的点击跟踪。** 解决了在更新的UI中创建的[!UICONTROL Recommendations]活动无法注册点击跟踪，导致报告转化为零的问题。 旧版UI中构建的活动可正确跟踪点击次数并报告预期转化。 此修复程序可确保在更新后的UI中创建的推荐活动现在包括正确的跟踪属性、恢复转化报表以及与A4T量度保持一致。 (TGT-53287)
...
* 在更新的UI中解决了&#x200B;**收藏集编辑器崩溃。** 修复了在更新的[!UICONTROL Visual Experience Composer] (VEC) UI中，从编辑器面板打开收藏集导致页面崩溃的问题，出现TypeError：无法读取未定义的属性（读取“customLocale”）。 此错误出现在多个活动类型中，包括[!UICONTROL Recommendations]和A/B测试。 (TGT-53703)
* **用于删除VEC中还原的选定集合的选项。** 修复了VEC中的一个问题：用户只能替换[!UICONTROL Recommendations]活动中的选定集合，但无法将其完全删除。 此限制阻止了需要在不进行替换的情况下干净移除集合的用例。 此修复引入了一个用于删除所选收藏集的清除选项，从而允许在活动设置方面更加灵活，并与旧版UI行为保持一致。 (TGT-53652)
* **自定义条件集合现在在[!UICONTROL Recommendations] UI中正确显示。** 修复了“推荐”界面中使用自定义标准构建的收藏集无法显示产品结果的问题。 尽管基于属性的标准收藏集按预期工作，但使用自定义筛选器的收藏集返回“未找到结果”，尽管目录匹配有效。 此修复确保使用自定义属性的收藏集现在能够在[!UICONTROL Product]选项卡中正确填充，从而恢复个性化推荐工作流的完整功能。 (TGT-53653)
* **修复了首次打开[!UICONTROL Products]页面时集合不显示产品的问题。** 访问[!UICONTROL Recommendations]分区中的收藏集的客户在首次打开[!UICONTROL Products]页面时遇到了空的产品结果。 此问题是由于GraphQL查询中的后端错误导致的，该错误无法加载具有自定义标准的集合的产品数据。 问题已得到解决，产品现在可以正确显示，而无需切换环境。 (TGT-53694)
* **修复了在基于表单的[!UICONTROL Recommendations]活动中无法删除收藏集的问题。** 使用[!UICONTROL Form-Based Experience Composer]创建[!UICONTROL Recommendations]活动的客户无法取消选择以前选择的收藏集。 UI要求在保存之前选择收藏集，以防用户恢复为“所有收藏集”。 此行为已更新以符合VEC功能，允许客户在没有选定收藏集的情况下进行保存，并按预期默认为“所有收藏集”。 (TGT-53708)
* **修复了由于缺少集合或筛选规则值而无法由属性设置促销的问题。** 在活动创建流程中按属性配置促销的客户遇到错误，指出促销缺少收集ID或筛选规则值。 即使设置似乎已完成，此验证问题也会阻止进程。 现在，在按属性配置促销活动时，可以成功保存促销活动。 (TGT-53750)
* **修复了由于体验名称重复而无法保存活动的问题。** 客户在保存包含特定标准和设计组合的活动时遇到无效用户输入错误。 即使设置似乎有效，错误也因体验名称重复而触发。 现在可以保存具有不同配置的活动，而不会出现命名冲突。 (TGT-53805)
...
* **修复了将促销活动应用于实时[!UICONTROL Recommendations]活动时触发错误的问题。** 客户在尝试将前端促销应用于实时[!UICONTROL Recommendations]活动时遇到错误“促销活动缺少收藏集ID或筛选规则值”，即使提供了有效的配置详细信息后也是如此。 现在可以将促销活动成功应用于实时活动，而不会触发验证错误，从而确保在更新的活动创建UI中实现更顺畅的体验。 (TGT-53738)
* **修复了产品在[!UICONTROL Recommendations] UI内的收藏集中不可见的问题。** 来自单个租户的客户报告，在新UI的[!UICONTROL Recommendations]部分中查看某些收藏集时加载产品列表失败。 通过切换环境暂时解决了此问题，但此解决方法会导致用户体验不佳。 现在，产品实体可以始终如一地加载，而无需切换环境。 (TGT-53783)
...
* **[!UICONTROL Total Revenue]指标现在包含在从活动报表导出的CSV中。** 解决了在更新的[!UICONTROL Overview] UI中，活动报表视图中正确显示总收入，但CSV导出中缺少总收入，显示为$0的问题。 这种差异导致用户无法依赖导出的数据进行离线分析和报告。 (TGT-53325)
...
* **改进了在未启用量度时[!UICONTROL Graph View]的错误消息传递。** 修复了VEC中的一个问题：当关联的[!DNL Analytics]报表包中未启用所请求的量度时，[!UICONTROL Graph View]显示一般的“出现问题”消息。 此问题是由后端GraphQL响应中的`not_enabled_metric`错误触发的。 此修复用信息更丰富的消息取代了模糊的错误，帮助用户识别[!DNL Analytics]中的配置问题，从而减少混淆和不必要的支持升级。 (TGT-53577)
* **修复了报表持续时间超出支持的90天限制的问题。** 在[!UICONTROL Reports]部分中使用“[!UICONTROL Last X Days]”过滤器的客户能够选择超过90天的持续时间，这可能会导致性能问题和数据不完整。 更新了过滤器，强制实施最长90天的范围，以确保一致且可靠的报表。 (TGT-53795)
* **修复了使用默认环境而不是选定环境生成性能CSV报表的问题。** 以前，当客户在报表设置中更改环境并生成性能报表时，生成的CSV包含来自默认环境（而非选定环境）的数据。  UI现在可正确传递`environmentId`参数，从而确保报表反映所选环境。 此外，错误处理已得到改进。 如果在CSV生成期间出现GraphQL错误，则UI现在会显示一条明确的错误消息，而不是生成一个空的CSV文件。 (TGT-53064)
* **修复了Analytics for Target (A4T)报表无法在[!UICONTROL Graph View].**&#x200B;中显示数据的问题 将[!DNL Target]与A4T集成一起使用的客户在A/B测试活动的“报表”选项卡中切换到图形视图时遇到“出现错误”错误。 尽管[!UICONTROL Table View]已正确加载，[!UICONTROL Graph View]仍无法呈现选定时间范围内的量度趋势。 [!UICONTROL Graph View]现在可按预期显示所有受支持量度的性能数据，从而提高更新UI中的可见性和报告准确性。 (TGT-53573)
...
* **元素元数据现在显示在悬停在痕迹导航菜单中的位置。** 改进了VEC中的痕迹导航菜单，可在将鼠标悬停在项目上时显示其他元素详细信息，例如ID、类和名称。 此增强功能可帮助用户在活动设置期间更轻松地识别和区分元素。 (TGT-53409)
* **痕迹导航悬停现在突出显示VEC中的相应元素。** 改进了[!UICONTROL Visual Experience Composer]，以在痕迹导航菜单中将鼠标悬停在项上时突出显示编辑器中的相应元素。 还会显示元素类型，如容器、粗体文本或按钮。 此行为适用于同级元素，并根据验证列表排除不支持的类型，例如SVG。 (TGT-53411)
...
* **在VEC的页面刷新期间，自定义代码更改现在持续存在。** 修复了VEC中的以下问题：在网站刷新期间，自定义代码修改丢失。 在多次重新加载的页面上发生此问题，导致编辑器重置并还原未保存的更改。 修复程序可确保自定义代码编辑内容保持不变，即使页面在编辑期间重新加载也是如此，从而防止意外失去进度。 (TTGT-53501)
* **已在VEC中解决网站访问权限的登录问题。** 修复了用户在通过VEC访问网站时无法登录到网站的问题。 登录流会反复地将用户重定向回登录页面，从而阻止活动设置和预览。 此修复可确保VEC不再干扰登录行为，从而恢复经过身份验证的用户的预期访问权限。 (TGT-53524)
* **VEC Helper扩展中已解决Cookie复制问题。** 修复了[!UICONTROL Adobe Experience Cloud Visual Editing Helper]扩展在通过预览链接进行QA期间复制`at_qa_mode` Cookie的问题。 当手动切换预览索引时，会创建多个具有跨域冲突值的Cookie，从而阻止测试者可靠地切换变量。 甚至在Target UI外部也观察到此行为，它同时影响内部帐户和客户端帐户。 此修复通过防止重复条目并调整域范围来确保一致的Cookie处理，从而允许无缝的变体切换，而无需手动Cookie清理。 (TGT-53579)
...
* **修复了在加载某些活动时VEC崩溃并显示空白屏幕的问题。** 特定租户的客户报告编辑器无法加载特定活动。 VEC在崩溃并显示空白屏幕之前停留在“应用初始修改”上。 现在，VEC在更新的活动创建过程中加载受影响的活动，而不会出现错误。 (TGT-52932)
* **修复了[!UICONTROL Automated Personalization]活动中的[!UICONTROL Manage Content]边栏显示不一致位置标签的问题。** 客户报告[!UICONTROL Manage Content]边栏在[!UICONTROL Experiences]和[!UICONTROL Offers]选项卡中显示的位置编号不匹配。 （例如，体验中的位置2和位置4，以及选件中的位置1和位置2），即使仅配置了两个位置也是如此。 位置标签现在是一致的，并准确地映射到修改，从而提高活动创建过程中的清晰度和可用性。 (TGT-52934)
* **修复了VEC中的修改在保存后丢失的问题。** 客户报告，在VEC中保存修改后，页面将刷新并将更改恢复到其先前版本。 此问题导致最新更新丢失，除非立即保存整个活动。 现在，修改在保存后会正确保留，并且编辑器不会再意外还原更改，从而确保在活动创建工作流中提供可靠的体验。 (TGT-53500)
* **通过在悬停和选定内容上显示元素类型，增强了VEC中的元素选择。** 为了提高活动创建期间的可用性，现在，VEC会在将鼠标悬停在上面或选中时，根据HTML元素的类型对其进行修饰。 此增强功能可帮助客户更轻松地识别和选择正确的元素。 选定的元素以不同的颜色突出显示，悬停的元素以蓝色列出。 元素类型也会显示，在编辑过程中提供更清晰的上下文。 (TGT-53502)
...
此更新可确保配置的一致性，并防止在多沙盒环境中发生冲突。 有关详细信息，请参阅&#x200B;*Adobe Target目标*&#x200B;指南中的[Experience Platform连接](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/destinations/catalog/personalization/adobe-target-connection){target=_blank}。
...
* **已改进[!UICONTROL AEM Experience Fragment]选件的用户归因。** 解决了VEC中[!UICONTROL AEM Experience Fragment] (XF)选件的“[!UICONTROL Last updated]”字段错误地显示代码ID而非用户名的问题。 在更新的UI中选择选件期间出现此差异，导致与旧版UI和HTML选件不一致，后者正确显示最后编辑者的名称。 此修复确保跨选件类型的一致用户归因，从而提高透明度并与预期行为保持一致。 （TGT-52880 和 TGT-52898）
...
* 已解决&#x200B;**ODS选件呈现问题。** 解决了在更新的UI中使用VEC生成活动时，通过[!DNL Adobe Journey Optimizer] (AJO)创建的[!DNL Offer Decision Service] (ODS)选件无法呈现的问题。 相同的配置在旧版UI中可正常使用，这会导致混淆并阻止活动执行。 此修复可确保两个UI版本之间提供一致的选件，从而恢复AJO驱动型个性化的预期行为。
...
* **下载选项在更新的报告概述UI的报告部分中恢复。** 解决了新概述UI中“报表”部分缺少[!UICONTROL Download]按钮，导致用户无法导出属性重要性得分的问题。 此更新将恢复完整的导出功能，从而简化对用于分析和报告的可下载数据的访问。 (TGT-53222)
* 已在[!UICONTROL Important attributes]视图中恢复&#x200B;**[!UICONTROL Download full CSV report]按钮。** 解决了在更新的活动创建UI中，报告视图的[!UICONTROL Important Attributes]部分中缺少[!UICONTROL Download full CSV report]按钮的问题。 此修复可恢复对可下载见解的访问权限，从而确保更新和旧版UI中的功能一致。 (TGT-53238)
...
* **已更正更新的UI中参数存在条件的表单验证。** 解决了在更新的UI中，选择“[!UICONTROL Custom mbox parameter value is present]”或“[!UICONTROL Parameter is present]”错误地要求客户输入值的问题。 此行为与预期逻辑冲突，预期逻辑只是检查参数是否存在，而不管其值如何。 该修复将表单验证与预期行为保持一致，从而能够更顺畅地设置活动并支持完全采用更新后的UI。 (TGT-53638)
* 针对页面交付中的参数存在规则更正了&#x200B;**表单逻辑。&quot;** 解决了在更新的UI中选择页面投放规则（如&quot;[!UICONTROL Parameter is present]&quot;、&quot;[!UICONTROL Parameter is not present]&quot;、&quot;[!UICONTROL Parameter value is present]&quot;或&quot;[!UICONTROL Parameter value is not present]&quot;）时错误地要求用户输入其他参数值的问题。 此行为与旧版UI不一致，并且与在不指定值的情况下检测参数存在的预期逻辑相冲突。 此修复可恢复预期的规则配置行为，从而简化活动设置并提高可用性。 (TGT-53640)
* 在更新的UI中，**多页面规则生成器的验证逻辑已得到改进。** 解决了更新后的UI中多页面规则生成器存在的多个验证问题。 这些修复包括：
...
* **在更新的UI中修复了MVT和AP活动的位置名称持久性。** 解决了在更新的UI中，在[!UICONTROL Multivariate Test] (MVT)和[!UICONTROL Automated Personalization] (AP)活动中编辑的位置名称未正确保存的问题。 重命名位置后，在选项卡（如[!UICONTROL Targeting]和[!UICONTROL Experiences]）之间导航会导致名称恢复到其以前的状态。 此修复确保在选项卡间进行一致的位置命名，并提高了活动设置期间的可靠性。 (TGT-52927)
* **在MVT活动的“管理体验”面板中更正了位置标签。** 解决了在更新的UI中，[!UICONTROL Multivariate Test] (MVT)活动中的[!UICONTROL Manage Experiences]面板显示不一致的位置编号的问题。 此修复程序可确保位置标签按顺序排列并正确与用户定义的修改保持一致，从而提高体验设置期间的清晰度和可用性。 (TGT-52929)
...
* **计划在未来的活动在[!UICONTROL Activity]概述中错误地显示“[!UICONTROL Live]”状态**：客户观察到计划在未来开始的活动在[!UICONTROL Activity]概述中错误地标记为“[!UICONTROL Live]”。 已解决此状态不匹配，计划活动现在可正确显示为“[!UICONTROL Scheduled]”，而无需刷新页面。 (TGT-52835)
...
* [!UICONTROL Reports]部分中的&#x200B;**[!UICONTROL Graph view]未显示来自[!DNL Analytics]**&#x200B;的值：在Re端口部分中访问[!UICONTROL Graph view]的客户遇到数据丢失问题，所有值都显示为零。 此问题是由从[!UICONTROL Analytics]检索的数据不正确引起的。 [!UICONTROL Graph view]现在可按预期显示准确值。 (TGT-52792)
+++
...
* **使用Backspace清除所有日期值会导致页面崩溃**：使用Backspace清除“[!UICONTROL Specified Date & Time]”字段中的所有值时，[!UICONTROL Goals & Settings]分区中的计划活动的客户遇到崩溃。 此问题是由日期处理逻辑中的空引用错误导致的。 页面现在可以正常处理空日期输入而不会崩溃。 (TGT-53624)
* **由于有效负载无效，[!UICONTROL Product Catalog Search]中未出现任何产品**：访问[!UICONTROL Product Catalog Search]中[!UICONTROL Recommendations]分区的客户遇到了GraphQL有效负载无效导致的空结果。 此后端错误会导致产品数据无法正确加载。 现在，产品在更新后的UI中按预期显示。 (TGT-53630)
...
* **已停用更新的VEC**&#x200B;中的[!UICONTROL Failed]和[!UICONTROL Draft]活动部分：已从更新的VEC中删除[!UICONTROL Failed]和[!UICONTROL Draft]活动选项。 新的UI不再支持草稿状态，并且失败的营销活动不会存储在后端。 这些选项不再相关。 相关筛选器和后端字段（例如，`uiSyncFailed`、`errorMessage`）也已移除，以简化活动管理。 (TGT-53150)
...
* **修复了保存**&#x200B;后[!UICONTROL Offer Decisions]从活动中消失的问题：在活动创建过程中添加的[!UICONTROL Offer Decisions]在保存并重新打开活动后未保留。 在编辑动态内容并使用特定选择器和属性插入[!UICONTROL Offer Decisions]时，出现此问题。 [!UICONTROL Offer Decisions]在保存后现在可正确保留，而选择器将保持不变，从而确保定位和编辑行为保持一致。 (TGT-53434)
...
* **修复了在启用不带数据的[!DNL Recommendations] [!UICONTROL Promotion]时触发不明确的错误消息的问题**：在[!DNL Recommendations]活动中启用[!UICONTROL Front]或[!UICONTROL Back Promotion]而不指定所需值会导致泛型“输入错误无效”消息。 基础问题是缺少配置字段，但错误消息未明确说明原因，因此难以进行故障诊断。 现在，活动创建流程可在必填字段（如`collectionId`或规则）缺失时提供明确且可操作的错误消息，帮助客户快速识别并解决配置问题。 (TGT-52616)
* **修复了导致[!UICONTROL Product]列表无法在[!UICONTROL Recommendations]选项卡**&#x200B;的[!UICONTROL Edit]模式中显示的问题：在[!UICONTROL Recommendations]选项卡中编辑[!UICONTROL collection]或[!UICONTROL exclusion]时，客户无法查看筛选的产品列表。 该列表应基于应用的规则实时更新，但似乎未按预期进行。 此问题已得到解决，产品列表现在可正确显示，并且随着规则的修改而动态更新。 (TGT-53481)
* **修复了更新的UI中“查看详细信息”对话框的布局问题**：更新的UI中的“查看详细信息”模式布局已修改，以提高清晰度和可用性。 该对话框现在包含两个选项卡：
   ...
   * **修复了在保存后推荐活动中移除的促销重新显示的问题**：客户报告说，当[!DNL Recommendations]活动中移除[!UICONTROL front]或[!UICONTROL back]促销并保存该活动时，重新打开时促销会继续显示。 此问题在暂存环境和生产环境中都发生，并影响更新的活动创建过程。 问题已得到解决。 现在，从活动中移除的促销活动在保存后可正确保留。 (TGT-53490)
...
* **修复了活动创建过程中阻止升级到AP活动中的[!UICONTROL Targeting]步骤的问题**：修复了活动创建过程中客户无法继续到[!UICONTROL Automated Personalization] (AP)活动中的[!UICONTROL Targeting]步骤的问题（除非添加了两个位置）。 此行为与以前的体验不同，在以前的体验中，具有多个选件的单个位置便已足够。 该要求已得到纠正，允许客户继续将单个位置设置作为其AP工作流的一部分。 (TGT-53426)
...
* **修复了阻止客户在更新的UI中搜索报表包的问题**：在更新的UI中，[!UICONTROL Goals & Settings]部分中的[!UICONTROL Report Suites]下拉列表不允许客户键入和搜索，这使得难以找到特定的报表包，尤其是对于具有大量条目的租户。 与以前的UI不同，列表未排序，缺少基于输入的筛选。 此问题已得到解决。 客户现在可以键入以搜索和筛选报表包，从而恢复旧版UI中提供的功能。 (TGT-53514)
...
* **修复了以下问题：在活动创建过程中选择[!UICONTROL All Workspaces]时，受限于单个工作区的客户可以查看来自其他工作区的活动**：受限于一个工作区的客户意外地可以查看所有工作区的活动。 此可见性可能会对其他工作区中的实时活动造成意外更改，从而可能影响网站性能。 Workspace访问控制已得到增强，以确保客户只能查看其分配的工作区中的活动并与之交互。 (TGT-53101)
...
* **修复了在点击跟踪模式下跟踪的元素字段行为，以防止未保存的输入并提高清晰度**：修复了活动创建过程中出现的问题，即[!DNL Click Track]模式中的[!UICONTROL Tracked Element]字段可编辑，但未保留输入的名称，从而导致客户混淆。 字段现已禁用，以防止未保存的输入，并且已阐明选定元素的命名方式，以提高目标配置和跟踪准确性。** (TGT-53458)
...
* **修复了活动创建过程中允许对AEM导出的片段进行意外HTML编辑的问题**：修复了活动创建过程中允许客户编辑从[!DNL Target]中的[!DNL Adobe Experience Manager] (AEM)导出的[!DNL Experience Fragments] (XF)的HTML的问题。 这是意外行为，因为从AEM发布后，XF应该保持锁定状态以进行编辑。 此修复可确保“编辑HTML”选项不再可用于AEM导出的片段，并保留内容完整性和预期治理。 (TGT-53286)
...
* **修复了活动创建过程中无法在推荐视图中显示产品消息的问题**：修复了活动创建过程中即使存在消息，[!DNL Recommendations]视图中的[!UICONTROL Message]列也不会显示产品数据的问题。 客户必须手动移除并重新添加列以临时显示内容，在滚动或搜索时，这些内容通常会再次消失。 此更新可恢复产品消息的一致可见性，并改进目录导航和审核工作流。 (TGT-52777)
...
* **修复了VEC中自定义代码更改无法可靠地跨编辑会话保存的问题**：修复了在可视化体验编辑器(VEC)中进行的自定义代码修改无法在一次尝试中可靠地保存的问题。 客户报告，即使同时使用[!UICONTROL Edit Content]和最终[!UICONTROL Save]按钮，网站和QA URL中仍间歇性地缺少更改，例如样式更新或HTML编辑。 此回归已被解决，确保所有自定义代码更改在编辑会话间按预期持续存在。** (TGT-53418)
...
* 修复了更新的[!DNL Target] UI中的一个问题：在从[!UICONTROL Targeting]选项卡导航回[!UICONTROL Experiences]后，在[!UICONTROL Visual Experience Composer] (VEC)中做出的HTML选件修改无法保留。 (TGT-52874)
...
* 修复了源创建向导的[!UICONTROL Environment]下拉列表中的字符串“[!UICONTROL All host groups]”未正确本地化的问题。 (TGT-46737)
...
* 修复了在[!UICONTROL Goals & Settings]下选择[!UICONTROL Viewed an mbox]转化量度导致页面崩溃的问题。 （TGT-53346、TGT-53343 和 TGT-53348）
...
* 修复了在创建或编辑[!UICONTROL Automated Personalization] (AP)活动时，在[!UICONTROL Form-Based Experience Composer]中单击&#x200B;**[!UICONTROL Manage Content]**&#x200B;图标（ ![管理内容图标](/help/main/assets/icons/Experience.svg)）后导致编辑器崩溃的问题。 (TGT-53047)
...
* 修复了用户无法在[!DNL Target]用户界面中编辑site_cart_z1 [!DNL Recommendation]活动的问题。 尝试打开活动在[!UICONTROL Overview]页面上触发了一个错误，阻止了对编辑器的访问。 (TGT-53221)
...
* 修复了在VEC中配置图像选件时，[!UICONTROL Insert Before]和[!UICONTROL Insert After]选项的[!UICONTROL Undo]功能存在的问题。
...
* 已将[!UICONTROL Goals & Settings]中的旧版功能恢复到[!UICONTROL Priority]滑块，允许客户直接输入数值，如旧版UI中所支持。 （TGT-53185 和 TGT-53219）
...
* 修复了在更改收藏集或促销活动时，[!UICONTROL A/B Test]活动中作为选件的[推荐](/help/main/c-recommendations/recommendations-as-an-offer.md)未正确更新的问题。 (TGT-52884)
...
* 修复了一个问题，如果在流量分配设置期间首先选择[!UICONTROL Auto-Allocate] (AA)，则该问题阻止用户创建[!UICONTROL Auto-Target] (AT)活动。 此问题会导致后端验证错误，并阻止保存活动。 (TGT-53096)
...
* 修复了一个问题，在[!UICONTROL Goals & Settings]步骤中选择[!DNL Adobe Analytics] (A4T)作为报表源时，该问题阻止客户创建或保存活动。 选择[!UICONTROL Custom Event]量度（例如，“自定义事件16”）时具体出现问题，导致以下错误：“用户输入无效。” (TGT-52910)
* 修复了单击“[!UICONTROL View in Analytics]”链接会将用户重定向到主页而非预期的[!DNL Analytics]仪表板的问题。（TGT-53092和TGT-53093）
...
* 修复了在更新的[!UICONTROL Overview] UI中查看[!DNL Recommendations]活动时，选择[!DNL Adobe Analytics] (A4T)作为报表源时无法加载[!UICONTROL Goals & Settings]部分的问题。 显示以下错误消息：“出现错误。 我们无法完成您的请求。 如果问题仍然存在，请联系 Adobe 客户关怀部门。“ (TGT-52999)
...
* 修复了在使用[!UICONTROL promotions]添加[!DNL Recommendations]时影响活动创建工作流的问题。 当用户选择“[!UICONTROL Promote by Attribute]”并添加筛选规则（例如[!UICONTROL Parameter Matching]）时，在保存和重新编辑活动后未保留所选规则类型和操作数值。 重新打开时，筛选规则类型将意外更改，并且缺少操作数值。 (TGT-53059)
* 修复了[!DNL Recommendations] UI中的一个问题：使用单个规则创建的任何促销活动被错误地解释并显示为“项目列表”促销类型，而不管该规则的逻辑如何。 (TGT-53063)
* 修复了在使用更新的[!UICONTROL Overview]UI时，包含[!DNL Recommendations]的[!UICONTROL Experience Targeting] (XT)活动缺少“[!UICONTROL Download Recommendations Data]”按钮的问题。 （TGT-52730 和 TGT-52756）
...
* 修复了从[!UICONTROL Reports]页面中选择“[!UICONTROL Export order details to CSV]”导致下载空文件的问题。 即使活动中存在有效的订单数据，也会发生此问题。 (TGT-52225)
...
* 修复了在活动创建期间同时打开多个[!UICONTROL Visual Experience Composer] (VEC)实例的问题。 当用户在[!UICONTROL Page Delivery]步骤中禁用[!UICONTROL Enhanced Experience Composer] (EEC)并从网站URL中删除结尾斜杠时，会发生此问题。 (TGT-52782)
* 修复了以下问题：[!UICONTROL Goals & Settings]步骤中的[!UICONTROL Revenue]量度下拉列表错误地默认为[!UICONTROL Revenue per Visit] (RPVISIT)，即使用户选择了其他量度也是如此。  折叠和重新展开指标配置面板时出现问题，导致重置之前选择的值。 （TGT-52811 和 TGT-52878）
...
* 修复了以下问题：某些客户无法创建或保存活动，而其他客户可以正常执行相同的操作。 问题在帐户之间不一致。(TGT-52842)
...
* 解决了在检索[!DNL Analytics]量度列表时导致GET请求失败的问题。 (TGT-51106)
...
若要在手动呈现建议时受益于此优化，请查看您的[[!DNL Platform Web SDK implementation]](https://experienceleague.adobe.com/zh-hans/docs/target-dev/developer/client-side/aep/aep-web-sdk-overview){target=_blank}，以确保在手动呈现体验后或使用`applyPropositions`方法（或相应的[!DNL Launch]操作作为助手）呈现体验时发送通知。
...
* **拆分调用（也称为页面事件的顶部和底部）。**
  ...
  此实现类型是[!DNL Adobe]推荐的新[拆分调用实现方法](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/use-cases/top-bottom-page-events){target=_blank}。 使用此方法时，新的优化不会影响[!DNL Analytics]或A4T报表。
...
有关详细信息，请参阅&#x200B;*Adobe Target开发人员指南*&#x200B;中的[Target边缘节点](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/allowlist-edges.html?lang=zh-Hans){target=_blank}。
...
| 功能 | 详细信息 |
|--- |--- |
| 已为[!UICONTROL Auto-Allocate]和[!UICONTROL Auto-Target]优化A4T指标<p>（发布日期：2023 年 3 月 30 日） | 在为[!UICONTROL Auto-Allocate]和[!UICONTROL Auto-Target]活动使用[!UICONTROL A4T]时，[!DNL Target]允许您选择基于二项式事件的量度或基于连续事件的量度。<P>请注意，在支持的指标中发生了以下变化：<ul><li>[!DNL Target] 保留现有活动以前的行为直至 2023 年 9 月 9 日。 在此日期之后，将不再继续执行使用不支持的指标的活动，以强制现有活动采用新行为。</li></ul>有关详细信息，请参阅[!UICONTROL Auto-Allocate]和[!UICONTROL Auto-Target]活动[&#128279;](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md#supported)的A4T支持中的“支持的目标量度”。<br>以下教程已随此功能一并更新：<ul><li>[在 [!DNL Analysis Workspace] 中为[!UICONTROL Auto-Allocate]个活动设置A4T报告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=zh-Hans){target=_blank}</li><li>[在 [!DNL Analysis Workspace] 中为[!UICONTROL Auto-Target]个活动设置A4T报告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-target-activities.html?lang=zh-Hans){target=_blank}</li></ul> |
...
有关详细信息，请参阅&#x200B;*Adobe Target开发人员指南*&#x200B;中的[模型API概述](https://experienceleague.adobe.com/docs/target-dev/developer/api/models-api/models-api.html?lang=zh-Hans){target=_blank}。
...
| 适用于Google Chrome的新[!UICONTROL Visual Experience Composer]扩展 | Chrome Web Store中提供了适用于Chrome的新[!DNL Adobe Target] [!UICONTROL Visual Experience Composer] (VEC)扩展。<br>从2023年1月开始，当前[!DNL Target] VEC Helper扩展将停止在Google Chrome中运行，因为Google不允许使用清单V2的扩展。 下载新的扩展程序，从新的一年开始，继续在 [!DNL Target] 中以视觉方式创作您的网站。<br>以下链接显示了 Chrome 网络商店中的两个扩展：<ul><li>[新扩展](https://chrome.google.com/webstore/detail/adobe-experience-cloud-vi/kgmjjkfjacffaebgpkpcllakjifppnca){target=_blank}</li><li>[旧的扩展](https://chrome.google.com/webstore/detail/adobe-target-vec-helper/ggjpideecfnbipkacplkhhaflkdjagak){target=_blank}</li></ul>有关更多信息，请参阅[可视化编辑帮助程序扩展](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)。 |
...
| *Adobe Target 开发人员指南* | *Adobe Target 开发人员指南* 便于合并所有 [!DNL Target] 开发人员内容。 该指南包含有关实施[!DNL Target]和[!DNL Recommendations]、[!DNL Target] SDK以及[!DNL Target] API的信息。<br>有关详细信息，请参阅[Adobe Target Developer Guide](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank}。 |
...
* 为启用了[Business ID帐户](https://helpx.adobe.com/cn/enterprise/using/identity.html){target=_blank}和基于策略的身份验证(PBA)的组织启用了[!DNL Target]用户界面令牌刷新。 (TGT-42590)
...
| [!UICONTROL Audiences] UI刷新 | 作为[!DNL Adobe Target]团队持续努力改进[!DNL Target]用户体验的一部分，此版本刷新了[!DNL Target] UI中的[!UICONTROL Audiences]和[!UICONTROL Profile Scripts]页面。 此更新统一并标准化了以前不一致的设计模式，并添加了新的增强功能，例如：<ul><li>同时选择和删除多个受众的功能</li><li>刷新的[受众生成器设计](/help/main/c-target/c-audiences/create-audience.md)</li><li>[!UICONTROL Audience]库规则生成器中的排除规则支持</li><li>新的“受众Source”筛选器，可用于更快地发现受众</li><li>会话持久搜索和筛选选项</li><li>为 [!DNL Target Premium] 客户在工作区之间移动受众的功能。</li></ul>有关详细信息，请参阅[受众](/help/main/c-target/target.md).<br>**注意**：此功能将在八周后向不同地区的客户推出。 |
| [!UICONTROL Profile Scripts] UI刷新 | [!UICONTROL Profile Scripts]库也已更新，并且包含一个已刷新的界面和几项生产力更新：<ul><li>同时选择和删除多个轮廓脚本的功能</li><li>轮廓脚本的新代码编辑器</li><li>代码编辑器中的语法突出显示和错误检查</li><li>通过键盘快捷键自动完成令牌（mbox 或 profile）参数</li></ul>有关详细信息，请参阅[访客配置文件](/help/main/c-target/c-visitor-profile/visitor-profile.md).<br>**注意**：此功能将在八周后向不同地区的客户推出。 |
...
有关详细信息，请参阅[SDK开发人员指南](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank}中的[Adobe Target文档](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/python/overview.html?lang=zh-Hans){target=_blank}。
...
| 功能 | 详细信息 |
| --- | --- |
| [!UICONTROL Analytics for Target] (A4T)的重定向支持 | 使用[A4T](/help/main/c-integrating-target-with-mac/a4t/a4t.md)时，Platform Web SDK现在支持[!DNL Target]重定向。<br>有关详细信息，请参阅[Analytics for [!DNL Target] 实现](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)。 |
...
| 功能 | 详细信息 |
| --- | --- |
| ![Premium 徽章](/help/main/assets/premium.png) 自动定位活动支持 Analytics for Target (A4T) | [!UICONTROL Auto-Target]活动现在支持[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>此集成允许您使用[!UICONTROL Auto-Target]集成机器学习算法，根据每位访客的个人资料、行为和上下文选择最适合该访客的体验。<br>如果您已[实施A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以用于A/B测试和体验定位活动，则您已准备就绪！<br>有关详细信息，请参阅自动分配和自动定位活动支持[A4T](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| 对[!UICONTROL Auto-Allocate]活动的Analytics for Target (A4T)支持 | [!UICONTROL Auto-Allocate]活动现在支持[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>此集成允许您使用[!UICONTROL Auto-Allocate]多臂赌博机功能将流量引向入选体验，同时使用[!UICONTROL Adobe Analytics]目标量度和/或[!UICONTROL Adobe Analytics]报告和分析功能。<br>如果您已[实施A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以用于A/B测试和体验定位活动，则您已准备就绪！<br>有关详细信息，请参阅自动分配和自动定位活动支持[A4T](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
| 在 [!DNL Analysis Workspace]<br> 中支持 A4T 2020 年 6 月 25 日 | [!DNL Analysis Workspace]现在支持[!UICONTROL Anaytics for Target] (A4T)。 [!UICONTROL Analytics for Target (A4T) panel]允许您分析[!DNL Analysis Workspace]中的[!DNL Adobe Target]活动和体验。<br>有关详细信息，请参阅&#x200B;*A4T报告*&#x200B;中的[Analytics中的报告](/help/main/c-integrating-target-with-mac/a4t/reporting.md)和&#x200B;*Analytics工具指南*&#x200B;中的[Analytics for Target (A4T)面板](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/panels/a4t-panel.html?lang=zh-Hans)。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| at.js 1.8.2 | at.js 的此版本是一个维护版本，其中包括以下修复：<ul><li>修复了在使用CNAME和边缘覆盖时，at.js 1.*x*&#x200B;可能无法正确地创建服务器域，从而导致[!DNL Target]请求失败的问题。 (TNT-35064)</li></ul>有关更多信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html){target=_blank}。 |
| at.js 2.3.1 | 此版本的 at.js 是一个维护版本，它包括以下增强功能和修复：<ul><li>使得可通过 [targetGlobalSettings](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetglobalsettings.html){target=_blank} 覆盖 `deviceIdLifetime` 设置。 (TNT-36349)</li><li>修复了在使用CNAME和边缘覆盖时，at.js 2.*x*&#x200B;可能无法正确地创建服务器域，从而导致[!DNL Target]请求失败的问题。 (TNT-35065)</li><li>修复了一个问题，在使用 [!DNL Target] [!DNL Launch] 扩展程序 v2 和 [!DNL Adobe Analytics] [!DNL Launch] 扩展程序时，[!DNL Target] 延迟了 [!DNL Analytics] `sendBeacon` 调用。 （TNT-36407、TNT-35990、TNT-36000）</li></ul>有关更多信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html){target=_blank}。 |
...
* 以下指标已被弃用，无法在创建新 A4T 活动时被选为主要目标指标。
  ...
  | 平均访问深度 | 不适用 未建议作为主要目标指标 |
  | 机器人 | 不适用 未建议作为主要目标指标 |
  | 移动崩溃率、移动平均以前会话长度、移动应用商店平均排名、移动应用性能崩溃率、移动应用商店平均评分 | 不适用 未建议作为主要目标指标 |
...
| 功能/增强 | 描述 |
| --- | --- |
| Java SDK | 通过 [!DNL Target] Java SDK，可部署 [!DNL Target] 服务器端。 此Java SDK帮助您轻松将[!DNL Target]与其他[!DNL Adobe Experience Cloud]解决方案（如[!DNL Adobe Experience Cloud Identity Service]、[!DNL Adobe Analytics]和[!DNL Adobe Audience Manager]）集成。<br>Java SDK引入最佳实践，并消除在通过我们的投放API与[!DNL Target]集成时产生的复杂因素，这样工程团队即可专心处理业务逻辑。 以下是我们在最新版本中引入的重要功能：<ul><li>支持预取和通知，使您可以通过缓存优化性能。</li><li>支持在网页和服务器端都混合集成 [!DNL Target] 时优化性能。 我们正在引入一个称为 `serverState` 的设置，由通过服务器端检索的体验填充它，这样 at.js 2.2 将不再进行额外的服务器调用以检索体验。 此方法可优化页面加载性能。</li><li>支持通过 Java SDK 检索 VEC 创建的活动（通过新的投放 API 实现此检索）。</li><li>已开源，这样您的开发人员即可向 [Target Java SDK](https://github.com/adobe/target-java-sdk) 贡献代码。</li></ul>请在 Adobe Tech Blog - [用新的 Target Java SDK 进行服务器端优化](https://medium.com/adobetech/server-side-optimization-with-the-new-target-java-sdk-421dc418a3f2)上详细了解 Target Java SDK。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| ![Premium 徽章](/help/main/assets/premium.png) 基于用户的推荐<br>（2019 年 10 月 24 日） | 根据每位访客的浏览、查看和购买历史记录推荐商品。 这些项目通常称为“为您推荐”。<br>通过此标准，您可以为新访客和回访访客提供个性化的内容和体验。 推荐的列表根据访客最近的活动进行加权，在会话中更新，并随着访客浏览您的网站而逐渐更加个性化。<br>有关更多信息，请在[标准/算法](/help/main/c-recommendations/c-algorithms/algorithms.md#criteria-algorithms)中参阅“基于用户的建议”。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| Node.js SDK 1.0 版 | 通过Target Node.js SDK，您可以部署Target服务器端。<br>此Node.js SDK可帮助您轻松将Target与其他Experience Cloud解决方案（如Adobe Experience Cloud Identity Service、Adobe Analytics和Adobe Audience Manager）集成。<br>Node.js SDK引入最佳实践，并消除在通过我们的投放API与Adobe Target集成时产生的复杂因素，这样工程团队便可以专注于业务逻辑。 以下是我们在最新版本中引入的重要功能：<ul><li>支持预取和通知，使您可以通过缓存优化性能。</li><li>支持在网页和服务器端都混合集成 Target 时优化性能。 我们正在引入一个称为 `serverState` 的设置，将由通过服务器端检索的体验填充它，这样 at.js 2.2 将不再进行额外的服务器调用以检索体验。 此方法可优化页面加载性能。</li><li> 支持通过 Node.js SDK 检索 VEC 创建的活动（通过新的投放 API 实现此检索）。</li><li>已开源，这样您的开发人员即可向 Node.js SDK 贡献代码。</li></ul> |
...
| 功能/增强 | 描述 |
| --- | --- |
| 可视化体验编辑器 (VEC) | **新 VEC 菜单选项**：单击 VEC 中的某个页面元素时，相应菜单会显示适用于该元素类型的选项。<ul><li>您现在可以使用[!UICONTROL Styles > Background]选项更改所选元素的背景图像和颜色。 (TGT-15001)</li></ul>查看[可视化体验选项](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)中的&#x200B;*样式*。<br>**点击跟踪改进**：我们改进了VEC和单页应用程序(SPA) VEC中配置点击跟踪的过程。<ul><li>当选择要在点击跟踪中使用的元素时，所有可用元素的名称都会显示在右侧的“修改”面板中，从而允许快速、轻松地选择所需元素。</li><li>三步引导式活动工作流的[!UICONTROL Goals & Settings]页面会显示一个数字，用于指示为点击跟踪选择的元素数量。 您可以将鼠标悬停在此数字上，以查看所有选定元素的名称。 (TGT-33878)</li></ul>请参阅[点击跟踪](/help/main/c-activities/r-success-metrics/click-tracking.md)。 |
| 单页应用程序可视化体验编辑器 (SPA VEC) | **引导式工作流**：新的引导式工作流可帮助您了解应如何配置页面交付规则设置，以便为单页应用程序成功执行和运行活动。 (TGT-33718)<br>请参阅[单页应用程序(SPA)可视化体验编辑器](/help/main/c-experiences/spa-visual-experience-composer.md#page-delivery-settings)。<br>**克隆修改**：您现在可以使用SPA VEC定义修改，然后克隆该修改，以供在单页应用程序的其他视图中使用。 (TGT-33882)<br>请参阅[单页应用程序 (SPA) 可视化体验编辑器](/help/main/c-experiences/spa-visual-experience-composer.md)。 |
| ![Premium 徽章](/help/main/assets/premium.png) 自动个性化 (AP) 和自动锁定 | **将体验指定为控制**：在创建 AP 或自动锁定活动时，您可以选择要用作控制的体验。 通过此功能，您可以根据活动中配置的流量分配百分比，将整个控制流量路由到特定体验。 然后，您可以根据该体验的控制流量评估个性化流量的性能报表。 当前控制选项（随机提供的体验）将继续可用。 （TGT-32801、TGT-26572和TGT-26571）<br>请参阅[选择Automated Personalization或自动定位活动的控件](/help/main/c-activities/t-automated-personalization/experience-as-control.md)。<br>**Personalization Insights报表**：当访客在特定位置看到特定内容时，对营销人员友好的属性命名可提供更多有意义的信息。 （TGT-33421 和 TGT-34957）<br>请参阅[为 Target 个性化算法收集数据](/help/main/c-activities/t-automated-personalization/ap-data.md)。 |
...
| 客户端 Analytics 日志记录 | 无论是在客户端还是服务器端，均允许客户完全控制如何将分析数据发送到Adobe Analytics。<br>有关详细信息，请参阅&#x200B;*实施之前*&#x200B;中的[客户端Analytics日志记录](/help/main/c-integrating-target-with-mac/a4t/before-implement.md#client-side)。 |
| 发送通知 | 允许开发人员在通过代码而不是使用`applyOffer()`或`applyOffers()`呈现体验时发送通知。<br>有关详细信息，请参阅[adobe.target.sendNotifications(options)](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-sendnotifications-atjs-21.html){target=_blank}。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| [!UICONTROL Visual Experience Composer] | [!UICONTROL Visual Experience Composer] (VEC)包含以下增强功能，可让您更快捷、更高效地完成工作：<ul><li>现在，设置点击跟踪时，可使用 DOM 路径功能。<br>有关更多信息，请参阅[点击跟踪](/help/main/c-activities/r-success-metrics/click-tracking.md#considerations)。</li><li>使用“样式”面板可查看或编辑所选元素现有样式的值。 您还可以添加其他样式。<br>要访问“样式”面板，请单击VEC中的某个页面元素，然后单击“[!UICONTROL Edit]”>“[!UICONTROL Styles]”。<br>样式面板将显示在VEC的右侧。 该面板包含一个样式列表，允许您编辑样式或将其添加到所选元素。 如果您习惯使用层叠样式表 (CSS)，或者从开发人员那里接收代码，则可以使用实时 CSS 编辑器查看更改并添加样式。<br>有关更多信息，请参阅&#x200B;*可视化体验编辑器选项*&#x200B;中的[样式](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)。</li><li>富文本编辑器现在支持嵌套的HTML5元素。<br>HTML5规范允许嵌套标记的新组合。 富文本编辑器的早期版本不支持 HTML5 规范所允许的新嵌套标记。 因此，在 VEC 中选择的任何嵌套元素均无法正确处理，从而导致不必要的 HTML 更改。 (TGT-33618)<br>有关更多信息，请参阅&#x200B;*可视化体验编辑器选项*&#x200B;中的[编辑文本/HTML](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#edit-text-html)。</li> |
...
>[!NOTE]
>
>如果您需要[!DNL Adobe]选择加入对《通用数据保护条例》 (GDPR)提供支持，则您应当实施at.js 1.7.1。 at.js 2.*x*&#x200B;当前不支持选择加入支持。
...
| 功能/增强 | 描述 |
| --- | --- |
| at.js 版本 2.0.0<br>2019 年 2 月 19 日 | at.js 2.x 现已可用。<br>最新版本的 at.js 提供了丰富的功能集，使您的企业能够在下一代客户端技术上实现个性化。 此新版本着重升级了at.js，以便与单页应用程序(SPA)进行良性的交互。<br>以下是使用at.js 2.x的一些好处，这些好处在以前的版本中未提供：<ul><li>能够在页面加载时缓存所有产品建议，将多次服务器调用减少至一次服务器调用。</li><li>由于产品建议是通过缓存立即显示的，不存在传统服务器调用引入的任何时间延迟，因此极大地提升了最终用户在您网站上的体验。</li><li>通过简单的单行代码和一次性开发人员设置，您的营销人员能够通过可视化体验编辑器 (VEC) 在单页应用程序上创建和运行 A/B 和体验 (XT) 活动。</li></ul>at.js 2.x 引入了以下新函数：<ul><li>getOffers()</li><li>applyOffers()</li><li>triggerView()</li></ul>以下函数在引入 at.js 2.x 后被弃用：<ul><li>mboxCreate()</li><li>mboxDefine</li><li>registerExtension()</li></ul>有关详细信息，请参阅[从at.js 1.x升级到at.js 2.x](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html){target=_blank}和[at.js函数](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html){target=_blank}。<br>**注意**：如果您需要Adobe选择加入对[通用数据保护条例](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hans){target=_blank} (GDPR){target=_blank}的支持，则当前必须使用at.js 1.7.0。 at.js 2.x不支持选择加入支持。 |
| at.js 版本 1.7.0<br>2019 年 2 月 14 日 | at.js 1.7.0 现在可用。<br>此版本提供了 Adobe 选择加入支持。 通过 Adobe 选择加入，可轻松将 Adobe 解决方案与同意管理平台集成。<br>有关Adobe选择加入的更多信息，请参阅[隐私和《通用数据保护条例》](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hans){target=_blank} (GDPR){target=_blank}。<br>此版本还修复了Target可能使用来自重定向URL的参数覆盖重定向URL参数的问题。<br>**注意**：如果您需要Adobe选择加入对GDPR提供支持，则当前必须使用at.js 1.7.0。 at.js 2.x不支持选择加入支持。<br>有关所有版本的列表，请参阅[at.js版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html){target=_blank}。 |
...
| 轮廓脚本<br>2019 年 1 月 17 日 | 出于性能原因，我们建议返回值不要再超过 256 个字符。<br>对于字符串返回值，如果返回值的大小超过 2,048 个字符，则系统将禁用该脚本。<br>对于数组返回值，如果数组的连接值的大小超过 2,048 个字符，则系统将禁用该脚本。<br>有关字符限制和其他限制（选件大小、受众、配置文件、值、参数等）的详细信息 影响Target中活动和其他元素的其他限制，请参阅[限制](/help/main/r-troubleshooting-target/target-limits.md)。 |
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>这是一个维护版本，该版本解决了以下问题： </p> <p> 
     ...
     </ul> </p> <p> <p>重要信息：此外，at.js版本1.6.2还包含at.js版本1.6.1和1.6.0中包含的所有增强功能和修复。 这些版本不再可供下载。 如果您使用的是 1.6.1 或 1.6.0，我们建议您升级到 1.6.2 版本。 </p> </p> <p>有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html" format="html" scope="external">at.js 版本详细信息</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_5A60FFE5E86148F4BDC6A7031D03D6BA"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>可视化体验编辑器 (VEC) </p> </td> 
   <td colname="col2"> <p>单击“修改”面板中的操作时，VEC 会自动滚动网页，并高亮显示相应的元素。 您不再需要手动向下滚动以查找受修改影响的 HTML 元素。 </p> <p> <img src="assets/modifications_panel.png" id="image_6E01280636E34ADDA9527AD18A34310B" /> </p> <p>(TGT-30441) </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_6D99C48B72D24728BF623608053931D3"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>通过在Target活动中使用在AEM中创建的体验片段，您可以将AEM的易用性和强大与Target中强大的自动智能(AI)和机器学习(ML)功能结合使用，从而测试和大规模个性化体验。&amp;nbsp；&amp;nbsp； </p> <p>AEM 可将您的所有内容和资产汇集到一个中心位置，以帮助实施您的个性化策略。 通过 AEM，您能够在一个位置轻松创建适用于桌面、平板电脑和移动设备的内容，而无需编写代码。 无需为每个设备创建页面，AEM会自动使用您的内容调整每个体验。 </p> <p> 通过Target，您可以根据包含行为、上下文和离线变量的基于规则和人工智能驱动的机器学习方法的组合，大规模提供个性化体验。&amp;nbsp；通过Target，您可以轻松设置和运行A/B和多变量活动，以确定最佳的选件、内容和体验。 </p> <p>现在，有些从事优化和个性化工作的专业人士正在使用 Target 促进其业务发展，对这类人士而言，体验片段代表了在将内容/体验创建器与管理器关联方面迈出了巨大的一步。 </p> <p>有关更多信息，请参阅 <a href="/help/main/c-experiences/c-manage-content/aem-experience-fragments.md#topic_1E1E4EA01F074349B2CF8785387B5FE8" format="dita" scope="local">AEM 体验片段</a>。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>报表 </p> </td> 
   <td colname="col2"> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_C595EEF916494342AD99FF0FDF999927"> 
      ...
      <li id="li_21B31F91685C46CAA47688FDE5735312"> <p>当报表中具有统计意义时，Target现在会显示提升度范围。（TGT-27301、TGT-27794和TGT-26387） </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> <span class="filepath">at.js</span> 版本 0.9.6 现已可用。 有关更多信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html" format="dita" scope="local">下载 at.js</a>。 </p> <p><span class="filepath">at.js</span> 版本 0.9.6 中包含以下增强功能和修复： </p> <p> 
     <ul id="ul_108DF85393614C69988E299485D338FD"> 
      ...
      <li id="li_3B1F618DEC744056B5BB172C4DBB359A"> <p>用于提取保存Cookie时应使用的顶级域的算法在<span class="codeph"> at.js </span>版本0.9.6中已更改。 由于此更改，Cookie无法保存到使用IP的地址。 在大多数情况下，IP 地址都用于测试目的，但作为变通方法，您可以使用 DNS 条目或调整本地框中的主机文件。 </p> </li> 
...
>[!NOTE]
>
>带有FP-11577的[!DNL Adobe Experience Manager] 6.2（或更高版本）现在支持通过其[!UICONTROL Adobe Target Cloud Services]集成来实施[!DNL at.js]。 有关更多信息，请参阅 Adobe Experience Manager 6.2 ** 文档中的[功能包](https://experienceleague.adobe.com/docs/?lang=zh-Hans)和[与 Adobe Target 集成](https://experienceleague.adobe.com/docs/?lang=zh-Hans)。
...
[Target Standard 1.7版本网络研讨会](https://my.adobeconnect.com/p1oabaz3cxi/)
