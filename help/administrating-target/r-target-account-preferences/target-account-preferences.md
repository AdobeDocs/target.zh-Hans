...
| 加载混合内容 | 使用增强体验书写器(EEC)打开网站时启用混合内容。启用此选项可避免通过Target代理服务器加载静态资源的额外开销。<br>此选项很有用，例如，如果您的内容安全策略(CSP)标题允许加载混合内容，而不在启用EEC的情况下使用代理服务器。<br>如果您的HTTP网站在EEC中增加加载时间，则此选项也很有用，其中JavaScript、图像等更长时间通过代理加载。 |
