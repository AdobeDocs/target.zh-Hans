---
description: “活动”可让您测试页面设计并将内容定位到特定受众。
keywords: 活动列表;活动;活动类型;编辑活动;活动操作;活动属性;活动列表筛选器;活动限制
seo-description: 通过Adobe Target中的活动，您可以测试页面设计和针对特定受众的目标内容。
seo-title: Adobe Target 中的活动
solution: Target
title: 活动
topic: Standard
...
“活动”可让您测试页面设计并将内容定位到特定受众。活动可决定网站访客可能遇到的体验。

例如，您可以设计一种可测试两个不同登陆页面的活动，一个突出显示女鞋的信息，另一个是突出显示更一般夏季服装的登录页。此活动确定了用于控制这两个登陆页面分别何时显示的条件，以及用于决定哪个页面更为成功的量度。此活动配置为在满足特定条件时开始和结束，例如在具体日期开始和结束，或者在活动得到批准后开始并在活动停用后结束。
...
打开 [!UICONTROL 时，“活动] ”列表是默认视图 [!DNL Target]。您可以从此页面创建新活动并管理现有活动。

您还可以通过单击 [!UICONTROL UI顶部] 的 [!UICONTROL “活动] ”选项卡来显示“活动”列表 [!DNL Target] 。
...
| 属性 | 显示活动 [的属性](/help/administrating-target/c-user-management/property-channel/property-channel.md) 。 |
...
| 删除 | 删除草稿或活动。<BR>**注意**：删除的活动无法恢复。除非绝对确定您绝对不需要此活动，否则请使用 [!UICONTROL 存档] 操作。然后，您可以根据需要重新激活活动。 |
...
## 对活动列表进行排序和筛选 {#section_41DAD479FFF740E2BB67BF4825955670}
...
## 提示与技巧 {#section_F77F30A246A14B538D9363B7F3639F97}
...
## 已将导入Target的属性用于Target之外更新的属性 {#section_802B0D174E6A44E1A96F404CA81AAE44}
...
>[!VIDEO](https://video.tv.adobe.com/v/18550?captions=chi_hans)
