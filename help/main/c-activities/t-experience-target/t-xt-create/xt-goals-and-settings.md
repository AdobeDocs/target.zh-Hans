...
# [!UICONTROL Experience Targeting] (XT)活动中的目标和设置

在[!UICONTROL Goals & Settings]页面中输入有关测试目标的信息：
...
如果未在[!UICONTROL Administration]中启用此选项（默认），请指定优先级： [!UICONTROL Low]、[!UICONTROL Medium]或[!UICONTROL High]。
...
对于在以前的[!DNL Target]版本中创建的活动，[!UICONTROL Low]优先级将转换为0，[!UICONTROL Medium]优先级将转换为5，[!UICONTROL High]优先级将转换为10。 您可以根据需要调整这些值。
...
>[!NOTE]
>
>如果您将 [!DNL Analytics] 用作报表源，则设置会由 [!DNL Analytics] 服务器来管理。[!UICONTROL Advanced Settings]选项不可用。
...
3. 将所需量度从左侧窗格拖放到右侧窗格中，然后单击&#x200B;**[!UICONTROL Reached]**&#x200B;以在[!UICONTROL Reached]和[!UICONTROL Not Reached]之间切换设置。
...
* 选择[!UICONTROL Increment Count & Keep User in Activity]以指定递增计数的方式。
