...
这可以通过列入允许列表以下IP地址来解决。 这些IP地址用于Adobe服务器，用于EEC代理。 仅在编辑活动时才需要这些地址。访客到您的站点不需要这些IP地列入允许列表址。
...
* **问题：** 您的网站域(ISP)正在阻止增 [!UICONTROL 强体验编辑器]。

   **Remedy:** 允许列表上面列出的IP地址。

* **问题：** IP地址已 [!DNL Target] ，但您的网站不支持TLS版本1.2。 [!DNL Target] 当前使用默认配置1.2。在18.4.1（2018年4月25日）之前， [默认配置支持TLS 1.0。有关详细信息，请参阅](/help/c-implementing-target/c-considerations-before-you-implement-target/tls-transport-layer-security-encryption.md#concept_CC1001E9D3AE4BABAF90B8311B0A6451)TLS（传输层安全）加密更改。
...
您可能会看到上面“The [!UICONTROL Enhanced Visual Experience Composer] won&#39;t load on my site上的安全页面”中所述的错误消息。 如果上述IP地列入允许列表址已，但您的网站不支持TLS版本1.2。[!DNL Target]当前使用默认配置1.2。在[!DNL Target] 18.4.1（2018年4月25日）之前，默认配置支持TLS 1.0。有关详细信息，请参阅[TLS（传输）层安全性)加密更改](/help/c-implementing-target/c-considerations-before-you-implement-target/tls-transport-layer-security-encryption.md#concept_CC1001E9D3AE4BABAF90B8311B0A6451)。
