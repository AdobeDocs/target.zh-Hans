...
description: 了解如何在Adobe Target Recommendations中创建标准和促销的包含规则。 添加其他动态或静态筛选规则以获得更好的效果。
...
有关在[!DNL Adobe Target]中为标准和促销创建包含规则以及添加其他动态或静态筛选规则以更好地实现推荐的信息。
...
* 动态包含规则通过匹配用户的用户档案参数或mbox调用中的属性来提供推荐。
   ...
   例如，您可以创建“最受欢迎的标准”推荐，然后创建返回的推荐集，然后根据用户访问显示推荐的页面时传递的属性筛选任何推荐（实时）。
...
以下示例提供了有关如何在营销工作中使用动态促销的一般说明：

| 运算符 | 示例 |
| --- | --- |
| 等于 | 在动态促销中使用“equals”运算符，当访客查看您网站上的项目（如产品、文章或电影）时，您可以从以下位置提升其他项目：<ul><li>同一品牌</li><li>同一类别</li><li>同一类别和同一厂商</li><li>同一商店</li></ul> |
| 不等于 | 在动态促销中使用“does not equal”运算符，当访客在您的网站上查看项目（如产品、文章或电影）时，您可以从以下位置提升其他项目：<ul><li>不同的电视连续剧</li><li>不同的类别</li><li>不同的产品系列</li><li>不同的风格 ID</li></ul> |
| 介于 | 在动态促销中使用“is between”运算符，当访客在您的网站上查看项目（如产品、文章或电影）时，您可以提升以下项目：<ul><li>更贵</li><li>更便宜</li><li>费用高或低 30%</li><li>同一季的后续集数</li><li>一套书中之前出版的书</li></ul> |
...
| 操作 | 适用选项 | 详细信息 |
|--- |--- |--- |
| [!UICONTROL 忽略此筛选规则] | [!UICONTROL 用户档案属] 性匹配 [!UICONTROL 与参数匹配] | 这是[!UICONTROL 用户档案属性匹配]和[!UICONTROL 参数匹配]的默认操作。<br>该选项指定忽略该规则。例如，如果有三个筛选规则，第三个规则不传递任何值，则您只需忽略具有空值的第三个规则，而不是不返回任何结果。 |
