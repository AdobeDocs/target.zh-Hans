...
1. 单击 **[!UICONTROL Create Script]**。
   ...
   | 参数类型 | 描述 |
   |--- |--- |
   | mbox | 在创建 mbox 时通过页面代码直接传入。 请参阅&#x200B;*Target开发人员指南*&#x200B;中的[将参数传递到全局Mbox](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/global-mbox/pass-parameters-to-global-mbox.html){target=_blank}。<P>**注意**：[!DNL Target] 对于每个 mbox 调用有 50 个唯一轮廓属性的限制。 如果必须将超过50个配置文件属性传递到[!DNL Target]，请使用[!UICONTROL Profile Update API]方法传递它们。 有关详细信息，请参阅&#x200B;*Target开发人员指南*&#x200B;中的[更新配置文件](https://experienceleague.adobe.com/docs/target-dev/developer/api/profile-apis/profile-api-overview.html){target=_blank}。 |
...
由于配置文件脚本在服务器端执行，因此它们无法直接读取页面。 数据必须通过mbox请求或将数据传入Target](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/methods-to-get-data-into-target.html){target=_blank}的其他[方法传入。 当数据进入 [!DNL Target] 之后，轮廓脚本可按照 mbox 参数或轮廓参数的形式读取数据。

## 脚本轮廓参数的 JavaScript 参考

需要具备简单的Javascript知识才能有效地使用脚本配置文件
...
| `//` | 检查源是否包含目标布尔值包含的所有元素（源数组，目标数组）。<br>`//` 从目标（对应于regexp）中提取子字符串并将它解码`Array/*String*/ decode(String encoding, String regexp, String target)`。<br>此功能还支持使用常量字符串值、分组(`condition1 \|\| condition2) && condition3`)和正则表达式(`/[^a-z]$/.test(landing.referring.url)`)。 |
