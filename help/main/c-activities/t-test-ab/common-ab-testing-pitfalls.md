---
keywords: AB;A/B;AB...n；错误；陷阱；错误；陷落；显着性；入选者；统计差异；统计功能；流量分配；分配
description: 了解如何避免公司在Adobe中执行A/B测试时最常出现的陷阱和错误 [!DNL Target] 和其他测试解决方案。
title: 如何避免常见的A/B测试错误？
feature: A/B Tests
...
# A/B 测试常见的十个隐患以及如何避免它们

中的A/B测试 [!DNL Adobe Target] 是大多数数字营销优化计划的支柱，可帮助营销人员为其访客和客户提供经过优化和有针对性的体验。 本节概述了公司在执行A/B测试时可能遇到的十种最重要的陷阱。 此外，本节还介绍了避免这些陷阱的方法，从而使您的公司可以通过这种测试工作获得更高的投资回报率 (ROI)，并对报告的 A/B 测试结果更有信心。

## 陷阱1:忽视显着性水平的影响 {#section_55F5577A13C6470BA1417C2B735C6B1D}
...
## 陷阱2:在没有具有统计意义显着差异的情况下，确定多个选件测试的入选者 {#section_FA83977C71DB4F69B3D438AF850EF3B6}
...
## 陷阱3:忽视统计功效的影响 {#section_0D517079B7D547CCAA75F80981CBE12A}
...
## 步骤4:使用单侧检验 {#section_8BB136D1DD6341FA9772F4C31E9AA37C}
...
## 陷阱5:监控测试 {#section_EA42F8D5967B439284D863C46706A1BA}
...
## 陷阱6:提前终止测试 {#section_DF01A97275E44CA5859D825E0DE2F49F}
...
## 陷阱7:在测试期间更改流量分配 {#allocation}

我们建议您不要更改测试期间的流量分配百分比，因为这可能会使测试结果在数据正常化之前出现偏差。
例如，假设您有一个A/B测试活动，其中80%的流量被分配给体验A（控制），20%的流量被分配给体验B。在测试期间，您会将每个体验的分配更改为50%。 几天后，您会将流量分配更改为100%以包含体验B。

在此方案中，如何将用户分配到体验？

如果您为体验B手动将分配拆分更改为100%，则最初分配到体验A（控制）的访客将保留在最初分配的体验（体验A）中。 流量分配的更改仅影响新参加者。

如果您想要更改百分比或极大地影响进入每个体验的访客流量，我们建议您创建一个新活动或复制活动，然后编辑流量分配百分比。

如果您在测试期间更改了不同体验的百分比，则需要几天时间数据才能正常化，尤其是当许多购买者是回访访客时。
再举一个示例，如果A/B测试的流量分配被拆分为50/50，然后您将拆分更改为80/20，那么在更改后的前几天，结果可能看起来会偏斜。 如果转化的平均时间较长（即某人需要数小时甚至数天才能完成购买），则这些延迟的转化可能会影响您的报表。 因此，在第一个体验中，数字从50%变为80%，而转化的平均时间为两天，在测试的第一天只有50%的访客进行转化，尽管今天有80%的访客正在进入体验。 这样看来转化率直线下降，但在80%的访客花了两天时间进行转化后，转化率将恢复正常。

## 陷阱8:不考虑新奇效应 {#section_90F0D24C40294A8F801B1A6D6DEF9003}
...
## 陷阱9:未考虑考虑考虑期间的差异 {#section_B166731B5BEE4E578816E351ECDEA992}
...
## 陷阱十：使用的量度不反映业务目标 {#section_F0CD6DC7993B4A6F9BEEBB31CD1D9BEE}
...
## 结论：通过识别并避免陷阱成功进行A/B测试 {#section_54D33248163A481EBD4421A786FE2B15}
