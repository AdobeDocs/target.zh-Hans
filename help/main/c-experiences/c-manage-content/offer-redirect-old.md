...
>[!NOTE]
>
> * 可在[!UICONTROL 选件] > [!UICONTROL 代码选件]页面上或[基于Forms的体验编辑器](/help/main/c-experiences/form-experience-composer.md)中创建重定向选件。 您无法在可视化体验编辑器(VEC)中创建或应用重定向选件。 内容将插入到[!DNL Target]请求位置，因此这些位置可能不适合全局[!DNL Target]请求。
>
...
您必须使用[!UICONTROL 基于表单的体验编辑器]来应用重定向选件。 您当前无法使用VEC应用重定向选件。

[!DNL Adobe Target] [!UICONTROL 基于表单的体验编辑器]是一个非可视化体验和选件创建界面，当可视化体验编辑器不可用或不实用时，它有助于创建在[!UICONTROL A/B测试]、[!UICONTROL 体验定位] (XT)、[!UICONTROL Automated Personalization] (AP)和[!UICONTROL 推荐]活动中使用的体验。 例如，您可以使用基于表单的[!UICONTROL 体验编辑器]创建使用重定向选件的体验。
...
1. 单击&#x200B;**[!UICONTROL 内容]**&#x200B;部分中的下拉列表，然后单击&#x200B;**[!UICONTROL 更改重定向选件]**。

   ![更改重定向选件选项](/help/main/c-experiences/c-manage-content/assets/change-redirect-offer-option2.png)

1. 从[!UICONTROL 选择远程选件]对话框中选择所需的重定向选件，然后单击&#x200B;**[!UICONTROL 完成]**。
