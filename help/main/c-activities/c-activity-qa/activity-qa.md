...
[!UICONTROL 活动QA]允许您在启动[!DNL Target]活动之前，对其进行全面测试。 [!UICONTROL 活动QA]功能包括：
...
* 能够对整个用户历程进行 QA。 在活动 QA 过程中，只需使用 QA 链接访问网站一次，即可浏览整个网站。 在结束会话或使用[QA Target书签](/help/main/c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)强制自己退出[!UICONTROL 活动QA]之前，您将一直保留在活动QA中。 如果您的活动跨多个网页，则此功能非常有用。

  >[!NOTE]
  >
  >此功能适用于版本2.*x*&#x200B;或更高版本的at.js实施。 对于at.js 1.*x*&#x200B;实施，仅当访客的浏览器不阻止第三方Cookie时，此功能才为true。

## 访问和共享 QA URL {#section_1C59BAA247B247BDB125D1BE8EAD4547}

1. 在活动的[!UICONTROL 概述]页面中，单击&#x200B;**[!UICONTROL 活动QA]**。
...
1. 与组织成员共享活动链接URL以进行测试。
   ...
   活动链接永不过期，如果有人更改了活动或体验，则无需重新发送链接。 但是，如果您应用与[!UICONTROL 受众库]不同的受众，而不是简单地编辑该活动，则会生成一个您必须再次共享的新链接。
   ...
   您可以导航网站以查看其他页面，因为[!UICONTROL 活动QA]模式具有粘性。 对于版本2.*x*&#x200B;或更高版本的at.js实施，此情况为真。 对于at.js 1.*x*&#x200B;实施，仅当访客的浏览器不阻止第三方Cookie时，才会出现这种情况。

1. 要查看从活动链接URL生成的报表，请单击活动的&#x200B;**[!UICONTROL 报表]**&#x200B;页面，单击&#x200B;**[!UICONTROL 设置]**&#x200B;图标（![icon_gear图像](assets/icon_gear.png)），然后从&#x200B;**[!UICONTROL 环境]**&#x200B;下拉列表中选择&#x200B;**[!UICONTROL QA模式流量]**。

## 正在从QA模式中释放您自己

[!UICONTROL 活动QA]为粘性。 在[!UICONTROL 活动QA]中浏览网站后，必须终止[!DNL Target]会话，或者必须让[!DNL Target]从[!UICONTROL 活动QA]中释放您，然后才能像普通访客一样查看您的网站。

### at.js 2.*x*

如果您的网站部署了at.js 2.*x*，请使用[Target QA书签](/help/main/c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)强制自己退出[!UICONTROL 活动QA]。 在网站上加载具有空值的页面（如下一个项目符号中所述），在部署at.js 2.*x*&#x200B;时，*不会*&#x200B;从浏览器中删除QA Cookie。

### at.js 1.*x*

如果您的网站部署了at.js 1.*x*，则除了使用[Target QA书签](/help/main/c-activities/c-activity-qa/activity-qa-bookmark.md#concept_A8A3551A4B5342079AFEED5ECF93E879)之外，您还可以手动强制自己退出，方法是在网站上加载一个具有空值的`at_preview_token`参数的页面。 例如：
...
* 如果帐户中保存的活动过多，则可能无法加载已保存活动的[!UICONTROL 活动QA]预览链接。 重试预览链接应该有效。 为防止这种情况继续发生，请存档不再主动使用的已保存活动。
* [!UICONTROL 活动QA] URL可用于将[Analytics作为报表源](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T)的活动。 使用[!UICONTROL 活动QA]执行QA时生成的点击量流向与活动数据流相同的报表包，即使活动已上线。
* [!UICONTROL 活动QA]不显示已存档活动或已超过其结束日期的活动内容。 如果取消激活已结束的活动，则必须再次保存该活动，以便[!UICONTROL 活动QA]正常工作。
...
* 所有 `at_preview` 参数和值均已进行 URL 编码。 大多数时候，一切都会按预期运行。 但是，有些客户必须加载均衡器或Web服务器，以尝试对查询字符串参数再次进行编码。
  ...
  [!DNL Adobe]建议您与IT团队联系，以确保所有预览参数均被转换，以便这些值不会发生任何形式的转换。
...
* [!UICONTROL Target QA模式] UI仅显示多页面活动中体验的第一个URL。 假设您要创建一个历程测试，并从URL1移动到URL2。 但是，如果要单独转到 URL2，请复制针对 URL1 提供的所有 URL 参数，并在放置“?”后将其应用于 URL2， 正如 URL1 中所看到的一样。
...
| [!UICONTROL Auto-Target（自动定位）] | 是 | 是 | 是 |
