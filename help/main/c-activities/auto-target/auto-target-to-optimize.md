...
* 注意活动形状。 与原始体验计数相比，性能更多地取决于位置×选件的数量。 大型笛卡尔组合可能会减慢[!UICONTROL Visual Experience Composer]中的加载和编辑速度，即使低于记录的体验限制也是如此。

  作为最佳实践，请将[!UICONTROL Auto-Target]和[!UICONTROL Automated Personalization]活动保留到4-6个位置，每个位置具有4-6个选件。 不建议使用较大的配置。 由于这些活动直接在[!UICONTROL Experiences]步骤中生成，因此，当配置超出支持的阈值时，[!DNL Target] UI可能会显示内联警告或阻止保存。
