...
description: 了解如何在Adobe [!DNL Target] Recommendations活动中根据使用访客行为上下文的键使用推荐来显示相关结果。
title: 如何使推荐基于推荐键？
...
基于算法的Recommendations使用访客行为上下文来显示[!DNL Adobe Target] [!DNL Recommendations]活动中的相关结果。

[!DNL Target Recommendations]中有四种算法类型：
...
>[!NOTE]
>
>下表描述了[!UICONTROL 基于购物车]类型，该类型即将推出。
...
有关创建标准以及定义其算法类型和算法的更多信息，请参阅[创建标准](/help/c-recommendations/c-algorithms/create-new-algorithm.md)。

不同的推荐算法会将自己放置在不同类型的页面上。 有关每种算法类型及其可用算法的更多信息，请参阅以下各节。

## [!UICONTROL 基于热门程度]

[!UICONTROL 基于热门程度]算法类型允许您根据网站上某个项目的整体热门程度或根据用户最喜爱或最常查看的类别、品牌、流派等项目的热门程度进行推荐。

[!UICONTROL 基于热门程度]算法类型提供以下算法：
...
通过[!UICONTROL 基于项目的]推荐类型，您可以根据查找与用户当前正在查看或最近已查看的项目类似的项目来进行推荐。

[!UICONTROL Item-Based]算法类型提供以下算法：
...
[!UICONTROL User-Based]算法类型提供以下算法：

### 最近查看的项目 {#recently-viewed}

使用访客的历史记录（跨会话）根据设计中的版块数量显示访客最近查看过的 *x* 个项目。

“最近查看的项目”算法返回特定于给定[环境](/help/administrating-target/hosts.md)的结果。 如果两个网站属于不同的环境，并且访客在两个网站之间切换访问，则每个网站仅显示相应网站的最近查看的项目。如果两个网站位于同一环境中，并且访客在两个网站之间切换访问，则访客将看到两个网站的最近查看的相同项目。

>[!NOTE]
>
>您不能使用[!UICONTROL 最近查看的项目]标准进行备用推荐。
...
此功能意味着您可以使用[!DNL Target]在离线计算的推荐或管理的自定义列表上添加个性化。 这样做可以将数据科学和研究与 Adobe 行之有效的众多功能（包括交付、运行时筛选、A/B 测试、定位、报表、集成，等等）结合起来。
...
在您的网站上使用[!UICONTROL 上次购买的项目]推荐键：
...
在您的网站上使用[!UICONTROL 上次查看的项目]推荐键：
