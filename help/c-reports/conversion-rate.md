---
keywords: 定位
description: 了解Adobe Target如何显示和计算每个体验的转化率、提升、置信度（统计意义）和置信区间。
title: 如何视图转化率、提升和信心级别？
feature: Reports
...
对于 A4T，我们会使用 Student t 检验来计算连续变量（而非二进制量度）。在 Analytics 中，会始终跟踪访客，并计入所执行的每项操作。因此，如果访客进行了多次购买或多次访问了某个成功量度，则会计入这些额外的点击。这会使量度变为连续变量。为了进行学生的T测试计算，需要“平方和”来计算方差，该方差用于t统计的分母。 [本文档介绍](/help/assets/statistical-calculations.pdf) 所用数学公式的详细信息。可从[!DNL Analytics]检索平方和。 要获取平方和数据，您需要导出样本时间段内要优化的量度在访客级别对应的数值。
...
[!DNL Target] 建议使用两个样 [本学生t测试](https://en.wikipedia.org/wiki/Student%27s_t-test#:~:text=The%20t%2Dtest%20is%20any,the%20test%20statistic%20were%20known.)，因为这些测试比z测试等替代测试需要更少的假设，并且是在控制体验和备用体验之间对业务指标进行成对比较（定量）的适当统计测试。

### 更详细

运行在线A/B测试时，每个用户/访客会随机分配给单个变体。 随后，我们会计量所关注的业务指标（例如转换率、订单、收入等） 适用于每个变体中的访客。 我们使用的统计测试然后测试平均业务量度的假设验证(例如转化率、每用户订单、每用户收入等) 等于控件和给定的替代变体。

虽然业务度量本身可能根据某些任意分布进行分配，但此度量的平均分布（在每个变体中）应通过[中心极限定理](https://en.wikipedia.org/wiki/Central_limit_theorem)收敛到正态分布。 请注意，虽然不能保证此平均采样分布将以多快的速度收敛到正常，但是，在在线测试中，通常要考虑访客的规模，才能达到此条件。

在平均值的这种正常性下，可以显示要使用的测试统计值遵循t分布，因为它是通常分布值（业务度量的手段的差）与基于数据的估计的缩放项的比值（装置差的标准误差）。 **学生的t-test**&#x200B;是适当的假设验证测试，假定测试统计服从t分布。

### 为什么不使用其他测试

**z-test**&#x200B;是不合适的，因为在典型的A/B测试场景中，测试统计的分母不是从已知方差派生的，而是必须从数据中估计。

**不使用** Chi-quarded测试，因为这些测试适用于确定两个变体之间是否存在定性关系(即变体之间没有差异的空假设验证)。T-测试更适合于&#x200B;_定量_&#x200B;比较度量的情景。
