...
[!DNL Adobe Target]中的自动分配活动可在两个或更多体验中标识一个入选者，并在测试继续运行和学习期间，自动为入选者重新分配更多流量以提高转化。
...
>[!NOTE]
>
>如果活动只有两个体验，则两个体验的流量会相等，直到[!DNL Target]找到置信度为75%的入选体验为止。 此时，2/3的流量会分配给入选者，1/3的流量会分配给失败者。 之后，当体验达到95%的置信度时，90%的流量会分配给入选者，10%的流量会分配给失败者。 为了避免长期出现误报（即保持一些探索），我们始终会保持一些流量会被发送到“丢失”体验。
...
是. 有关详细信息，请参阅[自动分配和自动定位活动支持 A4T](/help/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。
...
您可以使用现有的[样本量计算器](https://experienceleague.adobe.com/tools/calculator/testcalculator.html)来估算测试运行的时长。 （与传统A/B测试一样，如果您测试的选件超过两个或多个转化量度/假设，则应用Bonferroni校正。） 请注意，此计算器专为传统的固定水平A/B测试而设计，仅提供估算。 将计算器用于自动分配活动是可选的，因为自动分配将为您声明入选者（您无需选择固定时间点来查看测试结果），提供的值始终具有统计学上的有效性。 在我们的实验中，我们发现了以下内容：
* 在恰好测试两个体验时，如果体验之间的性能差异较大，则自动分配会比固定时间域测试（即样本量计算器建议的时间范围）更快地找到入选者，但当体验之间的性能差异较小时，可能需要额外的时间来确定入选者。 在这些情况下，固定水平测试通常会在没有统计意义显着结果的情况下结束。
* 在测试两个以上的体验时，如果单个体验的效果远远超出所有其他体验，则自动分配会比固定水平测试（即样本量计算器建议的时间范围）更快地找到入选者。 如果两个或多个体验都是与其他体验“入选”，但是它们之间存在紧密的匹配，则自动分配可能需要额外的时间来确定哪个体验更优越。 在这些情况下，固定视野测试通常会通过以下结论结束： “入选”体验优于性能较低的体验，但没有确定哪个体验更优。

### 我是否应该从自动分配活动中删除性能不佳的体验，以加快确定入选者的过程？

确实没有理由删除性能不佳的体验。 自动分配可以更频繁地自动提供高性能体验，而较少地提供性能不佳的体验。 在活动中保留性能不佳的体验不会对确定入选者的速度产生重大影响。

20% 的访客将被随机分配给所有体验。向性能不佳的体验提供的流量很少（20%除以体验数量）。

### 我是否可以在自动分配活动的中途更改目标量度？{#change-metric}
...
### 在运行自动分配活动时能否使用“重置报表数据”选项？

不建议对[!UICONTROL 自动分配]活动使用[!UICONTROL 重置报表数据]选项。 尽管它删除了可见的报表数据，但此选项并未从[!UICONTROL 自动分配]模型中删除所有培训记录。 对于[!UICONTROL 自动分配]活动，请不要使用[!UICONTROL 重置报表数据]选项，而是创建新活动并取消激活原始活动。 (注：本指南还适用于[!UICONTROL 自动定位]和[!UICONTROL Automated Personalization]活动。)

### 自动分配如何针对环境构建模型？

[!UICONTROL 自动分] 配仅基于默认环境中记录的流量和转化行为构建模型。默认情况下，[!UICONTROL Production]是默认环境，但可以在Target [Administration > Environments](/help/administrating-target/environments.md)中更改此设置。

如果点击发生在其他（非默认）环境中，则流量将根据默认环境中观察到的转化行为进行分配。 该点击的结果（转化或非转化）将记录为用于报告目的，但未在[!UICONTROL 自动分配]模型中考虑。

选择其他环境时，报表将显示该环境的流量和转化。 报表的默认选定环境将始终为所选帐户范围的默认环境。 不能按活动设置默认环境。
