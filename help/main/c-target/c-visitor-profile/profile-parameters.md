...
由于配置文件脚本在服务器端执行，因此它们无法直接读取页面。数据必须通过 mbox 请求或其他 [将数据导入 Target 的方法](https://developer.adobe.com/target/before-implement/methods-to-get-data-into-target/methods-to-get-data-into-target/){target=_blank}。当数据进入 [!DNL Target] 之后，配置文件脚本可按照 mbox 参数或配置文件参数的形式读取数据。
