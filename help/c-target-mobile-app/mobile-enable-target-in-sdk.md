...
title: 如何在AdobeMobile SDK中启用 [!DNL Target] ?
feature: 实施移动设备
role: Developer
...
1. 添加客户端代码和超时，并启用 SSL。
   ...
   添加 Target 客户端代码和超时。客户端代码是您的帐户或公司的唯一代码。超时是指 Target 在显示默认内容之前将等待响应的时间（以秒为单位）。请确保在 Adobe Mobile Services 的“管理应用程序设置”页面上选中&#x200B;**[!UICONTROL 使用 HTTPS]** 选项。如果未启用HTTPS，则iOS 9及以上版本中的所有调用都将被阻止，除非您允许列表Target服务器。
