...
* **基于项目的算法**：包含遵循以下逻辑的算法：“查看/购买此项目的用户也查看/购买了这些项目”。 这些算法被分组到伞形术语“项 — 项协同筛选”以及[!UICONTROL 具有相似属性的项]算法下。

* **基于用户的算法**：包括[!UICONTROL 最近查看的]和[!UICONTROL 为您推荐的算法]。
...
* **模型训练**：
  ...
  * **tf-idf计算**：下一步涉及创建tf-idf矢量以反映项描述中令牌的相对重要性。 对于项目i中的每个令牌/术语t，使用的目录D |D|项中，首先计算项频率TF(t，i)（项出现在项i中的次数），以及文档频率DF(t，D)。 实质上，存在令牌的项目数。 则tf-idf度量为
...
对[!DNL Target]推荐算法套件的最新添加为[!UICONTROL 为您推荐]和一系列基于购物车的推荐算法。 这两种算法都使用协同过滤技术来形成基于项目的个人推荐。 然后，在服务时，用户的浏览历史记录（针对[!UICONTROL 为您推荐]）或用户当前购物车（针对基于购物车的推荐）中的多个项目用于检索这些基于项目的推荐，然后将这些项目合并以形成推荐的最终列表。 请注意，存在多种风格的个性化推荐算法。 多键算法的选择意味着，当访客有任何浏览历史记录后，即可立即使用推荐，并且推荐可以更新以响应最新的访客行为。
...
* **模型服务**：与以前的算法不同，以前的算法服务推荐涉及为检索指定单个键，然后应用业务规则，而[!UICONTROL 为您推荐的算法]和基于Cart的算法则采用更复杂的运行时过程。

  * **多键检索和合并**：对于基于购物车的推荐，最多将购物车中传递的10个项目视为检索的键，并且每个推荐的权重相等。 对于[!UICONTROL 为您推荐]，最多前五个唯一查看的项目和最近五个唯一购买的项目被视为检索键，其中来自购买项目的推荐权重是来自查看项目的推荐的两倍。 在合并推荐时，如果一个项目出现在多个推荐的单个列表中，则会添加其加权相似度分数。 此阶段的最终推荐列表是随后合并的重新加权推荐列表，按降序排列。
...
* [!UICONTROL 在网站中查看次数最多]，在网站中为[!UICONTROL 最畅销商品]，它们按查看或购买这些项目的会话总数排名项目。 输出是推荐项目的单个（无键）列表。
* 按类别/项目属性显示的“查看次数最多”/“最畅销商品”是一些推荐，其中项目按查看或购买这些项目的会话的聚合计数排序，但按项目类别或特定项目属性分组。 输出是推荐项目的列表，由类别值或项目属性值键控。
* [!UICONTROL 按配置文件属性查看次数最多]和[!UICONTROL 按配置文件属性最畅销商品]也按聚合视图或购买计数对项目进行排名，但分组键是访客配置文件属性，如国家/地区、成员层或成员层，而不是项目属性。 每个会话的计数都归因于访客在交互时持有的任何配置文件值，而不是项目的任何属性。 结果是一组推荐列表，由该配置文件属性的不同值作为键值。
...
自定义标准允许客户[将自己的推荐 [!DNL Target]](/help/main/c-recommendations/c-algorithms/recommendations-csv.md)上传到，这提供了很大的灵活性，并允许“自带模型”功能。 自定义标准取代了[!UICONTROL 基于项目的]推荐的“离线培训”部分，但在在线内容交付阶段其行为与基于项目的推荐算法类似，即使用单个键检索推荐，然后应用业务规则/过滤器。
