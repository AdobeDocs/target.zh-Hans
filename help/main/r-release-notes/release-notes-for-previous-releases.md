...
* **允许的URL的Regex验证。** 修复了在&#x200B;**[!UICONTROL 允许的URL]**&#x200B;字段中输入无效正则表达式而不进行正确验证的问题。 (TGT-55095)
...
* [!UICONTROL 编辑受众]对话框中的&#x200B;**按钮。** 修复了&#x200B;**[!UICONTROL 编辑受众]**&#x200B;对话框中的按钮不再正确呈现的问题。 (TGT-54638)
...
* **[!UICONTROL 图形视图]与表和下载不同步。** 修复了以下问题：即使&#x200B;**[!UICONTROL 表视图]**&#x200B;和下载的报表仍显示正确的值，活动报表仍会在某些日期范围的&#x200B;**[!UICONTROL 图形视图]**&#x200B;中显示缺失或零量度。 (TGT-54998)
...
* **受众使用情况列表未完全呈现。** 修复了受众详细信息中的&#x200B;**[!UICONTROL 使用情况]**&#x200B;部分只能显示映射活动的子集的问题，即使其他活动与该受众关联也是如此。 (TGT-55094)
...
* **对最后一个八位字节IP模糊化的确认更清晰。** 当您在&#x200B;**[!UICONTROL 管理]** > **[!UICONTROL 实现]**&#x200B;上将&#x200B;**[!UICONTROL 模糊处理访客IP地址]**&#x200B;更改为&#x200B;**[!UICONTROL 最后一个八位字节]**&#x200B;时，确认对话框现在会说明[!DNL Target]隐藏访客IP地址的最后一个八位字节。 (TGT-44821)
...
* 使用增强型体验编辑器(EEC) **空白或不完整的页面。** 修复了在启用&#x200B;**[!UICONTROL 增强型体验编辑器]**&#x200B;时，[!UICONTROL 可视化体验编辑器]无法在编辑器中加载网站的问题。 (TGT-54576)
...
* **报表中的受众筛选器出错。** 修复了更改&#x200B;**[!UICONTROL 目标和设置]**&#x200B;中的受众筛选器导致[!DNL Target]用户界面的“报表”部分出现错误的问题。 (TGT-55006)

* **按优先级排序活动。** 使用&#x200B;**[!UICONTROL 优先级]**&#x200B;列标题在活动列表中添加了按优先级排序，其升序和降序与其他可排序列一致。 (TGT-54948)

* **其他活动属性在保存后未保留。** 修复了保存并重新打开活动后，某些&#x200B;**[!UICONTROL 属性]**&#x200B;选择未保留的问题。 (TGT-53889)
...
* [!UICONTROL 页面交付]规则运算符的&#x200B;**日语标签。** 修复了日语UI中页面投放规则运算符标签无法读取或损坏的字符串。 (TGT-53097)
...
* **在编辑器的错误体验上显示的修改。** 修复了在[!UICONTROL 可视化体验编辑器]中的体验之间切换后，删除或其他修改可能显示在错误体验上的问题。 (TGT-54955)

* **删除插入HTML时删除的修改。** 修复了以下问题：删除添加了&#x200B;**[!UICONTROL 此项前插入]**&#x200B;或&#x200B;**[!UICONTROL 此项后插入]**&#x200B;的额外&#x200B;**[!UICONTROL HTML]**&#x200B;块时，还删除了一个无CSS选择器的链接修改。 (TGT-54530)
...
* **某些活动中缺少位置。** 修复了某些活动中缺少&#x200B;**[!UICONTROL 位置]**&#x200B;的问题。 (TGT-54951)
...
* **应用于其他视图时保留的自定义代码。** 修复了在同一&#x200B;**[!UICONTROL 活动]**&#x200B;中为另一个&#x200B;**[!UICONTROL 视图]**&#x200B;添加或保存自定义代码时，可能会删除应用于一个&#x200B;**[!UICONTROL 视图]**&#x200B;的自定义代码的问题。 (TGT-53933)
+++
...
* **在“活动”视图中可见的受众属性。** 修复了从&#x200B;**[!UICONTROL 活动]**&#x200B;查看的受众规则详细信息时，未显示从&#x200B;**[!UICONTROL 受众]**&#x200B;部分打开同一受众时显示的特定属性的问题。 (TGT-54742)

* **导出“活动”列表页面上的CSV。** 添加了&#x200B;**[!UICONTROL 导出CSV]**&#x200B;操作，以便您可以从用户界面导出活动列表，包括在应用过滤器时，而无需仅依赖API进行例行导出。 (TGT-51466)

* **未找到选择器时标记的Experience修改。** 体验修改现在运行选择器存在性检查；在页面上未找到选择器时，修改将被标记为无效。 (TGT-54815)

* **[!UICONTROL 自动个性化]活动。** 修复了阻止用户可靠地创建、编辑或管理自动个性化活动的界面和活动加载问题，这些问题会阻止活动设置并延迟个性化用例。 (TGT-54421)
...
* **从活动创建受众时显示的受众名称和描述。** 修复了在从活动流创建或编辑受众时，受众&#x200B;**[!UICONTROL 名称]**&#x200B;和&#x200B;**[!UICONTROL 描述]**&#x200B;字段与直接在&#x200B;**[!UICONTROL 受众]**&#x200B;下创建受众相比，无法清晰脱颖而出的问题。 (TGT-54837)
...
* **[!UICONTROL 实时活动]计入分析。** 修复了分析功能板上的&#x200B;**[!UICONTROL 实时活动]**&#x200B;量度报告的总数可能高于&#x200B;**[!UICONTROL 所有活动]**&#x200B;中显示为实时的活动数的问题。 (TGT-54788)
...
* [!UICONTROL 全局排除].**中的**&#x200B;长ID列表 修复了以下问题：与旧版相比，**[!UICONTROL 全局排除项]**&#x200B;中粘贴或输入较长ID列表的ID在更新后的界面中可能会被截断，从而导致排除列表不完整。 (TGT-54422)
...
* [!UICONTROL 可视化体验编辑器]中的&#x200B;**增强型体验编辑器(EEC)状态指示器。** EEC指示器指示是否启用增强型体验编辑器。 其演示文稿已修订，不再类似于交互式切换，因为它仅用作非交互式状态显示。 (TGT-54828)

* **可视体验编辑器[!UICONTROL 中的可折叠左边栏]。** 现在，当活动处于打开状态以进行编辑时，可以折叠左边栏。 这改进了对于包含多个受众和页面的活动（包括在较小的显示器上）的&#x200B;**[!UICONTROL 组件]**&#x200B;和&#x200B;**[!UICONTROL 属性]**&#x200B;的访问权限。 (TGT-54269)
...
* 可视化体验编辑器(Chrome)中的&#x200B;**专用网络URL。** 除非允许本地网络访问，否则Chrome可能会阻止专用网络上的页面。 **[!UICONTROL 可视化体验编辑器]**&#x200B;现在会请求在框架Experience Cloud中使用的权限，这样Chrome可以显示允许提示而不是静默失败。 在允许访问后，如果您的环境支持VEC，则可以在VEC中加载内部URL。 (TGT-54346)
...
* **在实施屏幕上添加闪烁管理切换。** 已向[!UICONTROL 实现]屏幕添加新的切换开关以控制启用闪烁管理设置。 此切换允许管理员直接从“实施”屏幕配置闪烁管理。 (TGT-52247)
...
* **在概述页面中显示受众和体验的全名。** 此增强功能会更新[!UICONTROL 概述]页面以显示受众和体验的全名。 以前，长名称会被截断并且无法完全显示，因此需要用户通过三次单击来选择所有文本，以查看完整名称。 此更新可确保完整受众和体验名称可见，从而使用户更容易识别和审查活动配置。 (TGT-53323)
...
* **已列入允许列表的URL管理功能。** 已向[!UICONTROL 管理]部分添加新的“列入允许列表的URL”管理功能，以管理允许的远程选件URL模式。 此功能包括在表中显示列入允许列表的URL、搜索和过滤URL、通过模式对话框添加新URL以及在确认后删除URL的功能。 该功能可帮助客户控制哪些URL可用于远程选件，从而提高安全性和合规性。 (TGT-54486)
...
* **为Web编辑器选件启用工作区列。** 此更新解决了Web编辑器中的其他工作区中出现[!UICONTROL 默认Workspace]的选件所造成的客户混淆。 虽然此行为按预期工作，但[!UICONTROL 默认Workspace]选件有意在所有工作区中均可见，但客户报告UI未明确工作区来源，尤其是在非默认工作区（如“审批者”）中创建活动时更是如此。 为了提高清晰度，Web编辑器的选件列表中现已启用[!UICONTROL Workspace]列，这使用户能够轻松区分每个选件属于哪个工作区，并防止误解显示的其他选件。 (TGT-54138)
* 在新选项卡中打开具有target=&quot;_blank&quot;的&#x200B;**链接。** 此修复了包含具有~target=&quot;_blank&quot;~的链接的已创作网站在单击[!UICONTROL 浏览]模式时将在新的浏览器选项卡中打开，中断编辑器内预览体验的问题。 出现此行为是因为创作页面的本机链接属性未被扩展插入的JavaScript截获，这与旧版UI不同，在旧版UI中，锚点元素会进行转换并覆盖其目标以保留在编辑器中导航。 此更新现在确保在Web编辑器中正确处理使用~target=&quot;_blank&quot;~的链接，以便在创作过程中不再打开外部选项卡。 (TGT-54134)
...
* **[!UICONTROL 工作区]下拉列表在[!UICONTROL 用户]部分中限制为20个。** 此修复程序解决了以下问题：[!UICONTROL 管理] > [!UICONTROL 用户]部分中的[!UICONTROL 工作区]下拉菜单仅显示20个工作区，即使用户有权访问更多工作区也是如此。 `licenseGroups`的基础GraphQL调用也被限制为20个结果，这会导致UI显示不完整的列表，尽管用户有权访问组织中的更多工作区。 更新会移除此硬限制，以便现在可以正确返回并显示整组可用工作区。 (TGT-53820)
* **修复了选件模式未显示工作区列的问题。** 修复了选件模式在更新的UI中不显示Workspace列的问题。 这会给客户带来困惑，因为[!UICONTROL 默认Workspace]的选件与选定工作区中的选件一起显示，但未指明它们的来源。 工作区列现在已启用，以便客户可以清楚地识别每个选件属于哪个工作区。 (TGT-52320)
...
* **如果已删除，Activity Edit则不应添加自动检测到的属性。** 此修复了以下问题：编辑活动会自动重新引入用户之前删除的自动检测属性。 重新打开活动进行编辑时，系统错误地还原了已删除的属性，导致[!UICONTROL 属性列表]中的行为不一致和混淆。 此更新可确保在删除自动检测到的属性后，该属性会在所有后续编辑期间保持被删除状态，并且除非用户明确将其添加回来，否则该属性不会重新出现。 (TGT-54182)
* **如果已删除，则不添加自动检测到的属性。** 此修复可确保，一旦用户手动从活动中移除自动检测到的属性，系统在活动编辑器中的后续导航期间将不再重新引入该属性。 以前，如果用户取消选择自动检测的属性，移至[!UICONTROL 定位]步骤，然后返回到[!UICONTROL 体验]，则编辑器将根据活动编辑器状态切片中存储的自动检测列表重新填充移除的属性。 更新后的逻辑现在将自动检测到的属性与~ActivityState~切片中的当前属性进行比较，并阻止重新添加用户已删除的任何自动检测到的属性。 这会导致各步骤之间出现一致的行为并尊重用户意图。 (TGT-54181)
* **将自动检测到的文本添加到属性列表。** 此增强功能会更新[!UICONTROL 属性列表]，以明确标记系统自动检测的任何属性。 当自动检测到的属性也出现在用户可见的[!UICONTROL 属性列表]中时，它现在会使用存储在~ActivityEditorSlice~状态中的值，在其名称旁边显示“（自动检测）”文本。 这反映了旧版UI的行为，并帮助用户轻松区分手动选择的属性和自动识别的属性。 (TGT-54120)
* **将自动检测到的[!UICONTROL 属性]添加到状态。** 此更新确保~ActivityEditorSlice.ExperienceEditor~状态一致地维护所有自动检测到的属性ID的最新列表，这些属性ID从Web编辑器传递到“活动[!UICONTROL 体验]”选项卡中。 每次用户导航到[!UICONTROL 体验]选项卡时，都会使用任何新检测到的属性刷新状态，同时会防止出现重复项，从而确保准确跟踪和可靠的下游行为。 (TGT-54119)
...
* **[!UICONTROL 环境]下拉列表仅显示100个结果。** 此修复解决了具有超过100个环境的客户在[!UICONTROL 推荐]中的[!UICONTROL 环境]下拉列表中只能看到前100个条目的限制。 基础GraphQL查询(~getEnvironmentsV2~)采用硬编码页面大小100分页，导致UI仅显示部分列表，即使有其他页面可用。 对于拥有超过100个环境的客户，此问题会导致缺少选项和不完整的选择体验。 更新将提高限制，以便返回并显示所有环境，从而确保完全可见，无论环境计数是多少。 (TGT-53903)
...
* **修复了[!UICONTROL 报告]箭头未明确指示可展开列的问题。** 修复了报表表格未明确显示其他列可以在更新后的UI中展开的问题。 在列标题附近的[!UICONTROL 报表]箭头中添加了消失的工具提示，以帮助客户了解更多列可用。 (TGT-53479)
...
* **[!UICONTROL 体验片段]名称在新VEC UI**&#x200B;中被截断(TGT-54312)
* **无法对[!UICONTROL 收入]指标使用[!UICONTROL 高级设置]。** 此修复了用户在[!UICONTROL 目标和设置]中为[!UICONTROL 收入]量度配置[!UICONTROL 高级设置]时遇到403“访问被拒绝”错误的问题。 添加与主目标关联的依赖项条件时，出现问题；后端错误地要求编辑者权限，即使用户已有足够的权限创建和编辑活动。 因此，尽管配置有效，但保存活动失败。 更新更正了权限检查，以便具有适当访问权限的用户可以成功添加收入量度依赖关系，而不会触发禁止的资源错误。 (TGT-54092)
* **修复了“添加”按钮未应用于选定图像的问题。** 修复了一个问题，该问题阻止客户在活动创建流程中选择或更新图像时添加某些图像。 例如，当客户搜索特定资产时，在搜索“ipp”时返回的图像，单击[!UICONTROL 添加]按钮未应用所选图像，并且未创建任何修改。 选择其他图像（如`Homepage-banner-1-moz.jpg`）可继续按预期工作。 此更新确保所有有效图像都可以在更新后的UI中一致应用。 (TGT-53610)
* **修复了删除URL条件会重置目标量度配置的问题。** 修复了删除[!UICONTROL 目标]量度中的单个URL条件导致整个配置在更新的UI中重置的问题。 当客户尝试删除[!UICONTROL 转化] > [!UICONTROL 查看了页面]下保存的URL条件时，目标类型意外切换为[!UICONTROL 查看了Mbox]，并且已删除所有之前配置的设置。 此更新确保仅删除选定的URL条件，并且所有剩余的目标设置保持不变。 (TGT-53271)
...
* **[!UICONTROL Message]列间歇性地无法在更新的UI中显示产品数据。** 解决了在更新的[!UICONTROL 推荐] UI中，[!UICONTROL 目录搜索]中的[!UICONTROL Message]列间歇性地无法显示产品数据的问题，即使信息源中存在值也是如此。 列现在可以一致地显示所有产品的正确报文值，确保可靠的可见性，而无需手动重新配置列。 (TGT-52777)
* 在更新的UI中保存活动后，**[!UICONTROL 下载推荐数据]按钮不可见。** 解决了更新后的UI中，即使在重新保存之后，某些保存的活动也不会显示[!UICONTROL 下载推荐数据]按钮的问题。 现在，按钮在所有活动中均可一致显示，从而确保用户能够可靠导出推荐数据而无需解决办法。 (TGT-53802)
...
* 在更新的UI中使用[!DNL Adobe Analytics]作为报表源时出现&#x200B;**[!UICONTROL 目标和设置]错误消息。** 解决了更新的[!UICONTROL 概述] UI中，目标部分显示“出现错误”错误的问题。 我们无法完成您的请求。 如果选择[!DNL Adobe Analytics] (A4T)作为报表源，如果问题仍然存在，请联系[!DNL Adobe Client Care]。 目标现在可以使用[!UICONTROL Adobe Analytics]指标正确显示，从而确保跨报表源的一致可见性。 (TGT-54021)
...
* **无法在更新的UI中编辑或替换决策选件。** 解决了在更新的UI中，无法通过[!UICONTROL 修改]面板编辑或替换决策选件，且选件名称显示为空白的问题。 Decisioning优惠现在完全可访问和编辑，从而恢复与旧版UI的等同性，并确保客户可以直接在活动中管理优惠。 (TGT-53884)
...
* 更新的UI中的[!DNL Recommendations]活动缺少&#x200B;**[!UICONTROL 下载推荐]按钮。** 解决了在更新的[!DNL Recommendations] UI中，使用推荐的A/B活动看不到[!UICONTROL 下载推荐]按钮的问题。 现在，按钮可正确显示，允许用户按预期导出推荐数据，这与旧版UI中的功能一致。 (TGT-53768)
* 更新后的概述UI中缺少&#x200B;**[!UICONTROL 下载推荐数据]按钮。** 解决了更新的[!UICONTROL 概述] UI中的问题，该问题导致包含推荐的活动无法显示[!UICONTROL 下载推荐数据]按钮。 按钮现在可正确显示，从而确保用户无需切换回旧版UI即可直接导出推荐数据。 (TGT-53772)
* **编辑活动条件有时会在更新的UI中导致空白屏幕。** 解决了更新后的UI中的一个问题：单击体验中的[!UICONTROL 编辑标准]有时会导致某些活动出现空白屏幕。 现在，标准编辑器可以在所有活动中可靠地加载，从而确保用户能够不受中断地编辑。 (TGT-53961)
* **无法在更新的UI中编辑序列条件。** 解决了在更新的UI中，尝试编辑[!UICONTROL 序列标准]导致标准弹出窗口在加载时停留并显示空白屏幕的问题。 标准编辑器现在可正确加载，允许用户编辑和更新序列标准而不会中断。 (TGT-53985)
...
* **[!UICONTROL 多变量测试] (MVT)位置和图形报告问题阻止生成报告。** 解决了MVT活动无法在Target UI中生成[!UICONTROL 位置贡献]和图形报表的问题，该问题显示错误“出现错误。 我们无法完成您的请求。” 现在，报告可在UI中正确加载，确保完全可见。 (TGT-53654)
* 由于[!UICONTROL 元素]贡献报表错误，**MVT报表未加载。** 修复了Target UI中MVT活动报表加载失败，并显示“无法获取元素贡献报表”错误的问题。 现在，报表可正确显示，从而确保元素贡献的全面可见。 (TGT-53691)
* **将订单详细信息导出到[!UICONTROL 体验定位] (XT)活动的CSV问题。** 修复了XT活动中[!UICONTROL 将订单详细信息导出到CSV]选项显示不正确并返回空文件的问题。 现在，仅对AP活动显示选项，以确保准确的导出功能并防止混淆。 (TGT-53798)
...
* **[!UICONTROL 删除修改]按钮问题阻止删除活动修改。** 解决了[!DNL Target] UI中的[!UICONTROL 删除修改]按钮不起作用，导致用户无法删除活动中的修改的问题。 现在，按钮按预期工作，允许毫不延迟地可靠地删除修改。 (TGT-53728)
...
* **[!UICONTROL 编辑]和[!UICONTROL 概述]页面之间的活动位置对齐不匹配。** 解决了[!UICONTROL 概述]页面中的活动位置编号与[!UICONTROL 编辑体验]页面中的更新不一致的问题。 现在，两个视图中的位置保持一致，确保精确的对齐并防止位置缺失或编号错误。 （TGT-53960 和 TGT-53954）
* **无法在更新的VEC中切换回[!UICONTROL 设计]模式。** 解决了更新后的VEC UI中的一个问题：用户在[!UICONTROL 浏览]模式下导航到新页面后，无法切换回[!UICONTROL 设计]模式。 现在，[!UICONTROL 设计]切换可正常工作，允许修改无缝地跨页面应用。 （TGT-53988 和 TGT-53993）
* **查询参数未显示在活动概述中。** 解决了在更新的UI中，查询参数未显示在活动的[!UICONTROL 概述]页面中，从而导致[!UICONTROL 概述]和页面交付URL之间存在差异的问题。 现在，查询参数可正确显示，从而确保活动位置在各个视图中完全表示并保持一致。 (TGT-53701)
...
+++ 查看详细信息
* **解决了更新后的UI中的可用性问题**。 [!UICONTROL 观察者]现在可以使用[!UICONTROL 查看活动]选项预览活动，就像在旧版UI中一样。 (TGT-51741)
* **[!UICONTROL 观察者]用户现在可以在更新的UI中查看活动内容。** 观察者角色用户在更新后的活动UI中恢复可见性。 以前，观察者无法查看旧版UI中可用的修改、选件和内容更改。 (TGT-53785)
* **[!UICONTROL 审批者]用户现在可以编辑活动目标，而不会出现编辑器权限错误。** 解决了活动创建UI中阻止审批者级别用户保存对高级目标设置更改的权限问题。 受影响的用户收到`403 Forbidden.Resource`错误，需要编辑器权限，尽管具有足够的访问权限。 (TGT-53819)
...
* **已在“仅此活动”报表中还原多受众选择。** 解决了活动创建UI中阻止用户在[!UICONTROL 目标和设置]中的[!UICONTROL 仅此活动]部分下选择多个受众的问题。 (TGT-53283)
* **基于受众的报告图表现在可正确显示转化数据。** 解决了[!UICONTROL 报表]选项卡中导致图形在选择非默认受众时失败的问题。 虽然提供了数据和置信度量度，但可视化图表只显示一条实线，导致分析困难。 (TGT-53769)
* **[!UICONTROL 定位] UI现在明确指示排除的受众规则。** 解决了活动创建UI的[!UICONTROL 定位]部分中，未明确显示设置为[!UICONTROL 排除]的受众规则的问题。 这会导致查看定位逻辑时出现混淆，尤其是对于排除特定URL的受众。 (TGT-53809)
* **受众定义值现在可在[!UICONTROL 定位]选项卡中选择和复制。** 解决了活动创建界面中阻止用户在[!UICONTROL 定位]选项卡中选择和复制受众规则值的问题。 旧版UI中提供了此功能，但更新后的UI中缺少此功能。 (TGT-53856)
...
* **页面加载事件选择在推荐修改中现在持续存在。** 解决了在活动创建UI中，将推荐的事件类型从[!UICONTROL 视图]切换到[!UICONTROL 页面加载]时，该问题阻止用户保存更改。 尽管选择似乎成功，但在导航离开并阻止活动发布后已恢复。 (TGT-53957)
...
* **“[!UICONTROL 将订单详细信息导出到CSV]”现在下载了完整数据。** 解决了更新的[!UICONTROL 概述] UI中导致“[!UICONTROL 将订单详细信息导出到CSV]”选项下载空文件的问题，即使存在有效的报表数据也是如此。 (TGT-53787)
...
* **VEC不再在“[!UICONTROL 应用初始修改]”上间歇性停止。“**” 解决了VEC中的一个间歇性问题，该问题导致体验加载失败并停留在“[!UICONTROL 应用初始修改]”上。 此行为不一致，有时会触发重定向循环或需要手动清除缓存。 (TGT-53916)
* 无法重现&#x200B;**VEC加载问题。** 我们调查了一个报告的问题，即在编辑现有活动时，VEC停留在“[!UICONTROL 应用初始修改]”上。 此行为疑似与缺少父元素的HTML内容有关。 我们将继续监控复发，并建议为HTML选件使用结构化容器以确保稳定性。 (TGT-53972)
* VEC中的&#x200B;**[!UICONTROL 设计]模式不再像[!UICONTROL 间歇性浏览]模式那样运行。** 解决了UI中[!UICONTROL 设计]模式间歇性地表现为[!UICONTROL 浏览]模式的问题，该模式允许可单击的`<a>`链接并阻止元素选择。 这会导致悬停框消失并阻止修改工作流。 (TGT-53136)
* **在[!UICONTROL 编辑器]模式下单击按钮不再触发页面重定向。** 解决了在更新的VEC UI中，单击[!UICONTROL 编辑器]模式中的按钮会导致意外重定向到该按钮的目标URL的问题。 这会阻止用户编辑call-to-action (CTA)元素，并中断创作工作流。 (TGT-53137)
* **自动个性化活动中的选件代码更新现在无错误保存。** 解决了在更新[!UICONTROL Automated Personalization]活动中的选件代码时，在新建用户界面中导致出现“[!UICONTROL 无效用户输入]”错误的问题。 该错误阻止用户保存更改，即使输入有效也是如此。 (TGT-53586)
* VEC中的&#x200B;**[!UICONTROL 设计]模式现在阻止可编辑组件的链接导航。** 解决了更新后的VEC中存在的一个问题，即在[!UICONTROL 设计]模式下，可单击元素（如按钮和链接）会触发页面重定向。 此行为模拟[!UICONTROL 浏览]模式，并阻止用户修改关键组件。 (TGT-53696)
* **“[!UICONTROL 已单击元素]”量度现在可以正常使用，且没有重定向或错误。** 解决了在新建用户界面中，选择“[!UICONTROL 单击了元素]”转化量度时导致意外重定向和空白屏幕的问题。 在安装期间单击按钮触发导航而不是注册元素，从而导致“[!UICONTROL 找不到元素]”错误。 (TGT-53817)
...
* **[!UICONTROL 定位]部分未指示受众规则已设置为排除。** 使用排除逻辑配置的受众错误地显示为包含在活动创建UI的[!UICONTROL 定位]部分中。 尽管后端正确应用了排除规则，但UI无法直观地表示该规则，在活动设置期间会忽略“排除”标签并导致混淆。 [!UICONTROL 定位]部分现在可以清楚地显示排除规则，确保受众配置和定位可视化之间的一致性。 (TGT-53809)
...
* **修改现有活动时无法编辑标准内容。** 编辑活动时，[!UICONTROL 标准]内容部分显示为已禁用，其中按钮灰显且无响应。 通过确保[!UICONTROL 标准]配置在活动更新期间完全可编辑，此问题已得到解决。 客户现在可以修改[!UICONTROL 标准]内容，而无需切换选择或使用解决方法，从而改进了活动创建更新过程中的灵活性和可用性。 (TGT-53812)
* **无法在活动中编辑标准。** 从活动中访问标准时，[!UICONTROL 编辑标准]和[!UICONTROL 删除标准]选项已禁用。 但是，可通过[!UICONTROL 推荐]选项卡成功编辑相同的标准。 现在，可以从活动编辑工作流和[!UICONTROL 推荐]选项卡中完全编辑标准，从而确保一致且高效的编辑体验。 (TGT-53814)
...
* **报告与滚动条交互后崩溃。** 单击[!UICONTROL 报告]选项卡中的滚动条导致页面崩溃，并出现JavaScript错误：
...
* **修复了[!UICONTROL 管理内容]模式的[!UICONTROL Automated Personalization]活动中出现的仅用于活动的受众的无效用户输入错误问题。** 在AP活动的[!UICONTROL 管理内容]模式中配置仅限该活动的受众时，客户遇到无效的用户输入错误。 尽管之前已成功使用受众，但还是出现了此问题。 现在，组合的受众配置可正确保存，而不会触发验证错误。 (TGT-53749)
...
* **优惠决策选项在初始活动创建期间始终可见。** 解决了在更新的UI中，首次创建A/B活动流程时，特别是在无痕模式下访问启用了优惠决策的租户时，无法显示[!UICONTROL 优惠决策]选项的问题。 该选项仅在导航到[!UICONTROL 定位]步骤并返回[!UICONTROL 体验]之后显示。 此修复程序确保[!UICONTROL 优惠决策]选项在初始设置期间立即可用，从而提高可用性并减少混淆。 (TGT-51888)
...
* 已恢复在更新的UI中创建的[!UICONTROL 推荐]活动的&#x200B;**点击跟踪。** 解决了在更新的UI中创建的[!UICONTROL Recommendations]活动无法注册点击跟踪，导致报告转化为零的问题。 旧版UI中构建的活动可正确跟踪点击次数并报告预期转化。 此修复程序可确保在更新后的UI中创建的推荐活动现在包括正确的跟踪属性、恢复转化报表以及与A4T量度保持一致。 (TGT-53287)
* **已恢复推荐活动的点击跟踪。** 解决了在更新的UI中创建的[!UICONTROL Recommendations]活动无法注册点击跟踪，导致报告转化为零的问题。 旧版UI将跟踪ID (`at-track-click`)正确应用于[!UICONTROL 推荐]内容，而更新的UI错误地插入了占位符(`__recsClickTrackIdPlaceholder__`)，从而阻止后端跟踪。 此修复确保[!DNL Recommendations]内容现在包含正确的跟踪ID，从而恢复转化报表并与A4T量度保持一致。 (TGT-53496)
* 在更新的UI中解决了&#x200B;**收藏集编辑器崩溃。** 修复了在更新的[!UICONTROL 可视化体验编辑器] (VEC) UI中，从编辑器面板打开收藏集导致页面崩溃的问题，出现TypeError：无法读取未定义的属性（读取“customLocale”）。 此错误出现在多个活动类型中，包括[!UICONTROL 推荐]和A/B测试。 (TGT-53703)
* **用于删除VEC中还原的选定集合的选项。** 修复了VEC中的一个问题：用户只能替换[!UICONTROL 推荐]活动中的选定集合，但无法将其完全删除。 此限制阻止了需要在不进行替换的情况下干净移除集合的用例。 此修复引入了一个用于删除所选收藏集的清除选项，从而允许在活动设置方面更加灵活，并与旧版UI行为保持一致。 (TGT-53652)
* **自定义标准集合现在在[!UICONTROL 推荐] UI中正确显示。** 修复了“推荐”界面中使用自定义标准构建的收藏集无法显示产品结果的问题。 尽管基于属性的标准收藏集按预期工作，但使用自定义筛选器的收藏集返回“未找到结果”，尽管目录匹配有效。 此修复确保使用自定义属性的收藏集现在可以在[!UICONTROL 产品]选项卡中正确填充，从而恢复个性化推荐工作流的完整功能。 (TGT-53653)
* **修复了首次打开[!UICONTROL Products]页面时，收藏集不显示产品的问题。** 访问[!UICONTROL 推荐]分区中的收藏集的客户在首次打开[!UICONTROL 产品]页面时遇到了空的产品结果。 此问题是由于GraphQL查询中的后端错误导致的，该错误无法加载具有自定义标准的集合的产品数据。 问题已得到解决，产品现在可以正确显示，而无需切换环境。 (TGT-53694)
* **修复了在基于表单的[!UICONTROL 推荐]活动中无法删除收藏集的问题。** 使用[!UICONTROL 基于表单的体验编辑器]创建[!UICONTROL 推荐]活动的客户无法取消选择之前选择的收藏集。 UI要求在保存之前选择收藏集，以防用户恢复为“所有收藏集”。 此行为已更新以符合VEC功能，允许客户在没有选定收藏集的情况下进行保存，并按预期默认为“所有收藏集”。 (TGT-53708)
...
* **修复了属性配置的促销活动的验证仍然无效的问题。** 客户在活动创建过程中按属性设置促销活动时遇到永久性验证错误，即使所有必填字段均已正确填充也是如此。 此问题是由于[!UICONTROL 推荐]工作流中的验证逻辑不正确导致的。 现在，基于属性的促销活动会按预期进行验证和保存。 (TGT-53811)
* **修复了将促销应用于实时[!UICONTROL 推荐]活动触发错误的问题。** 客户在尝试将前端促销应用于实时[!UICONTROL 推荐]活动时，遇到“促销活动缺少收藏集ID或筛选规则值”错误，即使提供了有效的配置详细信息后也是如此。 现在可以将促销活动成功应用于实时活动，而不会触发验证错误，从而确保在更新的活动创建UI中实现更顺畅的体验。 (TGT-53738)
* **修复了产品在[!UICONTROL 推荐] UI中的收藏集中不可见的问题。** 来自单个租户的客户报告，在查看新UI的[!UICONTROL 推荐]分区中的某些收藏集时，产品列表加载失败。 通过切换环境暂时解决了此问题，但此解决方法会导致用户体验不佳。 现在，产品实体可以始终如一地加载，而无需切换环境。 (TGT-53783)
...
* **[!UICONTROL 总收入]指标现已包含在从活动报表导出的CSV中。** 解决了更新的[!UICONTROL 概述] UI中的问题：总收入在活动报表视图中正确显示，但在CSV导出中缺失，显示为$0。 这种差异导致用户无法依赖导出的数据进行离线分析和报告。 (TGT-53325)
* **[!UICONTROL 总销售额]指标现已包含在从活动报表导出的CSV中。** 解决了更新后的UI中的一个问题：总计[!UICONTROL 销售额]指标在活动报表视图中正确显示，但在CSV导出中缺失。 这种差异导致用户无法访问下载报表中的完整性能数据。 此修复程序确保CSV导出中现在准确地包含[!UICONTROL 总销售额]值，从而恢复应用程序内报表和离线分析之间的一致性。 (TGT-53330)
* **改进了在未启用度量时[!UICONTROL 图形视图]的错误消息传递。** 修复了VEC中的一个问题：当关联的[!DNL Analytics]报表包中未启用所请求的量度时，[!UICONTROL 图形视图]显示一般的“出现错误”消息。 此问题是由后端GraphQL响应中的`not_enabled_metric`错误触发的。 此修复用信息更丰富的消息取代了模糊的错误，帮助用户识别[!DNL Analytics]中的配置问题，从而减少混淆和不必要的支持升级。 (TGT-53577)
* **修复了报表持续时间超出支持的90天限制的问题。** 在[!UICONTROL 报表]部分中使用“[!UICONTROL 最近X天]”过滤器的客户可以选择超过90天的持续时间，这可能会导致性能问题和数据不完整。 更新了过滤器，强制实施最长90天的范围，以确保一致且可靠的报表。 (TGT-53795)
...
* **修复了Analytics for Target (A4T)报表无法在[!UICONTROL 图形视图].**&#x200B;中显示数据的问题 将[!DNL Target]与A4T集成一起使用的客户在A/B测试活动的“报表”选项卡中切换到图形视图时遇到“出现错误”错误。 尽管[!UICONTROL 表视图]已正确加载，[!UICONTROL 图形视图]仍无法呈现选定时间范围内的量度趋势。 [!UICONTROL 图形视图]现在可按预期显示所有受支持量度的性能数据，从而提高更新UI中的可见性和报告准确性。 (TGT-53573)
...
* **痕迹导航悬停现在突出显示VEC中的相应元素。** 改进了[!UICONTROL 可视化体验编辑器]，可在将鼠标悬停在痕迹导航菜单中的项目上时，突出显示编辑器中的相应元素。 还会显示元素类型，如容器、粗体文本或按钮。 此行为适用于同级元素，并根据验证列表排除不支持的类型，例如SVG。 (TGT-53411)
...
* **VEC Helper扩展中已解决Cookie复制问题。** 修复了[!UICONTROL Adobe Experience Cloud可视化编辑帮助程序]扩展在通过预览链接进行QA期间复制`at_qa_mode` Cookie的问题。 当手动切换预览索引时，会创建多个具有跨域冲突值的Cookie，从而阻止测试者可靠地切换变量。 甚至在Target UI外部也观察到此行为，它同时影响内部帐户和客户端帐户。 此修复通过防止重复条目并调整域范围来确保一致的Cookie处理，从而允许无缝的变体切换，而无需手动Cookie清理。 (TGT-53579)
...
* **修复了[!UICONTROL Automated Personalization]活动中的[!UICONTROL 管理内容]边栏显示不一致的位置标签的问题。** 客户报告[!UICONTROL 管理内容]边栏在[!UICONTROL 体验]和[!UICONTROL 选件]选项卡中显示位置编号不匹配。 （例如，体验中的位置2和位置4，以及选件中的位置1和位置2），即使仅配置了两个位置也是如此。 位置标签现在是一致的，并准确地映射到修改，从而提高活动创建过程中的清晰度和可用性。 (TGT-52934)
...
* **已改进[!UICONTROL AEM体验片段]选件的用户归因。** 解决了VEC中的一个问题：针对[!UICONTROL AEM Experience Fragment] (XF)选件的“[!UICONTROL 上次更新]”字段错误地显示了一个代码ID而不是用户名。 在更新的UI中选择选件期间出现此差异，导致与旧版UI和HTML选件不一致，后者正确显示最后编辑者的名称。 此修复确保跨选件类型的一致用户归因，从而提高透明度并与预期行为保持一致。 （TGT-52880 和 TGT-52898）
...
* **下载选项在更新的报告概述UI的报告部分中恢复。** 解决了新概述UI中“报表”部分缺少[!UICONTROL 下载]按钮，导致用户无法导出属性重要性得分的问题。 此更新将恢复完整的导出功能，从而简化对用于分析和报告的可下载数据的访问。 (TGT-53222)
* 在[!UICONTROL 重要属性]视图中还原了&#x200B;**[!UICONTROL 下载完整CSV报告]按钮。** 解决了在更新的活动创建UI中，报告视图的[!UICONTROL 重要属性]部分中缺少[!UICONTROL 下载完整CSV报告]按钮的问题。 此修复可恢复对可下载见解的访问权限，从而确保更新和旧版UI中的功能一致。 (TGT-53238)
* **已解决影响[!UICONTROL 自动定位]报告的UI问题（在更新的概述UI中）。** 修复了更新的概述界面中影响[!UICONTROL 自动定位]活动报表的多个UI问题。 这些修复包括：

  * 摘要报表中缺少提升和置信度量度
  * “已构建模型”复选框的颜色指示器不正确
  * 尽管[!DNL Analytics]中存在数据差异，但无法正常显示图形报告
  * 缺少[!UICONTROL 自动化区段]和[!UICONTROL 重要属性]报告的下载链接
  * [!UICONTROL 自动化区段]报表显示已损坏

  这些修复恢复了预期的报告行为，并提高了在更新的UI中对[!UICONTROL 自动定位]性能的可见性。 (TGT-53484)

+++

**[!UICONTROL Visual Experience Composer（可视化体验编辑器）]**

+++查看详细信息

* **已更正更新的UI中参数存在条件的表单验证。** 解决了在更新的UI中，选择“[!UICONTROL 自定义mbox参数值存在]”或“[!UICONTROL 参数存在]”时无法正确要求客户输入值的问题。 此行为与预期逻辑冲突，预期逻辑只是检查参数是否存在，而不管其值如何。 该修复将表单验证与预期行为保持一致，从而能够更顺畅地设置活动并支持完全采用更新后的UI。 (TGT-53638)
* 针对页面交付中的参数存在规则更正了&#x200B;**表单逻辑。&quot;** 解决了在更新的UI中，选择页面投放规则(如&quot;[!UICONTROL Parameter present]&quot;、&quot;[!UICONTROL Parameter not present]&quot;、&quot;[!UICONTROL Parameter value present]&quot;或&quot;[!UICONTROL Parameter value not present]&quot;时，错误地要求用户输入其他参数值。 此行为与旧版UI不一致，并且与在不指定值的情况下检测参数存在的预期逻辑相冲突。 此修复可恢复预期的规则配置行为，从而简化活动设置并提高可用性。 (TGT-53640)
...
* **在A/B和MVT活动中解决了位置重命名问题。** 修复了更新后的UI中的一个错误，该错误导致在位置列表、定位和返回之间导航后，重命名[!UICONTROL A/B测试]或[!UICONTROL 多变量测试] (MVT)活动中的位置不再有效。 此更新可确保保存位置名称更改，并在整个活动工作流中始终如一地反映这些更改。 (TGT-52367)
* **在更新的UI中修复了MVT和AP活动的位置名称持久性。** 解决了在更新的UI中，在[!UICONTROL 多变量测试] (MVT)和[!UICONTROL Automated Personalization] (AP)活动中编辑的位置名称未正确保存的问题。 重命名位置后，在选项卡（如[!UICONTROL 定位]和[!UICONTROL 体验]）之间导航导致名称恢复到其以前的状态。 此修复确保在选项卡间进行一致的位置命名，并提高了活动设置期间的可靠性。 (TGT-52927)
* **在MVT活动的“管理体验”面板中更正了位置标签。** 解决了在更新的UI中，[!UICONTROL 多变量测试] (MVT)活动中的[!UICONTROL 管理体验]面板显示的位置编号不一致的问题。 此修复程序可确保位置标签按顺序排列并正确与用户定义的修改保持一致，从而提高体验设置期间的清晰度和可用性。 (TGT-52929)
...
* **客户无法从[!UICONTROL 活动概述]**&#x200B;中复制活动或文档名称：以前，客户无法直接从[!UICONTROL 活动概述]中复制更新的活动创建过程中的活动或关联选件/文档名称。 此限制会影响可用性，尤其是在较小的屏幕上。 客户现在可以轻松地复制活动和文档名称而无需变通办法。 (TGT-51850)
...
* **AP活动未在[!UICONTROL 报表]节**&#x200B;中获取模型就绪数据：查看[!UICONTROL 报表]节中的Automated Personalization (AP)活动的客户无法在报表组和选件级别看到模型就绪的指示器。 出现此问题的原因是无法从后端正确获取模型就绪的数据。 功能已恢复，并且模型就绪的数据现在按预期显示。 （TGT-53600 和 TGT-53601）
* **计划将来开展的活动在[!UICONTROL 活动]概述**&#x200B;中错误地显示了“[!UICONTROL 实时]”状态：客户注意到，计划在[!UICONTROL 活动]概述中在未来开始的活动错误地标记为“[!UICONTROL 实时]”。 此状态不匹配已解决，计划活动现在正确显示为“[!UICONTROL 已计划]”，无需刷新页面。 (TGT-52835)
...
* **产品列表在[!UICONTROL 查看收藏集]对话框中不可见：**&#x200B;以前，客户在[!UICONTROL 推荐]选项卡中查看收藏集时无法查看产品列表。 [!UICONTROL 查看收藏集]对话框现在可以正确显示关联的产品，从而提高更新的[!UICONTROL 推荐] UI中的透明度和可用性。 (TGT-50531)
* **修复了[!UICONTROL 产品目录搜索]高级搜索**&#x200B;中导致区分大小写筛选的问题：[!UICONTROL 产品目录搜索]页面中的高级搜索筛选现在正确忽略区分大小写的问题，这与后端和GraphQL服务的行为一致。 此更新确保无论文本大小写如何，都能为客户提供一致而准确的建议结果。 (TGT-53585)
* **更新的[!UICONTROL 产品目录搜索] UI中的高级搜索未提供建议**：在更新的[!UICONTROL 产品目录搜索] UI中使用高级搜索功能的客户需要输入拼写正确的精确值，因为未显示任何建议。 此问题使得难以有效地找到产品。 现在，在高级搜索输入过程中，建议会按预期显示。 (TGT-52008)
* **某些审批者无法查看[!UICONTROL 产品目录搜索]**&#x200B;中的产品：具有[!UICONTROL 审批者]权限的客户无法查看[!UICONTROL 产品目录搜索]中的任何产品，尽管具有相同角色的其他用户具有访问权限。 此问题是由影响目录可见性的权限不一致引起的。 所有审批者现在都可以按预期查看[!UICONTROL 推荐]部分中的产品。 (TGT-53617)
...
* **在“报表”页面上切换受众导致Target UI出错**：客户在[!UICONTROL 报表]部分中选择某些受众时遇到错误。 此问题是由后端GraphQL调用中的无效受众处理导致的，会导致意外错误和缺少数据。 该问题已得到解决，现在即使没有可用数据，桌面受众也加载且没有错误。 (TGT-53370)
* [!UICONTROL 报表]部分中的&#x200B;**[!UICONTROL 图形视图]未显示来自[!DNL Analytics]**&#x200B;的值：访问Re端口部分中的[!UICONTROL 图形视图]的客户遇到缺少数据的情况，所有值都显示为零。 此问题是由从[!UICONTROL Analytics]检索的数据不正确引起的。 [!UICONTROL 图形视图]现在可按预期显示准确的值。 (TGT-52792)
+++
...
* **使用[!UICONTROL 增强型体验编辑器] (EEC)单击“接受Cookie”失败，因为缺少函数**：客户报告说尝试通过EEC接受Cookie导致控制台错误： `handleclickAcceptAllButton is not defined`。 现在，Cookie接受功能可按预期工作，以确保在更新的UI中创建活动期间获得更流畅的体验。 (TGT-52794)
...
* **使用Backspace清除所有日期值导致页面崩溃**：使用Backspace清除“[!UICONTROL 指定的日期和时间]”字段中的所有值时，[!UICONTROL 目标和设置]分区中的计划活动的客户遇到崩溃。 此问题是由日期处理逻辑中的空引用错误导致的。 页面现在可以正常处理空日期输入而不会崩溃。 (TGT-53624)
* **由于有效负载无效，[!UICONTROL 产品目录搜索]中未出现任何产品**：访问[!UICONTROL 产品目录搜索]中的[!UICONTROL 推荐]分区的客户遇到了GraphQL有效负载无效导致的空结果。 此后端错误会导致产品数据无法正确加载。 现在，产品在更新后的UI中按预期显示。 (TGT-53630)
...
* **已停用更新的VEC**&#x200B;中的[!UICONTROL 失败]和[!UICONTROL 草稿]活动部分：已从更新的VEC中删除[!UICONTROL 失败]和[!UICONTROL 草稿]活动选项。 新的UI不再支持草稿状态，并且失败的营销活动不会存储在后端。 这些选项不再相关。 相关筛选器和后端字段（例如，`uiSyncFailed`、`errorMessage`）也已移除，以简化活动管理。 (TGT-53150)
* **无法登录到活动的VEC**：尝试通过VEC登录到其网站的客户被反复重定向到登录页面，从而无法访问活动编辑。 此问题无法在内部复制，并且可能与站点端会话处理相关。 登录流程已稳定，客户现在可以访问VEC而不会出现重定向错误。 (TGT-53524)
* **在[!UICONTROL 浏览]模式下按“返回”按钮两次会导致VEC崩溃**：在VEC中浏览[!UICONTROL 浏览]模式的客户在按两次浏览器的返回按钮时遇到了崩溃。 此问题导致编辑器冻结并需要刷新页面。 现在，该编辑器可以可靠地处理背面导航，而不会崩溃。 (GT-53568)
...
* **修复了计划活动在页面刷新之前错误地显示[!UICONTROL 实时]状态的问题**：客户发现，在计划活动以在未来的日期和时间上线时，状态立即显示为[!UICONTROL 实时]，而不是[!UICONTROL 已计划]。 这会导致混淆，即使确认消息正确指示已计划活动。 刷新页面更正了状态显示。 此问题现已得到解决，计划的活动可正确显示计划状态，而无需刷新。 (TGT-52937)
...
* **修复了在活动创建过程中客户无法键入报表包名称的问题**：在活动创建过程中使用[!DNL Adobe Analytics]作为报表源的客户无法键入到[!UICONTROL 报表包]下拉列表中以搜索特定的报表包。 这会影响具有大量报表包的组织的工作流，在这些报表包中，手动滚动会显着延迟设置。 下拉列表未按字母顺序排序，并且未始终响应键入的输入，因此很难找到“Office + Store - Web - Prod”等报表包。 此问题已得到解决，客户现在可以通过键入报表包名称来高效搜索。 (TGT-53345)
...
* **修复了阻止客户在UI中使用[!UICONTROL 此项前插入]或[!UICONTROL 此项后插入]插入体验片段的问题**&#x200B;客户在尝试在更新的UI中使用“此项前插入”或“此项后插入”选项将[!UICONTROL 体验片段]插入到活动中时遇到错误。 显示的错误消息为：“选件内容必须正好包含一个HTML元素。” 此问题特定于更新后的UI，而在以前的版本中未出现。 此问题现已得到解决，客户可以成功插入[!UICONTROL 体验片段]，而不会遇到此错误。 (TGT-53442)
...
* **修复了以下问题：[!UICONTROL 优惠决策]选择器保存后无法修改并意外更改**：在更新的活动创建过程中，客户无法按预期修改[!UICONTROL 优惠决策]选择器。 尝试更改选择器失败，选择器在保存后恢复到不正确的值。 此行为导致决策选件从可视化体验编辑器(VEC)中消失，阻止进一步编辑。 现在，选择器更改会正确保留，并且决策选件在保存之后在VEC中保持可见和可编辑状态。(TGT-53433)
* **修复了在保存并重新打开活动后[!UICONTROL 优惠决策]从活动中消失的问题**：在活动创建过程中添加的[!UICONTROL 优惠决策]未保留。 在编辑动态内容并使用特定选择器和属性插入[!UICONTROL 优惠决策]时，出现此问题。 [!UICONTROL 优惠决策]在保存后现在可正确保留，选择器将保持不变，以确保一致的定位和编辑行为。 (TGT-53434)
...
* **修复了[!UICONTROL 目录搜索]**&#x200B;中图像加载不一致的问题：修复了[!UICONTROL 目录搜索]中的缩略图和图像在活动创建过程中加载不一致的问题。 除非“缩略图URL”列可见，并且某些产品图像在导航或搜索操作后已加载部分或完全未加载，否则图像无法显示。 图像加载行为已稳定，并且无论列可见性或导航操作如何，缩略图现在都可以可靠地显示。 (TGT-52778)
...
* **修复了[!UICONTROL 推荐目录]间歇性地无法显示完整产品属性数据的问题**：在更新的[!DNL Recommendations] UI中，客户遇到了[!UICONTROL 目录搜索]结果中无法一致显示某些产品属性（如消息）的问题，即使数据存在于馈送中也是如此。 此问题要求客户手动重新配置列可见性以检索缺少的值。 [!UICONTROL 目录搜索]现在可靠地显示所有已配置的属性，无需手动重置列。 (TGT-52769)
* **修复了无法在实时活动中禁用[!UICONTROL 前端促销活动]的问题**：未保存尝试在实时活动中禁用[!UICONTROL 前端促销活动]。 选择[!UICONTROL 更改促销活动]并禁用它后，该促销活动在重新编辑活动时保持活动状态，从而阻止更新推荐配置。 促销活动设置现在可正确保存，这样客户就可以按预期禁用或修改实时活动中的促销活动。 (TGT-53231)
* **修复了在启用不带数据的[!DNL Recommendations] [!UICONTROL 促销活动]时触发不明确的错误消息**&#x200B;的问题：在[!DNL Recommendations]活动中启用[!UICONTROL 前置]或[!UICONTROL 后端促销活动]而未指定所需值会导致出现一般的“输入错误无效”消息。 基础问题是缺少配置字段，但错误消息未明确说明原因，因此难以进行故障诊断。 现在，活动创建流程可在必填字段（如`collectionId`或规则）缺失时提供明确且可操作的错误消息，帮助客户快速识别并解决配置问题。 (TGT-52616)
* **修复了导致[!UICONTROL 产品]列表无法在[!UICONTROL 推荐]选项卡**&#x200B;的[!UICONTROL 编辑]模式中显示的问题：在[!UICONTROL 推荐]选项卡中编辑[!UICONTROL 收藏集]或[!UICONTROL 排除项]时，客户无法查看筛选的产品列表。 该列表应基于应用的规则实时更新，但似乎未按预期进行。 此问题已得到解决，产品列表现在可正确显示，并且随着规则的修改而动态更新。 (TGT-53481)
* **修复了更新的UI中“查看详细信息”对话框的布局问题**：更新的UI中的“查看详细信息”模式布局已修改，以提高清晰度和可用性。 该对话框现在包含两个选项卡：
  * [!UICONTROL 详细信息]选项卡：显示选定项目的所有相关信息。
  * [!UICONTROL 清单]选项卡：显示按当前集合和排除规则过滤的所有产品。

  此增强功能可帮助客户更轻松地在活动创建过程中导航和了解特定于项目的数据和库存上下文。 (TGT-53503)

  * **修复了在保存后推荐活动中移除的促销重新出现的问题**：客户报告说，当[!DNL Recommendations]活动中移除[!UICONTROL 前置]或[!UICONTROL 后置]促销活动并保存该活动时，重新打开时促销活动会继续显示。 此问题在暂存环境和生产环境中都发生，并影响更新的活动创建过程。 问题已得到解决。 现在，从活动中移除的促销活动在保存后可正确保留。 (TGT-53490)
...
* **修复了[!UICONTROL 自动化区段]报表显示null受众值的问题**：客户报告活动报表中的[!UICONTROL 自动化区段]显示受众数据为null，从而无法准确分析区段性能。 在访问[!UICONTROL 报表]部分并选择[!UICONTROL 自动化区段]时，即使需要有效的受众数据也会发生此问题。 [!UICONTROL 自动化区段]现在可以正确显示受众值，从而确保获得可靠的报表和分段见解。 (TGT-52793)
...
* **修复了在更新的UI中在[!UICONTROL 浏览]和[!UICONTROL 设计]模式之间切换时重置SPA状态的问题**：客户报告了在更新的UI中在[!UICONTROL 浏览]和[!UICONTROL 设计]模式之间切换时导致Web编辑器重新加载，重置了SPA的状态。 此问题会中断工作流，并要求客户重新输入信息。 问题已得到解决。 现在，在模式之间切换时，会保留SPA状态，以确保在活动创建期间获得更顺畅且一致的体验。 (TGT-53074)
...
* **修复了活动创建过程中阻止推进到AP活动中[!UICONTROL 定位]步骤的问题**：修复了活动创建过程中客户无法继续到[!UICONTROL Automated Personalization] (AP)活动中[!UICONTROL 定位]步骤的问题，除非添加了两个位置。 此行为与以前的体验不同，在以前的体验中，具有多个选件的单个位置便已足够。 该要求已得到纠正，允许客户继续将单个位置设置作为其AP工作流的一部分。 (TGT-53426)
...
* **修复了阻止客户在更新的UI中搜索报表包的问题**：在更新的UI中，[!UICONTROL 目标和设置]分区中的[!UICONTROL 报表包]下拉列表不允许客户键入和搜索，这使得难以找到特定的报表包，尤其是对于具有大量条目的租户。 与以前的UI不同，列表未排序，缺少基于输入的筛选。 此问题已得到解决。 客户现在可以键入以搜索和筛选报表包，从而恢复旧版UI中提供的功能。 (TGT-53514)
...
* **修复了以下问题：在活动创建过程中选择[!UICONTROL 所有工作区]时，仅限单个工作区的客户可以查看其他工作区的活动**：仅限一个工作区的客户意外地可以查看所有工作区的活动。 此可见性可能会对其他工作区中的实时活动造成意外更改，从而可能影响网站性能。 Workspace访问控制已得到增强，以确保客户只能查看其分配的工作区中的活动并与之交互。 (TGT-53101)
* **修复了以下问题：客户在没有访问权限的情况下可以从[!UICONTROL 默认Workspace]查看活动：**&#x200B;只有暂存工作区访问权限的客户可以通过活动创建流程从[!UICONTROL 默认Workspace]查看活动。 尽管后端配置和访问权限正确，仍会发生此行为。 Workspace访问控制已得到增强，以确保客户只能在其分配的工作区内查看活动。(TGT-53297)
...
* **修复了在更新的UI中AP活动的活动创建过程中出现的体验要求问题**：修复了活动创建过程中出现的问题，即[!UICONTROL Automated Personalization] (AP)活动在更新的UI中需要两个位置而不是两个体验。 此行为阻止了客户使用以前支持的多个选件配置单个位置的用例。 该要求已更正以与原始功能匹配，允许客户使用两种体验继续进行AP活动，而不管位置数量如何。 (TGT-53429)
* **修复了点击跟踪模式下跟踪的元素字段行为，以防止未保存的输入并提高清晰度**：修复了活动创建过程中的问题，该问题导致在[!DNL Click Track]模式下的[!UICONTROL 跟踪的元素]字段可编辑，但未保留输入的名称，进而给客户造成混淆。 字段现已禁用，以防止未保存的输入，并且已阐明选定元素的命名方式，以提高目标配置和跟踪准确性。** (TGT-53458)
* **修复了活动创建过程中阻止在[!UICONTROL 点击跟踪]模式下命名跟踪组件的问题**：修复了活动创建过程中客户无法在[!UICONTROL 点击跟踪]模式下命名跟踪组件的问题。 输入名称后，该字段显示为可编辑，但未保留输入，在编辑模式下默认为通用标签，如“MY PRIMARY GOAL 0”。 跟踪的元素字段现已禁用，并且已阐明命名行为以提高目标设置和跟踪准确性。 (TGT-51396)
...
* **修复了目录搜索UI中高级搜索未能提供建议的问题**：修复了新[!UICONTROL 目录搜索] UI中[!UICONTROL 高级搜索]功能未能提供建议的问题。 要求用户输入具有精确拼写的精确值，这使得搜索体验繁琐且容易出错。 通过此修复，[!UICONTROL 高级搜索]现在会根据用户类型提供相关建议，从而提高可用性并减少查找产品时的摩擦。 (TGT-52008)
...
* **修复了按[!UICONTROL 上次更新]列排序不一致的问题：**&#x200B;修复了活动创建过程中按[!UICONTROL 上次更新]列排序目录导致结果不一致的问题。 客户无法根据更新的时间戳可靠地组织产品数据，导致目录审核和管理更加困难。 排序现在按预期工作，提高了更新UI中的准确性和可用性。 (TGT-53116)
...
* **修复了活动加载和[!UICONTROL 取消]活动创建流程中的按钮问题**：修复了活动创建流程中某些活动加载失败，导致客户无法访问修改的问题。 此外，[!UICONTROL 取消]按钮无响应，导致客户无法停止加载过程或退出编辑视图。 此修复程序可确保活动现在可靠地加载，并且[!UICONTROL 取消]按钮按预期运行，从而提高可视化体验编辑器中的整体稳定性和用户体验。 (VEC)(TGT-53218)
* **修复了更新的VEC UI中阻止编辑并禁用[!UICONTROL 取消]按钮的体验切换问题**：修复了更新的VEC UI中在体验定位(XT)活动中的体验之间切换时无法加载修改的问题。 客户无法编辑超出最初输入范围的体验，且[!UICONTROL 取消]按钮缺失或无响应。 此修复确保修改现在在所有体验中正确加载，并且[!UICONTROL 取消]按钮可靠运行，即使没有助手扩展也是如此，从而改善编辑工作流并减少挫折感。 (TGT-53256)
...
* **修复了VEC中自定义代码更改无法可靠地跨编辑会话保存的问题**：修复了在可视化体验编辑器(VEC)中进行的自定义代码修改无法在一次尝试中可靠地保存的问题。 客户报告，即使同时使用[!UICONTROL 编辑内容]和最终[!UICONTROL 保存]按钮，网站和QA URL中仍间歇性地缺少样式更新或HTML编辑等更改。 此回归已被解决，确保所有自定义代码更改在编辑会话间按预期持续存在。** (TGT-53418)
...
* 修复了更新的[!DNL Target] UI中，具有[!UICONTROL 编辑者]角色的用户能够访问和尝试编辑实时活动的问题，应限制该问题。 活动列表和概述屏幕错误地显示了实时活动的编辑选项，从而导致潜在的意外更改。 (TGT-53055)
* 修复了[!UICONTROL 高级搜索] UI中的一个问题：选择“value present”或“value not present”运算符会提示用户输入操作数，对于这些条件，该操作数不应该是必需的。 (TGT-51961)
...
* 修复了体验片段(XF)内容无法在[!UICONTROL 可视化体验编辑器] (VEC)预览中呈现的问题，尽管该内容在内容交付中正常工作。 (TGT-53318)
...
* 修复了更新的[!DNL Target] UI中的一个问题：在从[!UICONTROL 定位]选项卡导航回[!UICONTROL 体验]后，在[!UICONTROL 可视化体验编辑器] (VEC)中做出的HTML选件修改无法保留。 (TGT-52874)
* 修复了更新的[!DNL Target] UI中的一个问题：在同一选择器之前和之后插入一个HTML选件会导致不正确的位置生成。 当客户从[!UICONTROL 定位]选项卡返回到[!UICONTROL 体验]选项卡时，仅保留了一个选择器，从而导致修改丢失和内容投放中断。 此行为与旧版UI不同，旧版UI使用不同的位置标识符正确保留两个修改。 (TGT-52891)
* 修复了更新的[!DNL Target] UI中的一个问题：单击[!UICONTROL 修改]边栏中的已添加选件会导致右侧[!UICONTROL 配置]面板间歇性地出现和消失，从而使其难以与选件设置进行交互。 (TGT-53288)
...
* 修复了更新后的VEC中阻止客户对[!UICONTROL 单页应用程序] (SPA)视图进行修改的问题。 虽然在旧版UI中创建的活动支持特定于视图的定位，但新UI无法检测或允许对这些视图进行编辑，视图切换控件显示为禁用。 (TGT-52556)
...
* 修复了对[!UICONTROL 可视化体验编辑器]中的体验所做的修改不可见或在UI中显示不一致的问题。 (TGT-TGT-53381)
* 修复了更新后的VEC中，[!UICONTROL 管理内容]部分无法显示体验缩略图的替换文本的问题。 此问题使用户难以识别文档，尤其是在文件名较长或截断时。 (TGT-52291)
* 修复了客户在VEC活动中配置[!UICONTROL 页面交付]规则时遇到错误验证错误的错误。 具体而言，在输入文本值时，诸如“等于任意”和“不等于任意”之类的运算符会触发“运算符类型的输入无效”，即使应当支持文本也是如此。 (TGT-52629)
* 修复了在使用“选择选件”按钮选择选件时，导致更新UI的[!UICONTROL 修改]面板中行为不一致的几个问题。 UI未替换现有选件，而是添加了一个重复的选件，并尝试与任一选件交互会导致控制台错误，例如`selectorNotFound`。 此外，某些选件不会显示“选择选件”按钮，只会显示可编辑的属性。 (TGT-53321)
...
* 修复了在更新的[!DNL Target] UI中的工作区之间复制活动时的多个问题。 客户遇到与受众访问相关的错误，特别是实时活动，以及不正确的权限验证，即使用户在源和目标工作区中均具有“[!UICONTROL 审批者]”角色也是如此。 (TGT-53002)
...
* 修复了[!UICONTROL 创建标准]模式中的本地化问题，在[!UICONTROL 选择价格规则]下拉列表中选择“在以下值之间”选项时，[!UICONTROL 包含规则]部分中的字符串“to”未本地化。 (TGT-49754)
* 修复了源创建向导的[!UICONTROL 环境]下拉列表中的字符串“[!UICONTROL 所有主机组]”未正确本地化的问题。 (TGT-46737)
...
* 修复了阻止在QA环境中创建活动的问题。 进程已重定向到[!UICONTROL 活动]页面，而不是成功完成。 (TGT-53328)
...
* 修复了在更新的UI中[!UICONTROL 目录搜索]中的筛选器建议不准确的问题。 (TGT-52007)
...
* 修复了在编辑Automated Personalization (AP)活动时，当用户单击[!UICONTROL 管理内容]然后单击[!UICONTROL 完成]时，页面变为空白且无响应的问题。 （TGT-53047 和 TGT-52993）
* 修复了在[!UICONTROL 目标和设置]下选择[!UICONTROL 已查看mbox]转化量度导致页面崩溃的问题。 （TGT-53346、TGT-53343 和 TGT-53348）
* 修复了[!UICONTROL 重定向到URL]功能无法按预期运行的问题，即使URL有效，也不会进行重定向。 (TGT-53307)
...
* 修复了当工作区设置为“[!UICONTROL 所有工作区]”时，复制默认工作区中已存在的活动会导致错误的问题：
...
* 修复了将修改从[!DNL Page Load]移动到[!UICONTROL 视图]导致修改重复的问题，该问题保留在[!UICONTROL 页面加载]中，同时出现在[!UICONTROL 视图]中。 此外，从[!UICONTROL 视图]中删除修改也会错误地从[!UICONTROL 页面加载]中删除它。 (TGT-53270)
...
* 修复了[!UICONTROL 基于表单的体验编辑器]中的一个问题，在创建或编辑[!UICONTROL Automated Personalization] (AP)活动时，该问题导致编辑器在单击&#x200B;**[!UICONTROL 管理内容]**&#x200B;图标（![管理内容图标](/help/main/assets/icons/Experience.svg)）后崩溃。 (TGT-53047)
...
* 修复了一个问题，该问题导致[!UICONTROL 目录搜索]在滚动到列表底部时无法加载其他结果，从而恢复与旧版UI一致的行为。 (TGT-53088)
* 修复了阻止从[!UICONTROL 标准详细信息]对话框中删除项的问题。 (TGT-53245)
* 修复了导致无法打开或无法与无名称的产品交互的问题。 选择返回未命名结果的环境时，出现此问题，导致无法访问产品详细信息。 (TGT-53007)
* 修复了在选择某些产品时导致[!UICONTROL 目录搜索]页面崩溃并显示空白屏幕的问题。 (TGT-53087)
* 修复了用户无法在[!DNL Target]用户界面中编辑site_cart_z1 [!DNL Recommendation]活动的问题。 尝试打开活动时，在[!UICONTROL 概述]页面上触发了一个错误，阻止了对编辑器的访问。 (TGT-53221)
...
* 在Target后端中重新实现了已删除的选项持久层，以支持[!UICONTROL Automated Personalization] (AP)活动中的准确历史报表。 以前，删除选项时，会丢失其名称，从而难以解释过去的性能数据。
...
* 修复了在VEC中配置图像选件时，[!UICONTROL 此项前插入]和[!UICONTROL 此项后插入]选项的[!UICONTROL 撤消]功能存在的问题。

  以前，对图像选件撤消执行[!UICONTROL 此项前插入]或[!UICONTROL 此项后插入]操作会导致不一致的行为或无法正确还原修改，尤其是在旧版[!DNL Target] UI中创建的活动中。 此问题已得到解决，以确保撤消操作现在能够可靠地用于这些修改。 (TGT-52809)
...
* 已将[!UICONTROL 目标和设置]中的[!UICONTROL 优先级]滑块还原为旧版功能，允许客户直接输入数值，如旧版UI中所支持。 （TGT-53185 和 TGT-53219）
...
* 修复了客户在[!UICONTROL 目标和设置]页面上查看特定活动的报表时，[!UICONTROL 在Analytics中查看]链接错误地指向QA环境而不是生产环境的问题。 (TGT-53163)

+++

**[!UICONTROL 体验]和[!UICONTROL 选件]**

+++查看详细信息

* 修复了通过自定义代码调用`triggerView`导致无限循环的问题。 (TGT-52885)
* 修复了导致为活动定义的`LocalIds`与在体验定义中使用的那些`LocalIds`之间不匹配的问题。 (TGT-52669)
* 修复了活动[!UICONTROL 概述]页面上缺少量度名称的问题，该问题仅显示“选件”而不是正确的量度名称。 (TGT-53054)
...
* 修复了[!UICONTROL 基于表单的体验编辑器]中的一个问题，该问题阻止活动保存并触发错误消息：“无法读取未定义的属性（读取&#39;map&#39;）”。 (TGT-53145)
...
* 修复了单击[!UICONTROL 目录搜索]中的产品时显示“无法检索产品详细信息”错误并在没有关闭选项的情况下打开模式窗口的问题。 (TGT-53082)
* 修复了在更改收藏集或促销活动时，[!UICONTROL A/B测试]活动中的[推荐（选件](/help/main/c-recommendations/recommendations-as-an-offer.md)）未正确更新的问题。 (TGT-52884)
...
* 解决了[!UICONTROL 目标和设置]页面上的一个问题：多个体验中使用的选择器未一致地高亮显示为选定选定项。 (TGT-53062)
...
* 修复了在工作区之间更改属性时出现的问题。
  ...
  * 如果[!UICONTROL 属性]列表显示警告（可能表示某些属性不兼容），并且客户单击[!UICONTROL 添加]或[!UICONTROL 删除]，然后单击[!UICONTROL 保存]，则将删除目标工作区之外的所有属性。 如果客户单击[!UICONTROL 取消]，则所有属性都将保留，即使目标工作区中不存在这些属性也是如此。 (TGT-47094)
  * 如果保留在同一个工作区中，或从非默认工作区切换到默认工作区或其他工作区，则所有内容将保持不变。 (TGT-53078)
* 更新了实体验证逻辑以遵循活动的原始工作区上下文。 诸如[!UICONTROL 体验片段] (XF)之类的实体现在已根据最初创建该活动的工作区进行验证。 例如，如果默认工作区中存在XF，并且活动从工作区X复制到工作区Y，则只要XF在原始（默认）工作区中有效，验证仍会传递。 (TGT-53196)
* 在活动复制期间对复制临时受众的增强支持。
...
* 修复了[!UICONTROL 活动QA] URL包含不必要查询参数的问题： `at_preview_evaluate_as_true_audience_ids`。 (TGT-52907)
* 修复了预览URL错误地包含用户明确键入的受众以外的其他受众的问题。 已更正此行为，以确保在生成QA或预览链接时仅应用指定的受众。 (TGT-52912)
* 修复了在流量分配设置期间首先选择[!UICONTROL 自动分配] (AA)时阻止用户创建[!UICONTROL 自动定位] (AT)活动的问题。 此问题会导致后端验证错误，并阻止保存活动。 (TGT-53096)
...
* 修复了具有[!UICONTROL 审批者]角色的用户无法添加或保存仅限该活动的受众细化的问题。 尝试这样做会导致403禁止错误，表明需要“[编辑者]”权限，即使用户具有足够的权限来批准和管理活动。 (TGT-52984)
* 修复了使用[!UICONTROL 删除受众细化]选项删除特定于活动的受众时，该受众不再出现在受众列表中以供在同一活动中重新选择的问题。 除非从头开始重新创建受众，否则此行为会阻止用户重新添加相同的受众。 (TGT-52979)
* 修复了以下问题：从位置中删除后，甚至在保存活动之前，仅限该活动的受众细化都会立即从UI中消失。 此行为与预期功能和工具提示指导相冲突，该指导会指出：“保存活动后，将删除此库中所有未使用的受众。” (TGT-52982)
* 修复了尝试将[!UICONTROL 所有访客]以外的受众分配给活动时出现的问题。 保存后，显示以下错误消息：“我们无法完成您的请求。 如果问题仍然存在，请联系[!UICONTROL Adobe客户关怀团队]。” (TGT-53008)
* 修复了在活动编辑器中创建和分配新受众后阻止保存活动的问题。 显示的错误消息为：“我们无法完成您的请求。 如果问题仍然存在，请联系[!UICONTROL Adobe客户关怀团队]。” (TGT-52977)
...
* 修复了单击“在Analytics中查看”链接会将用户重定向到主页而不是预期的[!DNL Analytics]仪表板的问题。 （TGT-53092 和 TGT-53093）
...
* 修复了[!UICONTROL 管理内容]功能在删除所有内容选项后未正确验证位置状态的问题。 在尝试保存或继续进行活动配置时，此问题可能会导致不一致的行为或错误。 (TGT-52801)
* 修复了用户在添加新页面和删除不同体验中的特定元素时遇到“输入无效”错误的问题。 在元素操作期间，特别是在体验之间切换和修改共享页面结构时，通过生成重复的`LocalIds`触发了该错误。 (TGT-52720)
* 修复了使用[!UICONTROL 生成临时选件]功能导致[!UICONTROL 管理内容]面板中出现未定义位置的问题。 （TGT-53076 和 TGT-53070）
* 阐明了在客户处发生的行为，即从[!UICONTROL 定位]步骤导航回[!UICONTROL 体验]时，使用HTML选件进行的修改可能显示缺失。 对于此客户，受影响的网站会动态生成多个DOM选择器，这些选择器会随着每次页面加载而发生更改。 因此，在重新打开编辑器时，无法找到最初用于修改的选择器，从而导致修改丢失或无效。 此方案按设计要求运行。 为确保修改在编辑器中持久保留，建议客户端使用稳定、一致且不会因页面重新加载而更改的选取器。 (TGT-52874)
...
* 添加了一个新的[!DNL Recommendations]信息源[状态](/help/main/c-recommendations/c-products/feeds.md#status)： [!UICONTROL 部分导入失败]。 (KB-2215)
* 修复了在添加具有[!UICONTROL 促销活动]的[!DNL Recommendations]时影响活动创建工作流的问题。 当用户选择“[!UICONTROL 按属性提升]”并添加筛选规则（例如[!UICONTROL 参数匹配]）时，在保存和重新编辑活动后所选规则类型和操作数值未保留。 重新打开时，筛选规则类型将意外更改，并且缺少操作数值。 (TGT-53059)
* 修复了[!DNL Recommendations] UI中的一个问题：使用单个规则创建的任何促销活动被错误地解释并显示为“项目列表”促销类型，而不管该规则的逻辑如何。 (TGT-53063)
* 修复了在使用更新的[!UICONTROL 概述]用户界面时，包含[!DNL Recommendations]的[!UICONTROL 体验定位] (XT)活动“[!UICONTROL 下载推荐数据]”按钮缺失的问题。 （TGT-52730 和 TGT-52756）
...
* 修复了在具有推荐的基于表单的A/B活动中配置“[!UICONTROL 按属性促销]”促销活动时，客户无法保存筛选规则的问题。 保存并重新打开活动后，缺少筛选规则，无法成功保存活动。 (TGT-53057)
...
* 修复了编辑和保存自定义代码导致[!UICONTROL 修改]面板无响应的问题。 (TGT-53075)
* 修复了对变量体验中的自定义代码所做的修改意外地反映在[!UICONTROL 控制]体验中的问题。 这会导致投放行为发生意外更改。 现在，[!UICONTROL Control]体验仍然与对其他体验所做的自定义代码编辑隔离。 (TGT-52413)
* 修复了在编辑器完全加载之前用户单击了第二个体验（体验A）时，对某个体验（例如，体验B）所做的修改无意间复制到另一个体验（体验A）的问题。 如果最初选择的体验没有发生更改，此行为还可能导致修改丢失。 (TGT-52597)
* 修复了在活动创建[!UICONTROL 修改]步骤中所做的更改无法一致保存的问题。 在某些情况下，完成所有步骤并单击[!UICONTROL 保存]后，添加到[!UICONTROL 修改]部分的自定义脚本将不会在实时站点上反映，尽管在保存过程中没有可见错误。 (TGT-52661)
* 修复了以下问题：自定义代码更改无法正确保存，并且在同一活动中的多个体验间无意中进行了镜像。 此外，用户在打开或刷新某些活动时遇到访问问题，导致屏幕空白。 这些问题现已得到解决，以确保稳定的活动编辑和准确的体验隔离。 (TGT-52594)
* 修复了用户在[!UICONTROL 浏览模式]下无法浏览到其他URL的问题。 这会阻止测试人员和编辑人员验证或预览同一活动会话中的替代页面。 (TGT-53052)
* 修复了在活动创建期间同时打开多个[!UICONTROL 可视化体验编辑器] (VEC)实例的问题。 当用户禁用[!UICONTROL 增强型体验编辑器] (EEC)并在[!UICONTROL 页面交付]步骤中从网站URL中删除结尾斜杠时，会发生此问题。 (TGT-52782)
* 修复了以下问题：[!UICONTROL 目标和设置]步骤中的[!UICONTROL 收入]量度下拉列表错误地默认为[!UICONTROL 每次访问的收入] (RPVISIT)，即使用户选择了其他量度也是如此。  折叠和重新展开指标配置面板时出现问题，导致重置之前选择的值。 （TGT-52811 和 TGT-52878）
* 修复了活动创建工作流中与[!UICONTROL Automated Personalization] (AP)和[!UICONTROL Multivariate Testing] (MVT)活动中的选件命名和内容翻译相关的几个问题：
  ...
  * 通过右侧面板重命名选件更新了UI中的名称，但更改未反映在[!UICONTROL 管理内容]选项卡或[!UICONTROL 选件]选项卡中，从而导致持续验证错误。
...
* 已将[!UICONTROL 重新排列]选项添加到更新的[!UICONTROL 可视化体验编辑器] (VEC) UI，以便与旧版VEC中可用的功能保持一致。 （TGT-46957 和 TGT-52876）
* 修复了在[!UICONTROL A/B测试]活动中对变体体验（例如体验B）所做的修改未保留的问题。 在体验之间切换后，对变量的更改将消失。 此问题不会影响控制体验。 (TGT-52664)
* 修复了以下问题：某些客户无法创建或保存活动，而其他客户可以正常执行相同的操作。 问题在帐户之间不一致。(TGT-52842)
* 修复了在更新的VEC中，用户无法移动对[!UICONTROL 页面加载事件]的修改的问题，该功能存在于旧版UI中。 (TGT-52617)
* 修复了更新后的UI中的一个问题：在创建更改时[!DNL Target]中未显示[!UICONTROL 页面加载]事件；更新仅应用于视图。 (TGT-52604)
...
* 修复了在更新的VEC中应用修改时，更改最初正确显示的问题，包括预期的[!UICONTROL 体验片段]。 但是，在切换体验或进行其他编辑时，由于选择器问题，某些修改无法应用。 (TGT-52679)
* 修复了在通过克隆现有活动创建新活动时，克隆活动中的QA链接错误地保留原始活动中的页面URL的问题。 (TGT-52775)
* 修复了会无意中导致[!UICONTROL 设备上决策]在更新的VEC中不可用的问题。 (TGT-52371)
* 修复了阻止编辑产品[!DNL Recommendations]活动的问题。 尝试通过Target UI访问VEC时，[!UICONTROL 概述]页面上出现错误，导致无法进行任何编辑。 (TGT-52823)
...
* 修复了具有[!UICONTROL 编辑者]角色的用户无法编辑[!DNL Recommendations]活动的问题。 尝试更改设计并保存活动会导致403禁止错误，声明“[编辑者]”权限是必需的，即使用户已在相关工作区中拥有该角色。 (TGT-52836)
...
* 修复了无法为某些活动生成[!UICONTROL 自动化区段]和[!UICONTROL 重要属性]报告的问题。 (TGT-52904)
...
* 修复了[!UICONTROL 多变量测试] (MVT)活动中的一个问题，该问题导致不正确地不允许重用之前删除的体验中的`experienceLocalId`。 (TGT-52672)
...
* 修复了在更新实时[!UICONTROL 推荐]活动时由于请求有效负载中的属性无法识别而发生的错误。 系统现在可以正确处理“无效JSON”。 无法识别的属性名称”错误。 (TGT-52723)
* 修复了阻止创建[!DNL Recommendations]设计的问题。 单击[!UICONTROL 创建]触发消息：“脚本中应至少使用1个实体变量。” （TGT-52395 和 TGT-52899）
* 修复了阻止在不进行修改的情况下重新保存[!DNL Recommendations]设计的问题。 (TGT-52879)
* 修复了在保存[!UICONTROL 推荐]活动时导致出现“400错误请求”错误的后端验证错误。 (TGT-52716)
* 修复了[!UICONTROL 基于表单的体验编辑器]中的一个问题：在[!UICONTROL 位置]下拉列表中将鼠标悬停在具有特殊字符的mbox上，会导致编辑器变为空白并触发“无法对‘元素’执行‘querySelector’”。 错误。 (TGT-52717)
...
* 添加了[篇新的常见问题解答文章](/help/main/c-intro/updated-ui-faq.md)，以解决有关已更新的[!DNL Target] UI和[!UICONTROL 可视化体验编辑器] (VEC)的常见问题。
* 修复了[!UICONTROL 页面交付]中的“[!UICONTROL URL - does not contain]”规则不起作用的问题，即使本应阻止该规则，仍允许显示内容。 (TGT-52754)
* 修复了[!UICONTROL 页面交付]错误显示错误消息“不允许存在重复的页面URL”的问题。 (TGT-52765)
* 修复了以下问题：为包含体验片段的[!UICONTROL 页面交付] URL创建受众时，错误地附加了#。 (TGT-52786)
* 修复了复制活动并编辑[!UICONTROL 目标和设置]页面上的设置导致[!DNL Target] UI无响应的问题。 (TGT-52797)
* 修复了更新的[!UICONTROL 可视化体验编辑器] (VEC)中错误允许将[!UICONTROL A/B测试]活动中的其他页面重定向到同一URL的问题。 (TGT-51838)
* 修复了在编辑活动时未保存[!UICONTROL 目标和设置]页面上量度更改的问题。 (TGT-52799)
...
* 修复了更新的[!UICONTROL 可视化体验编辑器] (VEC)中未检测到`triggerView()`调用的问题。 (TGT-52575)
* 修复了更新的[!UICONTROL 可视化体验编辑器] (VEC)中的一个问题，该问题阻止用户向[!UICONTROL 单页应用程序] (SPA)视图添加修改。 (TGT-52556)
* 修复了更新的[!DNL Target] UI中阻止客户查看优惠详细信息的问题。 (TGT-52607)
* 修复了对[!UICONTROL 选件库]中的选件所做的更新未反映在更新的[!UICONTROL 可视化体验编辑器] (VEC)中的问题。 (TGT-52637)
...
* 修复了没有[!UICONTROL 活动QA]按钮的活动显示空选项选择器的问题。 (TGT-52733)
* 修复了QA链接无法正确呈现内容的问题。 (TGT-52718)
* 修复了将元素替换为体验片段时无法正确反映QA环境中更改的问题。 (TGT-52762)
* 修复了在更新的[!UICONTROL 可视化体验编辑器] (VEC)中当用户尝试添加体验片段时导致“输入无效”错误的问题。 (TGT-52701)
* 修复了在更新的[!UICONTROL 可视化体验编辑器] (VEC)中编辑受众定位时，“编辑受众”模式显示为空的问题。 (TGT-52749)
* 添加了消息，以在所选工作区中无法访问实体时通知用户。 (TGT-52767)
* 修复了UI无法允许将环境ID手动分配给标准的问题。 而是默认使用[!UICONTROL 产品目录搜索]主机组的ID。 此修复确保标准更改现在可以应用于所有环境，而不仅仅是默认环境。 (TGT-52817)
* 修复了包含推荐的[!UICONTROL 体验定位] (XT)活动缺少“[!UICONTROL 下载推荐数据]”选项的问题。 （TGT-52730 和 TGT-52756）
...
* 修复了当内部网页加载页面失败时，[!UICONTROL 可视化体验编辑器] (VEC)中的修改无法访问的问题。 (TGT-52488 &amp;TGT-52470)
* 修复了VEC中较小屏幕尺寸上看不到[!UICONTROL 修改]面板的问题。 (TGT-52470)
* 修复了更新后的VEC中的一个问题：从[!UICONTROL 浏览]模式切换回[!UICONTROL 设计]模式会导致控制台错误并阻止进一步的交互。 (TGT-52532)
...
* 修复了导致活动的[!UICONTROL 概述]页面显示[!UICONTROL 活动位置]的无效URL的问题。 (TGT-52695)
* 修复了更新的[!DNL Target] UI中导致活动位置出现重复条目的问题。 (TGT-52693)
* 修复了触发`getAudiencesV3`错误并阻止客户编辑或复制活动的问题。 (TGT-52709)
* 修复了在将[!UICONTROL 体验片段]或HTML选件添加到活动时导致无效有效负载错误的问题。 （TGT-52779 和 TGT-52773）
* 修复了更新的[!DNL Target] UI中，由于输入错误而导致无法正确显示E[!UICONTROL 体验片段]的问题。 (TGT-52701)
* 修复了由于无效的用户错误，客户无法编辑[!UICONTROL 基于表单的体验编辑器]中的活动的问题。 (TGT-52470)
...
* 在[!UICONTROL 报表] > [!UICONTROL 报表设置] （![报表设置图标](/help/main/assets/icons/Setting.svg)）下添加了QA模式流量设置，以便与旧版[!DNL Target] UI中的功能保持一致。 （TGT-52228 和 TGT-52329）
...
* 修复了更新的[!UICONTROL 可视化体验编辑器] (VEC)无法自动检测`at_property`值的问题。 (TGT-52347)
* 修复了一个问题，该问题导致在与表单元素交互时，在VEC中的[!UICONTROL 浏览]和[!UICONTROL 设计]模式之间切换后，预期仅记录一次修改。 (TGT-52455)
* 修复了由于错误表明选择器无效、已使用或不可见，导致无法在更新的VEC中选择[!UICONTROL 已单击元素]设置的问题。 (TGT-52467)
* 修复了在更新的VEC中添加[!UICONTROL 推荐选件]框导致显示重复（幽灵）框的问题。 在体验A和B之间切换时反复添加了更多虚框。 （TGT-52505 和 TGT-52519）
* 修复了在更新的[!DNL Target] UI中，通过[!UICONTROL 选件]菜单对HTML选件所做的更改未反映在相关活动中，反之亦然。 此行为现在与旧版UI匹配，在该旧版UI中，[!UICONTROL 选件]菜单与活动之间正确同步了更新。 （TGT-52540 和 TGT-52541）
* 修复了在活动中尝试使用[!UICONTROL 选件库]中的[!UICONTROL 体验片段]最近更新时，未反映它们的问题。 (TGT-52659)
...
* 修复了[!UICONTROL 活动]列表中的按名称搜索功能在多字查询中无法正常工作的问题。 (TGT-52529)
* 修复了阻止从[!UICONTROL Automated Personalization] (AP)活动中排除体验的问题。 (TGT-52383)
* 修复了在管理AP活动中的内容时，[!UICONTROL 筛选规则]中缺少“[!UICONTROL 包含]”选项的问题。 (TGT-52384)
* 修复了[!UICONTROL Automated Personalization] (AP)活动中报表不一致的问题，特别是与如何使用[!DNL Target]内部系统的`optionLocalId`值跟踪和报告默认选件有关。
* 修复了QA链接无法提供预期活动体验的问题。 (TGT-52163)
* 修复了具有[!UICONTROL 审批者]权限的用户被错误阻止编辑实时活动的问题，该问题会收到“访问被拒绝”错误消息。 (TGT-52416)
...
* 修复了序列条件无法在[!UICONTROL 可视化体验编辑器] (VEC)中正确显示[!DNL Recommendations]活动的问题。 (TGT-52435)
...
* 具有[!UICONTROL 产品管理员]和[!UICONTROL 系统管理员]权限的[!DNL Target]用户现在可以编辑[!UICONTROL 管理]页面上的所有设置，无论他们在[!DNL Target]中的角色如何。 没有这些权限的用户对这些设置具有只读访问权限。 此更新确保对[管理设置](/help/main/administrating-target/administrating-target.md)进行更严格的访问控制。 (TGT-48179)
* 修复了阻止保存活动[网站首选项](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)的缓存问题。 (TGT-52213)
* 修复了以下问题：在VEC中加载网站后，客户无法在[!UICONTROL 网站首选项]部分中按ID和类启用选择。 [!UICONTROL 网站首选项]设置自动还原为已禁用，即使它在启用之后也是如此。 (TGT-52207)
* 修复了当[页面交付](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)URL以正斜杠(/)结尾时，[!UICONTROL 可视化体验编辑器] (VEC)无法显示正确页面的问题。 (TGT-52237)
...
* 修复了阻止[!DNL Customer Journey Analytics]在活动[!UICONTROL 概述]页面上正确显示的问题。 (TGT-52359)
* 修复了阻止报表组在[!UICONTROL Automated Personalization] (AP)活动中持久存在的问题。 (TGT-52368)
* 修复了阻止保存包含Offer Decisioning的活动的问题。 (TGT-52390)
* 修复了以下问题：选择了默认选件，但在[!UICONTROL Automated Personalization] (AP)和[!UICONTROL 多变量测试] (MVT)活动中显示了其他选件内容。 (TGT-52372)
* 修复了GET权限逻辑，以检查完全组织访问权限与特定组织+用户访问权限之间的OR。 (TGT-52374)
* 修复了为[!UICONTROL 托管内容]和[!UICONTROL 报告受众]选择受众后未显示受众名称的问题，即使启用了[!UICONTROL 仅显示选定项]也是如此。 (TGT-52393)
...
* 修复了导致[!UICONTROL 活动]设置页面和[!UICONTROL 报表]概述页面之间的受众列表不一致的问题。 (TGT-52203)
...
* 修复了导致活动[!UICONTROL 概述]页面上的位置不正确的问题。 (TGT-52182)
...
* 修复了在添加其他页面然后导航回[!UICONTROL 体验]选项卡后导致修改消失的问题。 (TGT-52264)
* 修复了阻止客户更改[!UICONTROL 体验定位] (XT)活动中的受众的问题。 (TGT-52191)
* 修复了由于不支持的UI规则而阻止编辑XT活动的错误。 (TGT-52273)
* 修复了更新的[!UICONTROL 可视化体验编辑器] (VEC)中的问题，该问题导致痕迹导航不始终显示在编辑器底部，从而难以准确选择元素。 (TGT-52169)
* 修复了[!UICONTROL 受众]下拉列表因分页而无法显示所有受众的问题。 (TGT-52204)
* 修复了在[!UICONTROL Automated Personalization] (AP)活动中添加新选件时导致用户输入消息无效的问题。 (TGT-52210)
* 修复了以下问题：即使客户无权访问A4T，[!UICONTROL Analytics for Target] (A4T)仍错误地选作报表源。 (TGT-52226)
* 修复了阻止使用[!UICONTROL 查看页面] URL量度保存活动的问题。 (TGT-52260)
...
自2025年4月22日起，只有[!UICONTROL 产品]和[!UICONTROL 解决方案]管理员能够更新[!UICONTROL 管理]部分中的设置，无论他们在[!DNL Target]工作区中担任什么角色。 没有此权限的用户将对[!UICONTROL 管理]部分具有只读访问权限。
...
* 修复了阻止具有[!UICONTROL 编辑者]权限的客户保存活动的问题。 (TGT-52227)
...
* 修复了阻止客户打开某些[!UICONTROL 体验定位] (XT)活动的受众信息弹出窗口的错误。 (TGT-52049)
* 修复了在[!UICONTROL 可视化体验编辑器] (VEC)中进行更改后，受众设置恢复为“[!UICONTROL 所有访客]”的问题。 (TGT-52132)
* 修复了未针对特定活动显示受众细化的问题(TGT-52057)
* 修复了阻止客户在默认工作区中插入[!UICONTROL 体验片段]的问题。 (TGT-52073)
* 修复了选件显示为“找不到内容”并且未在[!UICONTROL Automated Personalization] (AP)活动的[!UICONTROL 选件]页面上显示的问题。 (TGT-52150)
* 添加了在活动中允许重复受众的功能。 (TGT-51200)
* 修复了在编辑后XT活动的[!UICONTROL 目标和设置]页面上显示错误的mbox名称的问题。 (TGT-52026)
* 修复了`defaultContent`尽管不在`experiences/optionLocations`中，却显示在选项中的问题。 (TGT-52036)
* 修复了一个问题，以确保未将空字符串转换为null值。 (TGT-52037)
* 修复了一个问题，该问题要求客户在编辑后在[!UICONTROL 目标和设置]页面上的[!UICONTROL 报告设置]中重新配置[!UICONTROL 优化目标]。 (TGT-52071)
* 修复了无页面交付规则的活动在[!UICONTROL 概述]页面上显示多个规则的问题。 (TGT-52084)
* 为尝试保存包含基本多语言平面以外的字符（如表情符号）的选件的用户添加了错误消息。 (TGT-52105)
* 修复了打开活动时触发错误消息“此活动使用一个或多个在源中删除的受众”的问题。 (TGT-52120)
* 修复了在编辑期间更新的[!UICONTROL 可视化体验编辑器] (VEC)中未显示ClickTrack量度的问题。 (TGT-52152)
* 修复了将查询参数作为活动位置的URL未在活动的[!UICONTROL 概述]页面上显示查询参数的问题。 (TGT-51635)
* 修复了导致整个体验URL无法在[!UICONTROL 可视化体验编辑器] (VEC)的[!UICONTROL 浏览模式]中显示的问题。 (TGT-52101)
* 修复了编辑活动导致页面交付在URL末尾添加“/”使其无效的问题。 (TGT-52114)
* 修复了[!UICONTROL 基于表单的体验编辑器]中的[!UICONTROL 活动QA]链接错误地重定向到[!DNL Adobe Experience Cloud]主页的问题。 (TGT-52055)
* 修复了在保存并重新打开后未保留添加到[!UICONTROL A/B测试]活动中的其他页面的问题。 (TGT-51994)
* 修复了阻止客户删除内联样式部分中的样式的问题。 (TGT-52070)
* 已恢复[!UICONTROL 活动QA]对话框中[受众定义信息卡](/help/main/c-target/c-audiences/audiences.md#section_11B9C4A777E14D36BA1E925021945780)的访问权限，类似于旧版UI。 (TGT-52056)
...
* 修复了一个问题，该问题阻止客户在Ove[!UICONTROL r]视图页面上的定位图形中以及活动编辑期间查看受众。 (TGT-52050)
* 修复了一个问题，该问题阻止客户在[!UICONTROL 体验定位] (XT)活动中按优先级重新排序体验。 (TGT-52054)
...
* 修复了在未进行任何更改的情况下打开和关闭[!UICONTROL 目标和设置]页面上的mbox选取器后触发无效名称错误的问题。 (TGT-51983)
...
* 修复了[!UICONTROL 此项前插入]选项无法用于导航栏前插入内容的问题。 (TGT-52031)
...
* 解决了导致[!UICONTROL 活动]页面加载缓慢的问题。 (TGT-51151)
...
* 解决了在活动创建或编辑期间将受众替换为“[!UICONTROL 所有访客]”而导致“不允许存在重复的受众”错误的问题。 (TGT-51978)
* 解决了在保存[!UICONTROL A/B测试]活动时导致“用户输入无效”错误的问题。 (TGT-51976)
* 解决了阻止计算量度在[!UICONTROL 目标和设置]页面上正确显示的问题。 (TGT-51975)
* 解决了阻止在`pageviews`量度的[!DNL Analytics]配置中匹配`companyName`和`reportSuite`的问题。 (TGT-51965)
* 解决了在活动中切换体验时删除修改的问题。 (TGT-51945)
* 解决了删除页面受众时也删除了[!UICONTROL ClickTrack]选择器的问题。 (TGT-51935)
* 解决了在打开活动的[!UICONTROL 概述]页面后使其不可编辑的问题。 (TGT-51931)
* 解决了在活动创建期间导致出现`[Unused optionLocalIds: 0]]`错误的问题。 (TGT-51920)
* 解决了删除文本样式更改后某些更改无法正确翻译的问题。 (TGT-51876)
* 解决了导致目标受众无法在[!UICONTROL 基于表单的体验编辑器]中正确更新的问题。 (TGT-51845)
* 解决了[!UICONTROL 可视化体验编辑器]中的URL在活动导航期间未正确更新的问题。 (TGT-51832)
* 解决了一个问题，该问题使得即使创建活动和添加优惠时显示得正确，仍无法在[!UICONTROL 优惠] UI中显示优惠。 (TGT-51805)
* 解决了在无法交付个性化或目标内容时，某些活动缺少用于显示默认内容的回退屏幕的问题。 (TGT-51638)
* 解决了导致实时优惠和某些文件夹无法在[!UICONTROL 优惠]用户界面中正确显示的问题。 (TGT-51628)
* 解决了导致某些URL字符串和goURL无法正确本地化的问题。 (TGT-35741)
* 修复了导致无法在[!DNL Target] UI中正确本地化角色（[!UICONTROL 审批者]、[!UICONTROL 编辑者]和[!UICONTROL 观察者]）的问题。 (TGT-29925)

### [!DNL Target Standard/Premium] 25.3.6（2025年3月14日）

此版本包含以下修复和更新：

* 解决了当多次使用同一[!UICONTROL ClickTrack]选择器时，[!UICONTROL 可视化体验编辑器] (VEC)活动中启用了[!UICONTROL 点击跟踪]的“用户输入无效”错误。 (TGT-51921)
...
* 解决了阻止用户更改[!UICONTROL 修改]面板中选件的问题。 (TGT-51800)
* 解决了操作和受众在左侧面板中错误显示的问题，包括在[!UICONTROL ClickTrack]模式下。 (TGT-51895)
...
* 解决了仅用于活动的受众在[!UICONTROL 受众]面板中不可见，导致无法编辑或重用的问题。 (TGT-51860)
...
* 修复了阻止编辑具有仅限该活动的受众的[!UICONTROL 体验定位] (XT)活动的问题。 (TGT-51846)
* 修复了[!UICONTROL 可视化体验编辑器] (VEC)在首次编辑时无法正确将修改应用于体验的问题。 (TGT-51843)
* 修复了单击VEC中的某些元素时触发“ID”错误的问题。 (TGT-51814)
* 在活动创建期间更新了VEC中的错误处理。 (TGT-51759)
* 修复了在保存活动时，[!UICONTROL 修改]面板中缺少视图导致“用户输入无效”错误的问题。 (TGT-51827)
...
* 组合受众可以包括子组，每个子组包含多个受众。 此版本修复了导致子组受众无法在[!UICONTROL 规则]对话框中显示的问题。 (TGT-51813)
* 解决了在打开旧版活动时，某些体验受众被替换为[!UICONTROL 所有访客]的问题。 (TGT-51812)
* 解决了阻止编辑仅具有活动受众的活动的问题。 (TGT-51807)
* 解决了阻止在更新的[!DNL Target] UI中编辑页面标题修改的问题。 (TGT-51797)
* 解决了在复制体验、删除其他体验然后尝试保存活动时发生的空错误。 (TGT-51796)
* 解决了在创建活动的[!UICONTROL 定位]步骤期间，受众信息面板中无法显示受众排除规则的问题。 (TGT-51579)
...
* 解决了在取消活动创建流程（选择[!UICONTROL 取消]而不是[!UICONTROL 添加受众]）时删除所有状态中受众更改的问题。 （TGT-51769 和 TGT-51770）
* 解决了无法为某些活动（特别是使用自定义代码时）加载[!UICONTROL 可视化体验编辑器] (VEC)的问题。  问题导致VEC显示空白屏幕或[!DNL Target] UI恢复到其旧版本。 (TGT-51758)
* 解决了在编辑受众的页面交付后丢弃修改的问题。 (TGT-51756)
* 解决了在[!UICONTROL 目标和设置]页面上更改量度类型时，从活动中删除所有非量度受众（页面和体验受众）的问题。 (TGT-51753)
* 解决了在编辑活动时单击[!UICONTROL 取消]将Target UI导航到[!UICONTROL 活动列表]而不是[!UICONTROL 活动详细信息]页面的问题。 (TGT-51731)
* 解决了阻止客户通过[!UICONTROL 将报表导出到CSV]选项下载报表的问题。 (TGT-51708)
* 解决了在基于表单的体验编辑器中将[!DNL Target Standard]客户错误地显示为使用[!DNL Target Premium]功能[!UICONTROL 属性]的问题。 (TGT-51678)
* 修复了在创建新优惠时阻止显示[!DNL Adobe Experience Platform]属性的问题。 (TGT-51665)
* 已将[!DNL Recommendations]清单的所有活动筛选器移动到快速搜索，将UI与[!UICONTROL 目录搜索]对齐，而不是[!UICONTROL 筛选器]边栏。 (TGT-50723)
...
随着[!DNL Adobe Target] UI现代化工作的继续，我们很高兴地宣布已正式提供更新的[!UICONTROL 活动]用户界面。

>[!NOTE]
>
>从2月17日开始，客户将逐步拥有新[!UICONTROL 活动] UI的访问权限。 为确保为所有客户无缝部署，此版本将分阶段部署。 第一阶段会将初始的[!DNL Target]客户组升级到新的[!UICONTROL 活动] UI。 后续阶段将升级剩余客户。
...
* [[!UICONTROL 已更新更改日志]](/help/main/c-activities/change-log.md)页面，现在从[[!DNL Audit Query API]](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/landing/governance-privacy-security/audit-logs/audit-api/overview){target=_blank}获取信息以进行实时分析。
...
* 完全[重新生成了[!UICONTROL 可视化编辑编辑器]](/help/main/c-experiences/c-visual-experience-composer/visual-experience-composer.md)，支持浏览器提供商的最新安全更新以及新式用户界面。
...
>[!NOTE]
>
>从2月17日开始，客户将逐步拥有新[!UICONTROL 推荐] UI的访问权限。 为确保为所有客户无缝部署，此版本将分阶段部署。 第一阶段会将初始的[!DNL Target]客户组升级到新的[!UICONTROL 活动] UI。 后续阶段将升级剩余客户。

此更新基于最新的[!DNL Adobe Spectrum]设计系统，标准化了以前不一致的设计模式，同时添加了新的增强功能，例如：

* [产品目录搜索](/help/main/c-recommendations/c-products/catalog-search.md)现在具有更新的数据库，允许实时同步产品。
* 通过API创建的[!UICONTROL Recommendations]对象（[!UICONTROL 标准]、[!UICONTROL 设计]、[!UICONTROL 收藏集]和[!UICONTROL 排除项]）[现在可在UI](/help/main/c-recommendations/c-recommendations-faq/recommendations-faq.md)中使用。
* [推荐设置](/help/main/administrating-target/recommendations-settings.md)已合并到[!UICONTROL 管理]部分下。
...
#### [!UICONTROL 选件库]用户界面更新

为了增强[!DNL Adobe Target]用户的用户体验，此版本更新了[!UICONTROL 选件库]用户界面。
...
* **[!UICONTROL 代码编辑器]升级**：使用语法高亮显示和行编号刷新了HTML和JSON编辑器。
...
* 使用JSON选件
* 在[[!UICONTROL 基于表单的体验编辑器]](/help/main/c-experiences/form-experience-composer.md)中创建的活动中使用自定义决策范围
* 在获取使用全局`__view__`作用域、基于[!UICONTROL 表单的体验编辑器]创建的活动时不使用`renderDecisions: true`
...
* **单个个性化和测量调用。**
  ...
  * 跳出率下降。
  * A4T和[!UICONTROL 页面视图]点击未拼合在一起，因此使用[!DNL Analytics]个eVar和事件在A4T报表中执行某些划分和关联比较困难。
...
### [!UICONTROL 可视化体验编辑器]助手扩展（2024年4月23日）

旧版[!DNL Target]可视化体验编辑器助手扩展是使用清单V2创建的。 [!DNL Google]宣布从2024年6月起，将不再允许使用清单V2创建的扩展。 有关详细信息，请参阅[[!UICONTROL 可视化体验编辑器]帮助程序扩展](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/vec-helper-browser-extension.md)。
...
### [!UICONTROL 浏览器]受众属性中`Browser:iPad`和`Browser:iPhone`的更新（2024年4月30日）
...
* 修复了在[!UICONTROL 可视化体验编辑器] (VEC)中执行创作时导致扩展重复Cookie的问题。
...
* 修复了导致使用[!UICONTROL Analytics for Target] (A4T)报表配置的[!UICONTROL 多变量] (MVT)活动无法正确显示的问题。 (TGT-47490)
...
* 具有收入目标指标的[!UICONTROL Analytics for Target] (A4T)活动未显示“收入”作为列名称，并且收入指标未在报表中以($)格式显示。 这是一个已经解决的外观问题。 (TGT-46995)
* 修复了一个问题，该问题导致报告日期间隔不起作用。 (TGT-47396)
* 修复了在客户使用[!UICONTROL 更多操作]图标激活或停用活动后，导致[!UICONTROL 所有活动]页面上显示错误状态的问题。 (TGT-47367)
* 修复了导致[!UICONTROL 重要属性]报表不针对单个客户显示的问题。 (TGT-47272)
...
* 修复了以下问题，这些问题会影响使用[[!UICONTROL Analytics for Target]](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T)作为报表源的活动：
  ...
  * 修复了有时导致活动报告页面上的[!UICONTROL 在Analytics中查看]链接无法正常工作的问题。 (TGT-46731)
  * 修复了导致[!UICONTROL 提升]和[!UICONTROL 置信度]的数据无法在[!DNL Target] UI中正确显示的问题。 （TGT-46592、TGT-46554 和 TGT-46586）

### [!UICONTROL 活动]页面用户界面刷新（2023年10月25日）

作为[!DNL Adobe Target]团队持续努力改进[!DNL Target]用户体验的一部分，此版本刷新了[!DNL Target] UI中的[!UICONTROL 活动]页面。 此更新统一了以前不一致的设计模式并使其标准化，同时添加了新的增强。
...
* 增强了新的[!UICONTROL 活动] UI，以便在创建新活动时打开[!UICONTROL 可视化体验编辑器] (VEC)，其中包含`selectorCriteria`的默认设置。 (TGT-46586)
* 修复了一个问题，在使用VEC时，该问题阻止某些客户在[!UICONTROL 编辑器]模式下编辑元素。 (TGT-46470)
* 添加了在使用自定义属性时指定通用首选选择器的功能。 (TGT-46545)
* 修复了有时会阻止使用[!UICONTROL Analytics for Target] (A4T)的[!UICONTROL 自动定位]报告在[!DNL Target] UI中显示的问题，即使该报告在[!DNL Adobe Analysis Workspace]中正确显示也是如此。 (TGT-46494)
...
* 为动态框架添加了[!UICONTROL 可视化体验编辑器] (VEC)增强功能。 (TGT-44064)
...
* 增强了[!UICONTROL 可视化体验编辑器] (VEC)以支持闪电Web组件（轻量级DOM）。 (TGT-45422)
...
* 修复了VEC [!UICONTROL 叠加]功能的问题，该功能现在支持影子DOM中的元素。 （TGT-45202 和 TGT-45262）
* 修复了在VEC中打开单页应用程序(SPA)页面，然后转到[!UICONTROL 浏览]模式时导致“返回”和“前进”箭头无法正确工作的问题。 (TGT-45956)
...
* 修复了对于使用[!UICONTROL Analytics for Target] (A4T)作为报表源的[!UICONTROL 自动分配]活动，导致[!DNL Target] UI和[!DNL Adobe Analytics] UI中的报表数据不一致的问题。 (TGT-46112)
...
* 修复了有时会阻止活动正确同步的问题，如[!UICONTROL Activity]列表页面上的“[!UICONTROL Status]”列所示。 （TGT-46010 和 TGT-44831）
* 修复了一个问题，该问题有时阻止将[!UICONTROL Analytics for Target] (A4T)用作报表源的活动的[!UICONTROL 报表]页面上显示“[!UICONTROL 在Analytics中查看]”链接。 (TGT-45808)
* 调整了表格中值的显示方式，改为显示百分比，而非带小数的数字。 例如，8% 而非 .08。 (TGT-45548)
* 修复了一个问题，该问题阻止客户使用键盘焦点移动到[!UICONTROL 体验定位] (XT)活动的[!UICONTROL 目标和设置]页面中的下一个元素。 (TGT-44526)
* 修复了在创建活动时打开“[!UICONTROL 添加受众]”对话框后导致键盘失去焦点的问题。 (TGT-44525)

### [!DNL Target] Standard/Premium 23.7.1（7 月 24-26 日）

此版本包含以下增强和修复：

* 改进了[在[!UICONTROL 可视化体验编辑器] (VEC)中使用DOM路径](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)导航元素以包含影子DOM元素时的搜索。 (TGT-45262)
...
| 功能 | 详细信息 |
|--- |--- |
| [!UICONTROL Automated Personalization]活动的[!UICONTROL QA模式] | [!DNL Adobe Target] [!UICONTROL QA模式]现在可用于[!UICONTROL Automated Personalization]活动，取代了[!UICONTROL 预览链接]功能。<P>有关更多信息，请参阅[活动 QA](/help/main/c-activities/c-activity-qa/activity-qa.md)。 |
...
* 修复了当图像名称包含GB 18030字符时，导致无法在[!UICONTROL 创建设计]面板中显示图像的问题。 (TGT-44614)
* 修复了一个问题，在该问题中，体验中的文本/HTML 内有一些 GB 18030 符号字符转义不当。 (TGT-44600)
* 修复了在分析期间导致[!UICONTROL 自动Personalization]活动报表冻结的问题。 (TGT-44820)
* 修复了当活动名称包含方括号（ [或]）时，导致无法在[!UICONTROL 活动]页面上搜索活动的问题。 (TGT-44777)
...
* 修复了向URL添加查询参数导致页面无法在[!UICONTROL 可视化体验编辑器] (VEC)中加载的问题。 (TGT-44873)
* 纠正了整个 [!DNL Target] UI 中的多处本地化错误。

### 与[!DNL Target]共享的Real-Time CDP配置文件属性[!UICONTROL Real-Time CDP配置文件属性]（2023年6月13日）

此版本包含以下增强功能：

| 功能 | 详细信息 |
|--- |--- |
| 与 [!DNL Target] 共享的 Real-Time CDP 轮廓属性 | [!UICONTROL Real-Time CDP配置文件属性]可以与[!DNL Target]共享，以便在HTML和JSON选件中使用。<P>有关更多信息，请参阅 [与  [!DNL Target]](/help/main/c-integrating-target-with-mac/integrating-with-rtcdp.md#rtcdp-profile-attributes) 共享 Real-Time CDP 轮廓属性。 |
...
| 功能 | 详细信息 |
|--- |--- |
| 用于Headless个性化和试验的AEM [!UICONTROL 内容片段] | 在[!DNL Target]活动中使用[!DNL Adobe Experience Manager] (AEM) [!UICONTROL 内容片段]。 将简单易用的 AEM 与 [!DNL Target] 中强大的人工智能 (AI) 和机器学习 (ML) 功能结合使用，帮助进行 Headless 个性化和试验。<P>有关详细信息，请参阅[AEM [!UICONTROL 内容片段]](/help/main/c-integrating-target-with-mac/aem/content-fragments-aem.md)。 |
| [*Adobe Target 开发人员指南*](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank} | *Adobe Target开发人员指南*&#x200B;已重新定位到&#x200B;*[!UICONTROL Adobe Experience League]*。 移动到&#x200B;*[!UICONTROL Experience League]*&#x200B;有助于本地化其他语言的文本，统一了&#x200B;*Experience League*&#x200B;中的搜索范围，并提供了来自&#x200B;*[!UICONTROL Adobe Target商业从业者指南]*&#x200B;和&#x200B;*[!UICONTROL Adobe Target开发人员指南]*&#x200B;的搜索结果，还提供了更多好处。<P>系统会自动将您从上一个位置重定向到&#x200B;*[!UICONTROL Experience League]*。 请根据需要更新您的书签。 |
...
* 在创建[组合受众](/help/main/c-target/combining-multiple-audiences.md)时，加载旋转器时禁用了“[!UICONTROL Done]”选项，因为该选项在[!DNL Target] UI中可见。 (TGT-44079)
* 修复了[!UICONTROL 受众]页面底部的[!UICONTROL 语言]链接，以便该链接正确链接到“[!UICONTROL 帐户通信首选项]”页面。 (TGT-43562)
* 解决了在[!UICONTROL 管理] > [!UICONTROL 报表] > [!UICONTROL 报表Experience Cloud解决方案]下选择[!UICONTROL Adobe Analytics]选项后，有时阻止客户创建[!UICONTROL A/B测试]活动的问题。 (TGT-44844)
* 修复了一个问题，该问题阻止客户从[!UICONTROL 可视化体验编辑器] (VEC)中查看具有许多体验的[!UICONTROL 多变量测试]活动中的最后一个体验。 VEC 底部的 [DOM 路径](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)有时会阻止客户看到上次体验。 (TGT-44578)
* 修复了导致 VEC 中的浏览 URL 无法反映在正常浏览器会话中可见的当前页面（如果页面需要授权或调用重定向）的问题。 (TGT-44350)
* 修复了一个问题，该问题阻止客户更改[!UICONTROL 推荐] > [!UICONTROL 设置]中的[!UICONTROL 筛选不兼容的标准]设置。 (TGT-44398)
* 修复了在将[!UICONTROL Analytics分类]与名称中包含点的报表包结合使用时导致POST请求创建[!DNL Recommendations]馈送失败的问题。 (TGT-44598)
...
* 使用[!UICONTROL 可视化体验编辑器] (VEC)创作自定义Web组件的更新：
...
* 已从[!UICONTROL 选件级别]报告中为[!UICONTROL Automated Personalization]活动显示的选件名称中删除位置。 此更改使报告更具可读性。 (TGT-44294)
* 已在[!DNL Target] UI中从AP和[!UICONTROL 自动定位] [!UICONTROL Personalization Insights]和[!UICONTROL 重要属性]报告中删除45天和90天日历选项。 由于使用模式和为了提高性能，已弃用这些日期范围。 UI 已更新，以反映当前允许的范围：15 天、30 天和 60 天。 (TGT-39357)
...
新的[!DNL Adobe Target]模型API（也称为API）可让用户查看和管理机器学习模型中用于[!UICONTROL Automated Personalization] (AP)和[!UICONTROL 自动定位] (AT)活动的功能的列表。
...
* 在[!DNL Target] UI中添加了工具提示，以帮助客户在创建或编辑使用A4T的[!UICONTROL 自动定位]活动时选择优化标准。
...
| 功能 | 详细信息 |
| --- | --- |
| [!DNL Adobe Experience Manager] (AEM) 体验片段 | AEM 体验片段功能更新包括以下内容：<ul><li>添加了在[!UICONTROL 选件]列表中按类型（HTML或JSON）筛选AEM体验片段的功能。 (TGT-43121)</li><li>修复了允许客户在使用VEC时插入不受支持的JSON [!UICONTROL 体验片段]优惠的问题。 只有在使用[!UICONTROL 基于表单的体验]编辑器时，才能插入JSON选件。 (TGT-43846)</li></ul>有关详细信息，请参阅 AEM [体验片段](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)。 |
| 适用于Google Chrome的新[!UICONTROL 可视化体验编辑器]扩展 | Chrome Web Store中提供了适用于Chrome的新[!DNL Adobe Target] [!UICONTROL 可视化体验编辑器] (VEC)扩展。<br>从2023年1月开始，当前的[!DNL Target] VEC Helper扩展将停止在Google Chrome中运行，因为Google不允许使用清单V2的扩展。 下载新的扩展程序，从新的一年开始，继续在 [!DNL Target] 中以视觉方式创作您的网站。<br>以下链接显示了 Chrome 网络商店中的两个扩展：<ul><li>[新扩展](https://chrome.google.com/webstore/detail/adobe-experience-cloud-vi/kgmjjkfjacffaebgpkpcllakjifppnca){target=_blank}</li><li>[旧的扩展](https://chrome.google.com/webstore/detail/adobe-target-vec-helper/ggjpideecfnbipkacplkhhaflkdjagak){target=_blank}</li></ul>有关更多信息，请参阅[可视化编辑帮助程序扩展](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)。 |

* 修复了导致受众规则信息无法在[!UICONTROL 受众细化]信息窗口中正确显示的问题。 (TGT-43917)
...
* 修复了一个问题，在VEC中从[!UICONTROL 撰写]切换到[!UICONTROL 浏览]模式后创建或编辑活动时，该问题导致无法在[!UICONTROL 体验]页面上的[!UICONTROL 修改]面板中正确显示某些组件。 (TGT-43300)
* 修复了一个问题，该问题阻止某些客户存档使用[!UICONTROL 自动定位]的[!UICONTROL A/B测试]活动。 (TGT-40978)
...
* 修复了导致[!UICONTROL Decision Offers]在[!UICONTROL 可视化体验编辑器] (VEC)中无法正常工作的问题。 (TGT-43866)
* 修复了在创建[!UICONTROL Multivariate Testing] (MVT)活动时选择[!UICONTROL 单击了一个元素]转化目标时导致显示错误消息的问题。 (TGT-43842)
* 修复了导致[!UICONTROL 展示次数]列无法显示在[!UICONTROL Automated Personalization] (AP)活动的已下载CSV报告文件中的问题。 (TGT-43780)
* 修复了在使用[!UICONTROL 基于表单的体验编辑器]时客户在复制体验后无法编辑HTML/JSON优惠的问题。 (TGT-43633)
* 修复了阻止客户将[!UICONTROL A/B测试]活动从非默认工作区复制到另一个非默认工作区的问题。 (TGT-41910)
* 修复了一个问题，以确保客户可以正确地显示包含推荐的[!UICONTROL A/B测试]和[!UICONTROL 体验定位] (XT)活动中[!DNL Recommendations]对象（设计、条件、集合等）的使用情况，还可以从[!DNL Target] UI和[!DNL Recommendations]后端删除不再使用的条件对象。 (TGT-42331)
...
* 具有[!UICONTROL 编辑者]角色的用户无法再编辑实时活动中的受众。 (TGT-43582)
...
* 修复了在[!UICONTROL 体验定位] (XT)活动中呈现[!UICONTROL 优惠决策]时间歇性地导致超时的问题。 (TNT-44611)
...
* 修复了一个问题，如果在部署新的[!UICONTROL 受众] UI之前创建活动的体验级细化和受众，该问题阻止单个客户编辑这些体验级细化和受众。 (TGT-43433)
* 修复了一个问题，该问题允许客户在编辑活动的报告受众时选择重复的 [!DNL Adobe Audience Manager] (AAM) 受众。 (TGT-43430)
* 修复了一个阻止客户在不同工作区创建重复受众的问题。 (TGT-43423)
* 修复了一个问题，该问题阻止客户删除在[!UICONTROL 基于表单的体验编辑器]中创建的活动中具有临时优惠的位置。 (TGT-43315)
...
* 通过此版本，您可以预取[!UICONTROL 自动Personalization] (AP)和[!UICONTROL 自动定位] (AT)活动（以前不由[!DNL Target]返回）的内容。 如果AP/AT活动在交付路径上，并且其优先级高于使用相同位置进行内容交付的其他AB/XT活动，则这可能会更改最终用户在预取调用时看到的体验（不会更改“执行”流程）。
...
* 将为所有[!DNL Target Standard]客户启用新的[!UICONTROL 受众] UI。

### Target Standard/Premium客户工程修复（2022年3月22日）

此维护版本包含以下增强功能：

* 添加了以下功能：在将[!UICONTROL 投放API]与使用[!UICONTROL Analytics作为报表源] (A4T)的活动结合使用时，返回`prefetch`视图和`pageLoad`点击量度的[!DNL Analytics]有效负载数据。 (TNT-43198)
...
* 在从[!DNL Adobe Experience Platform]、[!DNL Adobe Experience Cloud]和[!DNL Adobe Target Classic]导入的受众的[!UICONTROL 受众]列表中添加了[!UICONTROL 删除]选项。 还添加了批量删除功能。 (TGT-42914)

### at.js 版本 2.8.1（2022 年 1 月 28 日）

* 修复了`pageLoad`在[!UICONTROL 设备上决策] (ODD)混合执行模式中未映射到target-global-mbox的问题。
...
| [!UICONTROL 受众] UI刷新 | 作为[!DNL Adobe Target]团队持续努力改进[!DNL Target]用户用户体验的一部分，此版本刷新了[!DNL Target] UI中的[!UICONTROL 受众]和[!UICONTROL 个人资料脚本]页面。 此更新统一并标准化了以前不一致的设计模式，并添加了新的增强功能，例如：<ul><li>同时选择和删除多个受众的功能</li><li>刷新的[受众生成器设计](/help/main/c-target/c-audiences/create-audience.md)</li><li>[!UICONTROL 受众]库规则生成器中的排除规则支持</li><li>新的“受众Source”筛选器，可用于更快地发现受众</li><li>会话持久搜索和筛选选项</li><li>为 [!DNL Target Premium] 客户在工作区之间移动受众的功能。</li></ul>有关详细信息，请参阅[受众](/help/main/c-target/target.md).<br>**注意**：此功能将在八周后向不同地区的客户推出。 |
| [!UICONTROL 配置文件脚本]用户界面刷新 | [!UICONTROL 配置文件脚本]库也已更新，并且包含一个更新后的界面和几项生产力更新：<ul><li>同时选择和删除多个轮廓脚本的功能</li><li>轮廓脚本的新代码编辑器</li><li>代码编辑器中的语法突出显示和错误检查</li><li>通过键盘快捷键自动完成令牌（mbox 或 profile）参数</li></ul>有关详细信息，请参阅[访客配置文件](/help/main/c-target/c-visitor-profile/visitor-profile.md).<br>**注意**：此功能将在八周后向不同地区的客户推出。 |
...
| 功能 | 详细信息 |
| --- | --- |
| [!UICONTROL 可视化体验编辑器] (VEC) | 添加了对 [Web 组件](https://developer.mozilla.org/en-US/docs/Web/Web_Components)的支持。 可以在自定义元素以及自定义元素内部的元素上创建和测试个性化的体验和方案。<br>有关更多信息，请参阅[可视化体验编辑器选项](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#custom)。 |
...
* 修复了阻止客户在[!DNL Analysis Workspace]中通过单击[!DNL Target]活动报告中的[!UICONTROL 在Analytics中查看]按钮来打开[!UICONTROL A4T]面板的问题。 （TGT-42099、TGT-42100）
* 修复了在使用[!UICONTROL 基于表单的体验编辑器]编辑[!UICONTROL A/B测试]和[!UICONTROL 体验定位] (XT)活动时导致[!UICONTROL 编辑设计]按钮不显示的问题。 (TGT-41980)
...
在将[!DNL Target] [!UICONTROL 受众]与[!DNL Adobe Experience Platform Web SDK]一起使用时，添加了以下增强功能：
...
* 在 [!DNL Target] UI 中的各个位置添加了警告图标、弹出框和消息，以指示已从来源中删除受众，并且受众不再可用于 [!DNL Target] 活动中。
  ...
  * 活动创建工作流的[!UICONTROL 体验]步骤：
    ...
    ![在[!UICONTROL 体验]页面](assets/deleted-at-source-experiences.png)上的源消息中删除了受众

  * 活动创建工作流的[!UICONTROL 定位]步骤：

    ![在[!UICONTROL 定位]页面](assets/deleted-at-source-targeting.png)上的源消息中删除了受众

  * 活动创建工作流的[!UICONTROL 目标和设置]步骤：

    ![在[!UICONTROL 目标和设置]页面](assets/deleted-at-source-goals-settings.png)上的源消息中删除了受众

  * 受众细化（在活动创建工作流的[!UICONTROL 定位]步骤中，[!UICONTROL 替换受众]）：

* 如果您尝试使用“合并受众”功能，而其中某个受众已从来源中删除，则[!UICONTROL 保存]将被禁用。
...
| 功能 | 详细信息 |
| --- | --- |
| [!UICONTROL 受众] UI刷新 | 作为[!DNL Adobe Target]团队持续努力改进[!DNL Target]用户用户体验的一部分，此版本刷新了[!DNL Target] UI中的[!UICONTROL 受众]和[!UICONTROL 个人资料脚本]页面。 此更新统一并标准化了以前不一致的设计模式，并添加了新的增强功能，例如：<ul><li>同时选择和删除多个受众的功能</li><li>刷新的[受众生成器设计](/help/main/c-target/c-audiences/create-audience.md)</li><li>[!UICONTROL 受众]库规则生成器中的排除规则支持</li><li>新的“受众Source”筛选器，可用于更快地发现受众</li><li>会话持久搜索和筛选选项</li></ul>有关更多信息，请参阅[受众](/help/main/c-target/target.md)。 |
| [!UICONTROL 配置文件脚本]用户界面刷新 | [!UICONTROL 配置文件脚本]库也已更新，并且包含一个更新后的界面和几项生产力更新：<ul><li>同时选择和删除多个轮廓脚本的功能</li><li>轮廓脚本的新代码编辑器</li><li>代码编辑器中的语法突出显示和错误检查</li><li>通过键盘快捷键自动完成令牌（mbox 或 profile）参数</li></ul>有关更多信息，请参阅[访客轮廓](/help/main/c-target/c-visitor-profile/visitor-profile.md)。 |
| [!BADGE Premium]{type=Positive url="/help/main/c-intro/intro.md#premium newtab=true" tooltip="查看Target Premium中包含的内容。"}推荐标准创建和编辑 | [!UICONTROL 推荐标准]的创建和编辑工作流已经过简化，以简化选择正确的推荐算法和设置以实现目标的过程。<br>有关更多信息，请参阅[创建标准](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)。 |
...
* 修复了一个问题，该问题导致“编辑标准”按钮无法在VEC中[!DNL Recommendations]活动的[!UICONTROL 体验]页面上正常显示。 (TGT-39512)
* 修复了一个问题，该问题导致在复制到测试工作区时无法同步活动。 (TGT-40686)
* 修复了一个问题，该问题导致在VEC中使用“[!UICONTROL 在此之后插入]”时无法修改带[体验片段](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)的选择器。 (TGT-41802)
* 修复了一个问题，该问题导致无法将产品建议中的空 JSON 内容发送到后端。 [!DNL Target] 现在可发送 JSON 对象，即使该对象为空也是如此。 (TGT-41555)
* 修复了一个问题，该问题导致当客户在查看报告时单击“在Analytics中查看[!UICONTROL ”]时，打开旧[!DNL Analytics]报告而非[!DNL Analysis Workspace]。 (TGT-41867)
* 向客户尝试选择[!DNL Analytics]作为[!UICONTROL Automated Personalization]活动的报表源(A4T)时显示的UI消息中添加了附加说明。 该消息指出，“[!DNL Target]是[!UICONTROL Automated Personalization]活动唯一支持的源。” (TGT-41954)
* 向客户尝试使用“换行符”而不是逗号分隔主机时显示的错误消息中添加了附加说明。 (TGT-40671)
* 修复了一个问题，该问题导致某些活动的“[!UICONTROL 上次更新]”日期与西班牙客户和日本客户的英语UI中的不同（在查看西班牙语和日语版的UI时）。 (TGT-38980)
...
* 修复了一个问题，该问题导致在[!UICONTROL 基于表单的体验编辑器]中创建的[!UICONTROL 自动Personalization]活动报表引用报表中删除的选件。 此问题导致显示以下错误消息：“我们在检索此报表的数据时遇到问题。 如果问题仍然存在，请联系 Adobe 客户关怀部门。“ (TGT-41028)
...
| 功能 | 详细信息 |
| --- | --- |
| [!UICONTROL Analytics for Target] (A4T) | 在使用[!DNL Analytics]作为报表源(A4T)的活动的[!UICONTROL 报表]页面上单击“[!UICONTROL 在Analytics中查看]”链接时，[!DNL Analysis Workspace]现在会打开。 之前，该链接会打开 [!DNL Analytics] 报告。 (TGT-36959) |
...
* 修复了在刷新[!UICONTROL 受众]页面时导致默认工作区更改为其他工作区的问题。 (TGT-38871)
* 修复了[!UICONTROL 管理] > [!UICONTROL 实现]中有时会引发以下错误消息的问题：“您的全局mbox可能不同步。 请尝试重新保存它。”
...
* 修复了即使启用了[!UICONTROL 不允许重复项]选项，但在[!UICONTROL Automated Personalization]活动中的不同位置使用选件时仍阻止重复项的问题。 (TGT-39567)
* 修复了导致[!UICONTROL 管理] > [!UICONTROL Scene7配置]页面无法正确加载的问题。 (TGT-39918)
...
* 现在每天都生成[!UICONTROL Personalization Insights]报告，其中涉及[!UICONTROL 自动定位] (AT)和[!UICONTROL Automated Personalization] (AP)活动。 可选择提供过去15、30和60天[!UICONTROL 自动化区段]或[!UICONTROL 重要属性]的报告。 已删除 45 天和 90 天的选项以使其他回溯时段设置可每天运行。 (TGT-39472)
* 修复了客户在活动的[!UICONTROL 目标和设置]页面上单击[!UICONTROL 编辑依赖项]时导致不显示当前依赖项的问题。 (TGT-39340)
* 修复了在刷新工作区的[!UICONTROL 受众库]时发生的问题。 在刷新之前，显示当前所选工作区的受众。 在刷新之后，显示[!UICONTROL 默认Workspace]及其受众。 当前工作区及其受众在刷新之后继续存在。 (TGT-38871)
* 修复了在复制[!UICONTROL 推荐]活动并通过更改其标准顺序而稍后编辑原始活动时发生的问题。 原始活动中标准顺序的更改也被错误地应用于复制的活动。 (TGT-39155)
...
* 在[!UICONTROL 自动定位]活动中使用[!UICONTROL Analytics作为报表源] (A4T)时，在选择[!DNL Adobe Analytics]指标时添加了警告。 [!UICONTROL 自动定位]模型已优化为可与二进制（基于转化）量度一起使用。 选择连续指标（如收入）可能会产生次优结果，并且[!UICONTROL Personalization分析]报表可能不准确。 (TGT-38926)
* 在使用A4T的[!UICONTROL 自动定位]活动的[!UICONTROL 自动定位摘要]报表中添加了一个状态图标。 报表中每个体验旁边的绿色复选标记表示已为该体验生成个性化的机器学习模型。 时钟图标表示用于构建模型的流量不足。 (TGT-38925)
...
* 修复了一个问题，该问题阻止某些客户在[!UICONTROL 默认Workspace]中看到活动列表。 (TGT-38526)
...
* 修复了导致[!UICONTROL 平均提升置信区间]和[!UICONTROL 置信度]无法在[!UICONTROL 总计]行的[!DNL Auto-Target]报表中显示的问题。 但可正确地显示所有个别体验的测量结果。 (TGT-37301)
* 修复了一个问题，该问题在9月15日下午2:30 (PDT)至10月6日上午9:25 (PDT)影响[!DNL Adobe Target Premium]用户的[!UICONTROL 自动定位]报表。 在查看（使用“[!UICONTROL 查看了页面]”或“[!UICONTROL 单击了mbox]”选项配置的）受影响的转化量度的报表时，所报告的转化率有误。 目前没有已知的投放问题。
* 在[!UICONTROL 目录搜索]表中添加了一个可选的[!UICONTROL 上次更新时间]列，并添加了一个[!UICONTROL 上次更新时间]筛选器。 此增强可节省时间和精力，因为不必打开每个单独的项目以查看上次更新它的时间，并可按上次更新这些项目的日期进行筛选。
...
| 功能 | 详细信息 |
| --- | --- |
| ![Premium 徽章](/help/main/assets/premium.png) 自动定位活动支持 Analytics for Target (A4T) | [!UICONTROL 自动定位]活动现在支持[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>此集成允许您使用[!UICONTROL 自动定位]集成机器学习算法，根据每位访客的配置文件、行为和上下文选择最适合该访客的体验。<br>如果您已[实施A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以用于A/B测试和体验定位活动，则您已准备就绪！<br>有关详细信息，请参阅自动分配和自动定位活动支持[A4T](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
* 修复了一个问题，该问题有时在成功加载VEC之后阻止用户使用[!UICONTROL 可视化体验编辑器] (VEC)中的编辑功能。 (TGT-37162)
...
* 修复了一个问题，该问题在切换组织后有时阻止用户从[!UICONTROL 管理>实施]页面下载at.js。 (TGT-37668)
* 现在加载时禁用了 at.js 下载按钮，以防如果用户多次单击下载按钮，[!DNL Target] 发送多个请求。 (TGT-37633)
* 修复了[!UICONTROL 体验定位] (XT)活动中的一个问题，该问题导致体验长时间显示“正在获取结果”。 (TGT-37684)
...
* 修复了阻止某些用户显示[!UICONTROL 活动]列表的问题。 显示以下错误消息：“无法获取 URL 建议。” 在 Adobe 后端系统中对于在其名字中使用回车 (FirstName/r/n) 的用户发生该错误。 (TGT-37330)
* 修复了一个问题，如果（在[!UICONTROL Adobe Admin Console for Enterprise]中指定的）工作区名称包含撇号，则该问题阻止用户显示[!UICONTROL 活动]页面。 (TGT-37709)
* 修复了在选择优化和转化量度时[!UICONTROL 自动分配]活动中的一个问题：即使已指定报表包，仍有一条错误消息不正确地通知用户选择报表包。 (TGT-37689)
* 修复了一个问题，在导航到[!UICONTROL 定位]页面然后返回之后，该问题有时导致[!UICONTROL 目标和设置]页面上的量度为空。 (TGT-37691)
...
我们正在使用新技术栈逐渐改写整个 [!DNL Target] UI 以提高性能、减少发布新功能时所需的维护时间并改善整个产品的用户体验。 刷新的第一个部分是[!UICONTROL Setup]部分，它已被重命名为[!UICONTROL 管理]。

作为此刷新的一部分，您将能够使用[!UICONTROL 管理]部分中的页面轻松执行许多操作，例如：

* 从[!UICONTROL 实现]选项卡（**[!UICONTROL 管理]** > **[!UICONTROL 实现]**）下载最新的at.js文件。
* 自定义您的at.js设置，并可轻松地检查您的更改（**[!UICONTROL 管理]** > **[!UICONTROL 实施]**）。
* 修改增强的报表设置，如默认货币和时区、要从报表中排除的IP等。（**[!UICONTROL 管理]** > **[!UICONTROL 报表]**）
* 出于隐私原因，混淆访客IP地址（**[!UICONTROL 管理]** > **[!UICONTROL 实施]**）
* 在Adobe Admin Console中管理每个工作区及其角色之前，查看现有用户列表（**[!UICONTROL 管理]** > **[!UICONTROL 用户]**）。
...
* 修复了导致[!UICONTROL 此项后插入] > [!UICONTROL 图像]无法对可缩放矢量图形(SVG)图像正常工作的问题。 (TGT-37242)
* 修复了具有[!UICONTROL 发布者]角色的用户的一个问题，该问题阻止删除活动草案。 (TGT-37358)
* 修复了在选择[!UICONTROL 我的所有工作区]时阻止用户编辑活动的问题。 (TGT-37276)

### Target Standard/Premium 20.5.1（2020 年 6 月 17 日）

| 功能/增强 | 描述 |
| --- | --- |
| 对[!UICONTROL 自动分配]活动的Analytics for Target (A4T)支持 | [!UICONTROL 自动分配]活动现在支持[Analytics for Target](/help/main/c-integrating-target-with-mac/a4t/a4t.md)。<br>此集成允许您使用[!UICONTROL 自动分配]多臂赌博机功能将流量引向入选体验，同时使用[!UICONTROL Adobe Analytics]目标指标和/或[!UICONTROL Adobe Analytics]报表和分析功能。<br>如果您已[实施A4T](/help/main/c-integrating-target-with-mac/a4t/a4timplementation.md)以用于A/B测试和体验定位活动，则您已准备就绪！<br>有关详细信息，请参阅自动分配和自动定位活动支持[A4T](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md)。 |
...
* 当您登录到 [!DNL Adobe Experience Cloud] 时，您将被带入新的标题导航。 它看起来与以前的导航非常相似，只是顶部有一个黑色的栏，但它提供以下改进：
  ...
  * 改进了用户帮助：搜索结果包括 [!DNL Target] 产品文档以及社区论坛和更多视频内容所产生的结果，使您更轻松地访问更多内容以帮助充分利用 [!DNL Target]。 我们还直接在[!UICONTROL 帮助]菜单中添加了反馈机制，使报告问题或分享您的想法变得更轻松。

  * 改进了净推荐者分数(NPS)反馈功能，这样调查模式就不会打扰您的工作流程。

  * 当前在标头中的[!UICONTROL 通知]下拉菜单中无[!DNL Target]的通知可用。
...
| 功能/增强 | 描述 |
| --- | --- |
| Node.js SDK 1.0 版 | 通过Target Node.js SDK，您可以部署Target服务器端。<br>此Node.js SDK可帮助您轻松将Target与其他Experience Cloud解决方案集成。<br>Node.js SDK引入最佳实践，并消除在通过我们的投放API与Adobe Target集成时产生的复杂因素，这样工程团队即可专心处理业务逻辑。 以下是我们在最新版本中引入的重要功能：<ul><li>支持预取和通知，使您可以通过缓存优化性能。</li><li>支持在网页和服务器端都混合集成 Target 时优化性能。 我们正在引入一个称为 `serverState` 的设置，将由通过服务器端检索的体验填充它，这样 at.js 2.2 将不再进行额外的服务器调用以检索体验。 此方法可优化页面加载性能。</li><li> 支持通过 Node.js SDK 检索 VEC 创建的活动（通过新的投放 API 实现此检索）。</li><li>已开源，这样您的开发人员即可向 Node.js SDK 贡献代码。</li></ul> |
...
| 功能/增强 | 描述 |
| --- | --- |
| 可视化体验编辑器 (VEC) | **新 VEC 菜单选项**：单击 VEC 中的某个页面元素时，相应菜单会显示适用于该元素类型的选项。<ul><li>您现在可以使用[!UICONTROL 样式>背景]选项更改所选元素的背景图像和颜色。 (TGT-15001)</li></ul>查看[可视化体验选项](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)中的&#x200B;*样式*。<br>**点击跟踪改进**：我们改进了VEC和单页应用程序(SPA) VEC中配置点击跟踪的过程。<ul><li>当选择要在点击跟踪中使用的元素时，所有可用元素的名称都会显示在右侧的“修改”面板中，从而允许快速、轻松地选择所需元素。</li><li>三步引导式活动工作流的[!UICONTROL 目标和设置]页面会显示一个数字，用于表示为点击跟踪选择的元素数量。 您可以将鼠标悬停在此数字上，以查看所有选定元素的名称。 (TGT-33878)</li></ul>请参阅[点击跟踪](/help/main/c-activities/r-success-metrics/click-tracking.md)。 |
...
| 功能/增强 | 描述 |
| --- | --- |
| [!UICONTROL Visual Experience Composer（可视化体验编辑器）] | [!UICONTROL 可视化体验编辑器] (VEC)包含以下增强功能，可让您更快、更高效地完成工作：<ul><li>现在，设置点击跟踪时，可使用 DOM 路径功能。<br>有关更多信息，请参阅[点击跟踪](/help/main/c-activities/r-success-metrics/click-tracking.md#considerations)。</li><li>使用“样式”面板可查看或编辑所选元素现有样式的值。 您还可以添加其他样式。<br>要访问“样式”面板，请单击VEC中的页面元素，然后单击[!UICONTROL 编辑] > [!UICONTROL 样式]。<br>样式面板将显示在VEC的右侧。 该面板包含一个样式列表，允许您编辑样式或将其添加到所选元素。 如果您习惯使用层叠样式表 (CSS)，或者从开发人员那里接收代码，则可以使用实时 CSS 编辑器查看更改并添加样式。<br>有关更多信息，请参阅&#x200B;*可视化体验编辑器选项*&#x200B;中的[样式](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#styles)。</li><li>富文本编辑器现在支持嵌套的HTML5元素。<br>HTML5规范允许嵌套标记的新组合。 富文本编辑器的早期版本不支持 HTML5 规范所允许的新嵌套标记。 因此，在 VEC 中选择的任何嵌套元素均无法正确处理，从而导致不必要的 HTML 更改。 (TGT-33618)<br>有关更多信息，请参阅&#x200B;*可视化体验编辑器选项*&#x200B;中的[编辑文本/HTML](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#edit-text-html)。</li> |

#### 增强功能、修复和更改

* 我们改进了您在使用 VEC 删除资产时的工作流程。 删除的资产现在已从[!UICONTROL 选件库]和[!DNL Scene7]（如果适用）中移除。 已删除的资产将不会继续出现在搜索结果中。 (TGT-31981)
* 现在，即使资产文件夹中包含图像（非空文件夹），您也可以将其删除。 (TGT-33265)

  以前，您无法从Target图像选件库（[!UICONTROL 选件] > [!UICONTROL 图像选件]）中删除非空文件夹。 您会收到“文件夹不为空！” 通知。  借助此功能，我们将添加允许您执行文件夹删除的功能，以便删除包含任意数量的资产和子文件夹的整个文件夹。 此功能在 Target UI 以及 Adobe Experience Cloud Assets UI 中均可用。
...
* 现在，从“收藏集”选取器中选择[!UICONTROL Recommendations]收藏集后，必须单击[!UICONTROL 保存]按钮。 这种工作流程与 [!DNL Target] 中的其他工作流程保持一致。 (TGT-33205)
...
| ![高级徽章](/help/main/assets/premium.png)<br>在[!UICONTROL A/B测试]和[!UICONTROL 体验定位]活动中的推荐 | 您现在可以在[!UICONTROL A/B测试]（包括[!UICONTROL 自动分配]和[!UICONTROL 自动定位]）和[!UICONTROL 体验定位] (XT)活动中包含推荐。 此功能提供了几项全新的功能，例如：<ul><li>可在同一活动中测试和锁定推荐和非推荐内容。</li><li>可轻松尝试在页面上放置推荐内容，包括为多个推荐排序。</li><li>使用[!UICONTROL 自动分配]自动将流量推送到性能最佳的推荐体验。</li><li>使用[!UICONTROL 自动定位]，根据访客的个人资料为其动态分配量身定制的推荐体验。</li></ul>要开始使用此功能，请使用VEC创建[!UICONTROL A/B测试]或[!UICONTROL 体验定位]活动，然后执行[!UICONTROL 此项前插入]、[!UICONTROL 此项后插入]或[!UICONTROL 替换为]操作以向体验添加推荐。 (RECS-6166)<br>有关更多信息，请参阅[推荐作为产品建议](/help/main/c-recommendations/recommendations-as-an-offer.md)。 |
...
* 修复了一个问题，在[!UICONTROL 管理]页面上选择了Adobe Analytics作为报表源时，该问题阻止处于某些状况下的用户创建活动。 即使用户没有选择报表包的选项，他们仍会看到“请选择一个报表包”的消息。 (TGT-31968)
...
* 改进了用户在创建或编辑活动或产品建议时发生 [!DNL Target] 会话超时情况的工作流。 当用户单击[!UICONTROL 保存]时，会显示会话过期消息，但在重新登录后，会出现一个对话框，告知用户已成功登录，而[!DNL Target]中的UI将保留在同一个页面上，且不会丢失任何数据。
...
* 更新了 [!DNL Target] 用户界面中的以下几处位置：
  ...
  * 在报表和活动中，有些选项（[!UICONTROL 编辑]、[!UICONTROL 共享到信息源]、[!UICONTROL 查看体验URL]等） 现在可以通过单击[!UICONTROL 更多选项]图标（![icon_more_options图像](assets/icon_more_options.png)）来访问
...
* 在所有使用[!UICONTROL Analytics for Target] (A4T)报表的活动的[!UICONTROL 目标和设置]页面上添加了一个工具提示，用于告知用户如果活动的页面加载了at.js（0.9.1版或更高版本），则不需要跟踪服务器。 (TGT-22607)
...
* 修复了下载的[!UICONTROL 订单详细信息]CSV报表中未正确标记极端订单的问题。 (TGT-22500)
...
* 如果您使用基于表单的体验编辑器来编辑 mbox 而不是自动创建全局 mbox (`target-global-mbox`)，然后选择一个参与度量度作为成功量度，则该量度只会在具有活动中所用 mbox 的页面上递增。 例如，如果您的mbox为`homepage_mbox`，则[!UICONTROL 每次访问页数]量度便是该访问期间对`homepage_mbox`的点击数。
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>此版本对用户界面进行了重大更改。 最显著的更改包括： </p> <p> 
     ...
     </ul> </p> <p>此外，还更改了选择 Experience Cloud 解决方案（包括 Target）的方式。 要访问 Experience Cloud 解决方案和服务，请单击菜单图标： </p> <p> <img src="assets/menu-shell-400.png" id="image_6E9323E0EBEA41B1A7319D6BCC43E769" width="400" height="140" /> </p> <p>有关在登录到Experience Cloud后访问Target并将Target设为默认页面的更多信息，请参阅从Adobe Experience Cloud </a>访问Target <a href="/help/main/c-intro/target-access-from-mac.md#task_5467C72DAFCB4BB583762CAAFC00A5CF" format="dita" scope="local">。 </p></td> 
  </tr> 
  <tr> 
...
* 修复了当用户在[!UICONTROL 目标和设置]页面上单击&#x200B;**[!UICONTROL 选择链接]**&#x200B;时，所选链接未标记为已选择的问题。
