...
如果未在[!UICONTROL 管理] > [!UICONTROL 报告]中启用此选项（默认值），请指定优先级： [!UICONTROL 低]、[!UICONTROL Medium]或[!UICONTROL 高]。
...
对于在以前的[!DNL Target]版本中创建的活动，[!UICONTROL 低]优先级已转换为0，[!UICONTROL Medium]优先级已转换为5，[!UICONTROL 高]优先级已转换为10。 您可以根据需要调整这些值。
...
* [!UICONTROL 选择“递增计数并保持用户处于活动中”]以指定递增计数的方式。
* [!UICONTROL 选择“递增计数、释放用户并允许再次进入”]以指定用户再次进入活动时看到的体验。
* [!UICONTROL 选择“递增计数、释放用户并阻止再次进入”]，以指定用户看到的内容而非活动内容。
