...
* **自定义代码在应用于其他视图时丢失。**&#x200B;此修复了将自定义代码添加到其他视图并将其应用于其他视图时，应用于一个视图的自定义代码丢失的问题。 以前，当用户将自定义代码应用于活动中的多个视图时，以前应用的自定义代码将会消失。 此修复方法可确保在将修改应用于其他视图时，在所有视图间正确保留自定义代码。 (TGT-54230)
...
* **配置文件参数“介于”规则未保存在条件中。**&#x200B;此修复解决了对配置文件参数使用&#x200B;**In Between**&#x200B;运算符的标准规则无法正确保存的问题。 以前，当用户使用&#x200B;**介于**&#x200B;之间运算符配置规则并输入范围值时，条件似乎已成功保存，但在重新打开或编辑条件时，规则将消失。 此修复确保正确保存&#x200B;**In Between**&#x200B;规则，并在重新打开标准时保留这些规则。 (TGT-54498)
...
* **在所有活动后更改分析页面位置。** **分析页面**&#x200B;位置已更新为显示在导航中的&#x200B;**所有活动**&#x200B;页面之后。 “所有活动”页面现在是Target应用程序的默认页面。 (TGT-54672)
...
* **使左侧导航可折叠。**&#x200B;现在，所有选项卡的左侧导航均可折叠和展开。 折叠时，将显示带有工具提示的图标。 展开时，将显示图标和文本。 此增强功能可在用户需要时为其提供更多屏幕空间，同时保持对导航选项的快速访问。 (TGT-54608)
...
* **当后端返回null时，将experienceEditorType默认为“visual”。**&#x200B;此修复解决了在某些情况下将`experienceEditorType`值返回为null的问题，该问题会导致编辑器类型未正确初始化的UI中出现意外行为。 (TGT-54690)
...
* **无法将图像添加到活动 — 未知错误。**&#x200B;此修复解决了用户在尝试在活动中添加或替换图像时遇到“未知错误”的问题。 该问题与Scene7 URL同步和身份验证失败有关。 此修复确保可以在可视化体验编辑器中正确添加和替换图像。 (TGT-54137)
...
* **删除后未自动重新添加属性令牌。**&#x200B;此修复解决了在编辑活动时自动重新添加用户已取消选择的属性令牌的问题。 以前，当用户删除自动检测到的属性并在步骤之间导航时，系统将重新引入删除的属性，而不会出现任何警报或提示，从而导致在意外属性上启用活动。 此更新可确保删除资产令牌后，该令牌将在所有后续编辑和导航期间保持被删除状态。 (TGT-54040)
...
* **允许列出的URL管理功能。**&#x200B;新的“允许列出的URL”管理功能已添加到[!UICONTROL Administration]部分，以管理允许的远程优惠URL模式。 此功能包括在表中显示列入允许列表的URL、搜索和过滤URL、通过模式对话框添加新URL以及在确认后删除URL的功能。 该功能可帮助客户控制哪些URL可用于远程服务，从而提高安全性和合规性。 (TGT-54486)
...
* **支持在GraphQL中按createdAt和modifiedAt进行筛选。已向GraphQL API添加对**&#x200B;按`createdAt`和`modifiedAt`日期字段筛选活动的支持。 此增强功能可以在Target UI中进行准确筛选，减少客户端数据处理，并且无需多个API调用即可检索筛选后的活动数据。 (TGT-54499)
...
* **[!UICONTROL Workspaces]部分中的[!UICONTROL Users]下拉列表限制为20个。**&#x200B;此修复解决了以下问题： [!UICONTROL Workspaces] > [!UICONTROL Administration]部分中的[!UICONTROL Users]下拉列表仅显示20个工作区，即使用户有权访问更多工作区也是如此。 `licenseGroups`的基础GraphQL调用也被限制为20个结果，这会导致UI显示不完整的列表，尽管用户有权访问组织中的更多工作区。 更新会移除此硬限制，以便现在可以正确返回并显示整组可用工作区。 (TGT-53820)
* **修复了选件模式不显示Workspace列的问题。**&#x200B;修复了选件模式在更新的UI中不显示workspace列的问题。 这会给客户带来困惑，因为[!UICONTROL Default Workspace]中的选件与所选工作区中的选件一起显示，但未指明它们的来源。 工作区列现已启用，因此客户可以清楚地识别每个优惠所属的工作区。 (TGT-52320)
...
* **活动编辑不应添加自动检测到的属性（如果已删除）。**&#x200B;此修复解决了编辑活动会自动重新引入用户之前删除的自动检测属性的问题。 重新打开活动进行编辑时，系统错误地还原了删除的属性，导致[!UICONTROL Properties List]中的行为不一致和混淆。 更新可确保在删除自动检测到的属性后，该属性在后续所有编辑过程中都将被删除，并且除非用户明确将其添加回来，否则该属性不会再次出现。 (TGT-54182)
* **如果已经删除，请勿添加自动检测到的属性。**&#x200B;此修复可确保一旦用户手动从活动中删除自动检测到的属性，系统便不再在活动编辑器的后续导航期间重新引入该属性。 以前，如果用户取消选择自动检测到的属性，将其移动到[!UICONTROL Targeting]步骤，然后返回到[!UICONTROL Experiences]，则编辑器会根据存储在Activity Editor状态切片中的自动检测到的列表重新填充已删除的属性。 现在，更新的逻辑会将自动检测到的属性与~ActivityState~切片中的当前属性进行比较，并阻止重新添加用户已删除的任何自动检测到的属性。 这会导致各个步骤的行为保持一致，并且符合用户意图。 (TGT-54181)
...
* **修复了[!UICONTROL Reports]箭头未明确指示可展开列的问题。**&#x200B;修复了报表表未明确显示其他列可以在更新后的UI中展开的问题。 向列标题附近的[!UICONTROL Reports]箭头添加了消失的工具提示，以帮助客户了解更多列可用。 (TGT-53479)
...
+++查看详细信息
* 在更新的UI中使用&#x200B;**[!UICONTROL Goals & Settings]作为报表源时出现[!DNL Adobe Analytics]错误消息。**&#x200B;解决了更新[!UICONTROL Overview] UI中的问题，该问题导致目标部分显示“出现错误”错误。 我们无法完成您的请求。 如果选择[!DNL Adobe Client Care] (A4T)作为报表源，如果问题仍然存在，请联系[!DNL Adobe Analytics]。 目标现在可以使用[!UICONTROL Adobe Analytics]度量值正确显示，从而确保跨报告源的一致可见性。 (TGT-54021)
...
* 更新后的UI中&#x200B;**[!UICONTROL Download Recommendations]活动缺少[!DNL Recommendations]按钮。**&#x200B;解决了已更新的[!DNL Recommendations] UI中的问题，该问题中的[!UICONTROL Download Recommendations]按钮无法使用建议对A/B活动可见。 该按钮现在可以正确显示，从而允许用户按预期导出建议数据，与旧版用户界面中的功能一致。 (TGT-53768)
* 更新后的概述UI中缺少&#x200B;**[!UICONTROL Download Recommendation Data]按钮。**&#x200B;解决了已更新的[!UICONTROL Overview] UI中的问题，该问题导致包含建议的活动无法看到[!UICONTROL Download Recommendation Data]按钮。 按钮现在可正确显示，从而确保用户无需切换回旧版UI即可直接导出推荐数据。 (TGT-53772)
...
* 由于&#x200B;**贡献报表错误，[!UICONTROL Element]MVT报表未加载。**&#x200B;修复了Target UI中无法加载MVT活动报表，并显示“无法获取元素贡献报表”错误的问题。 现在，报告可以正确显示，从而确保元素贡献的完全可见性。 (TGT-53691)
...
+++查看详细信息
* **[!UICONTROL Delete Modification]按钮问题导致无法删除活动修改。**&#x200B;解决了[!UICONTROL Delete Modification] UI中的[!DNL Target]按钮不起作用的问题，从而阻止用户删除活动中的修改。 现在，该按钮可按预期工作，允许毫不延迟地可靠地删除修改。 (TGT-53728)
* **更新的UI无法识别首选选择器。**&#x200B;解决了更新后的UI中首选选择器（如`data-target-component-id`）未出现在VEC内的CSS选择器列表的问题。 用户现在可以可靠地选择首选属性而不是动态生成的类名，从而确保在SPA页面更新之间稳定定位。 (TGT-53908)
...
* **无法在更新的VEC中切换回[!UICONTROL Design]模式。**&#x200B;解决了更新的VEC UI中存在的一个问题，该问题导致用户在[!UICONTROL Design]模式下导航到新页面后，无法切换回[!UICONTROL Browse]模式。 [!UICONTROL Design]切换功能现在可以正常工作，允许修改无缝地跨页面应用。 （TGT-53988 和 TGT-53993）
...
* **[!UICONTROL Observer]用户现在可以在更新后的UI中查看活动内容。**&#x200B;在更新的活动UI中恢复了观察者角色用户的可见性。 以前，观察者无法查看修改、优惠和内容更改 — 这些功能在旧版UI中可用。 (TGT-53785)
* **[!UICONTROL Approver]用户现在可以编辑活动目标，而不会出现编辑器权限错误。**&#x200B;解决了活动创建UI中的权限问题，该问题导致审批人级别用户无法将更改保存到高级目标设置。 尽管具有足够的访问权限，但受影响的用户仍收到`403 Forbidden.Resource`错误，需要编辑器权限。 (TGT-53819)
...
+++查看详细信息
* **在“仅此活动”报告中恢复多受众选择。**&#x200B;解决了活动创建UI中的问题，该问题导致用户无法在[!UICONTROL This activity only]中的[!UICONTROL Goals & Settings]部分下选择多个访问群体。 (TGT-53283)
* **基于受众的报表图形现在可正确显示转化数据。**&#x200B;解决了[!UICONTROL Reports]选项卡中导致图形在选择非默认受众时失败的问题。 While data and confidence metrics were available, the visual graph showed only a solid line, making analysis difficult. (TGT-53769)
* **[!UICONTROL Targeting] UI现在清楚地指示了排除的观众规则。**&#x200B;解决了活动创建UI的[!UICONTROL Targeting]部分中的受众规则设置为[!UICONTROL Exclude]时未明确显示的问题。 在查看目标逻辑时，特别是在排除特定URL的受众时，这会导致混淆。 (TGT-53809)
* **受众定义值现在可在[!UICONTROL Targeting]选项卡中选择和复制。**&#x200B;解决了“活动创建”界面中导致用户无法选择和复制“[!UICONTROL Targeting]”选项卡中的访问群体规则值的问题。 旧版UI中提供了此功能，但更新后的UI中缺少此功能。 (TGT-53856)
...
* **更正了zh_CN页面编辑器上下文中“引号已删除”的错误。**&#x200B;更正了zh_CN区域设置中的一个翻译错误，该错误中“quote removed”未准确地呈现为“移除了报价”，这意味着提供了商业报价。 在“排版规则”>“标题样式”>“块引用”部分中，术语是指格式元素（引用块）而不是定价。 (TGT-53843)
...
* **现在可按预期保存活动中的更改。**&#x200B;解决了新创建UI中与权限相关的问题，该问题导致用户在编辑活动中的目标和高级设置时无法保存更改。 尽管具有适当的访问权限，受影响用户仍看到红色错误功能区和“Forbidden.Resource”消息。 (TGT-53816)
* **VEC UI现在保留跨视图的体验修改。**&#x200B;解决了更新后的VEC中影响体验开发的多个问题。 修改无法正确持久保留，尤其是在使用HTML选件或在视图之间切换时。 (TGT-53825)
* **现在，当修改跨越多个体验时，所有视图都会正确显示。**&#x200B;解决了活动创建UI中的一个问题，即在多个视图间应用修改时，只显示一个视图。 悬停工具提示无法列出所有关联的视图，即使正确应用了修改也是如此。 (TGT-53827)
* **VEC不再在“[!UICONTROL Applying initial modifications]”上间歇性停止。**&#x200B;解决了VEC中的间歇性问题，即体验无法加载且在“[!UICONTROL Applying initial modifications]”上滞留。 此行为不一致，有时还会触发重定向循环或需要手动清除缓存。 (TGT-53916)
* **无法重现VEC加载问题。**&#x200B;我们调查了一个报告的问题，即VEC在编辑现有活动时仍然卡在“[!UICONTROL Applying initial modifications]”上。 该行为被怀疑与缺少父元素的HTML内容有关。 我们将继续监控重复情况，并建议使用结构化容器来容纳HTML产品，以确保稳定性。 (TGT-53972)
* VEC中的&#x200B;**[!UICONTROL Design]模式不再像[!UICONTROL Browse]模式那样间歇性地工作。**&#x200B;解决了UI中的[!UICONTROL Design]模式间歇性行为类似于[!UICONTROL Browse]模式的问题，允许可单击的`<a>`链接并防止元素选择。 这会导致悬停框消失并阻止修改工作流。 (TGT-53136)
...
* **编辑期间，现有活动不再停滞在无限加载循环中。**&#x200B;解决了新创建UI中的问题，即在VEC中编辑现有活动会导致页面停滞在无限加载循环中。 此问题不影响新创建的活动，是由页面上已有的修改触发的。 (TGT-53913)
* **现在，修改过的现有活动页面可在VEC中正确加载。**&#x200B;解决了更新VEC中导致在编辑修改已保存的现有活动时，页面停滞在加载阶段的问题。 加载的新活动没有问题，但以前配置的活动无法呈现。 (TGT-53967)
...
* **报告在与滚动条交互后崩溃。** Clicking the scrollbar in the [!UICONTROL Reports] tab caused the page to crash, accompanied by a JavaScript error:
...
* 在VEC中使用痕迹导航时，**上下文菜单在第一次单击时消失。** Selecting a parent element via the breadcrumbs in the VEC caused the context menu to briefly appear and then disappear, making it difficult to access editing options. 现在，在浏览痕迹导航元素时，上下文菜单保持可见且功能正常，这提高了更新后的活动创建流程中元素选择工作流的可靠性。 (TGT-53424)
...
* **改进了VEC中的元素选择期间查看子元素时的工作流程。**&#x200B;为了提高活动创建期间的可用性和精度，现在，当鼠标悬停在或选择父HTML元素时，VEC会显示子元素。 此增强功能可让客户更好地了解页面结构，并做出更准确的修改，从而简化更新后的用户界面中的编辑工作流程。 (TGT-53416)
...
* **已恢复推荐活动的点击跟踪。**&#x200B;解决了在更新的UI中创建的[!UICONTROL Recommendations]活动无法注册单击跟踪的问题，导致报告的转换为零。 旧版UI已将跟踪ID (`at-track-click`)正确应用到[!UICONTROL Recommendations]内容，而更新的UI错误地插入了占位符(`__recsClickTrackIdPlaceholder__`)，从而阻止后端跟踪。 此修复确保[!DNL Recommendations]内容现在包括正确的跟踪ID、恢复转换报告并与A4T度量保持一致。 (TGT-53496)
* 在更新的UI中解决了&#x200B;**收藏集编辑器崩溃问题。**&#x200B;修复了更新的[!UICONTROL Visual Experience Composer] (VEC) UI中的一个问题：从“编辑器”面板中打开收藏集会导致页面崩溃并出现TypeError：无法读取未定义的属性（读取“customLocale”）。 此错误出现在多个活动类型中，包括[!UICONTROL Recommendations]和A/B测试。 (TGT-53703)
* **用于删除VEC中还原的选定集合的选项。**&#x200B;修复了VEC中的一个问题：用户只能替换[!UICONTROL Recommendations]活动中的选定收藏夹，但无法将其完全删除。 此限制阻止了需要彻底删除集合而不进行替换的用例。 此修复引入了一个用于删除所选集合的清晰选项，从而允许更灵活地设置活动并与旧版UI行为保持一致。 (TGT-53652)
* **自定义条件集合现在可在[!UICONTROL Recommendations] UI中正确显示。**&#x200B;修复了Recommendations界面中使用自定义条件构建的集合无法显示产品结果的问题。 尽管标准的基于属性的集合按预期方式工作，但尽管目录匹配有效，使用自定义筛选器的集合仍返回“未找到结果”。 此修复可确保使用自定义属性的集合现在能够在[!UICONTROL Product]选项卡中正确填充，从而恢复个性化建议工作流程的完整功能。 (TGT-53653)
...
* **在VEC帮助程序扩展中解决了Cookie复制问题。**&#x200B;修复了[!UICONTROL Adobe Experience Cloud Visual Editing Helper]扩展在通过预览链接进行QA期间复制`at_qa_mode` Cookie的问题。 当手动切换预览索引时，会创建多个具有跨域冲突值的Cookie，从而阻止测试者可靠地切换变量。 甚至在Target UI外部也观察到此行为，它同时影响内部帐户和客户端帐户。 此修复通过防止重复条目并调整域范围来确保一致的Cookie处理，从而允许无缝的变体切换，而无需手动Cookie清理。 (TGT-53579)
...
* **修复了[!UICONTROL Manage Content]活动中的[!UICONTROL Automated Personalization]边栏显示不一致位置标签的问题。**&#x200B;客户报告[!UICONTROL Manage Content]边栏在[!UICONTROL Experiences]和[!UICONTROL Offers]选项卡中显示不匹配的位置编号。 （例如，体验中的位置2和位置4，以及选件中的位置1和位置2），即使仅配置了两个位置也是如此。 位置标签现在一致并可准确地映射到修改，从而改进了活动创建过程中的清晰度和可用性。 (TGT-52934)
* **修复了在保存后VEC中的修改丢失的问题。**&#x200B;客户报告，在VEC中保存修改后，页面将刷新并将更改还原到之前的版本。 此问题导致最新更新丢失，除非立即保存整个活动。 现在，修改在保存后会正确保留，并且编辑器不会再意外还原更改，从而确保在活动创建工作流中提供可靠的体验。 (TGT-53500)
...
+++查看详细信息
* **改进了[!UICONTROL AEM Experience Fragment]优惠的用户归因。**&#x200B;解决了VEC中有关[!UICONTROL Last updated] (XF)的“[!UICONTROL AEM Experience Fragment]”字段错误地显示代码ID而非用户名的问题。 在更新的UI中选择优惠期间出现此差异，导致与传统的UI和HTML优惠不一致，旧版和UI正确显示了最后一个编辑器的名称。 此修复可确保各优惠类型的用户属性一致，从而提高透明度并与预期行为保持一致。 （TGT-52880 和 TGT-52898）
...
* 已解决&#x200B;**ODS选件呈现问题。**&#x200B;解决了在更新的UI中使用VEC生成活动时，通过[!DNL Offer Decision Service] (AJO)创建的[!DNL Adobe Journey Optimizer] (ODS)选件无法呈现的问题。 在旧版UI中，相同配置工作正常，导致混乱并阻止营销活动执行。 此修复可确保跨两个UI版本提供一致的选件，并恢复AJO驱动的个性化的预期行为。
...
+++查看详细信息
* **更新后的报告概述UI的报告部分中恢复了下载选项。**&#x200B;解决了新概述UI中的“报告”部分缺少[!UICONTROL Download]按钮的问题，从而阻止用户导出属性重要性分数。 此更新可恢复完整的导出功能，从而简化对用于分析和报告的可下载数据的访问。 (TGT-53222)
* **[!UICONTROL Download full CSV report]视图中恢复了[!UICONTROL Important attributes]按钮。**&#x200B;解决了更新后的活动创建UI中报告视图的[!UICONTROL Download full CSV report]部分中缺少[!UICONTROL Important Attributes]按钮的问题。 此修复程序可恢复对可下载见解的访问，从而确保更新的和旧版UI之间的功能一致。 (TGT-53238)
* **解决了影响[!UICONTROL Auto Target]在更新后的概述UI中报告的UI问题。**&#x200B;修复了更新的概述界面中影响[!UICONTROL Auto Target]活动报告的多个UI问题。 这些修复包括：
...
* **计划在将来进行的活动在[!UICONTROL Live]概述中错误地显示了“[!UICONTROL Activity]”状态**：客户观察到计划在将来开始的活动在[!UICONTROL Live]概述中错误地标记为“[!UICONTROL Activity]”。 此状态不匹配已解决，计划活动现在可以正确显示为“[!UICONTROL Scheduled]”，而无需刷新页面。 (TGT-52835)
...
+++查看详细信息
* **产品列表在[!UICONTROL View Collection]对话框中不可见：**&#x200B;以前，客户在“[!UICONTROL Recommendations]”选项卡中查看收藏集时看不到产品列表。 现在，[!UICONTROL View Collection]对话框可正确显示关联的产品，从而改进已更新的[!UICONTROL Recommendations] UI中的透明度和可用性。 (TGT-50531)
* **修复了导致[!UICONTROL Product Catalog Search]高级搜索中区分大小写筛选的问题**：“[!UICONTROL Product Catalog Search]”页中的高级搜索筛选现在会正确忽略区分大小写，从而与后端和GraphQL服务的行为保持一致。 此更新确保无论文本大小写如何，都能为客户提供一致而准确的建议结果。 (TGT-53585)
* **更新的[!UICONTROL Product Catalog Search] UI中的高级搜索不提供建议**：要求在更新的[!UICONTROL Product Catalog Search] UI中使用高级搜索功能的客户输入拼写正确的精确值，因为未显示任何建议。 此问题使得难以有效地找到产品。 现在，在高级搜索输入过程中会按预期显示建议。 (TGT-52008)
* **某些审批人无法查看[!UICONTROL Product Catalog Search]**&#x200B;中的产品：尽管具有相同角色的其他用户具有访问权限，但具有[!UICONTROL Approver]权限的客户无法查看[!UICONTROL Product Catalog Search]中的任何产品。 此问题是由于影响目录可见性的权限不一致而造成的。 所有批准者现在都可以按预期查看[!UICONTROL Recommendations]部分中的产品。 (TGT-53617)
...
+++查看详细信息
* **修复了“时间范围”已过期受众即使在删除后也阻止保存活动的问题**：由于与“时间范围”已过期受众相关的错误，客户无法保存已更新UI中的活动。 即使在删除受影响的受众后，错误消息仍然存在：“活动包含具有无效时间范围的受众。” 出现此问题的原因是，即使不再使用仅活动受众，系统仍会继续验证该受众。 时区差异进一步加剧了这种行为。 验证逻辑已更正，客户现在可以保存活动而不会遇到此错误。 (TGT-53517)
...
+++查看详细信息
* **修复了阻止客户使用[!UICONTROL Insert Before]或[!UICONTROL Insert After]在UI中插入体验片段的问题**&#x200B;客户在尝试使用更新的UI中的“此项前插入”或“此项后插入”选项将[!UICONTROL Experience Fragments]插入活动时遇到错误。 显示的错误消息为：“选件内容必须正好包含一个HTML元素。” 此问题特定于更新的UI，在以前的版本中未出现。 此问题现已解决，客户可以成功插入[!UICONTROL Experience Fragments]而不会遇到此错误。 (TGT-53442)
...
+++查看详细信息
* **修复了客户仅限一个工作区查看其他工作区活动的问题**：在活动创建流程中选择[!UICONTROL All Workspaces]时，仅限一个工作区访问的客户意外可查看所有工作区的活动。 此可见性可能会对其他工作区中的实时活动造成意外更改，从而可能会影响网站性能。 已加强工作区访问控制，以确保客户只能查看其分配工作区中的活动并与之交互。 (TGT-53101)
* **修复了客户无权查看[!UICONTROL Default Workspace]中的活动的问题：**&#x200B;只有临时工作区访问权限的客户能够通过活动创建流程查看[!UICONTROL Default Workspace]中的活动。 尽管后端配置和访问权限正确，但仍发生此行为。 加强工作区访问控制，以确保客户只能查看其分配的工作区中的活动。(TGT-53297)
...
+++查看详细信息
* **修复了更新的[!DNL Target] UI中的活动加载问题**：修复了更新的[!DNL Target] UI中某些活动在尝试编辑时加载失败的问题。 此问题导致客户将用户留在无限加载屏幕中。 此问题会影响多项活动，据报告在不同客户中发生的情况不一致。 通过此修复，现在可正确加载受影响的活动，从而实现无缝编辑并减少活动工作流的中断。 (TGT-53209)
* **修复了由于`optionLocalId`验证而导致活动创建过程中的保存错误**：修复了活动创建过程中的一个问题，该问题会导致客户无法保存由于后端验证错误而导致的更改： `OptionLocalIdReferentialIntegrity.ABActivity - Invalid optionLocalIds:`修改活动中的特定元素时发生此问题，从而导致保存操作失败。 修复程序可确保`optionLocalId`引用现在得到正确验证，从而允许客户保存活动而不会遇到此错误。 (TGT-53293)
* **修正了在切换页面时，活动创建过程因选项引用无效而崩溃的问题**：修正了活动创建过程中导致UI在编辑过程中切换页面三次后崩溃的问题。 崩溃是由无效的选项引用触发的，这会导致控制台错误，例如“找不到localId为&#39;7&#39;的选项”。 通过此更新，客户现在可以在页面之间切换并应用修改，而不会遇到系统故障或中断。 (TGT-53295)
...
* **修复了更新UI内活动创建流程中XF内容的间歇性预览问题**：修复了活动创建流程中XF内容在更新UI内间歇性无法以预览模式渲染的问题。 尽管内容交付正确，但预览并未一致地显示，因此客户很难验证优惠设置。 现在，XF预览可以可靠地加载，从而提高了活动配置期间的置信度和效率。 (TGT-53318)
...
* **修复了活动创建过程中无法在推荐视图中显示产品消息的问题**：修复了活动创建过程中即使存在消息，[!UICONTROL Message]视图中的[!DNL Recommendations]列也不会显示产品数据的问题。 客户必须手动移除并重新添加列才能临时显示内容，在滚动或搜索时，内容通常再次消失。 此更新可恢复产品消息的一致可见性，并改进了目录导航和审阅工作流程。 (TGT-52777)
...
* **修复了按[!UICONTROL Last Updated]列排序不一致的问题：**&#x200B;修复了activity-create过程中按[!UICONTROL Last updated]列排序目录导致结果不一致的问题。 客户无法根据更新时间戳可靠地组织产品数据，这增加了目录审阅和管理的难度。 排序现在按预期工作，从而提高了更新用户界面中的准确性和可用性。 (TGT-53116)
...
+++查看详细信息
* **修复了活动加载和[!UICONTROL Cancel]活动创建流程中的按钮问题**：修复了活动创建流程中某些活动加载失败，导致客户无法访问修改的问题。 此外，[!UICONTROL Cancel]按钮无响应，阻止客户停止加载过程或退出编辑视图。 此修复可确保活动现在可靠地加载，并且[!UICONTROL Cancel]按钮可按预期运行，从而提高可视化体验编辑器的整体稳定性和用户体验。 (VEC)(TGT-53218)
* **修复了已更新的VEC UI中阻止编辑并禁用[!UICONTROL Cancel]按钮的体验切换问题**：修复了已更新的VEC UI中的一个问题，该问题导致在体验定位(XT)活动中的体验之间切换时无法加载修改。 客户无法编辑超出他们最初输入的体验的体验，并且[!UICONTROL Cancel]按钮缺失或未响应。 此修复可确保立即在所有体验中正确加载修改，并且[!UICONTROL Cancel]按钮能够可靠地运行（即使没有帮助程序扩展），从而改进编辑工作流程，减少挫折感。 (TGT-53256)
...
* **修复了更新的UI中特定网页的活动创建过程中的视图加载问题**：修复了活动创建过程中出现的问题，即尽管已正确实施并在交付或交互调用中可见，但特定网页的更新的UI中未加载视图。 这影响了多个客户，并导致难以验证基于视图的定位。 现在，视图在支持的页面中填充得一致，从而改进了活动设置期间的可靠性。 (TGT-53246)
* **修复了更新UI中活动创建流程中间歇性视图加载的问题**：修复了活动创建流程中的问题：在活动编辑期间，视图间歇性无法显示在更新的UI中。 尽管网络负载中存在正确的视图名称，但无法在界面中一致地识别它，从而影响客户配置基于视图的个性化设置的能力。 视图现在可以可靠地显示，支持更流畅的设置和验证工作流程。 (TGT-53223)
* **修复了活动创建过程中被跟踪动作名称未保存在更新的UI中的问题**：修复了活动创建过程中被跟踪动作指标无法保存在更新的UI中的问题。 命名被跟踪元素并保存活动后，重新打开时名称将重置为默认标签，从而导致混乱并中断目标配置。 现在，跟踪的操作会保留其分配名称，从而允许客户准确地设置和管理转化量度。 (TGT-53453)
...
* 修复了更新后的VEC中，页面投放规则中的“等于任何”运算符无法接受输入值的问题。 在所有框上配置客户参数时，选择此运算符会导致“输入无效”错误，从而无法创建规则。 (TGT-52623)
...
* 修复了由于`copyActivityBatchOperations`突变失败，客户无法在工作区之间复制活动的问题。 尝试复制活动会导致服务器错误(500)和响应负载为空。 (TGT-52405)
* 修复了在所选工作区中无法访问配置中引用的提供时活动无法同步的问题。 这导致了发布错误，并使活动处于失败状态。 (TGT-52535)
...
+++查看详细信息
* 修复了[!UICONTROL Create Criteria]模态中的本地化问题，在[!UICONTROL Choose Price Rule]下拉列表中选择“在以下值之间”选项时，[!UICONTROL Inclusion Rules]部分中的字符串“to”未本地化。 (TGT-49754)
* 修复了“源创建”向导的[!UICONTROL All host groups]下拉列表中字符串“[!UICONTROL Environment]”的本地化问题未正确本地化。 (TGT-46737)
...
* 修复了由于HTML修改中意外包含`contentEditable`属性而导致QA预览页的整个内容变为可编辑的问题。 这允许用户单击并编辑页面上的任何文本，可能会在QA过程中引起布局问题和混淆。 (TGT-53247)
* 修复了将修改从[!DNL Page Load]移动到[!UICONTROL View]时导致复制修改的问题，该修改保留在[!UICONTROL Page Load]中，同时出现在[!UICONTROL View]中。 此外，从[!UICONTROL View]中移除修改也会从[!UICONTROL Page Load]中错误地将其移除。 (TGT-53270)
...
+++查看详细信息
* 修复了后端持久性层中的问题，即删除的选项存储正确，但无法通过现有API端点访问。 因此，前端应用程序无法为已删除的选项检索有意义的名称，从而影响历史报告视图。 此修复可确保现在可在用户界面中正确显示保留的已删除选项数据。 (TGT-52973)
* 实施了新的迁移终结点，以支持将已删除的活动选项从基于JCR的活动传输到[!DNL Target]中心。 此功能支持跨系统的一致跟踪和报告。 This feature ensures that deleted options are preserved and synchronized across the [!DNL Target] frontend and backend, improving historical reporting and data integrity. (TGT-53217)
* 引入了一个新的API端点，允许用户从辅助数据库恢复以前删除的活动选项。 此功能利用`RemovedCampaignElements`和`RemovedOptionInfo`类提供的现有基础结构，确保已删除的选项无缝地重新集成到活动活动中。 (TGT-52903)
* 修复了由于API限制，无法打开或编辑包含超过25个字符的度量名称的[!DNL Recommendations]活动的问题。 此修复可确保与超出字符限制的度量名称的兼容性，从而恢复对受影响活动的完全访问权限。 (TGT-52839)
...
* 在目标后端中重新实现了已删除的选项持久性层，以支持在[!UICONTROL Automated Personalization] (AP)活动中生成准确的历史报告。 以前，删除某个选项时，其名称会丢失，因此很难解释过去的性能数据。
...
+++查看详细信息
* 修复了VEC中将修改应用于视图会导致复制并触发“用户输入无效”错误的问题。 (TGT-52886)
* 修复了在VEC中配置图像产品时，[!UICONTROL Undo]和[!UICONTROL Insert Before]选项的[!UICONTROL Insert After]功能的问题。

  以前，撤消对图像提供的[!UICONTROL Insert Before]或[!UICONTROL Insert After]操作会导致不一致的行为或无法正确恢复修改，尤其是在旧版[!DNL Target] UI中创建的活动中。 此问题已得到解决，从而确保撤消操作现在能够可靠地用于这些修改。 (TGT-52809)

* 修复了`contentEditable`属性意外设置为true并保留在保存的HTML内容中的问题。 此更新可确保更干净的预期HTML输出，而不会出现意外的编辑行为。 (TGT-52319)
* 为防止永久失去已删除的选项并确保各服务间行为一致，已为UI和相关微服务中的选项实现了软删除功能。

  **键更改**：

   * 选项不再被永久删除。 相反，它们在parameters XML对象中用新的deleted： true标志标记。
...
+++查看详细信息
* 修复了将活动从非默认工作区复制到默认工作区或在非默认工作区之间复制时出现的问题。 现在，通过增强的跟踪和命名功能来复制选件以防止冲突。
...
+++查看详细信息
* 修复了[!UICONTROL Form-Based Experience Composer]中导致无法保存活动并触发错误消息“无法读取未定义的属性（正在读取“映射”）”的问题。 (TGT-53145)
...
+++查看详细信息
* 修复了在[!UICONTROL Catalog Search]中单击产品时显示错误“无法检索产品详细信息”并打开了模态而没有关闭选项的问题。 (TGT-53082)
...
* 修复了活动创建工作流中与[!UICONTROL Automated Personalization] (AP)和[!UICONTROL Multivariate Testing] (MVT)活动中的选件命名和内容翻译相关的几个问题：
   ...
   * 通过右侧面板重命名优惠会更新UI中的名称，但更改未反映在“[!UICONTROL Manage Content]”选项卡或“[!UICONTROL Offers]”选项卡中，导致出现永久验证错误。
   * 在MVT活动中，虽然重名错误在重命名后不会持续存在，但UI仍无法在选项卡中反映更新后的优惠名称。 (TGT-52933)
...
* 已将[!UICONTROL Rearrange]选项添加到更新后的[!UICONTROL Visual Experience Composer] (VEC) UI，以便与旧版VEC中可用的功能保持一致。 （TGT-46957 和 TGT-52876）
* 修复了对[!UICONTROL A/B Test]活动中的变体体验（例如，体验B）所做的修改未保留的问题。 在体验之间切换后，对变体的更改将消失。 此问题不影响控制体验。 (TGT-52664)
* 修复了以下问题：某些客户无法创建或保存活动，而其他客户可以正常执行相同的操作。 不同帐户间出现不一致的问题。(TGT-52842)
* 修复了在更新后的VEC中，用户无法移动对[!UICONTROL Page Load event]（旧版UI中的功能）的修改的问题。 (TGT-52617)
* 修复了在更新的UI中，创建更改时[!UICONTROL page load]事件在[!DNL Target]中不可见的问题；更新仅应用于视图。 (TGT-52604)
* 修复了导致某些活动修改无法在更新的VEC中正确显示的问题。 (TGT-52818)
...
* 修复了当体验名称超过50个字符时，阻止保存[!DNL Recommendations]活动的问题。 (TGT-52619)
...
* 修复了活动位置中的URL由于字符(如斜杠(/))无效而无法显示查询参数的问题。 (TNT52845)
* 通过后端API改进了[!DNL A/B Test]活动更新的验证错误消息。 当存在重复的位置名称时，消息现在会清楚地说明： `locations.selectors`不允许重名。 (TGT-52589)
...
* 修复了[!UICONTROL Form-Based Experience Composer]中的问题，即在[!UICONTROL Location]下拉列表中将鼠标悬停在带有特殊字符的mbox上会导致编辑器变为空白并触发“无法对‘元素’执行‘querySelector’”。 错误。(TGT-52717)
* 使用新的“PARTIALLY_IMPORTED”指示器提高了进给状态的准确性。 以前，即使文件中并非所有行都已导入，数据源仍被标记为“成功”，这是误导性的。 (TGT-52892)
* 修复了迁移到AP V2后，对`/admin/rest/ui/v1/campaigns`的某些API调用返回客户端错误(HTTP 4xx)的错误。 (TGT-52721)

### [!DNL Target Standard/Premium] 25.6.2（2025年6月12日）

此版本包括以下修复和更新：

* 添加了[新常见问题解答文章](/help/main/c-intro/updated-ui-faq.md)，以解决有关更新的[!DNL Target] UI和[!UICONTROL Visual Experience Composer]&#x200B;(VEC)的常见问题。
* 修复了[!UICONTROL URL - does not contain]中的“[!UICONTROL Page Delivery]”规则无法正常工作的问题，该问题允许显示内容，即使该内容本应被阻止也是如此。 (TGT-52754)
...
* 修复了在更新的[!DNL Target] UI中，通过[!UICONTROL Offer]菜单对HTML选件所做的更改未反映在关联活动中，反之亦然。 此行为现在与旧版UI匹配，旧版UI中的更新可在[!UICONTROL Offer]菜单和活动之间正确同步。 （TGT-52540 和 TGT-52541）
* 修复了在尝试在活动中使用[!UICONTROL Experience Fragments]中的[!UICONTROL Offers Library]的最新更新时，未反映这些更新的问题。 (TGT-52659)
* 修复了确认消息的简体中文翻译中的本地化问题。 之前的版本缺少位置名称周围的引号，而是使用非正式的语言，这与客户的风格指南相反。 更新后的翻译现在使用正确的标点符号和正式语调。 (TGT-52364)
...
* 修复了管理AP活动中的内容时，[!UICONTROL Contains]中缺少“[!UICONTROL Filter Rules]”选项的问题。 (TGT-52384)
* 修复了[!UICONTROL Automated Personalization] (AP)活动中的报表不一致问题，具体问题与如何使用`optionLocalId`内部系统中的[!DNL Target]值跟踪和报告默认选件有关。
* 修复了QA链接无法提供预期活动体验的问题。 (TGT-52163)
* 修复了具有[!UICONTROL Approver]权限的用户被错误阻止编辑实时活动的问题，该问题会收到“访问被拒绝”错误消息。 (TGT-52416)
* 修复了在更新的[!DNL Target] UI中，某些活动的受众优化无法显示的问题。 (TGT-52057)
* 修复了一个问题，该问题导致在更新后的UI中逆转受众细化和活动受众。 (TGT-52158)
* 修复了生成临时优惠导致重复优惠的问题。 (TGT-51938)
* 修复了阻止优惠更新并错误地显示“无效用户”错误的问题。 (TGT-52361)
* 修正了导致无法保存现有活动，从而触发“无效用户输入”错误的问题。 (TGT-52422)
* 修复了阻止编辑现有HTML产品的问题，即使未更改代码，在保存时也会触发“用户输入无效”错误。 (TGT-52351)
* 修复了导致[!DNL Target]无法识别网站URL中的“#”字符的问题。 (TGT-52093)
* 修复了阻止编辑[!DNL Recommendations]活动以添加或更新促销活动的问题，该问题会导致保存失败和重复促销活动。 (TGT-52343)
* 修复了一个问题，该问题阻止对[!DNL Recommendations]活动中的标准或设计进行更改，从而导致出现“无效的JSON：无法识别的属性名称”错误。 (TGT-52375)
* 修复了序列条件无法在[!UICONTROL Visual Experience Composer]活动的[!DNL Recommendations] (VEC)中正确显示的问题。 (TGT-52435)
...
我们很高兴在[!DNL AI Assistant]中宣布[!DNL Adobe Target]的启动！ 此强大的用户界面功能旨在帮助您轻松导航和了解[!DNL Target]概念。 在[!DNL Adobe Experience Cloud]的多个产品（包括[!DNL Target]）中均可用，[!DNL AI Assistant]将为您的体验带来翻天覆地的变化。

[!DNL AI Assistant]中的[!UICONTROL Target]是一个对话工具，可用于通过[!DNL Experience Platform]应用程序和服务加速工作流。 使用[!DNL AI Assistant]提高您的整体工作效率并增强您对产品知识的理解
...
* 具有[!DNL Target]和[!UICONTROL Product Administrator]权限的[!UICONTROL System Administrator]用户现在可以编辑[!UICONTROL Administration]页面上的所有设置，而不管他们在[!DNL Target]中的角色如何。 没有这些权限的用户对这些设置具有只读访问权限。 此更新确保对[管理设置](/help/main/administrating-target/administrating-target.md)进行更严格的访问控制。 (TGT-48179)
* 修复了导致无法保存活动[站点首选项](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings)的缓存问题。 (TGT-52213)
* 修复了在VEC中加载站点后，客户无法在[!UICONTROL Site Preferences]部分中启用按ID和类进行选择的问题。 [!UICONTROL Site Preferences]设置即使在启用后也自动恢复为禁用。 (TGT-52207)
* 修复了当[!UICONTROL Visual Experience Composer]页面传递[&#x200B; URL以正斜杠(/)结尾时，](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#settings) (VEC)无法显示正确页面的问题。 (TGT-52237)
...
* 修复了在将优惠移动到文件夹时导致用户输入错误的问题。 (TGT-52296)
* 为每个活动添加了一个`audienceMetadata`字段，并确保在编辑活动时已读取并更新该字段。 (TGT-51004)
...
* 修复了在选项保持不变时导致`location`和`OptionLocalIDs`错误地递增的问题。 (TGT-52188)
* 修复了导致活动[!UICONTROL Overview]页面上的位置不正确的问题。 (TGT-52182)
* 修复了没有为无效位置显示无效选择器警告的问题。 (TGT-52110)
* 修复了这样的问题，即下载的报告文件可以正确显示报告UI中的数据。 (TGT-52068)
* 修复了添加页面传递规则后批处理操作不会失败的问题。 (TGT-52097)
* 修复了导致[!DNL Target]从网站URL修剪所有查询参数的问题。 (TGT-52100)
...
* 修复了客户无权访问[!UICONTROL Analytics for Target] (A4T)但被错误选为报告源的问题。 (TGT-52226)
* 修复了阻止使用[!UICONTROL View a Page] URL度量保存活动的问题。 (TGT-52260)
* 修复了在活动中创建优惠时阻止客户选择工作区的问题。 (TGT-52289)
...
* 添加了一个错误消息，以引导用户解决活动中的重复选项。 (TGT-51927)
* 修复了在删除具有重定向选件的页面或体验时未删除`ClickTrack`选择器的问题。 (TGT-51952)
* 修复了允许空`ClickTrack`选择器导致的问题。 [!DNL Target]现在要求选择器字段不能为空。 (TGT-52107)
* 修复了错误地允许名称重复的指标的问题。 度量现在需要唯一的名称。 (TGT-52201)
* 修复了在[!UICONTROL Automated Personalization] (AP)活动中编辑优惠级别目标时，受众定义不可见的问题。 (TGT-52148)
...
* 修复了在[!UICONTROL All Visitors] (VEC)中进行更改后，受众设置恢复为“[!UICONTROL Visual Experience Composer]”的问题。 (TGT-52132)
* 修复了未针对特定活动显示受众细化的问题(TGT-52057)
* 修复了阻止客户在默认工作区中插入[!UICONTROL Experience Fragment]的问题。 (TGT-52073)
* 修复了选件显示为“找不到内容”并且未在[!UICONTROL Offers] (AP)活动的[!UICONTROL Automated Personalization]页面上显示的问题。 (TGT-52150)
...
* 修复了要求客户在编辑[!UICONTROL Optimization Goal]页面上的[!UICONTROL Reporting Settings]中重新配置[!UICONTROL Goals & Settings]的问题。 (TGT-52071)
...
* 修复了导致整个体验URL无法在[!UICONTROL Browse mode] (VEC)的[!UICONTROL Visual Experience Composer]中显示的问题。 (TGT-52101)
* 修复了编辑活动导致页面交付在URL末尾添加“/”使其无效的问题。 (TGT-52114)
* 修复了[!UICONTROL Activity QA]中的[!UICONTROL Form-Based Experience Composer]链接被错误地重定向到[!DNL Adobe Experience Cloud]主页的问题。 (TGT-52055)
* 修复了在保存并重新打开后未保留添加到[!UICONTROL A/B Test]活动中的其他页面的问题。 (TGT-51994)
* 修复了阻止客户删除内联样式部分中的样式的问题。 (TGT-52070)
* 类似于旧版UI，已恢复对[对话框中的](/help/main/c-target/c-audiences/audiences.md#section_11B9C4A777E14D36BA1E925021945780)受众定义卡片[!UICONTROL Activity QA]的访问。 (TGT-52056)
...
* 修复了在取消[!DNL Target]时导致[!UICONTROL ClickTrack]错误保存选择器的问题。 (TGT-51937)
...
* 解决了阻止在`companyName`量度的`reportSuite`配置中匹配[!DNL Analytics]和`pageviews`的问题。 (TGT-51965)
...
* 修复了导致无法在[!UICONTROL Approver] UI中正确本地化角色（[!UICONTROL Editor]、[!UICONTROL Observer]和[!DNL Target]）的问题。 (TGT-29925)

### [!DNL Target Standard/Premium] 25.3.6（2025年3月14日）

此版本包含以下修复和更新：

* 解决了当多次使用同一[!UICONTROL Visual Experience Composer]选择器时启用了[!UICONTROL Click Tracking]的[!UICONTROL ClickTrack] (VEC)活动中的“用户输入无效”错误。 (TGT-51921)
...
* 解决了无法对仅包含活动的受众编辑活动的问题。 (TGT-51807)
...
* 解决了在[!UICONTROL Goals & Settings]页面上更改度量类型时从活动中删除所有非度量访问群体（页面和体验访问群体）的问题。 (TGT-51753)
* 解决了在编辑活动时单击[!UICONTROL Cancel]将目标UI导航到[!UICONTROL Activities List]而非[!UICONTROL Activity Details]页面的问题。 (TGT-51731)
* 解决了阻止客户通过[!UICONTROL Export Reports to CSV]选项下载报告的问题。 (TGT-51708)
* 解决了基于表单的Experience Composer中将[!DNL Target Standard]名客户错误地显示为使用[!UICONTROL Properties]（一项[!DNL Target Premium]功能）的问题。 (TGT-51678)
...
* 修正了`localStorage`不可用时的遥测日志记录问题。 遥测导致某些在其浏览器中禁用了`localStorage`的客户出现问题。

有关此版本及以前at.js版本的信息，请参阅[at.js版本详细信息](https://experienceleague.adobe.com/zh-hans/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions){target=_blank}。

### Standard/Premium目标版本25.2.1（2025年2月17日） {#ui-update-2}
...
随着[!DNL Adobe Target] UI现代化工作的继续，我们很高兴地宣布已更新的[!UICONTROL Activities]用户界面正式提供。

>[!NOTE]
>
>从2月17日起，客户将逐渐有权访问新的[!UICONTROL Activities] UI。 为确保为所有客户无缝部署，此版本将在受控阶段部署。 第一阶段将把[!DNL Target]个客户的初始组升级到新的[!UICONTROL Activities] UI。 后续阶段将升级其余客户。

>[!IMPORTANT]
>
>有关[!DNL Target] UI版本切换生命周期结束计划的重要信息，请参阅[[!DNL Target] UI版本切换弃用](/help/main/r-release-notes/release-notes.md#toggle)。
...
* [[!UICONTROL Updated Change Log]](/help/main/c-activities/change-log.md)页面，现在从[[!DNL Audit Query API]](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/landing/governance-privacy-security/audit-logs/audit-api/overview){target=_blank}获取实时分析的信息。
...
有关详细信息，请参阅[优惠](/help/main/c-experiences/c-manage-content/manage-content.md)和此部分中的子文章。 本部分中的所有优惠文章均已更新，以反映这些UI更改。

以下短视频重点介绍此版本中的更改：

![提供UI刷新视频](/help/main/r-release-notes/assets/offers-video-v2.gif)
...
所有新[!DNL Adobe Experience Platform Web SDK]客户都将启用此优化。 但是，如果客户是从at.js迁移而来，并且没有执行下面的实施步骤，则会禁用该优化。 我们敦促这些客户在2025年2月3日之前审查其实施。 在此日期之后，我们将为所有客户启用优化。 如以下所述，在此之前未审查和调整实施情况可能会影响报告。 如果需要确认您的实现是否受到影响，或者需要更多时间来调整实现，请联系[!DNL Adobe Customer Care]。
...
* 在获取使用使用全局`renderDecisions: true`作用域的[!UICONTROL Form-Based Experience Composer]创建的活动时不使用`__view__`

如果未按照[数据收集](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/personalization/rendering-personalization-content){target=_blank}指南中的&#x200B;*呈现个性化内容*&#x200B;中的说明实施通知，则[!DNL Target]和[Analytics for Target报表](/help/main/c-integrating-target-with-mac/a4t/a4t.md) (A4T)中可能缺少报表数据。 在某些情况下，您可能会注意到不正确的流量分摊，因为未捕获报表数据。 或者，在其他情况下，重复报告同一事件。
...
* **拆分调用（也称为页面事件的顶部和底部）。**
  ...
  此实现类型是[推荐的新](https://experienceleague.adobe.com/zh-hans/docs/experience-platform/web-sdk/use-cases/top-bottom-page-events){target=_blank}拆分调用实现方法[!DNL Adobe]。 使用此方法时，新的优化不会影响[!DNL Analytics]或A4T报表。
...
有关at.js版本的更多信息，请参阅[Adobe Target开发人员指南](https://experienceleague.adobe.com/zh-hans/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions){target=_blank}中的&#x200B;*at.js版本详细信息*。
...
### `Browser:iPad`受众属性中`Browser:iPhone`和[!UICONTROL Browser]的更新（2024年4月30日）

| 更新 | 详细信息 |
|--- |--- |
| 在创建受众时使用的[!UICONTROL Browser:iPad]浏览器属性[!UICONTROL Browser:iPhone]中更新了[和](/help/main/c-target/c-audiences/c-target-rules/browser.md)。 | [!DNL Adobe Target]允许您[定位多个类别属性中的任意一个](/help/main/c-target/c-audiences/c-target-rules/target-rules.md)，包括访问您的页面时使用特定[浏览器或浏览器选项](/help/main/c-target/c-audiences/c-target-rules/browser.md)的访客。<P>从[!DNL Target] Standard/Premium 24.3.1（2024年3月4日至6日）开始，将更新使用Target UI创建的内置受众，例如`Browser:iPad`和`Browser:iPhone`，以使用[!DNL iPad]、[!DNL iPhone]和`profile.mobile.deviceVendor`为`profile.mobile.isMobilePhone`和`profile.mobile.isTablet`执行正确的定位。<P>客户无需对此更新执行任何操作。<p><B>重要信息</b>：若要让客户在配置文件脚本（和JavaScript区段）中对[!DNL iPad]和[!DNL iPhone]执行正确的定位，客户必须在&#x200B;**2024年4月30日**&#x200B;之前手动进行更改。 有关必须手动更改的备用设置示例，请参阅[受众属性 [!DNL iPad] 中的 [!DNL iPhone] 更新[!UICONTROL Browser]和](/help/main/c-target/c-audiences/c-target-rules/browser.md#updates)。 |

### [!UICONTROL Visual Editing Helper]延期（2024年3月14日）

此版本包含针对[[!DNL Adobe Experience Cloud Editing Helper]](/help/main/c-experiences/c-visual-experience-composer/r-troubleshoot-composer/visual-editing-helper-extension.md)的[!DNL Google Chrome]扩展的以下增强功能和修复：
...
* 修复了导致使用[!UICONTROL Multivariate] (A4T)报表配置的[!UICONTROL Analytics for Target] (MVT)活动无法正确显示的问题。 (TGT-47490)
...
* 修复了在客户使用[!UICONTROL All Activities]图标激活或停用活动后，导致[!UICONTROL More Actions]页面上显示错误状态的问题。 (TGT-47367)
...
* 增强了[活动QA](/help/main/c-activities/c-activity-qa/activity-qa.md)功能，以支持[活动中的体验的](/help/main/c-activities/t-automated-personalization/managing-exclusions.md)不允许重复的选件[!UICONTROL Automated Personalization]。 (TGT-46627)
...
作为[!DNL Adobe Target]团队持续努力改进[!DNL Target]用户体验的一部分，此版本刷新了[!UICONTROL Activities] UI中的[!DNL Target]页面。 此更新统一了以前不一致的设计模式并使其标准化，同时添加了新的增强。
...
* 修复了有时会阻止使用[!UICONTROL Auto-Target] (A4T)的[!UICONTROL Analytics for Target]报告在[!DNL Target] UI中显示的问题，即使该报告在[!DNL Adobe Analysis Workspace]中正确显示也是如此。 (TGT-46494)
...
| 功能 | 详细信息 |
| --- | --- |
| [!DNL Recommendations] 实施模式 | “使用 at.js 的推荐实施模式”**&#x200B;文章帮助您在使用 at.js JavaScript 库时了解和创建您的 [!DNL Adobe Target Recommendations] 实施。<P>有关详细信息，请参阅[Adobe Target Developer Guide](https://experienceleague.adobe.com/docs/target-dev/developer/implementation-patterns/atjs/recs-implementation-pattern-atjs.html?lang=zh-Hans){target=_blank}中的&#x200B;*使用at.js的Recommendations实施模式*。 |
...
* 修复了对于使用[!DNL Target] (A4T)作为报表源的[!DNL Adobe Analytics]活动，导致[!UICONTROL Auto-Allocate] UI和[!UICONTROL Analytics for Target] UI中的报表数据不一致的问题。 (TGT-46112)
...
有关详细信息，请参阅[Adobe Target Developer Guide](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/allowlist-edges.html?lang=zh-Hans){target=_blank}中的&#x200B;*允许列表目标边缘节点*。

### [!DNL Target] Standard/Premium 23.8.1（2023 年 8 月 9 日）

此版本包含以下增强和修复：

* 修复了有时会导致活动无法正确同步的问题，如[!UICONTROL Status]列表页面上的“[!UICONTROL Activity]”列所示。 （TGT-46010 和 TGT-44831）
* 修复了有时在使用[!UICONTROL View in Analytics] (A4T)作为报告源的活动的[!UICONTROL Reports]页面上无法显示“[!UICONTROL Analytics for Target]”链接的问题。 (TGT-45808)
* 调整了表格中值的显示方式，改为显示百分比，而非带小数的数字。例如，8% 而非 .08。(TGT-45548)
* 修复了客户无法使用键盘焦点来移动到[!UICONTROL Goals & Settings]页面中用于[!UICONTROL Experience Targeting] (XT)活动的下一个元素的问题。 (TGT-44526)
...
* 改进了[在](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path) (VEC)中使用DOM路径[!UICONTROL Visual Experience Composer]导航元素以包含影子DOM元素时的搜索。 (TGT-45262)
...
| 功能 | 详细信息 |
|--- |--- |
| [!UICONTROL QA mode]活动的[!UICONTROL Automated Personalization] | [!DNL Adobe Target] [!UICONTROL QA mode]现在可用于[!UICONTROL Automated Personalization]活动，替换了[!UICONTROL Preview links]功能。<P>有关更多信息，请参阅[活动 QA](/help/main/c-activities/c-activity-qa/activity-qa.md)。 |

* 性能增强不允许在[活动中](/help/main/c-activities/t-automated-personalization/managing-exclusions.md#concept_4EF78013F80E48EFA024AE0274C9F037)管理排除项[!UICONTROL Automated Personalization]时使用重复项功能（包括减少加载时间）。
...
* 修复了导致[!UICONTROL Auto Personalization]活动的报告在分析期间冻结的问题。 (TGT-44820)
* 修复了当活动名称包含方括号（ [!UICONTROL Activity]或[ ）时，导致无法在]页面上搜索活动的问题。 (TGT-44777)
...
| 功能 | 详细信息 |
|--- |--- |
| 用于Headless个性化和试验的AEM [!UICONTROL Content Fragments] | 在[!DNL Adobe Experience Manager]活动中使用[!UICONTROL Content Fragments] (AEM) [!DNL Target]。 将简单易用的 AEM 与 [!DNL Target] 中强大的人工智能 (AI) 和机器学习 (ML) 功能结合使用，帮助进行 Headless 个性化和试验。<P>有关详细信息，请参阅[AEM [!UICONTROL Content Fragments]](/help/main/c-integrating-target-with-mac/aem/content-fragments-aem.md)。 |
...
| 功能 | 详细信息 |
|--- |--- |
| 已为[!UICONTROL Auto-Allocate]和[!UICONTROL Auto-Target]优化A4T指标<p>（发布日期：2023 年 3 月 30 日） | 在为[!DNL Target]和[!UICONTROL A4T]活动使用[!UICONTROL Auto-Allocate]时，[!UICONTROL Auto-Target]允许您选择基于二项式事件的量度或基于连续事件的量度。<P>请注意，在支持的指标中发生了以下变化：<ul><li>[!DNL Target] 保留现有活动以前的行为直至 2023 年 9 月 9 日。在此日期之后，将不再继续执行使用不支持的指标的活动，以强制现有活动采用新行为。</li></ul>有关详细信息，请参阅[和[!UICONTROL Auto-Allocate]活动[!UICONTROL Auto-Target]的](/help/main/c-integrating-target-with-mac/a4t/a4t-at-aa.md#supported)A4T支持中的“支持的目标量度”。<br>以下教程已随此功能一并更新：<ul><li>[在 [!DNL Analysis Workspace] 中为[!UICONTROL Auto-Allocate]个活动设置A4T报告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-allocate-activities.html?lang=zh-Hans){target=_blank}</li><li>[在 [!DNL Analysis Workspace] 中为[!UICONTROL Auto-Target]个活动设置A4T报告](https://experienceleague.adobe.com/docs/target-learn/tutorials/integrations/set-up-a4t-reports-in-analysis-workspace-for-auto-target-activities.html?lang=zh-Hans){target=_blank}</li></ul> |

* 增强了受众和活动同步，以便在 [!DNL Adobe Experience Platform] 和 [!DNL Adobe Audience Manager] 中创建的项目可更快地在 [!DNL Target] UI 中可用。(TGT-44568)
* 增强了UI，允许用户删除[!UICONTROL Default URL] > [!UICONTROL Administration] > [!UICONTROL Visual Experience Composer]下的[!UICONTROL Default URL]。 此更改允许客户将默认 URL 更改回空字符串，这在以前的初始配置后是不可能实现的。(TGT-44577)
* 删除了阻止客户编辑或删除开箱即用的受众（具有保留名称的受众）的限制。(TGT-44655)
* 在创建[!UICONTROL Done]组合受众[!DNL Target]时，加载旋转器时禁用了[选项，该选项在](/help/main/c-target/combining-multiple-audiences.md) UI中可见。 (TGT-44079)
* 修复了[!UICONTROL Language]页面底部的[!UICONTROL Audiences]链接，使其正确链接到“[!UICONTROL Account communication preferences]”页面。 (TGT-43562)
* 解决了有时在选择[!UICONTROL A/B Test] > [!UICONTROL Adobe Analytics] > [!UICONTROL Administration]下的[!UICONTROL Reporting]选项后阻止客户创建[!UICONTROL Reporting Experience Cloud Solution]活动的问题。 (TGT-44844)
* 修复了一个问题，该问题阻止客户查看[!UICONTROL Multivariate Test] (VEC)中具有许多体验的[!UICONTROL Visual Experience Composer]活动中的最后一个体验。 VEC 底部的 [DOM 路径](/help/main/c-experiences/c-visual-experience-composer/viztarget-options.md#dom-path)有时会阻止客户看到上次体验。(TGT-44578)
* 修复了导致 VEC 中的浏览 URL 无法反映在正常浏览器会话中可见的当前页面（如果页面需要授权或调用重定向）的问题。(TGT-44350)
* 修复了阻止客户在[!UICONTROL Filter Incompatible Criteria] > [!UICONTROL Recommendations]中更改[!UICONTROL Settings]设置的问题。 (TGT-44398)
* 修复了在将[!DNL Recommendations]与名称中包含点的报表包一起使用时导致POST请求创建[!UICONTROL Analytics Classifications]馈送失败的问题。 (TGT-44598)
...
有关所有at.js版本的信息，请参阅[Adobe Target开发人员指南](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}中的[at.js版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank}。
...
>[!NOTE]
>
>要确保交付在VEC中创作的更改，请确保您使用的是版本高于2.8的[!DNL Target]SDK ([at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}或[Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/release-notes.html?lang=zh-Hans){target=_blank} (alloy.js))。
...
有关所有at.js版本的信息，请参阅[Adobe Target Developer Guide](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}中的[at.js版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank}。
...
* 已从AP中删除45天和90天日历选项，并在[!UICONTROL Auto-Target] UI中删除[!UICONTROL Personalization Insights] [!UICONTROL Important Attributes]和[!DNL Target]报告。 由于使用模式和为了提高性能，已弃用这些日期范围。UI 已更新，以反映当前允许的范围：15 天、30 天和 60 天。(TGT-39357)
* 不允许在活动上线后更改[!UICONTROL Same as Optimization Goal]页面上的[!UICONTROL Goals & Settings]设置。 (TGT-43923)
...
新的[!DNL Adobe Target]模型API，也称为API，可让用户查看和管理机器学习模型中用于[!UICONTROL Automated Personalization] (AP)和[!UICONTROL Auto-Target] (AT)活动的功能的列表。

有关详细信息，请参阅[Adobe Target开发人员指南](https://experienceleague.adobe.com/docs/target-dev/developer/api/models-api/models-api.html?lang=zh-Hans){target=_blank}中的&#x200B;*模型API概述*。
...
* 修复了一个问题，在VEC中从[!UICONTROL Modifications]模式切换到[!UICONTROL Experiences]模式后创建或编辑活动时，该问题导致无法在[!UICONTROL Compose]页面上的[!UICONTROL Browse]面板中正确显示某些组件。 (TGT-43300)
* 修复了一个问题，该问题阻止某些客户存档使用[!UICONTROL A/B Test]的[!UICONTROL Auto-Target]活动。 (TGT-40978)
...
* 修复了在创建[!UICONTROL Clicked an Element] (MVT)活动时选择[!UICONTROL Multivariate Testing]转化目标时导致显示错误消息的问题。 (TGT-43842)
...
* 修复了一个问题，以确保客户可以正确显示包含推荐的[!DNL Recommendations]和[!UICONTROL A/B Test] (XT)活动中[!UICONTROL Experience Targeting]对象（设计、条件、集合等）的使用情况，还可以从[!DNL Target] UI和[!DNL Recommendations]后端删除不再使用的条件对象。 (TGT-42331)
...
* 如果客户尝试将带有感叹号( ！ )的受众另存为受众名称的第一个字符（例如！London），则会显示警告消息。 (TGT-43643)
...
* 添加了对[用户代理客户端提示](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/user-agent-and-client-hints.html?lang=zh-Hans){target=_blank}的支持。
* 修复了在[!UICONTROL Offer Decisions] (XT)活动中呈现[!UICONTROL Experience Targeting]时间歇性地导致超时的问题。 (TNT-44611)
...
* 修复了在部署新的[!UICONTROL Audiences] UI之前创建体验级改进和访问群体时，某个客户无法编辑活动的体验级改进和访问群体的问题。 (TGT-43433)
...
* 为启用了[!DNL Target]Business ID帐户[和基于策略的身份验证(PBA)的组织启用了](https://helpx.adobe.com/cn/enterprise/using/identity.html){target=_blank}用户界面令牌刷新。 (TGT-42590)
...
* 将为所有[!UICONTROL Audiences]客户启用新的[!DNL Target Standard] UI。

### Target Standard/Premium客户工程修复（2022年3月22日）

此维护版本包含以下增强功能：

* 添加了以下功能：在将[!DNL Analytics]与使用`prefetch` (A4T)的活动结合使用时，返回`pageLoad`视图和[!UICONTROL Delivery API]点击量度的[!UICONTROL Analytics as the reporting source]有效负载数据。 (TNT-43198)
...
* 在从[!UICONTROL Delete]、[!UICONTROL Audiences]和[!DNL Adobe Experience Platform]导入的受众的[!DNL Adobe Experience Cloud]列表中添加了[!DNL Adobe Target Classic]选项。 还添加了批量删除功能。(TGT-42914)
...
* 修复了一个问题，该问题阻止某些客户从[!UICONTROL IPs to exclude from [!DNL Target] reporting data] > [!UICONTROL Administration]中的“[!UICONTROL Reporting]”列表中删除IP地址。 (TGT-43384)
...
| 功能 | 详细信息 |
| --- | --- |
| 基于购物车的推荐 | 添加了新的算法系列，可根据访客购物车的内容提供推荐。<br>有关更多信息，请参阅[创建标准](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)中的“基于购物车”，[计划和实施推荐](https://experienceleague.adobe.com/docs/target-dev/developer/recommendations.html?lang=zh-Hans){target=_blank}中的“购物车添加/购物车查看/结账页面”和“排除访客购物车中已有的商品”，以及[根据推荐键提供推荐](/help/main/c-recommendations/c-algorithms/base-the-recommendation-on-a-recommendation-key.md)中的“基于购物车”。 |

### [!DNL Target Standard/Premium] 21.10.3（2021 年 10 月 19 日）

此维护版本包含以下增强功能、修复和更改：

* 修复了阻止客户在[!UICONTROL A4T]中通过单击[!DNL Analysis Workspace]活动报告中的[!UICONTROL View in Analytics]按钮来打开[!DNL Target]面板的问题。 （TGT-42099、TGT-42100）
* 修复了在使用[!UICONTROL Edit Design]编辑[!UICONTROL A/B Test]和[!UICONTROL Experience Targeting] (XT)活动时导致[!UICONTROL Form-Based Experience Composer]按钮不显示的问题。 (TGT-41980)
* 修复了在创建新[!UICONTROL Compatible]活动时，导致在标准选择中不显示[!UICONTROL Recommendations]复选框的问题。 (TGT-42053)
...
* 在 [!DNL Target] UI 中的各个位置添加了警告图标、弹出框和消息，以指示已从来源中删除受众，并且受众不再可用于 [!DNL Target] 活动中。
   ...
   * 受众细化（活动创建工作流的[!UICONTROL Replace Audience]步骤中的[!UICONTROL Targeting]）：
...
| [!UICONTROL Profile Scripts] UI刷新 | [!UICONTROL Profile Scripts]库也进行了更新，其中包括更新的界面以及若干工作效率更新：<ul><li>同时选择和删除多个轮廓脚本的功能</li><li>轮廓脚本的新代码编辑器</li><li>代码编辑器中的语法突出显示和错误检查</li><li>通过键盘快捷键自动完成令牌（mbox 或 profile）参数</li></ul>有关更多信息，请参阅[访客轮廓](/help/main/c-target/c-visitor-profile/visitor-profile.md)。 |
| [!BADGE 高级]{type=Positive url="/help/main/c-intro/intro.md#premium newtab=true" tooltip="查看Target Premium中包含的内容。"}Recommendations条件创建和编辑 | [!UICONTROL Recommendations Criteria]创建和编辑工作流已经过简化，以简化选择正确的推荐算法和设置以实现目标的过程。<br>有关更多信息，请参阅[创建标准](/help/main/c-recommendations/c-algorithms/create-new-algorithm.md)。 |
...
* 修复了导致“编辑标准”按钮无法在VEC中[!UICONTROL Experiences]活动的[!DNL Recommendations]页面上正常显示的问题。 (TGT-39512)
* 修复了一个问题，该问题导致在复制到测试工作区时无法同步活动。(TGT-40686)
* 修复了一个问题，该问题导致在VEC中使用“[”时无法修改带](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md)体验片段[!UICONTROL Insert After]的选择器。 (TGT-41802)
* 修复了一个问题，该问题导致无法将产品建议中的空 JSON 内容发送到后端。[!DNL Target] 现在可发送 JSON 对象，即使该对象为空也是如此。(TGT-41555)
* 修复了一个问题，该问题导致当客户在查看报告时单击“[!DNL Analytics]”时，打开旧[!DNL Analysis Workspace]报告而非[!UICONTROL View in Analytics]。 (TGT-41867)
...
* 修复了一个问题，该问题导致在[!UICONTROL Auto Personalization]中创建的[!UICONTROL Form-Based Experience Composer]活动的报表引用报表中删除的选件。 此问题导致显示以下错误消息：“我们在检索此报表的数据时遇到问题。如果问题仍然存在，请联系 Adobe 客户关怀部门。“(TGT-41028)
...
| 功能 | 详细信息 |
| --- | --- |
| [!UICONTROL Analytics for Target] (A4T) | 在使用[!UICONTROL View in Analytics]作为报表源(A4T)的活动的[!UICONTROL Reports]页面上单击“[!DNL Analytics]”链接时，[!DNL Analysis Workspace]现在会打开。 之前，该链接会打开 [!DNL Analytics] 报告。(TGT-36959) |
...
有关详细信息，请参阅[SDK开发人员指南](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/python/overview.html?lang=zh-Hans){target=_blank}中的[Adobe Target文档](https://experienceleague.adobe.com/docs/target-dev/developer/overview.html?lang=zh-Hans){target=_blank}。
...
* 对 at.js 的[设备上决策](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hans){target=_blank}支持。
...
| 功能 | 详细信息 |
| --- | --- |
| 对 at.js 的设备上决策支持<br>（日期尚未公布） | 通过设备上决策，营销人员和产品开发人员可在用户的浏览器中几乎无延迟地提供试验性和个性化。<br>有关更多信息，请参阅[针对 at.js 的设备上决策。](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hans){target=_blank} |
...
* 修复了即使启用了“[!UICONTROL Automated Personalization]”选项，在[!UICONTROL Disallow Duplicates]活动中的不同位置使用时，也不会复制优惠的问题。 (TGT-39567)
...
* 现在每天生成[!UICONTROL Personalization Insights] (AT)和[!UICONTROL Auto-Target] (AP)活动的[!UICONTROL Automated Personalization]报告。 可选择过去15、30和60天内提供[!UICONTROL Automated Segments]或[!UICONTROL Important Attributes]的报告。 已删除 45 天和 90 天的选项以使其他回溯时段设置可每天运行。(TGT-39472)
* 修复了客户在活动的[!UICONTROL Edit Dependency]页面上单击[!UICONTROL Goals & Settings]时导致不显示当前依赖项的问题。 (TGT-39340)
...
* 添加了一个警告，在[!DNL Adobe Analytics]活动中使用[!UICONTROL Analytics as the reporting source] (A4T)时选择[!UICONTROL Auto-Target]度量时显示该警告。 [!UICONTROL Auto-Target]模型已优化为可与二进制（基于转化）量度一起使用。 选择连续指标（如收入）可能会产生次优结果，并且[!UICONTROL Personalization Insights]报表可能不准确。 (TGT-38926)
...
* 已生成使用A4T和[!UICONTROL Automated Segments]转化量度的[!UICONTROL Important Attributes]活动的[!UICONTROL Auto-Target]和[!DNL Analytics]报告，并且它们看起来与使用[!DNL Target]作为报告源时相同。 (TGT-38931)
...
* 修复了[!UICONTROL Recommendations]中导致[!UICONTROL Create Sequence]页面在更改行业垂直项后挂起的问题。 (TGT-38160)
* 修复了用户无法从[!UICONTROL Automated Personalization] (AP)活动中的优惠中删除受众的问题。 (TGT-39058)
* 修复了导致某些客户在[!UICONTROL Audience Info]卡中显示错误时间范围（开始日期和结束日期）的问题。 (TGT-39150)
...
| 功能 | 详细信息 |
| --- | --- |
| 设备上决策 | 通过设备上决策，营销人员和产品开发人员均可在用户的设备内跨渠道几乎无延迟地提供试验性和机器学习驱动的个性化。<br>速度和性能至关重要 - 对于客户洞察和用户满意度。<br>通过设备上决策，可将 A/B 测试和体验定位 (XT) 活动类型中的关键个性化和试验指令编译为“优化工件”：通过 CDN 加载到客户设备上的 JSON 对象。并且由于设备上决策与 [!DNL Adobe Experience Cloud] 产品存在固有联系，因此 [!DNL Target] 用户可获得快速的分析和更快的体验迭代。<br>有关详细信息，请参阅*[at.js的设备上决策](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/on-device-decisioning/on-device-decisioning.html?lang=zh-Hans){target=_blank}和服务器端的[设备上决策简介](https://experienceleague.adobe.com/docs/target-dev/developer/server-side/on-device-decisioning/overview.html?lang=zh-Hans){target=_blank}。 |

此版本包含以下增强功能、修复和更改：

* 修复了导致[!UICONTROL Average Lift Confidence Interval]和[!UICONTROL Confidence]无法在[!DNL Auto-Target]行的[!UICONTROL Total]报表中显示的问题。 但可正确地显示所有个别体验的测量结果。(TGT-37301)
* 修复了一个问题，该问题在9月15日下午2[!DNL Adobe Target Premium] (PDT)至10月6日上午9[!UICONTROL Auto-Target] (PDT)影响:30用户的:25报表。 在查看（使用&quot;[!UICONTROL Viewed a page]&quot;或&quot;[!UICONTROL Clicked on mbox]&quot;选项配置的）受影响的转化量度的报表时，所报告的转化率有误。 目前没有已知的投放问题。
* 在[!UICONTROL Last Updated At]表中添加了一个可选的[!UICONTROL Catalog Search]列和[!UICONTROL Last Updated At]筛选器。 此增强可节省时间和精力，因为不必打开每个单独的项目以查看上次更新它的时间，并可按上次更新这些项目的日期进行筛选。
...
* 修复了一个问题，如果（在[!UICONTROL Activity]中指定的）工作区名称包含撇号，则该问题阻止用户显示[!UICONTROL Adobe Admin Console for Enterprise]页面。 (TGT-37709)
* 修复了在选择优化和转化量度时[!UICONTROL Auto-Allocate]活动中的一个问题：即使已指定报表包，仍有一条错误消息不正确地通知用户选择报表包。 (TGT-37689)
* 修复了一个问题，在导航到[!UICONTROL Goals and Settings]页面然后返回之后，该问题有时导致[!UICONTROL Targeting]页面上的量度为空。 (TGT-37691)
...
| 功能/增强 | 描述 |
| --- | --- |
| at.js 1.8.2 | at.js 的此版本是一个维护版本，其中包括以下修复：<ul><li>修复了在使用 CNAME 和边缘覆盖 at.js 1 时出现的问题。*x* 可能无法正确地创建服务器域，从而导致了 [!DNL Target] 请求失败。(TNT-35064)</li></ul>有关更多信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}。 |
| at.js 2.3.1 | 此版本的 at.js 是一个维护版本，它包括以下增强功能和修复：<ul><li>使得可通过 [targetGlobalSettings](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/targetglobalsettings.html?lang=zh-Hans){target=_blank} 覆盖 `deviceIdLifetime` 设置。(TNT-36349)</li><li>修复了在使用 CNAME 和边缘覆盖 at.js 2 时出现的问题。*x* 可能无法正确地创建服务器域，从而导致了 [!DNL Target] 请求失败。(TNT-35065)</li><li>修复了一个问题，在使用 [!DNL Target] [!DNL Launch] 扩展程序 v2 和 [!DNL Adobe Analytics] [!DNL Launch] 扩展程序时，[!DNL Target] 延迟了 [!DNL Analytics] `sendBeacon` 调用。（TNT-36407、TNT-35990、TNT-36000）</li></ul>有关更多信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}。 |
...
有关更多信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}。
...
* 当您登录到 [!DNL Adobe Experience Cloud] 时，您将被带入新的标题导航。它看起来与以前的导航非常相似，只是顶部有一个黑色的栏，但它提供以下改进：
   ...
   * 当前在标题中的[!DNL Target]下拉菜单中无[!UICONTROL Notifications]的通知可用。
...
| 功能/增强 | 描述 |
| --- | --- |
| ![Premium 徽章](/help/main/assets/premium.png)<br>A/B 测试和体验定位 (XT) 活动中的推荐 | 推荐产品建议（算法）状态将显示在包含推荐产品建议的 A/B 测试和 XT 活动的“概述”页面上。状态包括：“结果已准备就绪”、“结果未准备就绪”和“信息源故障”。(TGT-33649)<br>查看[Recommendations优惠](/help/main/c-recommendations/recommendations-as-an-offer.md#status)。 |
...
| Target通过Apple ID (ECID)库4.3支持Experience Cloud的ITP 2.1和ITP 2.2 | 现在，Target客户可以通过利用Apple的CNAME认证计划来应对Adobe的ITP 2.1和ITP 2.2。<br>在此版本中，Target引入了与ECID库4.3的无缝集成，该库利用服务器端Cookie来缓解ITP 2.1和ITP 2.2的问题。强烈建议目标客户将[ECID库4.3+](https://experienceleague.adobe.com/docs/id-service/using/release-notes/release-notes.html?lang=zh-Hans)与Target的JavaScript库一起部署，以缓解任何未来的ITP版本。 ECID 库将持续增强，针对各浏览器推出的不断变化的 Cookie 策略提供完善的解决方案。<br>请参阅 [Apple 智能防跟踪 (ITP) 2.x](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/apple-itp-2x.html?lang=zh-Hans){target=_blank}。 |
...
有关此版本 at.js 以及之前版本的信息，请参阅 [at.js 版本详细信息](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans){target=_blank}。
...
| 功能/增强 | 描述 |
| --- | --- |
| 支持 Adobe 选择加入 | 通过 Adobe 选择加入，可轻松将 Adobe 解决方案与同意管理平台集成。<br>有关 Adobe 选择加入的更多信息，请参阅[隐私和《通用数据保护条例》(GDPR)](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/privacy/cmp-privacy-and-general-data-protection-regulation.html?lang=zh-Hans){target=_blank}。 |
...
| 发送通知 | 允许开发人员在通过代码而不是使用 `applyOffer()` 或 `applyOffers()` 呈现体验时发送通知。<br>有关更多信息，请参阅 [adobe.target.sendNotifications(options)](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/adobe-target-sendnotifications-atjs-21.html?lang=zh-Hans){target=_blank}。 |
...
* 修复了一个问题，该问题会导致一小部分的洞察报告返回 0% 转化率，而不是实际的转化率。(TNT-32125)
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>at.js 版本 1.6.3 现已可用。 </p> <p> 
     ...
     </ul> </p> <p>有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_EF529199D1C741F7BDBC9C41A37B7D26"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1" class="premium"> <p>个性化洞察报告 </p> <p> <p>注意：可用日期为 2018 年 11 月 14 日。 </p> </p> </td> 
...
<table id="table_4785030753B24AA1A973E1DF790B83DD"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1" class="premium"> <p>个性化洞察报告 </p> </td> 
...
<table id="table_5A60FFE5E86148F4BDC6A7031D03D6BA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>添加了 Microsoft Edge 作为 Target UI 和内容交付支持的浏览器。 </p> <p>有关更多信息，请参阅 . <a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html?lang=zh-Hans" format="dita" scope="local">支持的浏览器</a> (TGT-14102) </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>推荐 </p> </td> 
...
<table id="table_76576D9D931B4DA99900F2C03175938E"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>at.js </p> </td> 
   <td colname="col2"> <p>at.js 版本 1.3.0 现已可用。有关更多信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hans" format="dita" scope="local">下载 at.js</a> 和 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> <p> 
     <ul id="ul_349BEB37B6C94FF0801F121042037803"> 
      <li id="li_4C2F82F4DD394ED5A0BFF978B15FEDDF"> <p>以下新事件可用于帮助跟踪、调试和自定义与 at.js 的交互： </p> <p> 
        ...
        </ul> </p> <p>有关详细信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hans" format="dita" scope="local"> at.js自定义事件</a>。 </p> </li> 
      <li id="li_E2704294F8BA47FFAABE7572F67FB5C0"> <p>您可以使用来自数据提供程序的其他参数来增强 at.js 请求。应将数据提供程序添加到 <span class="codeph">dataProviders</span> 键下方的 <span class="codeph">window.targetGlobalSettings</span>。 </p> <p>有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hans" format="dita" scope="local">targetGlobalSettings()</a> 中的“数据提供程序”。 </p> </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 
     <ul id="ul_BD0C5B260E7E4F139FBC1FBA286C0B81"> 
      <li id="li_FCDBABE6C5034A3596F5BBF024245FB9"> <p>现在，Target 支持在选件库中创建 JSON 选件，以供在基于表单的体验编辑器中使用。请参阅<a href="/help/main/c-experiences/c-manage-content/create-json-offer.md#concept_63C7BEE1F0DB4A7596D997219B7C136D" format="dita" scope="local">创建JSON优惠</a>。 (TGT-27064) </p> </li> 
      <li id="li_5500AE7DCF4146E88E4619382CE8E836"> <p>现在，您可以在每个选件的定义弹出卡片中查看引用了代码选件的活动。此功能不适用于图像选件。查看<a href="/help/main/c-experiences/c-manage-content/manage-content.md#concept_17874A6FCBB743AA84C5988E8571CCF3" format="dita" scope="local">优惠</a>。 (TGT-26277) </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1" class="premium"> <p>推荐 </p> </td> 
   <td colname="col2"> <p> 
     <ul id="ul_63613AD2D744442AA12CD23F4DAC75B4"> 
      <li id="li_4DD5CF06D93A4083BCB34A4FFA293C89"> <p>现在，UI 会显示为推荐上传自定义算法数据的状态。请参阅<a href="/help/main/c-recommendations/c-algorithms/recommendations-csv.md#task_1BBA49883E794670A09F0ABE1B3F4288" format="dita" scope="local">正在上载自定义条件</a>。 (TGT-23891) </p> </li> 
...
<table id="table_793CDDF1BD9E48BDBABBF6CD979BE186"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p>at.js </p> </td> 
   <td colname="col2"> <p>at.js 版本 1.2.2 现已可用。有关更多信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hans" format="dita" scope="local">下载 at.js</a>。 </p> <p> 
...
<table id="table_8457FAE3508F454F9DFDEF093FBD7E40"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p><b>2017 年 10 月 13 日</b> </p> <p> <span class="filepath">at.js</span> 版本 1.2.1 现已可用。有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>在活动创建工作流中提供了以下增强功能： </p> <p> 
     <ul id="ul_2D251AC11FC54E86AE84DEFFB6FDF43C"> 
      <li id="li_AB8F12B3CF654120BD16EAE570517741"> <p>编辑活动时，您可以对当前显示的步骤进行所需修改，单击拆分按钮上的下拉列表，选择“<span class="wintitle">下一步</span>”以前进到下一步，然后单击“<span class="wintitle">保存并关闭</span>”以保存所做的更改并显示活动“<span class="wintitle">概述</span>”页面，或单击“<span class="wintitle">保存</span>”以保存所做的更改并保留在该步骤。 </p> <p> <img src="assets/edit_split_button_2.png" id="image_ABC7EE42F5D341EC88AACC54CA98DA2F" /> </p> <p>有关详细信息，请参阅<a href="/help/main/c-activities/edit-activity.md#concept_BB064C0D4A194BD1A1AE7CCA1E6BB8F0" format="dita" scope="local">编辑活动</a>。 </p> </li> 
      <li id="li_4C71E2570ECF4BBAB08443D89230CE82"> <p>编辑活动时，您可以打开所需的工作流步骤，进行相应更改（例如更改体验百分比、受众等），然后保存或关闭活动，而不必按照三步引导式工作流进行操作。 </p> <p> <img src="assets/edit_activity.png" id="image_0B9A2EF729C34A1D9FA84B8B7B17A3C1" /> </p> <p>有关详细信息，请参阅<a href="/help/main/c-activities/edit-activity.md#concept_BB064C0D4A194BD1A1AE7CCA1E6BB8F0" format="dita" scope="local">编辑活动</a>。 </p> </li> 
...
* 现在，拥有“审批者”权限的用户可以生成并启用配置文件 API 身份验证令牌。(TGT-24074)
  ...
  有关更多信息，请参阅[配置文件 API 设置](https://experienceleague.adobe.com/docs/target-dev/developer/implementation/methods/profile-api-settings.html?lang=zh-Hans){target=_blank}。
...
<table id="table_701D8D53D1DF4F28ADAC6EC221B0208A"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p><b>2017 年 9 月 27 日</b> </p> <p> <span class="filepath">at.js</span> 版本 1.2.0 现已作为维护版本提供，其中包含大多数错误修复。有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> <p> 
...
<table id="table_1B43199F1AE64E69AE65313B23741444"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p><b>2017 年 8 月 3 日</b> </p> <p> <span class="filepath">at.js</span> 版本 1.1 现已可用。有关更多信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hans" format="dita" scope="local">下载 at.js</a>。 </p> <p><span class="filepath">at.js</span> 版本 1.1 中包含以下增强功能和修复： </p> <p> 
...
<table id="table_47117524922A472AA977C652B581B356"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 已将 Target 论坛移至新的 <a href="https://experienceleaguecommunities.adobe.com/t5/adobe-target/ct-p/adobe-target-community?profile.language=zh-Hans" format="https" scope="external">Adobe 社区平台</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_6167ECB7B44F40DCADF299F46F1F795C"> 
 ...
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="filepath"> at.js </span> </p> </td> 
   <td colname="col2"> <p> <span class="filepath">at.js</span> 版本 0.9.6 现已可用。有关更多信息，请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hans" format="dita" scope="local">下载 at.js</a>。 </p> <p><span class="filepath">at.js</span> 版本 0.9.6 中包含以下增强功能和修复： </p> <p> 
     <ul id="ul_108DF85393614C69988E299485D338FD"> 
      ...
      <li id="li_B11CE11D9A594CB1ABB85BD0D93C4A15"> <p>添加了 <span class="codeph">selectorsPollingTimeout</span> 设置。有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hans" format="dita" scope="local">targetGlobalSettings()</a>。 </p> </li> 
      <li id="li_D6F862099A374FE394F4DA3520A1BBF0"> <p>更改了来自 <span class="codeph">getOffer()</span> 的响应格式。有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/functions-overview/atjs-functions.html?lang=zh-Hans" format="dita" scope="local">adobe.target.getOffer(options)</a>。 </p> </li> 
     ...
     </ul> </p> <p>有关此版本 <span class="filepath">at.js</span> 以及之前版本的信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 在[!UICONTROL Offer Level Report]报表的[!UICONTROL Automated Personalization]中，“查看次数”现在为“访问次数”。
...
>[!NOTE]
>
>带有FP-11577的[!DNL Adobe Experience Manager] 6.2（或更高版本）现在支持通过其[!DNL at.js]集成来实施[!UICONTROL Adobe Target Cloud Services]。 有关更多信息，请参阅 Adobe Experience Manager 6.2 ** 文档中的[功能包](https://experienceleague.adobe.com/docs/?lang=zh-Hans)和[与 Adobe Target 集成](https://experienceleague.adobe.com/docs/?lang=zh-Hans)。
...
* 现在，[!UICONTROL Create Activity]页面上的[!UICONTROL Activities]按钮在列表中加载所有活动名称之前处于活动状态。 这项改进使用户可以更快地创建新活动，特别是当帐户配置了很多活动时。(TGT-21470)
...
* 在使用[!UICONTROL Goals & Settings] (A4T)的所有活动的[!UICONTROL Analytics for Target]页面上添加了一个工具提示，通知用户如果活动的页面加载了at.js（版本0.9.1或更高版本），则不需要跟踪服务器。 (TGT-22607)
* 度量名称现在显示在[!UICONTROL Goals & Settings]页上，用户无需展开每个度量即可查看整个度量名称。 这项改进可提高用户编辑量度的速度和效率。(TGT-21276)
* 现在，您可以将 [!DNL Recommendations] 包含规则应用到自定义标准（已通过 CSV 上传），操作方法与应用到任何其他标准一样。(TGT-21896)
* 改进了[!UICONTROL Offers]页面的用户界面和可用性，尤其是在创建或管理文件夹以及创建优惠时。 （TGT-22509 和 TGT-22187）
* 改进了选择要隐藏的项时[!UICONTROL Visual Experience Composer] (VEC)中的用户体验。
(TGT-22224)
* 改进了使用[!UICONTROL Form-Based Experience Composer]创建活动时的用户体验。 选择mbox位置时，单击[!UICONTROL Next]后验证边框保持高亮显示。 (TGT-22221)
  ...
  <tr> 
   ...
   <td colname="col2"> <p>2017 年 1 月 18 日 </p> <p> <span class="codeph">at.js</span> 版本 0.9.4 包含以下更改： </p> <p> 
     <ul id="ul_8F149C28E2D946B9888B4D2F45167C3C"> 
      <li id="li_93E866BBFE374E93BCDB65BCFAC33B62"> <p> mbox 名称现在可包含特殊字符，这些字符包括与号 (&amp;)。(TNT-26144) </p> <p>有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/deploy-at-js/implement-target-without-a-tag-manager.html?lang=zh-Hans" format="dita" scope="local">at.js 配置</a>。 </p> </li> 
     ...
     </ul> </p> <p>要详细了解 <span class="codeph">at.js</span> 各个版本中的更改，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="html" scope="external">at.js 版本详细信息</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_33B60910EAE24BAFA778F280F72FB683"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>2016 年 7 月 14 日 </p> <p> <span class="filepath">at.js</span> 版本 0.9.1 现已可用。 </p> <p>有关更多信息，请参阅 <a href="https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=zh-Hans" format="dita" scope="local">at.js 版本详细信息</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
<table id="table_EBA34BD2F5C745DD9EC5231AD79F6C00"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>现在，在自动个性化活动和多变量测试活动中，元素选择器可以基于类或 ID。在以前的版本中，此选项仅可用于 A/B 测试活动。 </p> <p>有关详细信息，请参阅可视化体验编辑器<a href="/help/main/c-experiences/c-visual-experience-composer/vec-selectors.md#concept_4EB7663E255F439B8D24079D23479337" format="dita" scope="local">中使用的</a>元素选择器。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_8525ECC9B6D0435ABEF8C27F747B7A0C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Target 界面现在支持 Internet Explorer 11。 </p> <p>请参阅<a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html?lang=zh-Hans" format="dita" scope="local">支持的浏览器</a>。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_5A2F2058ACFB455E9F69484CA0C8D3DE"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>现在可以选择仅在悬停时显示的内容（例如弹出菜单和迷你推车），以在可视化体验编辑器中进行编辑。 </p> <p>See <a href="/help/main/c-experiences/experiences.md#concept_A2E10F6AFB3D4AEAB6951EE14688848D" format="dita" scope="local"> Experiences </a>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>以可视化的方式预览可视化体验编辑器中的每个内容组合。 </p> <p>查看Automated Personalization测试<a href="/help/main/c-activities/t-automated-personalization/ap-preview-experiences.md#task_21A700587E88453A9FC2210C0DE53A28" format="dita" scope="local">的</a>预览体验。 </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_5D4C3C5695BA4A88BC65E2721CFB073A"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 检查与您的 Target 页面实施和活动/体验发送状态相关的详细信息，以改进故障诊断。 </p> <p>有关详细信息，请参阅<a href="/help/main/c-activities/c-troubleshooting-activities/content-trouble.md#concept_D2548B486C984B1E97ED7A72075B8EEA" format="dita" scope="local">内容传递疑难解答</a>。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* 修复了用户在&#x200B;**[!UICONTROL Select Link]**&#x200B;页面上单击[!UICONTROL Goals and Settings]时，所选链接未标记为已选的问题。

* 修复了在[!UICONTROL Activity List]页面上激活后阻止新活动在[!UICONTROL Overview]中显示的错误。
