---
keywords: 实施；javascript库；js;atjs；设备上决策；设备上决策；规则对象
description: 了解设备决策使用的JSON规则对象。
title: 设备上决策中的JSON规则对象有何用途？
feature: at.js
role: Developer
...
规则对象是您的 [!DNL Adobe Target] 设备上决策活动。 规则对象由 [!DNL Target] 并传播到Akamai CDN以确保存在尽可能接近访客的规则对象。 规则对象包含元数据，可确保活动的精确执行和交付，同时允许通过事件跟踪进行实时分析。
