...
## [!DNL Target Standard/Premium] [!UICONTROL Visual Experience Composer] 仅限或 [!UICONTROL Form-Based Experience Composer] 使用全局 [!DNL Target] 仅请求 {#section_4A0A317DFED345649B58B0CB5B410C8B}

如果您的公司使用 [!DNL Target Standard/Premium] 和VEC独占使用，因而可以为同一调用返回来自多个活动的内容。 活动将使用以下决策流程来交付：
...
1. [!DNL Target Standard/Premium] 拉取中运行的每个活动 [!DNL Target] 请求。
1. [!DNL Target] 尝试将访客与活动进行匹配。
...
1. 如果某个VEC活动具有最高优先级，则会返回所有匹配VEC活动中的内容，但不会返回 [!DNL Target Classic] 或返回基于表单的活动内容。
...
## [!DNL Target Standard/Premium] [!UICONTROL Form-Based Experience Composer] 具有非全局 [!DNL Target] 请求 {#section_C3F5F09B0B2D4EF795C5929D5C426A8C}
...
**有两个活动使用在中创建的优惠 [!UICONTROL Visual Experience Composer] 和两个 [!DNL Target Classic] 活动**
...
>[!NOTE]
>
>组合响应的顺序是 [!DNL Target Classic] 内容排在首位。 只有一个 [!DNL Target Classic] 按照用例1中的方式提供响应，然后 [!UICONTROL Visual Experience Composer] 按优先级颠倒排序的优惠响应。
