...
[!UICONTROL 流量估算器]预测所需的样本大小，以确保满足以下要求：
...
[!UICONTROL 流量估算器]还提供了反馈，让您知道您是否具有足够的流量可供设计成成功的测试。

1. 在[!UICONTROL 多变量]活动的[!UICONTROL 可视化体验编辑器]的[!UICONTROL 体验]页面中，单击[!UICONTROL 体验]页面左上角的&#x200B;**[!UICONTROL 流量]**&#x200B;图标（![流量估算器图标](/help/main/assets/icons/Gauge2.svg)）。
...
1. （视情况而定）提供典型转化率、预计每日访客数和测试持续时间。
   ...
   * **[!UICONTROL 典型转化率]**：转化率以百分比表示，根据估算或Analytics系统中的以往数据得出。
   * **[!UICONTROL 预计每日访客数]**：这是指根据定位条件有可能会查看此页面的访客数量。 可以基于您的分析数据。
   * **[!UICONTROL 测试持续时间]**：您希望该活动运行的天数。

   [!UICONTROL 流量估算器]使用这些统计信息来确定运行成功的测试所需的调整。

   当您更改这些数字时，估算的结果也会相应改变。 例如，如果您正在测试多个体验，而您的转化率和展示次数太低，则[!UICONTROL 流量估算器]会显示测试成功必须运行多久。 或者，如果您的流量较低，则[!UICONTROL 流量估算器]可能会建议较少的体验数量，以便您可以在所需的天数内运行测试。
